...
https://cronl1.web1.one/cront?pq=1&li=2&ex=1&related_skip=1&ts=1

https://cronl1.web1.one/cront_05_to_20_second
https://cronl1.web1.one/cront_20_to_30_second
https://cronl1.web1.one/cront_30_to_45_second
https://cronl1.web1.one/cront_45_to_90_second


gw/cront?pq=1&li=2&is=30&ex=1&related_skip=1

/cront?pq=1&li=10&ct=781&re=100&is=30
/cront?pq=1&li=10&ct=781&re=100&im=1
/cront?pq=1&li=10&day=0&day2=0&ct=781&re=100
/cront?pq=1&li=10&day=1&day2=1&ct=781&re=100
...
if($_SERVER["HTTP_HOST"]=='localhost'||$_SERVER["HTTP_HOST"]=='localhost:8080'||$_SERVER["HTTP_HOST"]=='localhost:98'||isset($_SESSION["http_host_loc"])){	//check localhost or not, if yes then define localhost in data and set true
	$localhosts=true; 
...
if(isset($_REQUEST['cront_response'])&&$_REQUEST['cront_response'])
	$cront_response=$_REQUEST['cront_response'];
else 
	$cront_response='RES';

if(isset($_GET['cront_response'])&&$_GET['cront_response'])
	$cront_response=$_GET['cront_response'];
...
if ((strpos ( $php_self1, "status_auto_update" ) !== false)||(strpos ( $php_self1, "cront_status_auto_update" ) !== false)) {
	...
	//	exit;
		...
		foreach($tra as $key=>$value){
			...
			*/
			...
			$data_send['cront_response']=$cront_response;
			...
			if(isset($_REQUEST['t'])&&$_REQUEST['t']&&(!in_array($value['trans_status'], ["0","1"]))&&($value['trans_status']!=0))
			{
		...
		if($value['trans_status']==1)
		{
			if($data['pq'])
			{
				echo "<br/><br/>{$j}. Allready Successful=>".$transID; echo "<br/><br/>";
			}

		}
		else {

			$surl=$data['Host']."/status{$data['ex']}?transID={$transID}&type={$value['acquirer']}&check_auth=cron_tab&cron_tab=cron_tab&cront_response={$cront_response}&tr_id={$value['id']}&tdateSkip=tdateSkip&action=webhook".(isset($dbad_link_2)&&trim($dbad_link_2)?$dbad_link_2:'').@$qp_pr;
