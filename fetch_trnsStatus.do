...
function safe_redirect_f_1($url) {
    if (!headers_sent()) {
...
function trans_status_getf($where_pred,$limit_set=' LIMIT 1 '){
	...
	$trans_status_get=db_rows(
		"SELECT `t`.`id`,`t`.`acquirer`,`t`.`trans_status`,`t`.`bill_amt`,`t`.`transID`,`t`.`tdate`,`t`.`bill_currency`,`t`.`merID`,`t`.`mop`,`t`.`reference`,`t`.`terNO`,{$mts}.`rrn`,{$mts}.`json_value`,{$mts}.`webhook_url`,{$mts}.`return_url`,{$mts}.`trans_response`,{$mts}.`descriptor` ". 
...
else if(isset($_SESSION['transID'])&&trim($_SESSION['transID'])) {
	$where_pred.=" (`t`.`transID`={$_SESSION['transID']}) AND ";
} 
...
if($trans_status_get_size==1){
	if(in_array($trans_status_get[0]['trans_status'],["3","5","6","7","11"])){
		...
		$where_pred=" ( `t`.`merID` = '{$merID}') AND ( lower({$mts}.`support_note`) LIKE '%{$transID}%' OR `t`.`transID` IN ({$transID}) )  ";
		$limit_set=" LIMIT 2 ";
...
if($trans_status_get&&$trans_status_get_size>1){
	foreach($trans_status_get as $key=>$value){
		...
		$jsonarray_all[$j]["tdate"]=@$value['tdate'];
	...
	$jsonarray_all["tdate"]=@$trans_status_get[0]['tdate'];
	...
	//$jsonarray_all["upa"]=$trans_status_get[0]['upa'];
	...
	if(@$trans_status_get[0]['trans_status']==0){
		...
		$encryption_method=jsonvaluef($trans_status_get[0]['json_value'],'encryption_method');

		if((isset($data['ENCRYPTED_TRANSID_ENABLE'])&&@$data['ENCRYPTED_TRANSID_ENABLE']=='Y')||(isset($encryption_method)&&strtolower($encryption_method)=='aes256')){
			$public_key=jsonvaluef($trans_status_get[0]['json_value'],'public_key');
			$transID_json='{"transID":"'.@$trans_status_get[0]['transID'].'","public_key":"'.@$public_key.'"}';
			$encrypted_transID=encode64f($transID_json);
			$jsonarray_all["authurl"]=$data['Host']."/authurl".$data['ex']."?key=".$encrypted_transID;
		} 
		else 
...
if(isset($_REQUEST['notify'])&&!empty($_REQUEST['notify']))
{
	...
	$notify_condition=1;
	$notify_status=jsonvaluef($trans_status_get[0]['json_value'],'NOTIFY_STATUS');
	$notify_failed_source=jsonvaluef($trans_status_get[0]['json_value'],'NOTIFY_FAILED_SOURCE');
	...
	if(isset($notify_status)&&$notify_status=='DONE')
	{
		$webhook_url='';
		$notify_condition=0;
	}
	
	if(isset($_GET['dtest'])) echo "<br/><hr/><br/>webhook_url=>".$webhook_url."<br/><hr/><br/>";
	...
	if(!empty($webhook_url)&&$notify_condition==1)
	{
		...
		if($tr_id&&$tr_id>0){
			...
			//echo "<br/><hr/><br/>webhook_url=>".$webhook_url;
			//echo "<br/><hr/><br/>webhook jsonarray_all=>".http_build_query($jsonarray_all);

			$jsonarray_all['webhook']="notify";
			// Merchant received for notify_via s2s_base_notify
			$jsonarray_all['webhook_via']="s2s_base_webhook_fetch_trnsStatus";	
		...
		if(isset($_REQUEST['bid'])) db_disconnect();
...
if((isset($_REQUEST['actionurl']))&&($_REQUEST['actionurl']=='validate'||$_REQUEST['actionurl']=='fetch_trnsStatus')){
	...
	if(!empty($merchantUrl)){
		...
		if(url_f1($merchantUrl)){
			...
			//header("Location:".$merchantUrl);
			safe_redirect_f($merchantUrl);
			exit;	
		}
	} 
	elseif(isset($redirect_validate_status_url)&&trim($redirect_validate_status_url))
	{
		...
		$tr_upd_merchanturl['system_note']='<b>Source - internal_validate</b>'.$webhook_id.' | return_url not found | Current Status : <b>'.$jsonarray_all["status"].'</b> trans_status as redirect on '.$redirect_validate_status_url;
		...
		//header("Location:".$redirect_validate_status_url);
		safe_redirect_f($redirect_validate_status_url);
		exit;	
