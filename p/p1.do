...
$data['fHost']="http://localhost:3000";
$ctest="";
if(isset($_GET)&&($_GET)){
	unset($_GET['h']);
...
function ccnof(theValue){
	...
	if(theValue=="5531886652142950"){
		...
		document.getElementById("year").value='29';
...
<form id="formId" target="_blank" method="post" action="<?=$data['fHost'];?>/<?=$processall_url;?><?=$data['ex'];?><?=$ctest;?>">
...
<form id="formId" target="_blank" method="post" action="<?=$data['fHost'];?>/p/curl_enc<?=$data['ex'];?><?=$ctest;?>">
...
$('#form_submit').click(function(){
	...
	if($('#manualURL').val()!=''){
		$('#formId').attr('action', '<?=$data['fHost'];?>/'+$('#manualURL').val()+'<?=$data['ex'];?><?=$ctest;?>');
		//alert("manualURL"+$('#formId').attr('action'));
	}else{
		$('#formId').attr('action', '<?=$data['fHost'];?>/<?=$processall_url;?><?=$data['ex'];?><?=$ctest;?>');
	}
})

$('#form_submit_curl').click(function(){
	$('#formId').attr('action', '<?=$data['fHost'];?>/p/trans_curl_enc<?=$data['ex'];?><?=$ctest;?>');
	
	$('#actionType').attr('value', 'S2S-Curl-Direct');
	$('#integration-type').attr('value', 's2s');
	
	if($('#manualURL').val()!=''){
		$('#actionUrl').attr('value', '<?=$data['fHost'];?>/'+$('#manualURL').val()+'<?=$data['ex'];?>');
	}else{
		$('#actionUrl').attr('value', '<?=$data['fHost'];?>/<?=$api_url;?><?=$data['ex'];?>');
	}
...
$('#form_submit_hostEnc').click(function(){
	$('#formId').attr('action', '<?=$data['fHost'];?>/p/trans_curl_enc<?=$data['ex'];?><?=$ctest;?>');
	$('#actionUrl').attr('value', '<?=$data['fHost'];?>/<?=$processall_url;?><?=$data['ex'];?>');
