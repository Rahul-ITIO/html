...
if($pfdate&&$ptdate){
	...
	$account_type_whr=" AND (`payout_date` BETWEEN ('{$wd_created_date_prev}', '%Y%m%d')) AND (DATE_FORMAT('{$wd_created_date}', '%Y%m%d')))";
}else{
	//	tdate
...
//end: class	
		...
		foreach($post['payout_mature'] as $key=>$value){
			...
			if(($value['status']==1 || $value['status']==4 || $value['status']==7 || $value['status']==8)&&($value['trname']=='cn'||$value['trname']=='ch')){ // completed
				...
				$completed_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".number_formatf2($value['transaction_amt']).";".number_formatf2($dis_rate).";".number_formatf2($accounts[0]['txn_fee']).";".number_formatf2($rol_fee).";".number_formatf2($this_total)."";
			}
