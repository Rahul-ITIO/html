...
function getacquireridsf($wid,$ajax){
	...
	if(isset($wid)&&$wid>0&&$wid!='undefined')
	{
...
function withdraw_max_prevf($uid=0,$tid=0,$custom_settlement_optimizer_v3=0){
	...
	$c_withdraw_max=db_rows(
		...
		" FROM `{$data['DbPrefix']}{$master_trans_table_set}`".
	...
	$remaining_balance_prev=db_rows(
		...
		" FROM `{$data['DbPrefix']}{$master_trans_table_set}` AS `t`".
...
function fetch_trans_balance($memId="", $json_frozen=0)
{
	...
	if(!empty($memId) && $memId>0){
		...
		//$where_clouse = "(`merID` IN ({$memId}) ) AND (`trans_status` NOT IN (0,2,7,8,15,16,17) )";	//fetch merID = memId
	}
	else
		$where_clouse = 1;
...
function fetch_prvious_balance_cross_db($uid,$project_type='')
{
   ...
   $result['total_refund_amt']=@$account[0]['total_refund_amt'];
...
function get_trans_graph($post,$grfType=1,$subAdminId='')
{
	...
	// grfType=4 for group by duration_category for runtime graph
	...
	elseif($grfType==3){ // group by `terNO`
		$order_by=" `t`.`terNO` "; 
	...
	if(isset($post['time_period'])&&$post['time_period']&&$grfType==1){
		if($post['time_period']==1){
			//Dev Tech : 24-09-26 bug fix
            if($data['connection_type']=='PSQL') {
				$g_tdate	=" CONCAT(to_char(date_trunc('day', `t`.`tdate`), 'YYYY/MM/DD'),' (',{$count_id},')') `tdates`";
				$group_by	=" GROUP BY DATE_TRUNC('day', `t`.`tdate`) ";	
				$order_by='';
			}		
			else {
		...
		}elseif($post['time_period']==3){  // group by year
			if($data['connection_type']=='PSQL') 
			{
				$g_tdate	=" CONCAT(to_char(date_trunc('year', `t`.`tdate`), 'YYYY'),' (',{$count_id},')') `tdates`";
				$group_by	=" GROUP BY DATE_TRUNC('year', `t`.`tdate`) ";	
				$order_by='';
			}		
			else {
	...
	if(isset($post['date_1st'])&&$post['date_1st']&&isset($post['date_2nd'])&&$post['date_2nd']){
		...
		$date_2nd_24=(date('His',strtotime($post['date_2nd'])));
		if($date_2nd_24=='000000'||$date_2nd_24=='235959'){
			$post['date_2nd'] = (date('Y-m-d 24:00:00',strtotime($post['date_2nd'])));

			if(isset($_REQUEST['dtest'])&&$_REQUEST['dtest']){
				echo "<br/>date_2nd_24=>".$date_2nd_24;
				echo "<br/>date_2nd=>".$post['date_2nd'];
			}
		}

		//if time period value is 5 means, fetch data according payout date
	
		if(isset($post['is_created_date_on'])&&@$post['is_created_date_on']=='1'){
			$sl_date=' `t`.`created_date` ';
		}elseif(isset($post['key_name'])&&@$post['key_name']=='created_date'){
			$sl_date=' `t`.`created_date` ';
		}elseif(isset($post['key_name'])&&@$post['key_name']=='settelement_date'){
			$sl_date=' `t`.`settelement_date` ';
		}elseif(isset($post['settelement'])&&@$post['time_period']==5){
			$sl_date=' `t`.`settelement_date` ';
		}else{
			$sl_date=' `t`.`tdate` ';
		}
	...
	if($grfType==4) {
		...
		FROM (
			SELECT 
				CASE 
					...
					WHEN runtime <= 20 THEN '20s'
					WHEN runtime > 20 AND runtime <= 40 THEN '40s'
					ELSE 'Custom'
				END as duration_category
			FROM 
				`{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`  AS `t` {$join_additional}  WHERE `t`.`runtime` IS NOT NULL {$where_pred} 
		) AS subquery
		GROUP BY 
			duration_category
		ORDER BY 
		...
		//fetch transaction data from two table of master & aditional for analytic graph 
		$query_set="SELECT ".$g_tdate.", SUM(`t`.`trans_amt`) AS `amounts`, {$count_id} AS `ids` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`  AS `t` {$join_additional}  WHERE `t`.`tdate` IS NOT NULL ".$where_pred." {$order_by} "; 
	}
	
	if(isset($_REQUEST['dtest'])&&$_REQUEST['dtest']){
		echo "<br/><hr><br/>query_set=><br/>";
		print_r($query_set);
		
	}

	$depositeData=db_rows($query_set,$q);
...
function common_trans_detail($id, $uid, $custom_settlement_optimizer_v3=''){
	global $data; $prnt=0;
	...
	// Table Withdraw of custom settlement optimizer v3
	if($custom_settlement_optimizer_v3==1){
		$q="SELECT * FROM `{$data['DbPrefix']}{$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3']}`  WHERE `id`='{$id}' LIMIT 1";
		$master_trans_table_set=$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3'];
	}
	// Trans Master Table & Additional Table
	else {
...
if(!isset($json_value['gst_amt'])) $json_value['gst_amt'] = 0;
		...
		*/
		...
		$result['acquirer_creds_processing_final']=@$trans['acquirer_creds_processing_final'];
		$result['payload_stage1']=@$trans['payload_stage1'];
		$result['acquirer_response_stage1']=@$trans['acquirer_response_stage1'];
		$result['acquirer_response_stage2']=@$trans['acquirer_response_stage2'];
		...
		if(@$trans['bill_ip']){
			$count_ip=db_rows("(SELECT COUNT(id) AS ip_count FROM `{$data['DbPrefix']}{$master_trans_table_set}` WHERE `bill_ip`='{$trans['bill_ip']}' LIMIT 1)");
...
function mer_trans_list($uid, $dir='both', $acquirer=-1, $trans_status=-1, $start=0, $count=0, $order='', $suser='', $sdate='', $custom_settlement_optimizer_v3='')
{
	...
	if($qp) echo "<br/>custom_settlement_optimizer_v3=>".$custom_settlement_optimizer_v3;
	...
	if ((isset($_REQUEST['mid'])) && ($_REQUEST['mid']!='') && (!empty($_SESSION['login_adm'])) ){
		...
		if($custom_settlement_optimizer_v3==1){
			$q1	="SELECT `t`.*  ";
			$q1.=" FROM `{$data['DbPrefix']}{$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3']}` AS `t`";
		}
		else { // Trans Master Table & Additional Table 
			$q1	="SELECT `t`.*  {$ad} ";
			$q1.=" FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` AS `t{$echeck}`";
			$q1.=$join_additional;
		}
		$q2	=$mid_query;
		...
		if($qp){
			echo "<h4 style='font-size:17px!important;font-weight:bold;color:#e10000;'>mer_trans_list limit=>{$limit}</h4>";
			echo "<br/>q1=>".$q1;
			echo "<br/>q2=>".$q2;
			echo "<br/><hr/>";
		}
	...
	foreach($trans as $key=>$value){	//fetch all data row by row
		...
		//fetch custom settlement optimizer v3 
		if($custom_settlement_optimizer_v3==1){
			$result[$key]['csov3'] ="&csov3=".@$custom_settlement_optimizer_v3;
		}
		elseif(isset($data['DB_CON'])&&function_exists('config_db_more_check_link'))
		{ //fetch  db 
		...
		{
			$result[$key]['runtime']		=@$value['runtime'];		
			//$result[$key]['runtime_mm']	=@$value['runtime'].'|'.($value['runtime']>0?number_formatf2(@$value['runtime']/60):'');		
		}
...
//Transaction List in Admin Dashboard 
function adm_trans_list($uid, $dir='both', $acquirer=-1, $trans_status=-1, $start=0, $count=0, $order='', $sort = 'DESC', $suser='', $sdate='', $custom_settlement_optimizer_v3='')
{
	...
	if(isset($_REQUEST['a'])&&$_REQUEST['a']=='cn2'&&isset($_SESSION['login_adm'])) 
	{
		...
		echo "<br/><hr/><br/>custom_settlement_optimizer_v3=>".@$custom_settlement_optimizer_v3."<br/>";
	...
	*/
	...
	foreach($trans as $key=>$value){		//fetch all data row by row
		...
		*/
		...
		if(@$value['channel_type']>0&&isset($data['channel'][@$value['channel_type']]['name1'])){
...
function update_trans_ranges($uid, $status, $trange='', $paydate='',  $update_tr=true,  $adm_login=true, $json_value='', $custom_settlement_optimizer_v3='')
{
	global $data; $result=[];
	...
	$qp=0; 
	if(isset($_REQUEST['dtest'])) $qp=@$_REQUEST['dtest'];
	...
	if($qp){
		echo "<h4 style='font-size:17px!important;font-weight:bold;color:#e10000;'>update_trans_ranges
	=>{$custom_settlement_optimizer_v3}</h4>";
		
		echo "<b style='color:#062377;'>master_trans_table_set=> ".@$master_trans_table_set."</b>";
		echo "<br/><b style='color:#9d7c05;'>join_additional=> ".@$join_additional."</b>";
	...
	$tresult=db_rows(
			"SELECT * FROM `{$data['DbPrefix']}{$master_trans_table_set}` {$join_additional} WHERE {$wheres} ",0
	);
	...
	foreach($tresult as $key=>$value){
		...
		if($value['acquirer']==2||$value['acquirer']==3||$value['acquirer']==4){
			...
			//for settlement webhook
			//$settlement_webhook_array['timestamp']=@$value['tdate'];
			$settlement_webhook_array['transID']=@$value['transID'];
			$settlement_webhook_array['settlement_amount']=str_replace_minus(@$value['trans_amt']);
			$settlement_webhook_array['settlement_currency']=@$value['bank_processing_curr'];
			

			$data['settlement_webhook_array']=$settlement_webhook_array;
			$data['settlement_webhook_url']=@$value['webhook_url'];

			//echo "settlement_webhook_url=>"; exit;
		}
	...
	if(isset($is_multiple_cross_db)&&$is_multiple_cross_db==1) {
		...
		if(isset($data['MASTER_TRANS_ADDITIONAL'])&&$data['MASTER_TRANS_ADDITIONAL']=='Y'){
			...
			if($custom_settlement_optimizer_v3==1)
			db_query("UPDATE `{$data['DbPrefix']}{$master_trans_table_set}` SET ".$additional_update." WHERE `id`='".$tr_id."' ",0); 
			else db_query("UPDATE `{$data['DbPrefix']}{$data['ASSIGN_MASTER_TRANS_ADDITIONAL']}` SET ".$additional_update." WHERE `id_ad`='".$tr_id."' ",0); 
		}
	...
	*/
	...
	if(isset($data['DB_CON'])&&isset($data['DBCON_DEFAULT'])&&trim($data['DBCON_DEFAULT'])&&function_exists('config_db_more_connection')&&isset($DBCON_DEFAULT)&&isset($is_multiple_cross_db)&&$is_multiple_cross_db==1) 
	{
		...
		if(isset($calculation_tr_fee_upd)&&$calculation_tr_fee_upd==true){
			...
			calculation_trans_fee($tr_id,"",false,"",$cpost,$custom_settlement_optimizer_v3);
		}
	...
	//email 
	  if(empty($json_value)){
		...
		if($status_email==3||$status_email==5||$status_email==6||$status_email==11||$status_email==12){
			$webhook_url = "{$data['Host']}/fetch_trnsStatus{$data['ex']}?notify=1&actionurl=fetch_trnsStatus&bid={$tr_id}";
			$webhook_url=curl_url_replace_f($webhook_url);
			//echo "<br/><hr/><br/>webhook_url=>".$webhook_url; exit;
...
function calculation_trans_fee($trange,$uids=0,$all=false,$pdate='',$cpost='',$custom_settlement_optimizer_v3='')
{
	global $data; $wheres = ""; $all_id=""; $qp=$data['cqp'];
	$sortmode=" DESC ";
	//echo "<hr/>trange:".$trange;echo "<hr/>uids:".$uids;echo "<hr/>all:".$all;echo "<hr/>";
	if($qp) echo "<hr/><h1><==fun calculation_trans_fee=></h1><br/>";
	...
	*/
	...
	foreach($post['select_transactions'] as $key=>$value){		//execute all trans row by row
		if($value){
			...
			if((($value['trans_status']==2 || $value['trans_status']==23)&&$value['trans_type']==19)||(trim($value['reference'])=='')||($tdate!=$cdate)||((isset($json_value['ar_tdate'])&&$json_value['ar_tdate'])||(isset($_REQUEST['tdateSkip'])))){
				$json_value_find=1;
			}
			...
			if($data['con_name']=='clk'){
				$trans_count=11;
			}elseif($value['acquirer']!=4&&(	(($value['trans_status']==2 || $value['trans_status']==23)&&($value['trans_type']==11)&&($cpost)&&($cpost['trans_status']==1)) || (($value['trans_status']==2 || $value['trans_status']==23)&&($value['trans_type']==11)&&(empty($cpost))) )&&$txn_fee_success_or_failed>0)
			{
			...
			elseif(($value['trans_status']==2)&&($value['trans_type']==15)){
				...
				//, `risk_ratio`='".$total_ratio."' not use 

				//update query
				$update_query .=" `trans_amt`='{$value['bill_amt']}',`trans_currency`='{$value['bill_currency']}', `buy_mdr_amt`='0', `buy_txnfee_amt`='0', `rolling_amt`='0', `mdr_cb_amt`='0', `mdr_cbk1_amt`='0', `mdr_refundfee_amt`='0', `payable_amt_of_txn`='0', `available_balance`='".$available_balance."'"; 
				...
				//update query
				$update_query .=" `bill_amt`='".$bill_amt."',`trans_amt`='".$trans_amt."', `buy_mdr_amt`='0', `buy_txnfee_amt`='0', `rolling_amt`='0', `mdr_cb_amt`='0', `mdr_cbk1_amt`='0', `mdr_refundfee_amt`='0', `payable_amt_of_txn`='0', `available_balance`='".$available_balance_sender."' ".$update_query_mas; 
			...
			elseif(($value['trans_status']==2 || $value['trans_status']==23)&&($value['trans_type']==11)&&($cpost)&&($cpost['trans_status']==1)){ // success to Cancelled manually in card and check 
			...
			elseif(($value['trans_status']==2 || $value['trans_status']==23)&&($value['trans_type']==11)&&(empty($cpost))){ // Cancelled in card 
			...
			elseif(($value['trans_status']==11)){ // CBK1 
				...
				//query for update
				$update_query .=" `trans_amt`='".$trans_amt."',`buy_mdr_amt`='0', `buy_txnfee_amt`='".$txn_fee_success."', `rolling_amt`='0', `mdr_cb_amt`='0', `mdr_cbk1_amt`='0', `mdr_refundfee_amt`='0', `payable_amt_of_txn`='".$payable_amt_of_txn."', `available_balance`='".$available_balance."' "; 
				...
				//query for update
				$update_query .=" `trans_amt`='".$trans_amt."',`buy_mdr_amt`='', `buy_txnfee_amt`='', `rolling_amt`='".$trans_amt."', `mdr_cb_amt`='', `mdr_cbk1_amt`='', `mdr_refundfee_amt`='', `available_rolling`='".$available_rolling."' "; 
				...
				//query for update
				$update_query .=" `trans_amt`='{$value['bill_amt']}',`trans_currency`='{$value['bill_currency']}', `buy_mdr_amt`='0', `buy_txnfee_amt`='0', `rolling_amt`='0', `mdr_cb_amt`='0', `mdr_cbk1_amt`='0', `mdr_refundfee_amt`='0', `payable_amt_of_txn`='0', `available_balance`='".$available_rolling."', `rolling_amt`='0', `available_rolling`='".$available_rolling."' "; 
			...
			if($update_query){
				...
				if(isset($data['TIME_TO_COMPLETION_TRANSACTION_SECONDS'])&&@$data['TIME_TO_COMPLETION_TRANSACTION_SECONDS']=='Y')
				{
					...
					$differseconds=humanizef(date_dif($value['created_date'], $now_micro, 'second')); // minute || second
					$differseconds=(int)str_ireplace([' second',' minutes'],'',$differseconds);

					$update_query.=", `runtime`='{$differseconds}'";
...
function delete_mer_setting($gid){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}mer_setting_id_seq', (SELECT MAX(id) FROM {$data['DbPrefix']}mer_setting)+1);"
		);
	}
}
...
function scrubbed_trans($uid,$account_type,$trans_id='',$email=''){
	...
	$min_limit=number_formatf2($min_limit);
	$max_limit=number_formatf2($max_limit);
	...
	if(($scrubbed_period>0)&&(!empty($max_limit))&&(!empty($email))){
		...
		$last_amt=number_formatf2($last_amt);
...
function create_new_trans($merID, $bill_amt, $acquirer, $trans_status,$fullname='', $bill_address='', $bill_city='', $bill_state='', $bill_zip='', $bill_email='', $ccno='', $bill_phone='', $product_name='', $source_url=''){
	...
	if(isset($data['cqp'])&&$data['cqp']>10) $qp=@$data['cqp'];
	...
	try {
		...
		$trans_type='11';
		$reference=trim($_SESSION['reference']);
		$terNO=trim($_SESSION['terNO']);
		...
		if(!empty($source_url)) $source_url=lf($source_url,236);
		if(!empty($webhook_url)) $webhook_url=lf($webhook_url,236);
		if(!empty($return_url)) $return_url=lf($return_url,236);
		
		$tdate_micro=$_SESSION['tdate_micro']=micro_current_date();
		...
		$qry="INSERT INTO `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`".
			...
			if(isset($data['cqp'])&&$data['cqp']==11){
				echo "<br/><br/><hr/><br/> qry <=create_new_trans=>";
				echo $qry;exit;
			}
...
function gen_transID_f($id=0,$acquirer=0,$upd=0){
	...
	if(isset($data['DB_CON'])&&@$data['DB_CON']) $DB_CON_COUNT=countNumericKeys(@$data['DB_CON']);
	$transID = $acquirer.$DB_CON_COUNT.$id.date("s");
...
//function for return date for compare of micro 6 digit time wise.
function dateReplace_2($date, $format="Y-m-d H:i:s")	
{
	$formattedDate = "";
...
//function for return date for compare of micro 6 digit time wise.
function dateReplace_3($date, $format="Y-m-d H:i:s")	
{
	$formattedDate = "";
...
//function for return date with micro 6 digit time wise.
function fetchFormattedDate_2($date, $format="Y-m-d H:i:s")	
{
	//if(empty($date) || $date=='0000-00-00') return;	 return $date;
	$formattedDate = "";
	...
	$formattedDate = dateReplace_2($date);
...
function get_blacklist_details($uid, $action='list', $post=array()){
	...
	if($action=='list')	//if action is list then fetch full active black list data
	{
		...
		{
			$countQ=db_rows(
				"SELECT count(*) AS count FROM `{$data['DbPrefix']}blacklist_data`".
				" WHERE $q `status`='1' LIMIT 1",$qprint
			);
	...
	elseif($action=='addnew')	//if action is addnew, then add new entry in blacklist table
	{
		...
		if(!empty($uid)&&@$uid>1) { $post['merchant_list_id'] = explode(",",$uid);}
...
function blacklist_scrubbed($uid)
{
	...
	//Initialized variables with null or empty
	$client_ip = $bill_ip = $client_ccno = $client_email = $client_country = $client_city = $client_phone = $client_vpa = "";

	//client bill_ip
	if(isset($_SESSION['bill_ip'])&&@$_SESSION['bill_ip'])		$client_ip		= @$_SESSION['bill_ip'];
	
	if(isset($post['bill_ip'])&&@$post['bill_ip'])			$bill_ip		= @$post['bill_ip'];
	...
	$blackListData=db_rows(
		"SELECT * FROM `{$data['DbPrefix']}blacklist_data`".
		" WHERE `clientid` IN ('0','1','$uid') AND `status`='1' ORDER BY blacklist_type",0
	);
	...
	if($blackListData)
	{
		foreach ($blackListData as $key => $val) {
			...
			//check IP address for client_ip
			...
			//check IP address for bill_ip
			if($blacklist_type=="IP"&&$bill_ip)
			{
				if($bill_ip==$blacklist_value)	//check via IP
				{
					$scrubbed_msg = "This IP address is black listed";
					$tran_db=true;
					break;
	...
	for($i=0;$i<count($strarr);$i++)
	{
		...
		if($str)
		{
			...
			if(is_numeric($dd)&&is_numeric($mm)&&is_numeric($yy))
			{
				...
				$new_date_title= prndates($date);
				$newstr .='<div class="rmk_row d-flex justify-content-start "><div class="rmk_date col-sm-2" style="width:109px!important;" data-title="'.$date.'" title="'.$new_date_title.'">'.$new_date.$str.'</div>';
			}
		}
	}
	return $newstr;
...
function set_default_email($gid){
	...
	if(isset($emails[0])&&$emails[0]){
		...
		if($data['connection_type']=='PSQL'){
			db_query(
				"SELECT setval('{$data['DbPrefix']}clientid_emails_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}clientid_emails`)+1);",0
			);
		}
	}
}
...
function update_confirmation($cid){
	...
	//echo $last_login_ip; 
	//delete 15-day old data from confirms table

	if($data['connection_type']=='PSQL')
		$unregistered_delete=" DELETE FROM `{$data['DbPrefix']}unregistered_clientid` WHERE `created_date` < NOW() - INTERVAL '15 days' ";
	else $unregistered_delete=" DELETE FROM `{$data['DbPrefix']}unregistered_clientid` WHERE (TO_DAYS(NOW())-TO_DAYS(`created_date`)>=15) ";

		
	db_query($unregistered_delete);
	...
	foreach($principal_exp as $principal_value){	//execute all rows one by one
		if($principal_value){
			...
			if($rows && $rows <= $i ){
				...
				$result['ims_type']=jsonvaluef($value,"ims_type");
...
function delete_clients($uid){
  ...
  json_log_upd($gid,'clientid_table','Delete',$dresult,'');	//update log
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}clientid_table_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}clientid_table`)+1);",0
		);
	}
...
function sub_admin_details($ajax=0){
	...
	if((isset($_SESSION['sub_admin_id']))&&($_SESSION['get_mid']!='M. All')){
		...
		$query_mid ="  WHERE  ( id IN ({$sub_admin_id}) ) OR ( `access_id`='{$sub_admin_id}' OR `multiple_merchant_ids` LIKE '%,{$get_mid},%' OR `multiple_subadmin_ids` LIKE '%,{$sub_admin_id},%'  )  ";
...
//Dev Tech: 24-12-30 List for Account manager ID 
function account_manager_id($id=0,$list_type=0)
{
	global $data; $pre_qr=''; $limit='';
	if($id>0){
		$pre_qr=" AND id={$id}";
		$limit=' LIMIT 1 ';
	}
	//$post['account_manager_option']=array();
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y'))
	{
		$select_pt=db_rows(
			"SELECT * FROM {$data['DbPrefix']}account_manager".
			" WHERE manager_status=1 ".$pre_qr.
			" ORDER BY id ASC {$limit} ",0
		);
		foreach($select_pt as $key=>$value){
			
			$opt_text=$value['manager_name'].' | '.$value['contact_number'].' | '.$value['bcc_name'].' | '.$value['bcc_email'].' | '.$value['skype_id'].' | '.$value['whatsapp_no'].' | '.$value['telegram_no'];
			if($list_type==2) @$post['account_manager_option'] .= "<option value=\"{$value['id']}\">{$opt_text}</option>";
			else  @$post['account_manager_option'][$value['id']]=$opt_text;
		}	
	}
	return @$post['account_manager_option'];
}


//Dev Tech: 24-12-30 view for Account manager ID 
function account_manager_fetch($id,$is_admin=0)
{
	global $data; $result=[];
	
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y')&&$id>0)
	{
		$select_pt=db_rows(
			"SELECT * FROM {$data['DbPrefix']}account_manager".
			" WHERE id={$id} ".
		...
		foreach($select_pt as $key=>$value){
			...
			$result['manager_name']=$value['manager_name'];
			$result['contact_number']=$value['contact_number'];

			if($is_admin==1)
			{
				//bcc_name, bcc_email , BCC Name , BCC Email

				if(trim($value['bcc_name'])) $result['am_array'][]='<a href="tel:+'.$value['bcc_name'].'"><i class="fa fa-user-check text-info fa-fade" title="BCC Name"></i> '.$value['bcc_name'].'</a>';
				if(trim($value['bcc_email'])) $result['am_array'][]='<a href="mailto:'.$value['bcc_email'].'"><i class="fa fa-envelope-open-text text-info fa-fade" title="BCC Email Address"></i> '.$value['bcc_email'].'<hr style="margin: 17px 0 0 0;"/></a>';
			}
			
			if(trim($value['contact_number'])) $result['am_array'][]='<a href="tel:+'.$value['contact_number'].'"><i class="fa fa-headphones text-info fa-fade" title="Contact"></i> '.$value['contact_number'].'</a>';
			if(trim($value['email_id'])) $result['am_array'][]='<a href="mailto:'.$value['email_id'].'"><i class="fa fa-envelope-open-text text-info fa-fade" title="Email Address"></i> '.$value['email_id'].'</a>';
			if(trim($value['skype_id'])) $result['am_array'][]='<a href="skype:'.$value['skype_id'].'?chat"><i class="fa-brands fa-skype text-info fa-fade" title="Skype"></i> '.$value['skype_id'].'</a>';
			if(trim($value['whatsapp_no'])) $result['am_array'][]='<a href="https://wa.me/'.$value['whatsapp_no'].'" target="_blank"><i class="fa-brands fa-whatsapp text-success fa-fade" title="WhatsApp"></i> '.$value['whatsapp_no'].'</a>';
			if(trim($value['telegram_no'])) $result['am_array'][]='<a href="https://t.me/'.$value['telegram_no'].'" target="_blank"><i class="fa-brands fa-telegram text-info fa-fade" title="Telegram"></i> '.$value['telegram_no'].'</a>';

		}	
	}

	if(isset($result[0]['json_log_history'])) unset($result[0]['json_log_history']);
	if(isset($result['am_array'])){ 
		
		$am_icon=implode("<br/>", @$result['am_array']);
		$result['am_icon']=@$am_icon;
	}
	return $result;
}
...
function select_sold($uid,$store_id){
	...
	$trans=db_rows("SELECT SUM(`trans_amt`) AS `summ`, COUNT(`id`) AS `count`".
		" FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`".
		" WHERE (`trans_status` IN (1,7) ) AND `terNO` IN ({$store_id}) ".$where_pred." LIMIT 1",0);
...
//echo $post_email.'---'.$post['registered_email'];exit;
	if($admin){
		...
		$date_fetch=(new DateTime())->format('Y-m-d H:i:s');

		$is_active='1';
		$adm_fld=",`status`,`ip_block_client`,`edit_permission`,`default_currency`,`description`,`password_updated_date`";
		$adm_pst=",'2','1','{$post['edit_permission']}','{$post['default_currency']}','{$post['description']}','{$date_fetch}'";
	...
	$newid=newid(); //fetch newly added client id

	
	
	if($newid>0){

		//[confcode] for reset passward clientID is encoded 
		$post['ccode']=$post['confcode']=$data['ccode']=encode_f($newid,0);
...
function acquirer_tablef($list=0){
	...
	$select_pt=db_rows(
		"SELECT `acquirer_status`,`acquirer_id`,`acquirer_name`,`channel_type`,`acquirer_refund_policy`,`default_acquirer`,`mop` FROM `{$data['DbPrefix']}acquirer_table` WHERE `acquirer_status` NOT IN (0) ",0
	);
	...
	foreach($select_pt as $key=>$value){
		if($list==1){
			...
			$result['mop'][$value['acquirer_id']]=$value['mop'];
		}else{
...
//if admin or merchant then fetch MOP
if((!isset($_SESSION['actDb_mop']))&&(isset($_SESSION['adm_login']) || isset($_SESSION['login'])))
{
	$actDb_mop=@acquirer_tablef(10);
	$_SESSION['actDb_mop']=$actDb_mop['mop'];
}
