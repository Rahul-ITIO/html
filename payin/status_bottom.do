...
if(isset($transID)&&!empty($transID)&&$transID>0)
{
	...
	//start: cron step: 1 -----------------------------------------
		...
		if( (@$is_expired=='Y') && ((!isset($_SESSION['acquirer_status_code']))||(isset($_SESSION['acquirer_status_code'])&&$_SESSION['acquirer_status_code']==1)) && (!isset($_REQUEST['cron_host_response'])) )
		{	
	...
	if(isset($_SESSION['bank_processing_amount'])&&$_SESSION['bank_processing_amount']&&isset($_SESSION['responseAmount'])&&$_SESSION['responseAmount']&&$_SESSION['bank_processing_amount']<>""&&isset($_SESSION['acquirer_status_code'])&&$_SESSION['acquirer_status_code']==2){
		...
		if(isset($_REQUEST['action'])&&trim($_REQUEST['action'])) {
			$data_send['action']=@$_REQUEST['action'];
			$subQuery=$subQuery."&action=".@$_REQUEST['action'];
		}
		
		
		//bug check and display query for dev
		if(isset($_REQUEST['cqp'])&&trim($_REQUEST['cqp'])) {
			$data_send['cqp']=@$_REQUEST['cqp'];
			$subQuery=$subQuery."&cqp=".@$_REQUEST['cqp'];
		}
	...
	//end: cron step: 1 -----------------------------------------
		...
		if($data_send['acquirer_status_code']==2){ //success
			$return_url = $host_path."/return_url{$r2}{$data['ex']}?transID=$transID".@$subQuery;
		...
		elseif($data_send['acquirer_status_code']==1){ //pending
			...
			{
				$return_url = @$host_path."/trans_processing{$r2}{$data['ex']}?transID=".@$transID."".@$subQuery;
			}
...
#######################################################################################################
//Dev Tech : 2024-07-21 if cron_host_response for exit & not redirect when not empty trans_response & trans_status is 0 & acquirer response is 0 via 1 = pending and 1 not in  trans_status & not in admin session 
// Bug fix for delay from cron instance via crontab in ubuntu 

	if(((isset($_REQUEST['cron_host_response'])&&trim($_REQUEST['cron_host_response']))||(isset($_REQUEST['webhook_id'])&&trim($_REQUEST['webhook_id']))||(isset($_REQUEST['action'])&&@$_REQUEST['action']=='webhook'))&&isset($_SESSION['acquirer_status_code'])&&@$_SESSION['acquirer_status_code']) 
	{

		if(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==2||$_SESSION['acquirer_status_code']=="2")){
			$trans_status_update=1;
		}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==1||$_SESSION['acquirer_status_code']=="1")){
			$trans_status_update=0;
		}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==22||$_SESSION['acquirer_status_code']==23)){
			$trans_status_update=$_SESSION['acquirer_status_code'];
		}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']<1)){
			$trans_status_update=2;
		}

		if(!empty(trim($td['trans_response']))&&((@$td['trans_status']==$trans_status_update) || (@$td['trans_status']==1))&&(!isset($_SESSION['adm_login']))) 
		{
			db_disconnect();
		...
		if($td['trans_status']==1&&!isset($_SESSION['adm_login']))
		{
			...
			echo "Not required to return url because all ready success !!!!";
			db_disconnect();
			exit;
		}
			...
			else
			{
				if((isset($_REQUEST['actionurl'])&&$_REQUEST['actionurl']=='notify')||(isset($_REQUEST['action'])&&($_REQUEST['action']=='notify' || $_REQUEST['action']=='webhook')))
				{
					if(isset($_REQUEST['cron_host_response'])&&($td['trans_status']==1||$td['trans_status']==2)){
						...
						//Dev Tech : 24-07-27 check log from postman as a webhook 
						if(@$transID=='5115409140'|| isset($_REQUEST['wlog']))
						{
							
							echo "<br/><hr/><br/>action=><br/>"; print_r(@$_REQUEST['action']);
							echo "<br/><hr/><br/>return_url=><br/>".@$return_url;
							echo "<br/><hr/><br/>_REQUEST=><br/>"; print_r(@$_REQUEST);
							echo "<br/><hr/><br/>use_curl=><br/>"; print_r(@$use_curl);
							echo "<br/><hr/><br/>data_send=><br/>"; print_r(@$data_send);
						}
