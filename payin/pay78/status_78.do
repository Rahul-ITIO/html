...
$time_log=array();
$time_log['log_step_0']=(new DateTime())->format('Y-m-d H:i:s.u');

$is_curl_on = true;
if(!isset($data['STATUS_ROOT'])){
	$time_log['webhook_step_1']=(new DateTime())->format('Y-m-d H:i:s.u');
	include($root.'config_db.do');
	$str=file_get_contents("php://input");
	$res_arr = json_decode($str, true);
	$object_input=$res_arr;
	$time_log['webhook_step_2']=date('Y-m-d H:i:s');
	$res_message='';
	...
	if(isset($res_arr[0]['webhook_id'])&&$res_arr[0]['webhook_id']) $_REQUEST['webhook_id']=$res_arr[0]['webhook_id'];
	elseif(isset($res_arr['webhook_id'])&&$res_arr['webhook_id']) $_REQUEST['webhook_id']=$res_arr['webhook_id'];
	...
	if(isset($res_message)&&$res_message){
		...
		$time_log['webhook_step_3']=(new DateTime())->format('Y-m-d H:i:s.u');
	}

	include($data['Path'].'/payin/status_top'.$data['iex']);
	$time_log['webhook_step_4_for_find_trans_from_db']=(new DateTime())->format('Y-m-d H:i:s.u');
}
...
if(!empty($transaction_id))
{
	...
	$time_log['status_step_start_1']=(new DateTime())->format('Y-m-d H:i:s.u');
		...
		if(empty($acquirer_status_url)) $acquirer_status_url='https://merchantprod.timepayonline.com/evok/qr/v1/qrStatus';

		/*
		$shttp='https';
		$surl='merchantuat.timepayonline.com';
		if(isset($_REQUEST['url'])&&$_REQUEST['url']){
			$shttp='http';
			$surl=$_REQUEST['url'];
		}
		$acquirer_status_url="{$shttp}://{$surl}/evok/cm/v2/status";
		*/
		...
		if(isset($_REQUEST['url'])&&$_REQUEST['url']){
			$shttp='http:';
			$surl=$_REQUEST['url'];
			$acquirer_status_url=str_replace(['https:','merchantprod.timepayonline.com'],[$shttp,$surl],$acquirer_status_url);
		}
	...
	$req = [
		...
		curl_setopt_array($curl, array(
			...
			$time_log['status_step_fetch_from_acquirer_2']=(new DateTime())->format('Y-m-d H:i:s.u');	
	}
	...
	#######	upa, rrn, acquirer_ref update from status get :start 	###############
		...
		elseif(@$res['data'][0]['extTransactionId'])  $acquirer_ref= @$res['data'][0]['extTransactionId'];
		elseif(@$res['data'][0]['txnId'])  $acquirer_ref= @$res['data'][0]['txnId'];
		elseif(@$res['txnId'])  $acquirer_ref= @$res['txnId'];
		//up acquirer_ref : update if empty acquirer_ref and is upiTxnId 
		if((empty(trim($td['acquirer_ref']))||$td['acquirer_ref']=='{}')&&!empty($acquirer_ref)){
	...
	#######	upa, rrn, acquirer_ref update from status get :end 	###############
		...
		if($res['status']&&@$mopType=='QRINTENT') 
			$message	= $res['status'];
		...
		//elseif($res['status']&&@$mopType=='QRINTENT') $message	= $res['status'];
		...
		$time_log['status_check_for_retrun_url_1']=(new DateTime())->format('Y-m-d H:i:s.u');	
			...
			#######################################################################################################
			//Dev Tech : 2024-07-23 if fetch the acquirer status code then not redirect when not empty trans_response & db trans_status & acquirer response is match or db trans_status is 1-approved, 2-declined  & not in admin session 
			// Bug fix for delay from cron instance via crontab in ubuntu 

				if(!isset($_SESSION['adm_login'])&&isset($_SESSION['acquirer_status_code'])&&@$_SESSION['acquirer_status_code']) 
				{

					if(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==2||$_SESSION['acquirer_status_code']=="2")){
						$trans_acquirer_status_code=1;
					}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==1||$_SESSION['acquirer_status_code']=="1")){
						$trans_acquirer_status_code=0;
					}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']==22||$_SESSION['acquirer_status_code']==23)){
						$trans_acquirer_status_code=$_SESSION['acquirer_status_code'];
					}elseif(isset($_SESSION['acquirer_status_code'])&&($_SESSION['acquirer_status_code']<1)){
						$trans_acquirer_status_code=2;
					}

					if(!empty(trim($td['trans_response']))&&((@$td['trans_status']==$trans_acquirer_status_code) || (@$td['trans_status']==1 || @$td['trans_status']==2))&&(!isset($_SESSION['adm_login']))) 
					{
						//echo "Not required to return url because status is match !!!!";
	...
	*/
	$time_log['status_check_for_retrun_url_2']=(new DateTime())->format('Y-m-d H:i:s.u');	
}

#######################################################

if(!isset($data['STATUS_ROOT'])){
	$time_log['status_go_to_block_for_status_bottom_1']=(new DateTime())->format('Y-m-d H:i:s.u');	
