...
//'secretkey']=(isset($data['apJson']['secretkey'])?$data['apJson']['secretkey']:'');
...
if(!empty($transID))
{
   ...
   $postData['prepayId']			= $acquirer_ref;
	$postData['merchantTradeNo']	= $transID;
...
//exit;
	...
	if (isset($res) && count($res)>0)
	{
		...
		{
			if( strtoupper($status)=='SUCCESS' || strtoupper($status)=='PAID' ){ //success
