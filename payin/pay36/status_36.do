...
if((!isset($json_value['client_id'])||empty($json_value['client_id']))&&(isset($td['merID'])&&trim($td['merID']))){
	$ac_ms=mer_settings($td['merID'], 0, true, $td['type']);
...
if(!empty($transID))
{
	...
	if (isset($responseParamList) && count($responseParamList)>0)
	{
		...
		if(isset($responseParamList['results'][0]['customeridentIfication'])&&$responseParamList['results'][0]['customeridentIfication']&&empty($td['upa']))
		{
			$tr_upd_order=array();
			$tr_upd_order['upa']=$responseParamList['results'][0]['customeridentIfication'];	// store payer vpa
			trans_updatesf($td['id'], $tr_upd_order);		//update json_values
		}
