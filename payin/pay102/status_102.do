...
if(!empty($transID))
{
	...
	*/
	...
	if($is_curl_on)
	{
		...
		$acquirer_status_url	= @$acquirer_status_url.'/'.$merchant_id.'/order/'.@$transID.'/transaction/trans'.$transID;  	
	...
	*/
	...
	if (isset($responseParamList) && count($responseParamList)>0)
	{
		...
		/*
			...
			//up acquirer_ref : update if empty acquirer_ref_1 and is ["transaction"]["acquirer"]["batch"] 
