...
if(isset($apc_get['terNO'])&&$apc_get['terNO']) $bank_terNO=$apc_get['terNO'];
	...
	$curlPost["return_url"]=$status_default_url;
	...
	$tr_upd_order1['transID_acquirer_ref']=@$results["transID"];
	...
	if((isset($results["Error"]) && ($results["Error"]))||(isset($results["error"]) && ($results["error"])))
	{
		...
		if(@$results["Error"]) $error_description.=@$results["Error"];
		if(@$results["error"]) $error_description.=@$results["error"];
		
		if(@$results["Message"]) $error_description.=' - '.@$results["Message"];
		elseif(@$results["message"]) $error_description.=' - '.@$results["message"];
		elseif(@$response) $error_description.=' - '.htmlentitiesf(@$response);

		$return_response_arr['Error']=$acquirer;
		$return_response_arr['Message']=$_SESSION['acquirer_response']=@$error_description;
	...
	if(isset($results["authurl"]) && ($results["authurl"])){ //3D Bank data on Host URL
		...
		//if($acquirer==105) $auth_3ds2_action='redirect_base64';

		if(isset($apc_get['redirect_action'])&&$apc_get['redirect_action']) $auth_3ds2_action=$apc_get['redirect_action'];
...
##########################################################################
	...
	if(isset($_SESSION['b_'.$acquirer]['billing_descriptor'])&&$_SESSION['b_'.$acquirer]['billing_descriptor']) $_SESSION['acquirer_descriptor']=@$_SESSION['b_'.$acquirer]['billing_descriptor'];
