...
if(isset($data['DB_CON'])&&isset($_REQUEST['DBCON'])&&trim($_REQUEST['DBCON'])&&function_exists('config_db_more_connection'))
{
	...
	$link_db=config_db_more_check_link($DBCON,$dbad,$dbmt);
...
$host_path=$data['Host'];
	...
	if((isset($_REQUEST['admin'])&&$_REQUEST['admin'])||(isset($_REQUEST['actionurl'])&&$_REQUEST['actionurl']=="admin_direct"))
	{
		$is_admin=true;
	...
	if(isset($_REQUEST['cron_tab'])&&$_REQUEST['cron_tab']){
		$subQuery.='&destroy=2&cron_tab='.$_REQUEST['cron_tab'].@$dbad_link_2;
	}
	
	/* 
	if(isset($_GET['cron_host_response'])){
		$subQuery.='&cron_host_response='.$_GET['cron_host_response'];
	}
	*/
	if(isset($_REQUEST['acquirer_retrun_json_response'])&&$_REQUEST['acquirer_retrun_json_response']){
		$subQuery.='&acquirer_retrun_json_response='.$_REQUEST['acquirer_retrun_json_response'];
	}
	...
	if(isset($_REQUEST['session_destroy'])&&$_REQUEST['session_destroy'])
	{
		$subQuery.="&session_destroy=".$_REQUEST['session_destroy'];
	}
...
if((isset($_REQUEST['transID'])&&!empty($_REQUEST['transID']))){
	...
	if(isset($data['ACQUIRER_REF_ENABLE'])&&$data['ACQUIRER_REF_ENABLE']=='Y') $where_pred.=" (`transID`={$transID} OR {$data['DbPrefix']}{$data['ASSIGN_MASTER_TRANS_ADDITIONAL']}.`acquirer_ref`='{$transID}') AND";

	else $where_pred.=" (`transID`={$transID}) AND";
...
if((isset($_REQUEST['cron_host_response'])||isset($_GET['cron_host_response'])||isset($_REQUEST['webhook_id'])||isset($_GET['webhook_id']))&&($td['trans_status']==1||$td['trans_status']=='1'||$td['trans_status']==2||$td['trans_status']=='2'))
{
	echo $data_send['log_cron_via']='all_ready_send';
	exit;
}


//Dev Tech:24-12-23 Update acquirer_ref, rrn is action ?transID=&acquirer_ref_update=&rrn=
if(isset($_REQUEST['acquirer_ref_update'])&&(@$td['trans_status']!=1))
{

	$force_update='N';

	if(isset($_REQUEST['action'])&&$_REQUEST['action']=='force_update') $force_update='Y';

	//acquirer_ref update
	$acquirer_ref_upd='';
	if(isset($_REQUEST['acquirer_ref_update'])&&!empty($_REQUEST['acquirer_ref_update'])&&trim($_REQUEST['acquirer_ref_update'])) $acquirer_ref_upd = @$_REQUEST['acquirer_ref_update'];
	if((empty($td['acquirer_ref'])||$td['acquirer_ref']=='{}'||$force_update=='Y')&&(!empty($acquirer_ref_upd)&&$acquirer_ref_upd!=NULL)){
		$acquirer_ref_upd=trim($acquirer_ref_upd);
		//echo "<br/>acquirer_ref_upd=>".$acquirer_ref_upd;
		db_trf(@$td['id'], 'acquirer_ref', $acquirer_ref_upd);
	}

	//rrn update
	$rrn_upd='';
	if(isset($_REQUEST['rrn'])&&!empty($_REQUEST['rrn'])&&trim($_REQUEST['rrn'])) $rrn_upd = @$_REQUEST['rrn'];
	if((empty($td['rrn'])||$force_update=='Y')&&(!empty($rrn_upd)&&$rrn_upd!=NULL)){
		$rrn_upd=trim($rrn_upd);
		//echo "<br/>rrn_upd=>".$rrn_upd;
		db_trf(@$td['id'], 'rrn', $rrn_upd);
	}
...
if(isset($td['acquirer'])&&$td['acquirer']>0){
	...
	// `id`,`acquirer_status`,`acquirer_id`,`default_acquirer`,`acquirer_name`,`notification_email`,`acquirer_descriptor`,`acquirer_prod_mode`,`acquirer_uat_url`,`acquirer_status_url`,`acquirer_refund_url`,`cron_bank_status_response`,`mer_setting_json`,`acquirer_processing_creds`,``,``,``,``

	$acquirer_table_db=db_rows(
		"SELECT `id`,`acquirer_status`,`acquirer_id`,`default_acquirer`,`acquirer_name`,`notification_email`,`acquirer_descriptor`,`acquirer_prod_mode`,`acquirer_uat_url`,`acquirer_status_url`,`acquirer_refund_url`,`cron_bank_status_response`,`mer_setting_json`,`acquirer_processing_creds` FROM `{$data['DbPrefix']}acquirer_table`".
		" WHERE `acquirer_id`=".$td['acquirer'].
		" ORDER BY id DESC LIMIT 1",$qp
	);
	...
	$default_acquirer=@$acquirer_table['default_acquirer'];
	$apc_get_a=isJsonDe(@$acquirer_table['acquirer_processing_creds']);

	if(isset($apc_get_a['live']))	$apc_get_al=$apc_get_a['live'];
...
//Dev Tech : 2024-03-11 for check the status url is working or not than email for admin - start and if found value from Bank Status Path via br_status_path ------------------

$br_status_path = @$bank_acquirer_json_arr['br_status_path'];
if(isset($br_status_path)&&!empty(trim($br_status_path))&&strtoupper($br_status_path)!='NA'&&(isset($data['ACQUIRER_STATUS_CHECK_EMAIL_TO_ADMIN'])&&@$data['ACQUIRER_STATUS_CHECK_EMAIL_TO_ADMIN']=='Y'))
{
...
if($qp){
	...
	echo '<div type="button" class="btn btn-success my-2" style="background: #fff2d4;color:#2c2c2c;padding:5px 10px;border-radius:2px;margin:10px auto;width:fit-content;display:block;max-width:99%;word-wrap:anywhere;">';
	
	echo "<hr>Acquirer Table : apc_get_a=><br/><br/>";
	print_r(@$apc_get_a);

	echo "<br/><br/><hr>step apc_get=> ".$step_apc_get;
	echo "<hr>Trans. Table apc_get=><br/><br/>";print_r(@$apc_get);
	
	if($qp==2) { 
		echo "<hr>json_value=><br/><br/>"; print_r(htmlentitiesf(@$json_value)); 
	}
...
if(isset($data['retrun_via_status'])&&@$data['retrun_via_status']) $retrun_via_status=$data['retrun_via_status'];

// If trans_status is above of 0 than retrun via status  
if(isset($td['transID'])&&trim($td['transID'])&&isset($td['acquirer'])&&$td['acquirer']>0&&@$td['trans_status']>0&&@$retrun_via_status=='Y' && (!isset($_REQUEST['cron_host_response'])))
{
	...
	if(isset($_REQUEST['cron_tab'])&&$_REQUEST['cron_tab']){
		...
		$fetch_trnsStatus_url=$host_path."/fetch_trnsStatus".$data['ex']."?transID=".@$td['transID'];

		$valid_data=array();
		$valid_data['transID']=@$td['transID'];

		/*
		if(isset($json_value['get']['public_key'])&&trim($json_value['get']['public_key'])) $valid_data['public_key']=trim($json_value['get']['public_key']);
		elseif(isset($json_value['post']['public_key'])&&$json_value['post']['public_key']) $valid_data['public_key']=$json_value['post']['public_key'];
