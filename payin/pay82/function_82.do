...
echo $passphrase = $apc_get['Header_Encryption_Key'];
...
abstract class CryptoJSAES {
     ...
     */
    public static function encrypt($headeRequest, $passphrase, $salt = null) {
        $salt = $salt ?: openssl_random_pseudo_bytes(8);
        list($key, $iv) = self::evpkdf($passphrase, $salt);

        $ct = openssl_encrypt($headeRequest, 'aes-256-cbc', $key, true, $iv);
