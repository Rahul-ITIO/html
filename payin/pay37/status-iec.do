<?php 
        ...
        // Credentials
		$client_id			= $siteid_get['IBL-Client-Id'];
		$client_secret		= $siteid_get['IBL-Client-Secret'];
		$apiUrl				= "https://indusapiuat.indusind.com/indusapi-np/uat/iec/etender/updateTenderId/v1";//$bank_status_url;
		$CustomerTenderId	= $siteid_get['CustomerTenderId'];
		$CustomerchannelId	= $siteid_get['channelId'];

//print_r($siteid_get);exit;
$transaction_id=21811;
        $Payload = [
            'request' => [
                'header' => [
                    'requestUUID' => time(),
...
print_r($Payload);
   ...
   //'indusinduat-publickey.txt'
        $key = JWKFactory::createFromKeyFile(
			'payin/pay'.$midcard.'/'.$siteid_get['mode'].'_indusapi.indusind.com-sscert.pem',
