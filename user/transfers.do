<?
$data['PageName']	= 'Transfers';
$data['PageFile']	= 'transfers';

###############################################################################
include('../config.do');
$data['PageTitle'] = 'Transfers - '.$data['domain_name']; 
##########################Check Permission#####################################
...
if(isset($post['action'])&&$post['action']=='generate_payout_token'){
	...
	db_query("UPDATE `{$data['DbPrefix']}payout_setting`"." SET `payout_token`='{$payout_token}' WHERE `clientid`='{$uid}'");
...
if(isset($post['step'])&&$post['step']==1){
	...
	$sql_query.=" AND `transaction_for`='3'";		
	////////////// for paging /////////////////
	...
	// Add  AND `transaction_for`='3' in main query for Transfer Transaction
