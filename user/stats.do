...
$whereqrtotal=" `merID` IN ('$uid') AND ( `trans_type` IN (11) ) AND (".date_format_return('`tdate`',1).") >=  '$date_1st'  AND (".date_format_return('`tdate`',1).") <=  '$date_2nd'  AND (`trans_status` IN (2)) LIMIT 1 ";

$fail=trans_count_where_pred($whereqrtotal);

$whereqractive=" `merID` IN ('$uid') AND ( `trans_type` IN (11) ) AND (".date_format_return('`tdate`',1).") >=  '$date_1st'  AND (".date_format_return('`tdate`',1).") <=  '$date_2nd' AND (`trans_status` IN (1,7)) LIMIT 1 ";

$succ=trans_count_where_pred($whereqractive);  // New Created Function 
