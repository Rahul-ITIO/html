<?
###############################################################################
$data['PageName']='Qr Code Statement';
$data['PageFile']='qr_code_statements';
$data['PageFileName']='Qr Code Statement';
###############################################################################
include('../config.do');
...
if(isset($post['step'])&&$post['step']==1){
	...
	$p_order_by=" `transaction_date` DESC ";
	...
	////////////// for paging /////////////////
	$sqlStmt	= "SELECT count(id) as ct FROM `{$data['DbPrefix']}softpos_setting_transaction` WHERE $sql_query";
	$result_ct	= db_rows($sqlStmt,0);
	$data['total_record']= $result_ct[0]['ct'];
	////////////// End paging /////////////////

   $sqlStmt = "SELECT * FROM `{$data['DbPrefix']}softpos_setting_transaction` WHERE $sql_query ORDER BY $p_order_by $limit";
