...
$data['ThisPageUrl'] = 'trans_withdraw-fund_v3_custom_settlement';
...
$data ['PageFile'] = 'withdraw-fund_wd_v3';

##############################################################################

$post_session_check=1;
...
$instancePrivateIpGet = gethostbyname(gethostname());
$merchant_settlement_private_ip_allow='merchant_settlement_private_ip_allow.php';
//if(isset($_REQUEST['secureCron'])&&@$_REQUEST['secureCron']==@$instancePrivateIpGet&&file_exists($merchant_settlement_private_ip_allow)) include($merchant_settlement_private_ip_allow);


//mn2 cron auto create withdraw then check more conection for default auto connect db 
if( (isset($_SERVER['HTTP_REFERER'])) && ((strpos($_SERVER['HTTP_REFERER'],'include/csv_data_click')!==false) || (strpos($_SERVER['HTTP_REFERER'],'signins/merchant_settlement')!==false) || (isset($data['secureCron'])) ) )
{
	$data['DBCON_DEFAULT_ROOT']='WV3_CURL'; // check more db connection is default 

	//Showing only Fee : SUM(`gst_amt`) 'total_gst_fee', SUM(`buy_mdr_amt`) AS `buy_mdr_amt`, SUM(`buy_txnfee_amt`) AS `buy_txnfee_amt` and using for cron base 
	$data['SHOW_FEE_ONLY_ENABLE_IN_WV3_CUSTOM']='Y';
	$post_session_check=0;


	//via cron of instance 
	if(isset($data['secureCron'])){
		 $_SERVER['HTTP_REFERER']='signins/merchant_settlement';
		// $data['secureCron']=@$data['secureCron'];
	}
...
/*
echo "\n _REQUEST secureCron v3=>".@$_REQUEST['secureCron']."\n";
echo "\n data secureCron v3=>".@$data['secureCron']."\n";
echo "\n HTTP_REFERER v3=>".@$_SERVER['HTTP_REFERER']."\n";
print_r($_REQUEST);
*/
...
if(isset($data['ROLLING_V3_CUSTOM_ENABLE'])&&$data['ROLLING_V3_CUSTOM_ENABLE']=='Y') $rolling_v3_custom_enable=1;
else $rolling_v3_custom_enable=0;
...
if(!isset($data['ThisTitle'])){
	$data ['ThisTitle'] = 'Withdraw Funds V3';
}
...
$_REQUEST['dtest']=1; $_REQUEST['cqp']=5; 
...
//print_r($_SESSION['uid_wv2'.$uid]['ab']);exit;

//if(isset($_REQUEST['action'])&&@$_REQUEST['action']=='convertamt')
if((isset($_REQUEST['action'])&&$_REQUEST['action']=="convertamt")&&($convertamtAction)&&(isset($_REQUEST['amount']))&&(isset($_SESSION['uid_wv2'.$uid]['s_amount'])))
{
 	 
	$from_currency_2 = $_SESSION['uid_wv2'.$uid]['ab']['ab']['account_curr'];

	//print_r($_SESSION['uid_wv2'.$uid]['ab']);exit;
	
	$to_currency_2 = $_REQUEST['requested_currency'];
	$amount_get=$_REQUEST['amount'];
	$withdraw_fee=$_REQUEST['withdraw_fee'];
	if(@$noFeeRequired) {
		$withdraw_fee=0;
	...
	$json["summ_mature_amt"]=number_formatf_2($_SESSION['uid_wv2'.$uid]['ab']['ab']['summ_mature_amt']); 
...
if(isset($data['settlement_optimizer_available'])&&$data['settlement_optimizer_available']=='Y'){
	if((isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&(strtolower($post['settlement_optimizer'])=='manually'||strtolower($post['settlement_optimizer'])=='custom'||strtolower($post['settlement_optimizer'])=='weekly'))||($is_admin&&isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&(strtolower($post['settlement_optimizer'])=='manually'||strtolower($post['settlement_optimizer'])=='custom'||strtolower($post['settlement_optimizer'])=='weekly'||strtolower($post['settlement_optimizer'])=='no_settlement'))){
...
if(!isset($_SESSION['uid_wv2'.$uid]['payout']))
{
	...
	$_SESSION['uid_wv2'.$uid]['payout']=$payout = payout_trans_newf_wv3_custom($uid,$post['type']);
...
if($uid_session_true)
{
	...
	$ab=trans_balance_wv3_custom($uid,@$post['type']);
...
//$post['ab']['account_curr']=@$post['default_currency'];
//$post['ab']['account_curr_sys']=get_currency(@$post['default_currency']);


if($cp)
{
    echo "<br/><hr/><br/><h1><==AB==></h1><br/>";
    print_r($ab_uid);
    echo "<br/><hr/><br/>";
	echo "<br/><b style='color:#e60000;'>last_withdraw_micro_current_date_=> ".$_SESSION['last_withdraw_micro_current_date_'.$uid]."</b>";
	echo "<br/>last_withdraw_micro_current_date=>".@$post['last_withdraw_micro_current_date'];

	echo "<br/><br/><b style='color:#026402;'>withdraw_from_date=> ".@$post['withdraw_from_date']."</b>";
	echo "<br/><b style='color:#0933b0;'>withdraw_to_date=> ".$post['withdraw_to_date']."</b>";

	echo "<br/><hr/><br/>last_withdraw_remaining_balance=>".@$post['last_withdraw_remaining_balance'];
	echo "<br/>ab summ_mature_amt=>".@$post['ab']['summ_mature_amt'];

	echo "<br/><br/><b style='color:#0933b0;'>total_month_no=>".@$post['total_month_no']."</b>";
	echo "<br/><b style='color:#0933b0;'>per_monthly_fee=>".@$post['per_monthly_fee']."</b>";
	echo "<br/><b style='color:#0933b0;'>total_monthly_fee=>".@$post['total_monthly_fee']."</b>";

	
	echo "<br/><hr/><b style='color:#e60000;'>previous_transID=> ".@$post['previous_transID']."</b>";
	echo "<br/><b style='color:#026402;'>last_withdraw_transID=> ".@$post['last_withdraw_transID']."</b>";
	echo "<br/><b style='color:#062377;'>last_withdraw_tdate=> ".@$post['last_withdraw_tdate']."</b>";
	echo "<hr/><b style='color:#9d7c05;'>first_success_tdate=> ".@$post['first_success_tdate']."</b>";
	echo "<br/><b style='color:#9d7c05;'>first_success_transID=> ".@$post['first_success_transID']."</b>";

	echo "<br/><b style='color:#026402;'>ab account_curr=> ".@$post['ab']['account_curr']."</b>";
...
if(isset($post['ab']['summ_immature_amt'])) $json_values['immature_fund'] = @$post['ab']['summ_immature_amt'];
if(isset($post['ab']['summ_withdraw_amt'])) $json_values['total_withdraw_made'] = @$post['ab']['summ_withdraw_amt'];

if(isset($post['ab']['summ_mature_roll'])) $json_values['mature_rolling_fund'] = @$post['ab']['summ_mature_roll'];
if(isset($post['ab']['summ_total_roll'])) $json_values['rolling_balance'] = @$post['ab']['summ_total_roll'];
if(isset($post['ab']['summ_immature_roll'])) $json_values['immature_rolling_fund'] = @$post['ab']['summ_immature_roll'];
if(isset($post['ab']['summ_withdraw_roll'])) $json_values['withdraw_rolling_fund'] = @$post['ab']['summ_withdraw_roll'];
...
$post['monthly_fee_rate'] = @$post['per_monthly_fee'];

// echo "<hr/>total_month_no=>".$post['total_month_no']; echo "<hr/>monthly_fee=>".$post['monthly_fee'];

//$post['monthly_fee'] = $payout['total_month_no'] * $post['monthly_fee'];
$post['monthly_fee'] = @$post['total_monthly_fee'];

if($cp){
	echo "<br/><h2><b>CALC TOTAL MONTHLY FEE=></b>".@$post['monthly_fee']."</h2><br/>";
	echo "<h2><b>total_month_no=></b>".@$post['total_month_no']."</h2><br/>";
}
...
if (($data['ThisPageLabel']=='Withdraw'||$data['ThisPageLabel']=='Send')) 
{
	...
	if(@$rolling_v3_custom_enable&&@$post['ab']['summ_immature_amt']>=@$post['ab']['summ_immature_roll']&&@$post['rolling_res']){
		if($cp){
	...
	else
	{
		...
		if(@$rolling_v3_custom_enable)
		{
			...
			if($post['frozen_balance'])
			{
				...
				//subtract the amount of rolling_hold  than re calculation as per frozen_balance
		...
		if($cp){
			...
			if(@$post['rolling_hold']) echo "<br/>rolling_hold=>".@$post['rolling_hold'];
...
if(isset($post['last_withdraw_tdate'])&&trim($post['last_withdraw_tdate'])) $json_values['last_withdraw_tdate'] = $post['last_withdraw_tdate'];
if(isset($post['last_withdraw_micro_current_date'])&&trim($post['last_withdraw_micro_current_date'])) $json_values['last_withdraw_micro_current_date'] = $post['last_withdraw_micro_current_date'];

$json_values['wd_transaction_period'] = @$post['transaction_period'];
$json_values['wd_date_from'] = $post['withdraw_from_date'];
$json_values['wd_date_to'] = $post['withdraw_to_date'];
$json_values['wd_total_month_no'] = $post['total_month_no'];
$json_values['wd_monthly_fee'] = $post['per_monthly_fee'];
	
$json_values['wd_account_curr']=$clk_arr['account_curr']=$post['ab']['account_curr'];
...
$json_values = array_merge ( @$json_values, @$ab_uid);

//removeSingleQuotes($json_values);

//if(isset($json_values['ab']['summ_withdraw'])) unset($json_values['ab']['summ_withdraw']);
...
if(isset($post['virtual_fee'])&&@$post['virtual_fee']) $req_amt['virtual_fee']=@$post['virtual_fee'];
$req_amt['cal_total_fee']=$post['cal_total_fee'];

if($cp){
  	echo "<br/><br/><hr/><br/>req_amt=>";
	print_r($req_amt); 
	echo "<br/>amount=>".(@$req_amt['amount'] + @$req_amt['settlement_fixed_fee'] + @$req_amt['monthly_fee'] + @$req_amt['virtual_fee']);
  
	//echo "<br/>ab=>";print_r($post['ab']);
	if(@$post['ab']['summ_immature_amt']) echo "<br/>summ_immature_amt:  Immature Fund=>".@$post['ab']['summ_immature_amt'];
	if(@$post['ab']['summ_total_roll']) echo "<br/>summ_total_roll: - Rolling Balance=>".@$post['ab']['summ_total_roll'];
	
	echo "<br/>post amount=>".@$post['amount'];
	if(@$post['rolling_res']) echo "<br/>rolling_res=>".@$post['rolling_res'];

	if(@$post['after_rolling_amt']) echo "<br/>after_rolling_amt=>".@$post['after_rolling_amt'];
...
if( $is_admin == true && isset($auto_amt_clk) && $auto_amt_clk== true && $noFeeRequired==0) {
	...
	if($amount>1){
		...
		$da['clk']['Error']='Cannot proceed because insufficient balance.'; json_print3($da);exit;
...
//Dev Tech : 24-05-15 not need below condition and remove here on dated 25-01-25
...
// all fee not add 
	...
	$json_values['wd_wire_fee']=$clk_arr['settlement_fixed_fee']=$post['settlement_fixed_fee'];
	$json_values['wd_total_monthly_fee']=$clk_arr['monthly_fee']=$post['monthly_fee'];
	$json_values['wd_virtual_fee']=$clk_arr['wd_virtual_fee']=$post['virtual_fee'];
	$json_values['cal_total_fee']=$clk_arr['cal_total_fee']=$post['cal_total_fee'];

	if(isset($post['ab']['total_mdr_txtfee_amt'])) $clk_arr['total_mdr_txtfee_amt']=@$post['ab']['total_mdr_txtfee_amt'];
	if(isset($post['ab']['total_gst_fee'])) $clk_arr['total_gst_fee']=@$post['ab']['total_gst_fee'];
	if(isset($post['ab']['total_gstotal_mdr_txtfee_amtt_fee'])) $clk_arr['total_mdr_txtfee_amt']=@$post['ab']['total_mdr_txtfee_amt']; //Transaction Fee
	if(isset($post['ab']['total_mdr_amt'])) $clk_arr['total_mdr_amt']=@$post['ab']['total_mdr_amt']; //Discount Rate 
	...
	if($cp){
		...
		echo "<br/><hr/><h4 style='font-size:22px!important;font-weight:bold;color:#3a8411;'>CRON ARRAY :: clk_arr</h4>";
		print_r(@$clk_arr);
	}
...
if((isset($post['send'])&&$post['send'])||(isset($post['action'])&&$post['action']=="convertamt")){
	...
	if ($post['step'] == 1||$post['step'] == 4) {
		...
		if (!isset($_POST['amount'])) { //_POST
			...
			$to_Currency = (@$post['requested_currency']?@$post['requested_currency']:@$_REQUEST['requested_currency']);
			...
			//echo $from_Currency . $to_Currency . $post['pay_amount'];exit;
			...
			if($post['previous_transID']){
				$json_values['previous_wd_transID'] = $post['previous_transID'];
			}
			...
			//removeSingleQuotes(@$clk_arr);

			$clk_arr['withdrawfee_calc']=@$json_values['withdrawfee_calc'];
			$clk_arr['total_monthly_fee']=@$json_values['wd_total_monthly_fee'];
		...
		$upa=@$post['BankType'];
			...
			$additional_fld=$master_fld=",`acquirer_ref`,`acquirer_response`,`support_note`,`source_url`,`system_note`,`json_value`,`trans_response`,`upa`,`webhook_url` " ;
			
			$additional_data=$master_data=",'{$acquirer_ref}','{$acquirer_response}','{$remark_upd}','{$source_url}','{$system_note}','{$json_value}','{$trans_response}','{$upa}','{$webhook_url}' " ;
			...
			{	
				...
				$default_db_connect=default_db_connect_f($cp); //Is default db connect ok
				$db_from=db_from_f($cp);

				@$master_fld .=",`db_from` "; 
				@$master_data .=",'".$db_from."' " ;
			}
			
			//cmn for cur tesing a=2 for exit 
			if(isset($_REQUEST['exit'])&&$_REQUEST['exit']==2) 
			{
				$clk_arr['db_from']=@$db_from; $json_values['clk']=$clk_arr; json_print3($json_values);exit;
			}
			...
			if($data['ThisPageLabel']=='Rolling') {
				...
				$payin_setting_updt="UPDATE `{$data['DbPrefix']}payin_setting`" .
					" SET `available_rolling`='{$last_available_rolling}' " .
					" WHERE `clientid`='{$uid}'";

				db_query_df($payin_setting_updt,$qp,$default_db_connect);
			...
			//echo "<br/>CONNECTION_TYPE_DEFAULT 15=>".$data['CONNECTION_TYPE_DEFAULT'].$db_from;echo "<br/><br/>";
			

			 
			if(isset($data['ROLLING_V3_CUSTOM_ENABLE'])&&$data['ROLLING_V3_CUSTOM_ENABLE']=='Y')
			{
				@$master_fld .=",`rolling_amt`,`available_rolling`,`mature_rolling_fund_amt`,`immature_rolling_fund_amt` "; 
				@$master_data .=",'".$rolling_amt."','".@$available_rolling_tr."','".@$json_values['mature_rolling_fund']."','".@$json_values['immature_rolling_fund']."' " ;
			}
				
			$sqlStmt = "INSERT INTO `{$data['DbPrefix']}{$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3']}`" . "(`tdate`,`merID`,`bill_amt`,`acquirer`,`trans_status`, `fullname`,`reference`,`transID`,`mop`,`bill_currency`,`bank_processing_curr`,`bill_ip`,`trans_type`,`trans_currency`,`buy_txnfee_amt`,`remark_status`,`trans_amt`,`payable_amt_of_txn`,`remaining_balance_amt`,`transaction_period`,`available_balance`,`settelement_date`,`channel_type` ".$master_fld.$query_own1.")
			VALUES" . 
			"('{$tdate_micro}','{$uid}','" . $bill_amt . "','{$acquirer}',{$trans_status}," . "'" . $fullname . "','" . $reference  . "','{$transactioncode}','" . $mop . "','" . $bill_currency . "','" . $bank_processing_curr . "','" . $current_ip . "','" . $trans_type . "','" . $trans_currency . "','-" . $buy_txnfee_amt . "'," .  "'1','" . "-" . $trans_amt . "','" . "" . $amount . "','".@$json_values['remaining_balance']."','" . $transaction_period . "','" . $last_available_balance . "','{$tdate_micro}','".$channel_type."'".$master_data.$query_own2." )";
			
			//echo $sqlStmt;exit;


			db_query_df($sqlStmt,$qp,@$default_db_connect);
			$tr_newtableid = newid_df(@$default_db_connect);
			...
			if($tr_newtableid){	
				...
				$tr_newtableid_updt="UPDATE `{$data['DbPrefix']}{$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3']}` SET `transID`='".$transactioncode."' ".$transaction_update_query." WHERE (`id`='{$tr_newtableid}') ";

				db_query($tr_newtableid_updt,$qp,@$default_db_connect);

				
				$tableName=$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3'];
