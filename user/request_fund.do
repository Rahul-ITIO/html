...
//echo 'aaaaaa'.$post['send'];
	...
	if(isset($post['step']) && $post['step']==1){	//if step 1 then increase
		...
		if(!isset($post['fullname']) || !$post['fullname']){
			...
			$post['currency_amount']=$post['default_currency_sym'].$post['amount'];

			//echo "<br/>amount===>".$post['amount'];exit;
...
if(isset($post['action']) && ($post['action']=='update' || $post['action']=='display')){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}request_trans_table_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}request_trans_table`)+1);",0
		);
	}
}
...
if(isset($_POST['invoice'])&&$_POST['invoice']=="inv"){
	...
	$inv_m_ph_eml='';
	if($receiver_email)
		$inv_m_ph_eml.='Email: '.$receiver_email.'<br/>';
	if($bill_phone)
		$inv_m_ph_eml.='Mobile: '.$bill_phone.'<br/>';
	
	//$post['ctable1']=$arr["inv_b_name"].'<br/>'.$inv_m_ph_eml.''.$arr["inv_b_address"].'<br/>'.$arr["inv_b_address2"];
	
	$post['ctable1']=$arr["inv_m_name"].'<br/>'.$arr["inv_m_address"];
		  
	$post['ctable2']=$arr["inv_b_name"].'<br/>'.$inv_m_ph_eml.''.$arr["inv_b_address"].'<br/>'.$arr["inv_b_address2"];
