...
<div class="container border my-2 bg-primary text-white p-1 vkg rounded " >
...
<div class="row vkg row clearfix_ice">
	<div class="col-sm-12 py-2">
		<h4 class="float-start"><i class="fa-solid fa-qrcode"></i> QR Code</h4>
	</div>
</div>
...
<form method="post" name="data">
	...
	if(($post['step']==1) && (isset($_GET['qrid'])&&$_GET['qrid']=="")){ 
	?>
	<div class="ro col-sm-12 row opal" id="s_box">
		<div class="text-end">
			<button type="submit" name="send" value="Generate QR Code" class="btn btn-primary my-1"><i class="fas fa-plus-circle" title="Generate QR Code"></i></button>
		</div>
	</div>
	...
	<table class="table table-hover bg-primary text-white">
		...
		<tr>
			...
			<td data-label="Status"><? if($value['status']==1){ echo "<i class='far fa-check-circle text-success' title='Active'></i>"; }else{ echo "<i class='fa-regular fa-circle-xmark text-danger' title='Not Active'></i>"; } ?></td>
			...
			<div class="btn-group dropstart"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action"><i class="<?=$data['fwicon']['circle-down'];?> text-link"></i></a>
                <ul class="dropdown-menu dropdown-menu-icon pull-right text-center" >
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/qr-code<?=$data['ex']?>?bid=<?=$value['id']?>&action=update<? if(isset($post['gid'])&&$post['gid']&&isset($_GET['admin'])&&$_GET['admin']) echo '&id='.$post['gid'].'&admin=1';?>" title="Edit QR Code" ><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a></li>
				  
           
                  <li> <a class="dropdown-item"  href="<?=$data['USER_FOLDER']?>/qr-code<?=$data['ex']?>?bid=<?=$value['id']?>&action=delete<? if(isset($post['gid'])&&$post['gid']&&isset($_GET['admin'])&&$_GET['admin']) echo '&id='.$post['gid'].'&admin=1';?>" title="Delete QR Code" onclick="return confirm('Are you Sure to Delete');"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a></li>
                  
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/qr-code<?=$data['ex']?>?qrid=<?=$value['id']?>&action=display<? if(isset($post['gid'])&&$post['gid']&&isset($_GET['admin'])&&$_GET['admin']) echo '&id='.$post['gid'].'&admin=1';?>"  title="View QR Code"><i class="<?=$data['fwicon']['eye-solid'];?>"></i> </a></li>
	...
	elseif($post['step']==2){ 
		...
		<?
			...
			if(empty($fullname))
			{
				$fname		= $clients_master_info['fname'];
				$lname		= $clients_master_info['lname'];
				$fullname	= trim($fname.' '.$lname);
			}
			$company_name			= $clients_master_info['company_name'];
			$address			= $clients_master_info['address'];
			$city				= $clients_master_info['city'];
			$country			= $clients_master_info['country'];
			$state				= $clients_master_info['state'];
			$zip				= $clients_master_info['zip'];
			$phone				= $clients_master_info['phone'];
			$email				= $clients_master_info['email'];
...
//			alert('Please enter valid VPA');
	...
	<div class="justify-content-center row">
		<div class="col-md-5 my-2 py-4 card card1 m_height rounded-tringle mx-1 bg-white text-dark row">
		<?php /*?>
		...
		<div class="col-sm-12 my-2 px-1">
			...
			<select name="store_id" id="storeType" class="form-select" title="Website Name" onchange="return checkAcquirer(this.value)" <? if(isset($post['action'])&&$post['action']=='update'&&isset($post['bid'])&&$post['bid']) echo 'disabled';?> required>
				<option value="">Select Website</option>
				<? 
				foreach($data['Store'] as $key=>$value){ 
					?>
					<option data-burl="<?=$value['bussiness_url'];?>" data-dba="<?=$value['dba_brand_name'];?>" data-val="<?=$value['api_token'];?>" value="<?=$value['id'];?>"><?=$value['name']?></option>
					<? 
					$k++; 
				} ?>
			</select>
			<? 
			if($post['store_id']){ ?>
				<script>$('#storeType option[value="<?=$post['store_id']?>"]').prop('selected','selected');</script>
			<? } ?>
		</div>
		...
		<div id="clients_detail" <?=$data['mem_det'];?>>
		<?
		if(!(isset($post['bid'])&&$post['bid']))
		{
		?>
...
<div class="container mb-2">
	...
	<div class="justify-content-center row">
		<div class="col-md-6 my-2 py-4 card card1 m_height rounded-tringle mx-1 bg-white text-dark">
			<div class="row d-flex77 justify-content-center customizecss" id="printarea">
				<div class="col-md-8" style="max-width:350px !important;">
					<div class="card">
						...
						if(isset($post['account_no'])&&$post['account_no'])
						{
							$bank_g = select_tablef("`id` in ({$post['store_id']})",'products',0,1,'`website_mcc_code`');
							$website_mcc_code = $bank_g['website_mcc_code'];
							...
							if($validate)
							{
								...
								<div class="text-center"><img src="https://quickchart.io/chart?chs=300x300&cht=qr&chl=<?=$url;?>&choe=UTF-8" class="img-fluid"></div>
				...
				<div class="col-md-8 px-2" style="max-width:350px !important;">
					...
					<form method="post" name="data">
						<input type="hidden" name="qrid" value="<?=((isset($_GET['qrid']) &&$_GET['qrid'])?$_GET['qrid']:'')?>">
							...
							<div class="text-center"><a href="<?=$data['USER_FOLDER']?>/index<?=$data['ex']?>" ><img src="https://quickchart.io/chart?chs=300x300&cht=qr&chl=<?=$url;?>&choe=UTF-8" class="img-fluid"></a></div>
