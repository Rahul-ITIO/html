...
$s_background_g="#ACACDE";//
//echo "background_g".$_SESSION['background_g'];
$s_color_g="#000";//
//echo "color_g".$_SESSION['color_g'];
...
<style>
.sbm-option {border-left:1px solid #ccc;border-right:1px solid #ccc}
.gpay-main .static-menu{ font-size:15px;font-family:'Avenir-Medium';padding:3px 10px 0;height:32px}
...
.static-menu{border:1px #ccc solid;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;background:#fff;height:48px;padding:13px 10px;position:relative;transition:all 400ms ease-in-out;width:100%;font-family:'Avenir-Book'}
...
#upi2IdRow .gpay-adrate{font-size:16px;font-weight:900;line-height:normal;vertical-align:middle;display:inline-block;margin:4px 0 0}
#upi2IdRow ul li:hover{background:var(--background-1);cursor:pointer;color:#fff;transition:none}
...
.appLogoDiv img{height:24px!important;width:auto!important;display:block!important;margin:7px auto!important;line-height:50px;vertical-align:middle}

.banksDiv.active, .appLogoDiv.active {background:#1c511c;color:#fff;}
.banksDiv, .appLogoDiv, .upisDiv {display:table-cell;vertical-align:middle; text-align:center;width:32%;height:36px;line-height:50px;border:1px solid #ccc;border-radius:3px;float:left;margin:2px;background:#efefef;cursor:pointer;}	
...
.merchant_logo_url {position:relative;top:13px;left:5px;margin-bottom:24px;text-align:center;}
.merchant_logo_url img {height: 70px;}
.submit_div {padding: 0px 0px;height:auto;}

<? if(isset($data['re_post'])&&$data['re_post']){?>
.merchant_logo_url {position: relative;top:-10px;margin-bottom:-5px;text-align: center;}
<? } ?>
...
@media (max-width:1049px){
	...
	.merchant_logo_url{position:relative;top:21px;margin-bottom:14px;text-align:center;}
...
.number .key_input {width: 15px !important;min-width: 15px !important;font-size: 18px !important; }
  ...
  <style>
	...
	.company_name {font-weight:bold;font-size:26px;text-align: center;color:<?=$s_color_g;?>;}
  ...
  <div class="processall container my-2">
    <div class="content_top" style="">
      <div class="capl hide bg-white" colspan="2" style="">
        ...
        <div class="col-sm-12 text-center"><img src="<?=($_SESSION['merchant_logo_url']);?>" class="img-fluid" alt="Logo" style="max-width: 100px;"/> </div>
        <? } ?>
     ...
     <script language="javascript" type="text/javascript"> 
        ...
        <div id="MainDivvkg" class="row slidePaymentDiv" <?=$c_style2;?> >
          ...
          <div class="col-sm-6 bg-vlight px-2 rounded-payment">
            ...
            <?php /*?><? if(isset($_SESSION['merchant_logo_url'])&&$_SESSION['merchant_logo_url']){?>
            <div class="merchantLogo text-center"> <img src="<?=($_SESSION['merchant_logo_url']);?>" class="img-fluid" alt="Logo" style="max-height:50px;"  /> </div>
...
<?if(isset($_SESSION['bearer_token_id'])&&trim($_SESSION['bearer_token_id'])){?>
	<div class="text-center my-2">
		Bearer Token : <b class="fs-3"><?=($_SESSION['bearer_token_id']);?></b>
	</div>
<?}?>
...
<p class="shadow-none p-2 bg-light rounded fs-5"><?=$post['curr']?$post['curr']:$_SESSION['curr'];?> <?=(isset($post['total'])&&$post['total']?$post['total']:(isset($post['bill_amt'])?$post['bill_amt']:''))?></p>
...
<?php /*?><div class="toast align-items-center text-message w-75 ms-2 toast-box show" role="alert" aria-live="assertive" aria-atomic="true" id="myToast">
  <div class="d-flex">
    ...
    <button type="button" class="btn-close btn-close-white me-2 m-auto" onclick="toastclose('.toast')" data-bs-dismiss="toast" aria-label="Close"></button>
			...
			<? if(isset($_SESSION['merchant_logo_url'])&&$_SESSION['merchant_logo_url']){?>
				<div class="merchantLogo text-center mt-1"> <img src="<?=($_SESSION['merchant_logo_url']);?>" class="img-fluid" alt="Logo" style="max-height:50px;"  /> </div>
...
<? } ?>	
	  ...
	  if(isset($post['acquirer_id_card'])&&!empty($post['acquirer_id_card'])){?>
		...
		?>
			...
			<input type="radio" value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" id="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" required name="account_types" class="account_types" data-value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" data-name="ecard" data-ccn="<?=(isset($_SESSION['b_'.$value['acquirer_id']]['mop'])?@$_SESSION['b_'.$value['acquirer_id']]['mop']:'')?>" data-currency="<?=$pcrcy;?>" data-ewlist="<?=acquirer_popup_msg_f($value['acquirer_id'])?>"  data-etype="2D"  data-count='<? if(isset($_SESSION["b_".$value['acquirer_id']]['deCon'])) echo $_SESSION["b_".$value['acquirer_id']]['deCon'];?>' data-dnmc='<? if(isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['block_countries'];?>' data-pco='<? if(isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['processing_countries'];?>' style="display:none;" />
                        <label for="<?=$value['acquirer_id']?>">
				...
				<div class="cname netBankEdiv">
					...
					?>
                      <div class="inputype the_icon  <?=countryCodeMatch2($post['country_two'],isset($_SESSION["b_".$value['acquirer_id']]['countries'])?$_SESSION["b_".$value['acquirer_id']]['countries']:'',isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])?$_SESSION["b_".$value['acquirer_id']]['processing_countries']:'',isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])?$_SESSION["b_".$value['acquirer_id']]['block_countries']:'')?> box-shadow my-1 w-100">
                        <input type="radio" value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" id="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" required name="account_types" class="account_types" data-value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" data-name="<?=(isset($dataname)?$dataname:'');?>" data-currency="<?=$pcrcy;?>" data-submitmsg="intent_submitMsg" data-ewlist="<?=acquirer_popup_msg_f($value['acquirer_id'])?>" data-etype="3D" data-count='<?=(isset($_SESSION["b_".$value['acquirer_id']]['deCon'])?$_SESSION["b_".$value['acquirer_id']]['deCon']:'');?>' data-dnmc='<? if(isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['block_countries'];?>' data-pco='<? if(isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['processing_countries'];?>' style="display:none;" <?=(isset($data_ccn)?$data_ccn:'');?> />
                        <label for="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" class="text-center">
						...
						?>
							<img src="<?=$data['Host']?>/images/<?=acquirer_logo_f($value['acquirer_id']);?>_logo.png" title="<?=$value['checkout_label_web'];?>"  />
						<?
						}elseif($value['checkout_label_web']=="UPI Wallets"){ 
							echo '<i class="'.$data['fwicon']['wallet_code'].' text-dark" title="UPI Wallets"></i>';
						}elseif(strstr($value['checkout_label_web'],"Net Banking")){ 
...
<? if(isset($post['t_name6'])&&strpos($post['t_name6'],'SabPaisawalletList') !== false){?>
						...
						<? if(isset($post['t_name6'])&&strpos($post['t_name6'],'qrcodOption') !== false){?>
							<div class="qrcodOption_div ewalist hide appLogoDiv desImg qrcode" onclick="checkupi('qrcode')"><img src="<?=$data['Host']?>/images/qr-code-logo.png" alt="QRcode" title="QRCode" style="display:block;width: 100%;"><span class="wallets txNm hide">QRcode</span></div>
							<?
							}
						}?>
							<div class="appLogoDiv desImg bhim" onclick="checkupi('bhim')"><img src="<?=$data['Host']?>/images/bhim.png" alt="bhim" title="Bhim" style="display:block;width: 100%;"><span class="wallets txNm hide">Bhim</span></div>
							
							<div class="appLogoDiv desImg whatsapp" onclick="checkupi('whatsapp')"><img src="<?=$data['Host']?>/images/whatsapp-pay.png" alt="Whatsapp Pay" title="Whatsapp Pay" style="display:block;width: 100%;height:50px"><span class="wallets txNm hide">WhatsApp</span></div>

<div class="appLogoDiv desImg google" onclick="checkupi('google')"><img src="<?=$data['Host']?>/images/google_pay.png" alt="Google Pay" title="Google Pay" style="display:block;width: 100%;height:50px "><span class="wallets txNm hide">Google Pay</span></div>
							
							<div class="appLogoDiv desImg mobikwik" onclick="checkupi('mobikwik')"><img src="<?=$data['Host']?>/images/mobikwik-logo-new.png" alt="Mobikwik" title="Mobikwik" style="display:block;width: 100%;"><span class="wallets txNm hide">MobiKwik</span></div>
							<div class="appLogoDiv desImg freecharge" onclick="checkupi('freecharge')"><img src="<?=$data['Host']?>/images/freecharge-logo-new.png" alt="FreeCharge" title="FreeCharge" style="display:block;width: 100%;"><span class="wallets txNm hide">Freecharge</span></div>
							<div class="appLogoDiv desImg jio" onclick="checkupi('jio')"><img src="<?=$data['Host']?>/images/jiomoney-logo-new.png" alt="JioMoney" title="JioMoney" style="display:block;width: 100%;"><span class="wallets txNm hide">JioMoney</span></div>
							<div class="appLogoDiv desImg ola" onclick="checkupi('ola')"><img src="<?=$data['Host']?>/images/olamoney-logo-pp.png" alt="OlaMoney" title="OlaMoney" style="display:block;width: 100%;"><span class="wallets txNm hide">OlaMoney</span></div>
							<div class="appLogoDiv desImg amazon" onclick="checkupi('amazon')"><img src="<?=$data['Host']?>/images/amazonpay_logo.png" alt="AmazonPay" title="AmazonPay" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">AmazonPay</span></div>
							<div class="appLogoDiv desImg phonepe" onclick="checkupi('phonepe')"><img src="<?=$data['Host']?>/images/phonepe.png" alt="PhonePe" title="PhonePe" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">PhonePe</span></div>
							<div class="appLogoDiv desImg paytm" onclick="checkupi('paytm')"><img src="<?=$data['Host']?>/images/paytm_logo.png" alt="PayTm" title="PayTm" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">PayTm</span></div>
							
							<div class="appLogoDiv desImg airtel" onclick="checkupi('airtel')"><img src="<?=$data['Host']?>/images/airtel_payments_bank_logo.png" alt="Airtel" title="Airtel" style="display:block;margin:12px auto !important;max-height:40%;"><span class="wallets txNm hide">Airtel</span></div>
							
							<div class="appLogoDiv desImg other" onclick="checkupi('other')"><img src="<?=$data['Host']?>/images/other_logo.png" alt="Other" title="Other" style="display:block;width:95%;margin:3px auto; height: 57px;width:100px;"><span class="wallets txNm hide">Other</span></div>
							
						  </div>
						  <? }?>

						  <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'appWalletList') !== false){?>
						  <div class="appWalletList_div ewalist hide">
							<select class="w94 wDropDown dropDwn required wallet_code_app" name="wallet_code_app" id="wallet_code_app" style="margin:5px 0;" onchange="checkupi(this.value)" >
                      ...
                      <? }?>
                        ...
                        <div class="modalMsgCont" style="display:none !important">
                          <div class="modalMsgContTxt" >
                            ...
                            <span class="submit_div">
                            <button id="cardsend_submit" type="submit" name="send123" value="CHANGE NOW!" class="checkValidityTr nopopup submitbtn btn btn-icon btn-primary" autocomplete="off" style='width:49%;padding:0;' onclick="checkValidity_tr_f('hideCloseButton');"><i class="far fa-check-circle"></i> <b class="contitxt">Yes</b></button>
                            </span> <a class='nopopup suButton btn btn-icon btn-primary' style='width:35%' onClick="document.getElementById('modal_msg').style.display='none';">No</a>
                            <p>You can always switch back to this tab if transactions is not successful. </p>
                          </div>
                        </div>
...
function cardvalidorf(){
	...
	$('#payment_form_id_ewallets').submit(function(e) { 
	  try {
		...
		if(($('.account_types').hasClass('active')) && (submitmsgVar !== undefined) && (submitmsgVar) && (submitmsgVar == 'intent_submitMsg') && ( $('.submitMsg').hasClass(submitmsgVar+'_div') )  ){
			//alert(submitmsgVar);
			$('#payment_form_id_ewallets').attr('target','_blank');
...
/*** Modify by DevTech:22-12-19		****/
	function checkupi(val)
	{
		...
		$('#upi-suffix, .upi-suffix').removeClass('dropdown-toggle'); 
		...
		else 
		{
			...
			*/
			...
			if(val=='phonepe')
			{
				...
				$('#upi-suffix, .upi-suffix').addClass('dropdown-toggle'); 
