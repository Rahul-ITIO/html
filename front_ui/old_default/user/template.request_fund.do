...
<? $convert_qrcode_from_url=$data['Host']."/@".prntext($post['username'])."/50.00"; ?>
  <div class="row vkg row clearfix_ice">
	...
	<div class="ro col-sm-12 row"> <span class="float-end d-none d-sm-block text-end" style="width:calc(100% - 150px);"> <a href="<?=$convert_qrcode_from_url;?>" target="_blank" title="Direct Checkout (UserId and Amount):" class="btn btn-primary my-rpnsive-btn text-break my-2 ">Payments Links: <?=$convert_qrcode_from_url;?></a></span>
...
<div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false" title="Action" ><i class="<?=$data['fwicon']['circle-down'];?> text-link"></i></a>
 <ul class="dropdown-menu dropdown-menu-icon pull-right text-center" >
   ...
   <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/request_fund<?=$data['ex']?>?id=<?=$value['id']?>&action=update" title="Edit"><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a></li>
				  
   <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/request_fund<?=$data['ex']?>?id=<?=$value['id']?>&action=delete"  title="Delete" onclick="return confirm('Are you Sure to Delete');"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a></li>
                  
   <li> <a class="dropdown-item modal_from_url_qrcode" title="Your QR code"  href="https://quickchart.io/chart?chs=300x300&cht=qr&chl=<?=$convert_qrcode_list;?>&choe=UTF-8" copyid="apic<?=$j;?>" style="width:40px;"><i class="<?=$data['fwicon']['qrcode'];?>"></i></a></li>
    ...
    <div class="row">
	  ...
	  <div class="col-sm-6 my-2 px-1">
		<div class="input-group ">
		  ...
		  <input type="number" step="00.01" name="amount"  placeholder="Enter The Full Amount ex. 10.00" class="form-control" value="<?=prntext(isset($post['amount'])?$post['amount']:'')?>" title="Enter the full amount ex. 10.00" data-bs-toggle="tooltip" data-bs-placement="bottom" required  />
		</div>
	</div>
