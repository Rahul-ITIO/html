    ...
    <div class="col-sm-9" style="height:100vh;">
	  ...
	  <div class="row px-2">
	    ...
	    <div class="col mb-2 text-center px-0">
		  ...
		  <div class="card-body cursor-pointer" onClick="ajaxf1(this,'<?=@$data['Host']?>/include/total_trans_merchant_default<?=@$data['ex'];?>?a=v2&actionAjax=sessionTrans&actionUI=trans_amt_and_refund_amt&curr=<?=@$default_currency?>','#trans_amt_3','0','4')" title="Click here for view the Transactions" data-bs-toggle="tooltip" data-bs-placement="top" >
              <div class="text-card cursor-pointer">
			  	<h3 class="cursor-pointer">Transactions Amount</h3>
					<h6 id="trans_amt_3">
						<?if(isset($_SESSION['tr_sum_count']['transactionAmt'])&&trim($_SESSION['tr_sum_count']['transactionAmt'])){
							echo $_SESSION['tr_sum_count']['transactionAmt'];
						}else{?>
        ...
        <div class="col mb-2 text-center px-0">
			<div class="card card1 rounded-tringle pull-up mx-1 bg-white text-dark">
				<div class="card-body cursor-pointer" onClick="ajaxf1(this,'<?=@$data['Host']?>/include/total_trans_merchant_default<?=@$data['ex'];?>?a=v2&actionAjax=sessionTrans&actionUI=trans_amt_and_refund_amt&curr=<?=@$default_currency?>','#refund_amt_3','1','4')" title="Click here for view the Refund Amount" data-bs-toggle="tooltip" data-bs-placement="top" >
					<div class="text-card cursor-pointer">
						<h3 class="cursor-pointer">Refund Amount</h3>
						<h6 id="refund_amt_3">
							<?if(isset($_SESSION['tr_sum_count']['refundAmt'])&&trim($_SESSION['tr_sum_count']['refundAmt'])){
								echo str_replace_minus($_SESSION['tr_sum_count']['refundAmt']);
							}else{?>
								<i class="<?=@$data['fwicon']['view'];?>"></i>
							<?}?>
						</h6>
    ...
    <div class="col-sm-3" align="center">
      <div class="rounded-tringle bg-white py-2 padding-mobile">
		...
		<div class="rounded-start text-start my-3 box-width ms-4 py-2 clearfix" >
		  ...
		  <div><i class="fas fa-bars text-icon"></i>&nbsp;&nbsp;Total Transaction : <a id="noOfTransaction_3" href="statement<?=@$data['ex']?>">
			<?if(isset($_SESSION['tr_sum_count']['noOfTransaction'])&&trim($_SESSION['tr_sum_count']['noOfTransaction'])){
				echo $_SESSION['tr_sum_count']['noOfTransaction'];
			}else{?>
				<i class="<?=@$data['fwicon']['view'];?>"></i>
			<?}?>
			</a></div>
...
$(document).ready(function () {
	...
	<?if(!isset($_SESSION['tr_sum_count']['transactionAmount'])){  ?>
		ajaxf1(this,'<?=@$data['Host']?>/include/total_trans_merchant_default<?=@$data['ex'];?>?a=v2&actionAjax=sessionTrans&actionUI=trans_amt_and_refund_amt&curr=<?=@$default_currency?>','#trans_amt_3','0','4');					
	<? } ?>
