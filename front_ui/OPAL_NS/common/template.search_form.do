...
<?*/?>
<style>
	.displ_json .btn.btn-primary.btn-sm {
		border: 0;
		margin: 0;
		top: 5px;
		position: relative;
	...
	.displ_json #payin_transaction_display_divid {
		margin-top: 43px
	}
...
?>
		...
		<div class="col-sm-1">
			...
			<a href="<?=$data['USER_FOLDER']?>/withdraw-fund<?=$data['ex']?>"
				onClick="javascript:$('#modalpopup_form_popup').show(200);" class="btn btn-primary my-1 text-white"
				style="" title="Withdraw Fund">Withdraw</a>
			<? }else{ ?>
			...
			<? 
		if((isset($data['con_name'])&&$data['con_name']=='clk')||(isset($_REQUEST['a'])&&$_REQUEST['a']=='csv'))
		{ ?>
			<form target="_blank" style="float:right;margin:5px 0 0 0;">

				<input type="hidden" name="searchkey" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['searchkey'])?prntext($_REQUEST['searchkey']):""?>">
				...
				<input type="hidden" name="key_name" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['key_name'])?prntext($_REQUEST['key_name']):""?>">
				<input type="hidden" name="status" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['status'])?prntext($_REQUEST['status']):""?>">
				<input type="hidden" name="time_period" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['time_period'])?prntext($_REQUEST['time_period']):""?>">
				<input type="hidden" name="date_1st" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['date_1st'])?prntext($_REQUEST['date_1st']):""?>">
				<input type="hidden" name="date_2nd" class="hide" style="display:none !important;"
					value="<?=isset($_REQUEST['date_2nd'])?prntext($_REQUEST['date_2nd']):""?>">

				<button type="submit" data-bs-toggle="modal" data-bs-target="#myModa30" name="downloadcvs"
					value="filterTrCSV" class="btn btn-icon btn-primary btn  mx-1 " style=""
					title="Download Transaction in CSV Format"><i class="fas fa-download"></i></button>
