...
if(isset($data['con_name'])&&$data['con_name']=='clk'){
	if(!isset($post['bill_address'])||empty($post['bill_address'])) $post['bill_address']='NA';
	if(!isset($post['bill_country'])||empty($post['bill_country'])) $post['bill_country']='IN';
	if(!isset($post['bill_state'])||empty($post['bill_state'])) $post['bill_state']='NA';
	if(!isset($post['bill_city'])||empty($post['bill_city'])) $post['bill_city']='NA';
	if(!isset($post['bill_zip'])||empty($post['bill_zip'])) $post['bill_zip']='NA';
}

//echo "<br/>con_name=>".$data['con_name'];

//print_r($data['frontUiName']);
...
html #modalpopup_processing_for_pending .modalpopup_form_popup_body {
	min-width:100%;
}
.modalpopup_processing_for_pending_div {
	min-width:752px;max-width:100%;margin:0 auto;position:fixed;bottom: 0;margin-left:7px;
}
...
.desImg{height:86px;cursor:pointer !important;line-height:normal; }
...
.fa-solid.fa-language.language-collapse {
  position: relative;
  z-index: 0;
}
...
.cname.cardDiv label {font-size:12px;width:96%;white-space:wrap !important;display:block;line-height:18px;cursor:pointer;margin:auto;}


.account_types.active {color:orange !important;}
label.active {color:#000000 !important;font-weight:bold;}
.active label.active, .active label.active * {color:#ffffff !important;font-weight:bold;}
...
.upi_qr_img { filter_XX: blur(2px); height:125px;}
.upi_qr_border {
...
.number .key_input {width: 15px !important;min-width: 15px !important;font-size: 18px !important; }
  ...
  <div class="processall container my-2">
    <div class="content_top" style="">
      ...
      <div class="border_con border rounded rounded-payment px-0 m-2 position-absolute top-50 start-50 translate-middle" >
...
?>
	 ...
	 ?>
        ...
        <div id="MainDivvkg" class="row slidePaymentDiv" <?=@$c_style2;?> >
          ...
          <div class="col-sm-6 bg-vlight p-4 rounded-payment">
		    <div class="row">
            <? 
			//if(isset($data['re_post'])&&$data['re_post'])
			{?>
            <div class="translateDiv col-sm-12 m-0 bg-vlight ms-2 mt-2">
				...
				<div class="float-start text-end language-adj" title="LANG" translate="no"><i class="<?=@$data['fwicon']['language'];?> language-collapse" title=" language" style="font-size:15px;" ></i><div id="google_translate_element"></div>
...
<span class="my-2">
          ...
          <? } ?>
          <div class="col-sm-6 rounded p-4 mb-2" <?=(isset($c_style)?$c_style:'');?>>
					...
					<?
						...
						?>
                          ...
                          <div class="ecol4">
                            ...
                            <input type="text" class="form-control" name="bill_amt" placeholder='Bill Amount' value="<?=@$post['bill_amt']?>" required >
                          </div>
                          <? } ?>
                        </div>
                        
                        <div id="contact_customer" class="row" style="clear:both;float:left;">
			<?
...
<? } */?>
<? if((!isset($post['bill_country'])||empty($post['bill_country'])) || (!isset($post['bill_state'])||empty($post['bill_state']))){?>
...
<? } ?>
                      ...
                      <div class="capc text-center my-2"><button id="cardsend_submit" type="submit" name="integration-type" value="CHECKOUT"  class="submit btn btn-icon btn-primary w-100 my-2"><i class="<?=@$data['fwicon']['check-circle'];?>"></i> <b class="contitxt">Continue</b></button></div>
...
<? if($post['step']==2){?>		 
			  ...
			  <div class="paymentContainerDiv mt-1"> 
...
<? } ?>	
	  ...
	  if(isset($post['acquirer_id_card'])&&!empty($post['acquirer_id_card'])){?>
		...
		?>
			<div class="inputype the_icon cc <?=countryCodeMatch2(@$post['country_two'],(isset($_SESSION["b_".$value['acquirer_id']]['countries'])?$_SESSION["b_".$value['acquirer_id']]['countries']:''),(isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])?$_SESSION["b_".$value['acquirer_id']]['processing_countries']:''),isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])?$_SESSION["b_".$value['acquirer_id']]['block_countries']:'')?> box-shadow my-1 w-100">
	...
	function goback(){	
		...
		window.location.href="<?=@$data['Host'];?>/checkout<?=@$data['ex']?>?action=vt";
                ...
                <div class="ewallets_div pay_div">
                  <form id="payment_form_id_ewallets" method="post" onSubmit="return validateForm(this);" target="payin_win" >
                    <div class="ecol2 text-center" style="width:100%">
                      <div style="display:none">
						...
						<?
							//Dev Tech : 23-07-19 start universal theme include for common1 ----  
...
//alert(55);
  ...
  var g_card_name=$.payform.parseCardType(crdnumber);
	       ...
	       if(g_card_name=="mastercard"){
			...
			// End For display default card on card number box added  on 200223
...
<div class="modalpopup_processing hide" id="modalpopup_processing_for_pending" style="position:relative;z-index:999999;">
	<div class="modalpopup_processing_for_pending_div position-absolute1 buttom-50 start-50 translate-middle" style="">
