...
function verifyAmountFunction(gid) {
  ...
  if (amount != null) {
  	window.location.href='<?="{$data['USER_FOLDER']}/bank".$data['ex'];?>?action=verify_account&gid='+gid+'&amount='+amount;
	}
}
...
// Function for display bank / crypto form from click on  bank type box Click  Added on 16122022 by vikashs
	...
	<? if(isset($_REQUEST['action'])&&$_REQUEST['action']=='addCryptoWallet'){?>
		  $("#CryptoCurrency_id").eq(0).trigger("click");
	<? }elseif(isset($_REQUEST['action'])&&$_REQUEST['action']=='addNewBank'){?>
		  $("#bankType_id").eq(0).trigger("click");
	<? } ?>
	
	<? if(isset($_REQUEST['action'])&&$_REQUEST['action']=='addCryptoWallet_2'){?>
		  $("#addCryptoWallet").eq(0).trigger("click");
	<? }elseif(isset($_REQUEST['action'])&&$_REQUEST['action']=='addNewBank_1'){?>
		  $("#addNewBank").eq(0).trigger("click");
	<? } ?>
 ...
 .bank_resp .float-start{ float: none !important;}
 }
 </style>
<div id="zink_id" class="container mt-2 mb-2 rounded border bg-primary">
  ...
  <form id="form1" method="post" name="data" enctype="multipart/form-data">
    ...
    ?>
		...
		<td scope="row"><div class="content"><a data-bs-toggle="modal" data-count="<?=prntext($value['id'])?>" class="tr_open_on_modal text-decoration-none" data-bs-target="#myModal"><i class="<?=$data['fwicon']['display'];?> data_display text-link" title="View bank details"></i></a></div></td>
              ...
              <div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action" ><i class="<?=$data['fwicon']['action'];?> text-link"></i></a>
                <ul class="dropdown-menu dropdown-menu-icon pull-right" >
				 ...
				 <li> <a class="dropdown-item  primaryto alnk nopopup" title="Set To Bank Account Primary" onclick="myConfirm('Do you want to set this as your primary bank account : <?=prntext($value['bname'])?>','<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=bank_account_primary');" ><i class="<?=$data['fwicon']['primary'];?> text-info float-start" title="Set To Primary"></i> <span class="action_menu">Set To Primary</span></a></li>
                  ...
                  <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=update" title="Edit"><i class="<?=$data['fwicon']['edit'];?> text-success float-start"></i> <span class="action_menu">Edit</span></a></li>
                  <? }?>
                  <li> <a class="dropdown-item remove" href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=delete" onclick="return confirm('Do you want to delete');" title="Delete"><i class="<?=$data['fwicon']['delete'];?> text-danger float-start float-start"></i> <span class="action_menu">Delete</span></a></li>
                </ul>
              </div>
           </td>
        </tr>
        <tr class="hide">
          <td colspan="6">
			  ...
			  <div class="mboxtitle hide"><?=(ucwords(strtolower($data['PageName'])));?> : <?=$value['bname']?> </div>
			  <div class="col-sm-12 border rounded mb-2 mboxcss">
			    <? if($value['bname']){ ?>
			    <div class="row m-2">
				<div class="col-sm-3">Bank Name:</div>
				<div class="col-sm-9">: <?=prntext($value['bname'])?></div>
			    </div>
				<? }if($value['baddress']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank Address:</div>
				<div class="col-sm-9">: <?=prntext($value['baddress'])?></div>
			    </div>
				<? }if($value['bcity']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank City:</div>
				<div class="col-sm-9">: <?=prntext($value['bcity'])?></div>
			    </div>
				<? }if($value['bzip']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank ZIP Code:</div>
				<div class="col-sm-9">: <?=prntext($value['bzip'])?></div>
			    </div>
				<? }if($value['bcountry']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank Country:</div>
				<div class="col-sm-9">: <?=prntext($data['Countries'][$value['bcountry']])?></div>
			    </div>
				<? }if($value['bstate']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank State:</div>
				<div class="col-sm-9">: <?=prntext($value['bstate'])?></div>
			    </div>
				<? }if($value['bphone']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Bank Phone:</div>
				<div class="col-sm-9">: <?=prntext($value['bphone'])?></div>
			    </div>
				<? }if($value['bnameacc']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Name:</div>
				<div class="col-sm-9">: <?=prntext($value['bnameacc'])?></div>
			    </div>
				<? }if($value['full_address']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Full Address:</div>
				<div class="col-sm-9">: <?=prntext($value['full_address'])?></div>
			    </div>
				<? }if($value['baccount']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Account Number:</div>
				<div class="col-sm-9">: <?=decrypts_string($value['baccount']);?></div>
			    </div>
				<? }if($value['btype']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Account Type:</div>
				<div class="col-sm-9">: <?=prntext($data['BankAccountType'][$value['btype']])?></div>
			    </div>
				<? }if($value['required_currency']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Requested Currency:</div>
				<div class="col-sm-9">: <?=prntext($value['required_currency'])?></div>
			    </div>
				<? }if($value['brtgnum']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">9 Digits Routing Number:</div>
				<div class="col-sm-9">: <?=prntext($value['brtgnum'])?></div>
			    </div>
				<? }if($value['bswift']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">S.W.I.F.T. Code:</div>
				<div class="col-sm-9">: <?=prntext($value['bswift'])?></div>
			    </div>
			   <? }if($value['settlement_webhook_url']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Settlement Webhook Url:</div>
				<div class="col-sm-9">: <?=prntext($value['settlement_webhook_url'])?></div>
			    </div>
			    <? } ?>
		...
		<th scope="row"><div class="content66"><a data-bs-toggle="modal" data-count="<?=prntext($value['id'])?>" class="tr_open_on_modal text-decoration-none" data-bs-target="#myModal"><i class="<?=$data['fwicon']['display'];?> data_display text-link" title="View crypto details "></i></a></div></th>
          ...
          <td align="center"><div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action" ><i class="<?=$data['fwicon']['action'];?> text-link"></i></a>
			...
			{
			?>
                  <li> <a class="dropdown-item  glyphicons star red primaryto alnk nopopup" title="Set To wallet Account Primary" onclick="myConfirm('Do you want to set this as your primary wallet account : <?=decrypts_string($value['coins_address'])?>','<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=crypto_wallet_primary');" ><i class="<?=$data['fwicon']['primary'];?> text-info float-start" title="Set To Primary"></i> <span class="action_menu">Set To Primary</span></a></li>
                  <?
			}
			?>
                  <li> <a class="dropdown-item  green verified" href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=update&insertType=Crypto Wallet"  title="Verified of This wallet Account"><i class="<?=$data['fwicon']['lock'];?> text-success float-start" title="Verified"></i> <span class="action_menu">Verified</span></a></li>
                  <?  }else{ if(isset($value['verify_status'])&&$value['verify_status']=='0' && isset($data['total_verify'])&&$data['total_verify']<2){
			?>
                  <li> <a class="dropdown-item " onclick="myConfirm('Are you sure to send account verification request? : <?=encode((isset($value['baccount'])&&$value['baccount'])?$value['baccount']:'',4)?>','<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=sent_verification_amt&tname=bank');" title="Verify"><i class="<?=$data['fwicon']['user-cross'];?> text-danger float-start" title="Self Verify"></i> <span class="action_menu">Self Verify</span></a></li>
                  <?
			}
			elseif(isset($value['verify_status'])&&$value['verify_status']=='2')
		...
		?>
                  ...
                  <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=update&insertType=Crypto Wallet"
		 title="Edit" ><i class="<?=$data['fwicon']['edit'];?> text-success float-start"></i> <span class="action_menu">Edit</span></a></li>
                  <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?id=<?=$value['id']?>&action=deleteCrypto" onclick="return confirm('Do you want to delete');" title="Delete"><i class="<?=$data['fwicon']['delete'];?> text-danger float-start"></i> <span class="action_menu">Delete</span></a></li>
                  <? 
		}?>
                </ul>
              </div></td>
        ...
        <tr class="hide">
          <td colspan="6"><div class="next_tr_<?=prntext($value['id'])?> hide">
              ...
              <div class="mboxtitle hide"><?=(ucwords(strtolower($data['PageName'])));?> : <?=prntext($value['coins_name'])?> </div>
			  <div class="col-sm-12 border rounded mb-2 mboxcss">
			    <? if($value['coins_name']){ ?>
			    <div class="row m-2">
				<div class="col-sm-3">Coins</div>
				<div class="col-sm-9">: <?=prntext($value['coins_name'])?></div>
			    </div>

                <? }if($value['coins_network']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Network</div>
				<div class="col-sm-9">: <?=prntext($value['coins_network'])?></div>
			    </div>
				
                <? }if($value['coins_address']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Address</div>
				<div class="col-sm-9">: <?=decrypts_string($value['coins_address']);?></div>
			    </div>

                <? }if($value['coins_wallet_provider']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">Wallet Provider</div>
				<div class="col-sm-9">: <?=prntext($value['coins_wallet_provider'])?></div>
			    </div>
				
                <? }if($value['withdrawFee']){ ?>
				
				<div class="row m-2">
				<div class="col-sm-3">Withdraw Fee:</div>
				<div class="col-sm-9">: <? if(isset($value['withdrawFee'])) echo prntext($value['withdrawFee'])?>
                  %
                  <? if(isset($post['ab']['account_curr'])) echo prntext(@$post['ab']['account_curr'])?></div>
			    </div>
                <? } ?>
              </div>
             </div>
...
if($cryptoWallet==1){
      ...
      <div class="tab-pane active" id="account-settings">
        ...
        <div class="container-sm my-2 mb-5 main_heading_type">
          <div class="row-fluid mt-5">
            ...
            <div class="bTypeDiv bank_resp" style="display:block;float:none;max-width:600px; margin:0 auto;">
			<div class="text-center">
             	<div class="rounded-tringle rounded bg-primary vkg p-2 m-2 float-start bTypebox" boxval="bankType" id="addNewBank" >
 ...
 <input type="radio" name="BankType" id="bankType_id" class="bType form-check-input my-2 hide" value="bankType" required/>
              ...
              <? if($data['con_name']=='clk'){ ?>
              <? }else{ ?>
				<div class="rounded-tringle rounded  vkg p-2 m-2 bg-primary float-start bTypebox" boxval="CryptoCurrency" id="addCryptoWallet" >
                <div class="both-side-margin text-center"> 
...
<div id="swiftmsgloader" style="display:none;"> <img src="<?=$data['Host']?>/images/loader.gif" style="height:50px !important; width:50px !important;" /></div>
	  ...
	  <? /*/?>
        ...
        <div id="typeBank" class="<? if($buttonname=="Add"||$cryptoWallet==1){ ?> hide <? } ?> input_col_3">
          <div class="row  p-1 rounded mb-2 text-white was-validated" id="bank_section">
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="bnameacc"  value="<?=prntext(@$post['bnameacc'])?>" autocomplete="off" required data-bs-toggle="tooltip" data-bs-placement="top" title="The bank account holder name should match the legal name of the business. Your account application may be rejected if you fail to provide a bank account matching this criteria." id="input_bnameacc"/>
				<label for="input_bnameacc" >Account Holder Name</label>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="full_address"  value="<?=prntext(((isset($post['full_address']) &&$post['full_address'])?$post['full_address']:''));?>" autocomplete="off" title="Enter account holder&rsquo;s address - as per in bank records" data-bs-toggle="tooltip" data-bs-placement="top" required id="input_full_address" />
				<label for="input_full_address" >Account Holder Address</label>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="baccount" value="<?=decrypts_string(@$post['baccount'],0)?>" autocomplete="off" title="Enter account number<? if($post['ovalidation']){ ?> / iban no<? } ?>" data-bs-toggle="tooltip" data-bs-placement="top"   required id="input_baccount"/>
              <label for="input_baccount" > Account Number<? if($post['ovalidation']){ ?> / IBAN No<? } ?></label>
			  </div>
            </div>
            <? if($post['ovalidation']){ ?>
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="brtgnum" value="<?=prntext(@$post['brtgnum'])?>" autocomplete="off" title="Enter bank short code (BSB, ABA, IFSC, NSC, NCC, NUBAN etc)" data-bs-toggle="tooltip" data-bs-placement="top" id="input_brtgnum" required/>
				<label for="input_brtgnum" > Short Code</label>
              </div>
            </div>
            <? } ?>
            <div class="col-sm-6">
             <div class="input-field mt-4">
                ...
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="bswift" id="bswift" value="<?=prntext(@$post['bswift'])?>" autocomplete="off" title="Enter your <?=$post['swift_con'];?> code" <? if($post['ovalidation']){ ?>   onBlur="checkAvailability()" <? } ?> data-bs-toggle="tooltip" data-bs-placement="top"    required/>
                <!--onfocusout="MakeRequest();"-->
				<label for="bswift" ><?=$post['swift_con'];?> Code</label>
              </div>
            </div>

            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="bname" id="bname"  value="<?=prntext(@$post['bname'])?>" autocomplete="off" title="Enter your bank name" data-bs-toggle="tooltip" data-bs-placement="top" required/>
				<label for="bname" >Bank Name</label>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="baddress" id="baddress"  value="<?=prntext(@$post['baddress'])?>" autocomplete="off" data-bs-toggle="tooltip" data-bs-placement="top" title="Enter your bank address" required/>
            ...
            <div class="col-sm-6">
              <div class="input-field mt-4">
                <input <?=$ep;?> type="text" class="form-control is-invalid" name="settlement_webhook_url" id="settlement_webhook_url"  value="<?=prntext(@$post['settlement_webhook_url'])?>" autocomplete="off" data-bs-toggle="tooltip" data-bs-placement="top" title="Enter your settlement webhook url" required/>
				        <label for="settlement_webhook_url" >Settlement Webhook Url</label>
              </div>
            </div>
            ...
            <div class="col-sm-6">
              <div  class="mt-4" >
                ...
                <div class="float-start pe-0 text-end" style="width:40px;" title="Click for add Intermediary Bank Details" data-bs-toggle="tooltip" data-bs-placement="left">
                  <? if($post['ovalidation']){ ?>
                  <label class="btn btn-primary px-2" for="intermediary_bank_info"> <i class="<?=$data['fwicon']['circle-plus'];?>"></i> </label>
                  <? } ?>
                </div>
              </div>
            </div>
            ...
            <div class="col-sm-6">
              <div class="input-field mt-4">
 <input <?=$ep;?> type="text" class="form-control is-invalid" name="adiinfo" id="adiinfo" value="<?=prntext(((isset($post['adiinfo']) &&$post['adiinfo'])?$post['adiinfo']:''));?>" autocomplete="off" title="Enter additional information (remarks)" data-bs-toggle="tooltip" data-bs-placement="top" />
				<label for="adiinfo">Additional information (Remarks)</label>
              </div>
            </div>
            ...
            <div class="col-sm-6">
              <div class="input-field select mt-4">
                ...
                <input <?=$ep;?> type="hidden" name="required_currency" id="required_currency" class="form-control is-invalid" value="<?=prntext(isset($post['required_currency'])&&$post['required_currency']?$post['required_currency']:'INR')?>" style="display:none !important"/>
				
                <span class="text-white btn btn-outline-primary" title="Requested currency">
                <?=prntext(isset($post['required_currency'])&&$post['required_currency']?$post['required_currency']:'INR')?>
                </span>
                <? }else{ ?>
                <select <?=$ep;?> class="form-select is-invalid" name="required_currency" id="required_currency" data-bs-toggle="tooltip" data-bs-placement="top" title="Select requested currency for payout" required >
				...
				<label for="input_adiinfo">Requested Currency</label>
                <script>
            ...
            <div class="row">
              ...
              <div  class="col-sm-12 rounded text-end">
                <div  class="col-sm-12 rounded">
                  <input  <?=$ep;?> <? if(isset($post['intermediary'])&&$post['intermediary']){echo "checked='checked'";} ?> type="checkbox" name='intermediary_bank_info' id='intermediary_bank_info'  class="hide" value="<?=prntext(((isset($post['intermediary_bank_info']) &&$post['intermediary_bank_info'])?$post['intermediary_bank_info']:''));?>"  />
                  <!-- Ajax DIVs -->
                  <!-- END  Ajax DIVs -->
                  <div class=" intermediary_div my-2 <? if(!isset($post['intermediary']) || empty($post['intermediary'])){echo "hide";} ?>" id="intermediary_div">
                    <div class="row border bg-primary rounded">
                      <div class="col-sm-4 px-2 my-2"><div class="input-field">
                        <input <?=$ep;?> type="text" class="form-control" name="intermediary" id="intermediary" title="Enter your intermediary <?=$post['swift_con'];?> code" value="<?=prntext(((isset($post['intermediary']) &&$post['intermediary'])?$post['intermediary']:''));?>"  onfocusout="IntMdtMakeRequest();" data-bs-toggle="tooltip" data-bs-placement="top"  />
                        <!--onfocusout="IntMdtMakeRequest();"-->
						<label for="intermediary" >Intermediary <?=$post['swift_con'];?> Code</label>
                      </div></div>
                      <div class="col-sm-4 px-2  my-2"><div class="input-field">
                        <input <?=$ep;?> type="text" class="form-control" name="intermediary_bank_name" id="intermediary_bank_name"  title="Enter your intermediary bank name" value="<?=prntext(((isset($post['intermediary_bank_name']) &&$post['intermediary_bank_name'])?$post['intermediary_bank_name']:''));?>" data-bs-toggle="tooltip" data-bs-placement="top" />
						<label for="intermediary_bank_name" >Intermediary Bank Name</label>
                      </div></div>
                      <div class="col-sm-4 px-2  my-2"><div class="input-field">
                        <input <?=$ep;?> type="text" class="form-control" name="intermediary_bank_address" id="intermediary_bank_address"  title="Enter your intermediary bank address" value="<?=prntext(((isset($post['intermediary_bank_address']) &&$post['intermediary_bank_address'])?$post['intermediary_bank_address']:''));?>" data-bs-toggle="tooltip" data-bs-placement="top" />
						<label for="intermediary_bank_address" >Intermediary Bank Address</label>
                      </div></div>
                    </div>
                  </div>
                  <script>
	...
	if ($("#intermediary_bank_info[type='checkbox']:checked").length >0){
	     ...
	     $('#intermediary_div').slideUp(700);
              ...
              <div class="row mt-4">
                ...
                Bank Account-->
                  Submit</button>
                  <a href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?" class="btn btn-primary btn-sm" autocomplete="off" style1="color: var(--color-1)!important;"><i class="<?=$data['fwicon']['back'];?>" ></i> Back</a> </div>
              </div>
            </div>
          </div>
          <? } ?>
  ...
  <div class="container px-0 text-white">
    <form id="form2" method="post" name="data" enctype="multipart/form-data">
      ...
      <div id="typeCrypto" class="<? if($buttonname=="Add"||$cryptoWallet==0){ ?>hide<? } ?> input_col_2">
        <div class="row  p-1 rounded mb-2 was-validated" id="bank_section">
		
		
          <div class="col-sm-6 px-1 mt-1"> 
		  <div class="col-sm-12 input-field select mt-4">
		  
            <select <?=$ep;?> name="coins_name" class="form-select is-invalid" id="coins" data-bs-toggle="tooltip" data-bs-placement="top" title="Select requested coins for payout" required >
              <option value="" disabled selected>&nbsp;&nbsp;</option>
              <option value="BTC">BTC</option>
              <option value="USDT">USDT</option>
              <option value="USDC">USDC</option>
            </select>
			<label for="coins" >Requested Coins For Payout</label>
            <script>
	$('#coins option[value="<?=prntext(@$post['coins_name'])?>"]').prop('selected','selected');
          ...
          <div class="col-sm-6 px-1 mt-1"> 
		  <div class="col-sm-12 input-field select mt-4">
            <select <?=$ep;?> name="coins_network"  class="form-select is-invalid" id="coins_network" data-bs-toggle="tooltip" data-bs-placement="top" title="Select network" required >
			...
			<label for="coins_network" >Select Network</label>
            <script>
	$('#coins_network option[value="<?=prntext(@$post['coins_network'])?>"]').prop('selected','selected');
		  ...
		  <div class="col-sm-12 input-field mt-4">
            <input <?=$ep;?> type="text" class="form-control is-invalid" name="coins_address" id="coins_address" value="<?=((isset($post['coins_address'])&&$post['coins_address'])?decrypts_string($post['coins_address'],0):'');?>" autocomplete="off"  data-bs-toggle="tooltip" data-bs-placement="top" title="Enter address" required/>
            <label for="coins_address" >Coin Address</label>
          </div>
		  </div>
          <div class="col-sm-6 px-1 mt-1"> <div class="col-sm-12 input-field mt-4">
            <input <?=$ep;?> type="text" class="form-control is-invalid" name="coins_wallet_provider" id="coins_wallet_provider" value="<?=((isset($post['coins_wallet_provider']) &&$post['coins_wallet_provider'])?$post['coins_wallet_provider']:'');?>" autocomplete="off" data-bs-toggle="tooltip" data-bs-placement="top" title="Enter provider" required/>
			 <label for="coins_wallet_provider" >Coin Provider</label>
          </div>
          </div>
          <div class="col-sm-12 row px-1 mt-4">
            <div class="upload-docimgx"> <?php /*?><span class="form-label text-wrap" id="upload-docimgxtt" title="Upload Screenshot of Wallet Dashboard">Screenshot of Wallet Dashboard </span><?php */?>
              ...
              <input <?=$ep;?> name="upload_logo" value="<?=((isset($post['bank_doc'])&&$post['bank_doc'])?$post['bank_doc']:'');?>" type="hidden" class="px-2">
              <? if(isset($post['bank_doc']) &&$post['bank_doc']){ ?>
			  <span class="px-2">
              <? $pro_img=display_docfile("../user_doc/",((isset($post['bank_doc'])&&$post['bank_doc'])?$post['bank_doc']:''));
          ...
          <div class="row mt-4">
            <div <?=$ep;?> class="text-center m_role" >
              ...
              <a href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>" class="btn btn-primary btn-sm"><i class="<?=$data['fwicon']['back'];?>"></i> Back</a> </div>
          </div>
        </div>
      </div>
    </form>
