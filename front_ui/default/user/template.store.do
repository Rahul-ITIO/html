...
<div id="zink_id" class="container border my-2 bg-primary rounded" >
  ...
  <div class="row vkg row clearfix_ice">
    <div class="col-sm-12 py-2">
      <h4 class="float-start"><i class="<?=$data['fwicon']['website'];?>"></i> My Website</h4>
      <? if(!isset($_GET['admin'])) { ?>
  ...
  <form method="post" name="data">
      ...
      <table id="display-large-screen" class="table table-hover bg-primary text-white">
        ...
        <thead>
          <tr>
            ...
            <th scope="col">URL</th>
            <th scope="col">ID</th>
            <th scope="col" title="aaaaaaa">API Token</th>
        ...
        <thead>
          <tr>
            <th scope="col" colspan="8"> <div class="my-2 ms-2 fs-5 text-start">Add your website to receive payments </div>
                <div class="text-start my-2 ms-2" style="max-width:400px !important;"> you can create new website, manage added website</div>
              <? if(!isset($_GET['admin'])) { ?>
        ...
        <? } ?>
        <tbody>
          <?  $idx=1;$k;foreach($data['Products'] as $value){ ?>
          <tr>
            <td><?php /*?><a href="<?=$data['USER_FOLDER'];?>/<?=($data['PageFileName']);?><?=$data['ex']?>?id=<?=$value['id'];?>&action=view" title="View <?=($data['PageName']);?> Details" ><i class="<?=$data['fwicon']['display'];?> data_display77 text-link" title="View website details"></i></a><?php */?>
            ...
            <td><!--<a class="database_plus btn btn-icon btn-primary btn-sm" href="<?=$data['USER_FOLDER'];?>/<?=($data['PageFileName']);?><?=$data['ex']?>?id=<?=$value['id'];?>&action=view" title="View <?=($data['PageName']);?> Details" >-->
              <span class="badge rounded-pill bg-primary text-white"><?=prntext($value['id'])?></span>
            <!--</a>--></td>
            <td class="text-break"><span id="apic_<?=$value['id'];?>" class="hide-999 float-start"><?=prntext($value['api_token'])?></span><i class="<?=$data['fwicon']['copy'];?> float-start ms-2 mt-1" title="Copy API Token" onclick="CopyVal('#apic_<?=$value['id'];?>','API Token')"></i></td>
		  ...
		  $('#store_status_<?=$k;?> option[value="<?=$value['active']?>"]').prop('selected','selected');</script>
            ...
            <td align="center"><div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false" title="Action"><i class="<?=$data['fwicon']['action'];?> text-link"></i></a>
                    <ul class="dropdown-menu dropdown-menu-icon pull-right" >
                      ...
                      <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/<?=($data['PageFileName']);?><?=$data['ex']?>?id=<?=$value['id']?>&action=delete" onclick="return confirm('Do you want to delete the website?');" title="Delete" ><i class="<?=$data['fwicon']['delete'];?> text-danger fa-fw float-start"></i> <span class="action_menu">Delete</span></a> </li>
                      <? if($data['clkStore']==1){ ?>
                      <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/requests<?=$data['ex']?>?wid=<?=$value['id']?>&type=<?=$value['api_token']?>&action=addrequests" title="Generate Code" ><i class="<?=$data['fwicon']['book-reader'];?> fa-fw float-start"></i> <span class="action_menu">Generate Code</span></a></li>
                      <? }else{ ?>
                      <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/generate<?=$data['ex']?>?id=<?=$value['id']?>&action=store" title="Generate Code"><i class="<?=$data['fwicon']['code'];?> fa-fw float-start"></i> <span class="action_menu">Generate Code</span></a></li>
                      <li> <a class="dropdown-item " href="<?=$data['USER_FOLDER']?>/website<?=$data['ex']?>?id=<?=$value['id']?>&action=re_generate_api_token&type=0&page=0" onclick="return confirm('Do you want to re-generate Api token? <?='\n';?> Regenerating a new token may result in losing access to the previous token');"><i class="<?=$data['fwicon']['arrow-rotate-right'];?> text-warning fa-fw float-start" title="Are you sure you want to Re-generate API Token"></i> <span class="action_menu">Re-generate</span></a></li>
                      <? } ?>
                    </ul>
            </div></td>
          </tr>
	...
	<div class="col-sm-12 border rounded mb-2 mboxcss">
				...
				<div class="col-sm-9 ">: <?=prntext($value['name'])?></div>
				</div>
                <? } ?>
		
		
                <? if(isset($value['api_token'])&&$value['api_token']){ ?>
				<div class="row m-2">
				<div class="col-sm-3">API Token</div>
				<div class="col-sm-9">: <?=substr($value['api_token'],0,10);?> ****************</div>
				</div>
		        <? } ?>
        ...
        <? } ?>
        <? if(isset($value['min_ticket'])&&$value['min_ticket']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Min. Ticket</div>
				<div class="col-sm-9">: <?=$value['min_ticket']?></div>
          </div>
        
        <? } ?>
        <? if(isset($value['max_ticket'])&&$value['max_ticket']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Max. Ticket</div>
				<div class="col-sm-9">: <?=$value['max_ticket']?></div>
          </div>
        
        <? } ?>
        <? if(isset($value['monthly_volume'])&&$value['monthly_volume']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Monthly Volume</div>
				<div class="col-sm-9">: <?=$value['monthly_volume']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($value['processing_mode'])&&$value['processing_mode']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Processing Mode</div>
				<div class="col-sm-9">: <?=$value['processing_mode']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($value['sales_region'])&&$value['sales_region']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Sales Region</div>
				<div class="col-sm-9">: <?=$value['sales_region']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($value['transaction_notification_email'])&&$value['transaction_notification_email']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">T.Notification Email</div>
				<div class="col-sm-9">: <?=encrypts_decrypts_emails($value['transaction_notification_email'],2);?>
          </div>
        </div>
        <? } ?>
        ...
        <? } ?>
        <? if(isset($value['merchant_logo'])&&$value['merchant_logo']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">URL of Logo</div>
				<div class="col-sm-9">: <?=$value['merchant_logo']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($value['ureturn'])&&$value['ureturn']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Success URL</div>
				<div class="col-sm-9">: <?=$value['ureturn']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($value['unotify'])&&$value['unotify']){ ?>
        <div class="row m-2">
				<div class="col-sm-3">Notify URL</div>
				<div class="col-sm-9">: <?=$value['unotify']?>
          </div>
        </div>
        <? } ?>
        ...
        <div class="row m-2">
				...
				<div class="col-sm-9">: <?=prntext($value['comments'])?>
          </div>
        </div>
        <? } ?>
		
		 <? if(isset($value['notification_to'])&&$value['notification_to']){ ?>
        <div class="row m-2">
          <div class="col-sm-3">Notification  Alert :</div>
		  <div class="col-sm-9">:
            <? if(is_array($value['notification_to'])){$notification_to=implode(",",$value['notification_to']);}else{$notification_to=$value['notification_to'];} ?>
            <?php 
			if($notification_to&&strpos($notification_to,"001")!==false){echo "Notification to Merchant on Approved, ";}
			if($notification_to&&strpos($notification_to,"004")!==false){echo "Notification to Merchant on Declined, ";}
			if($notification_to&&strpos($notification_to,"002")!==false){echo "Notification to Customer, ";}
			if($notification_to&&strpos($notification_to,"011")!==false){echo "Notify to Merchant on Withdraw, ";}
			if($notification_to&&strpos($notification_to,"012")!==false){echo "Notify to Merchant on Chargeback, ";}
			if($notification_to&&strpos($notification_to,"013")!==false){echo "Notify to Merchant on Refunded";}
  ...
  if($post['gid']&&$_SESSION['store_active']==1){$ep="disabled='disabled'";} ?>
    ...
    <div class="vkg"  >
      ...
      <h4 class="my-2"><i class="<?=$data['fwicon']['website'];?>"></i> Edit Website :
        <?=prntext($post['name'])?>
      </h4>
      <div class="row my-2 px-2 border bg-primary text-white rounded" >
        ...
        <div class="col-sm-8 text-end  my-2">
          <div><b id="generate_secret_key2" class="text-break d-none d-sm-block" >
            <?=$post['apikey'];?>
            </b> <a onClick="ajaxf1(this,'<?=$data['USER_FOLDER'];?>/<?=($data['PageFileName']);?><?=$data['ex']?>?bid=<? if(isset($post['bid'])) echo $post['bid'];?>&id=<?=$post['gid']?>&action=generate_secret_key&ajax=1','#generate_secret_key2')" class="btn btn-icon btn-primary btn-sm text-end mx-2" title="Regenerate Secret Key"><i class="<?=$data['fwicon']['arrow-rotate-right'];?>"></i></a><a class="btn btn-icon btn-primary btn-sm text-end" ><i class="<?=$data['fwicon']['copy'];?>" title="Copy Secret Key" onclick="CopyVal('#generate_secret_key2','API Token')"></i></a> </div>
        </div>
      </div>
      <? } else { ?>
      <h4 class="my-2"><i class="<?=$data['fwicon']['website'];?>"></i> Add New Website</h4>
      <? } ?>
      <div class="my-2 row rounded text-white p-1">
        <div class="p-1 col-sm-6 ">
          <label for="<?=($data['PageName']);?>" class="form-label">Website Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></strong></label>
          <input <? //=$ep;?> type="text" class="form-control" name="name" placeholder=" <?=($data['PageName']);?> of your choice" value="<? if(isset($post['name'])) echo $post['name']?>" title=" <?=($data['PageName']);?> of your choice" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" required >
        </div>
        <div class="p-1 col-sm-6">
        ...
        <div class="p-1 col-sm-6">
          <label class="form-label" for="Transaction Currency" >Transaction Currency </label>
          <input <? //=$ep;?> type="text" class="form-control" name="transaction_currency"  placeholder="Currency separated by commas i.e. SGD,USD,CNY" value="<? if(isset($post['transaction_currency'])) echo $post['transaction_currency']?>" title="Enter transaction currency - currency separated by commas i.e. SGD,USD,CNY" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off"  >
        </div>
        <div class="p-1 col-sm-6">
        ...
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="min_ticket" >Min Ticket </label>
          <input type="text" class="form-control" name="min_ticket" id="min_ticket" placeholder="Min ticket for your website " value="<? if(isset($post['min_ticket'])) echo $post['min_ticket']?>" title="Enter min ticket for your website" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="Max Ticket" >Max Ticket </label>
          <input type="text" class="form-control" name="max_ticket" placeholder="Max ticket for your website " value="<? if(isset($post['max_ticket'])) echo $post['max_ticket']?>" title="Enter max ticket for your website" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="Monthly Volume" >Monthly Volume </label>
          <input type="text" class="form-control" name="monthly_volume"  placeholder="Estimated Monthly Volume for this website (Please be specific)" value="<? if(isset($post['bussiness_url'])) echo $post['bussiness_url']?>" title="Enter estimated monthly volume for this website (Please be specific)" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="Processing Mode" >Processing Mode </label>
          <input type="text" class="form-control" name="processing_mode"  placeholder="Requested Processing Mode i.e. MOTO/API" value="<? if(isset($post['processing_mode'])) echo $post['processing_mode']?>"  title="Enter requested processing mode i.e. MOTO/API" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="Sales Region" >Sales Region </label>
          <input type="text" class="form-control" name="sales_region"  placeholder="Estimated Sales Region i.e. Singapore,USA,EU... etc. " value="<? if(isset($post['sales_region'])) echo $post['sales_region']?>" title="Enter estimated sales region i.e. Singapore,USA,EU... etc. " data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6 d-none">
          <label class="form-label" for="DBA/Brand Name" >DBA/Brand Name </label>
          <input type="text" class="form-control" name="dba_brand_name"  placeholder="DBA/Brand Name - This will be shown to your customers as your company_name name" value="<? if(isset($post['dba_brand_name'])) echo $post['dba_brand_name']?>" title="Enter DBA/Brand Name - This will be shown to your customers as your company_name name" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6">
        ...
        <div class="p-1 col-sm-6">
          ...
          <input type="text" class="form-control" name="merchant_logo"  placeholder="Your web url for logo" value="<? if(isset($post['merchant_logo'])) echo $post['merchant_logo']?>" title="Your web url for logo" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" >
        </div>
        <div class="p-1 col-sm-6">
          <label class="form-label" for="Description" >Description</label>
<textarea class="form-control" name="comments" placeholder="Describe your business in details" style="height:35px;" title="Describe your business in details" data-bs-toggle="tooltip" data-bs-placement="bottom"><? if(isset($post['comments'])) echo $post['comments']?>
</textarea>
        </div>
              ...
              <div class="accordion-body">
                <div class="row">
                  <div class="p-1 col-sm-6">
                    ...
                    <input type="text" class="form-control" name="transaction_notification_email"  placeholder="Email where we can send the transaction notification" value="<? if(isset($post['transaction_notification_email'])) echo encrypts_decrypts_emails($post['transaction_notification_email'],2);?>" autocomplete="off" title="Enter email id where we can send the transaction notification" data-bs-toggle="tooltip" data-bs-placement="bottom" >
                  </div>
                  <div class="p-1 col-sm-6">
                    <label class="form-label" for="Success URL" >Success URL</label>
                    <input type="text" class="form-control" name="ureturn" id="ureturn"  placeholder="Success url (additional field for custom programming)" value="<? if(isset($post['ureturn'])) echo $post['ureturn']?>" autocomplete="off" title="Enter success url (additional field for custom programming)" data-bs-toggle="tooltip" data-bs-placement="bottom" >
                  </div>
                  <div class="p-1 col-sm-6">
                    <label class="form-label" for="Notify URL" >Notify URL</label>
                    <input type="text" class="form-control" name="unotify"  placeholder="Notify url (additional field for custom programming)" value="<? if(isset($post['unotify'])) echo $post['unotify']?>" autocomplete="off" title="Enter notify url (additional field for custom programming)" data-bs-toggle="tooltip" data-bs-placement="bottom" >
                  </div>
                  <div class="p-1 col-sm-6">
                    <label class="form-label" for="Cancellation URL" >Cancellation URL</label>
                    <input type="text" class="form-control" name="ucancel"  placeholder="Cancellation url (additional field for custom programming)" value="<? if(isset($post['ucancel'])) echo $post['ucancel']?>" autocomplete="off" title="Enter cancellation url (additional field for custom programming)" data-bs-toggle="tooltip" data-bs-placement="bottom" >
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="row my-2 p-2 ms-1">
          ...
          <div class="col-sm-2" title="Notify to Merchant on Approval"> <span class="form-check form-switch  float-start">
            <input type="checkbox" name='notification_to[]2' id='notification_to1' class='checkbox_d form-check-input' size="100" value='001' <?php if($notification_to&&strpos($notification_to,"001")!==false){echo "checked='checked'";} ?> />
            </span>
            <label for="notification_to1" title="Merchant on Approval">Approved</label>
          </div>
          <div class="col-sm-2" title="Notify to Merchant on Declined"> <span class="form-check form-switch  float-start">
            <input type="checkbox" name='notification_to[]' id='notification_to4' class='checkbox_d form-check-input' size="100" value='004' <?php if($notification_to&&strpos($notification_to,"004")!==false){echo "checked='checked'";} ?>>
            </span>
            <label for="notification_to4" title="Merchant on Declined">Declined</label>
          </div>
          <div class="col-sm-2"> <span class="form-check form-switch  float-start" title="Notify to Merchant on Withdraw">
            <input type="checkbox" name='notification_to[]' id='notification_to_011' class='checkbox_d form-check-input' size="100" value='011' <?php if($notification_to&&strpos($notification_to,"011")!==false){echo "checked='checked'";} ?>>
            </span>
            <label for="notification_to_011" title="Notify to Merchant on Withdraw">Withdraw</label>
          </div>
          <div class="col-sm-2" title="Notify to Merchant on Chargeback"> <span class="form-check form-switch  float-start">
            <input type="checkbox" name='notification_to[]' id='notification_to_012' class='checkbox_d form-check-input' size="100" value='012' <?php if($notification_to&&strpos($notification_to,"012")!==false){echo "checked='checked'";} ?>>
            </span>
            <label for="notification_to_012" title="Notify to Merchant on Chargeback">Chargeback</label>
          </div>
          <div class="col-sm-2" title="Notify to Merchant on Refunded"> <span class="form-check form-switch  float-start">
            <input type="checkbox" name='notification_to[]' id='notification_to_013' class='checkbox_d form-check-input' size="100" value='013' <?php if($notification_to&&strpos($notification_to,"013")!==false){echo "checked='checked'";} ?>>
            </span>
            <label for="notification_to_013" title="Notify to Merchant on Refunded">Refund</label>
          </div>
          <div class="col-sm-2" title="Notify to Customer"> <span class="form-check form-switch  float-start">
            <input type="checkbox" name='notification_to[]' id='notification_to2' class='checkbox_d form-check-input' size="100" value='002' <?php if($notification_to&&strpos($notification_to,"002")!==false){echo "checked='checked'";} ?>>
            </span>
            <label for="notification_to2" title="Notification to customer for success and decline.">Customer</label>
          </div>
        </div>
    ...
    <div class="float-end"> <a class="sub_logins nopopup" href='<?=$data['Admins']?>/merchant<?=$data['ex']?>?bid=<?=$post['gid']?>&id=<?=$post['id']?>&action=update_stores&type=active&page=0&admin=1&hideAllMenu=1'><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a> </div>
	...
	<!--//Detail used on admin section merchant website details-->
	
    <div class="container my-2 px-0 table-responsive vkg">
      <h2 class="ms-1">Website Details :: <?=prntext($post['name'])?> !!</h2>
      ...
      <div class="row border bg-light my-2 text-start rounded p-2">
        <div class="row col-sm-6 my-2 p-2">
          <div><strong><?=($data['PageName']);?> ID: </strong> <?=$post['gid']?></div>
        </div>
        ...
        <? } ?>
        <? if(isset($post['api_token'])&&$post['api_token']){ ?>
        <div class="row col-sm-6 my-2 p-2 ">
          <div><strong>API Token:</strong>
            <?=substr($post['api_token'],0,10);?>
            ****************</div>
        </div>
        <? } ?>
        <!--<div class="row col-sm-12  my-2 p-2 ">
    <div><strong>Acquirer Id:</strong> <? if(is_array($post['notification_to'])){$notification_to=implode(",",$post['notification_to']);}else{$notification_to=$post['notification_to'];} ?>
          <?php if($notification_to&&strpos($notification_to,"007")!==false){echo "Request Money | ";} ?>
          <?php if($notification_to&&strpos($notification_to,"008")!==false){echo "VT |";} ?>
          <? $midcard=explode(',',$post['midcard']);
		foreach ($midcard as $value){
        ...
        <? } ?>
        <? if(isset($post['min_ticket'])&&$post['min_ticket']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
          <div ><strong>Min. Ticket:</strong>
            <?=$post['min_ticket']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($post['max_ticket'])&&$post['max_ticket']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
          <div><strong>Max. Ticket: </strong>
            <?=$post['max_ticket']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($post['monthly_volume'])&&$post['monthly_volume']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
        ...
        <? } ?>
        <? if(isset($post['processing_mode'])&&$post['processing_mode']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
          <div><strong>Processing Mode:</strong>
            <?=$post['processing_mode']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($post['sales_region'])&&$post['sales_region']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
          <div><strong>Sales Region:</strong>
            <?=$post['sales_region']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($post['transaction_notification_email'])&&$post['transaction_notification_email']){ ?>
        <div class="row col-sm-6  my-2 p-2 ">
          <div ><strong>T.Notification Email : </strong>
            <?=encrypts_decrypts_emails($post['transaction_notification_email'],2);?>
          </div>
        </div>
        <? } ?>
        ...
        <? } ?>
        <? if(isset($post['merchant_logo'])&&$post['merchant_logo']){ ?>
        <div class="row col-sm-6 my-2 p-2 ">
        ...
        <? } ?>
        <? if(isset($post['ureturn'])&&$post['ureturn']){ ?>
        <div class="row col-sm-6 my-2 p-2 ">
          <div><strong>Success URL:</strong>
            <?=$post['ureturn']?>
          </div>
        </div>
        <? } ?>
        <? if(isset($post['unotify'])&&$post['unotify']){ ?>
        <div class="row col-sm-6 my-2 p-2 ">
          <div><strong>Notify URL:</strong>
            <?=$post['unotify']?>
          </div>
        </div>
        <? } ?>
        ...
        <? } ?>
        <? if(isset($post['notification_to'])&&$post['notification_to']){ ?>
        <div class="row col-sm-6 my-2 p-2 ">
          <div><strong>Notification  Alert :</strong>
            <? if(is_array($post['notification_to'])){$notification_to=implode(",",$post['notification_to']);}else{$notification_to=$post['notification_to'];} ?>
            <?php 
			if($notification_to&&strpos($notification_to,"001")!==false){echo "Notification to Merchant on Approved, ";}
			if($notification_to&&strpos($notification_to,"004")!==false){echo "Notification to Merchant on Declined, ";}
			if($notification_to&&strpos($notification_to,"002")!==false){echo "Notification to Customer, ";}
      ...
      <? if(!isset($_GET['admin'])) { ?>
      <div class="text-center"> <a href="<?=$data['USER_FOLDER']?>/<?=($data['PageFileName']);?><?=$data['ex']?>?<?=($admin_subquery);?>" class="btn btn-primary my-2 text-dark" id="w100"><i class="<?=$data['fwicon']['back'];?>"></i> Back</a> </div>
      <? } ?>
    </div>
...
<?php /*?>js for copy API Token and resize width on mobile<?php */?>
<script>
