...
include('../include/country_state'.$data['iex']);
      ...
      $result_select=db_rows(
			" SELECT `country`, COUNT(`id`) AS ct, SUM(`transaction_amt`) AS amount".
			" FROM `{$data['DbPrefix']}transactions`".
			" WHERE `owner_id`='{$_SESSION['uid']}' AND `status` in (1) AND `country`!=''   GROUP BY `country` HAVING COUNT(`id`) > 0",0
		);
  ...
  <div class="col-sm-2 p-2">
      <div class="rounded-tringle bg-primary"> <span class="icon1 hover-for-all"> <i class="<?=$data['fwicon']['deshboard-website'];?> text-link"></i> </span> 
        <div class="mt-3 text-white" style="margin-top: 15px;">
          <h3>
            <div class="dropdown"> <a class="dropdown-toggle border border-primary rounded px-2" data-bs-toggle="dropdown" style="cursor:pointer;font-size: 14px;" title="<?=prntext($post['company']);?>" >Websites</a> 
              <div class="dropdown-menu " id="webmenu" aria-labelledby="dropdownMenuButton" >
                <? if($post['products']){
						foreach($post['products'] as $key=>$value){?>
                <a class="dropdown-item text-link" href="<?=$data['Members'];?>/dashboard<?=$data['ex'];?>?wid=<?=$value['id'];?>"><i class="<?=$data['fwicon']['hand'];?>"></i>&nbsp;
                <?=$value['name'];?>
                </a>
                <? }
					} ?>
          ...
          <span class="py-2" ><? if($post['company']){  echo prntext(substr($post['company'],0,15)); }else{ echo "&nbsp;&nbsp;";};?></span>
        </div>
        </div>
  </div>
</div>
...
$(function() {
	...
	var thisUrl = "<?=$data['Members']?>/dashboard<?=$data['ex'];?>?gid=range";
	/*
