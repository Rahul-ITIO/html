<? if(isset($data['ScriptLoaded'])){
	...
	//&&isset($_SESSION['uid_wv2'.$_SESSION['uid']]) 
	
	if($data['ThisPageUrl']=='trans_withdraw-fund_v3_custom_settlement'){
		$gpost['payout']['total_month_no']=@$post['total_month_no'];
		$gpost['payout']['per_monthly_fee']=@$post['per_monthly_fee'];
		//echo $data['ThisPageUrl'];
	}
	elseif(isset($data['WITHDRAW_INITIATE_TO_DATE_WISE'])&&@$data['WITHDRAW_INITIATE_TO_DATE_WISE']=='Y') {
		$gpost=@$_SESSION['uid_wv2'.$_SESSION['uid']];
...
if(isset($_GET['qp']))
{
	echo "<br/><hr/><br/><h3>POST AB</h3><br/>";
	print_r(@$post['ab']);
...
?>
<style>
.betaVersion{height:54px;position:absolute;margin:4px 0;z-index:9;right:6px}
...
$(document).ready(function(){
	...
	$('#requested_currency_id,#amount_id').focusout(function(){
		...
		if($('#requested_currency_id,#amount_id').hasClass('active_fn')){
			...
			var bnkVal=$("#bankList_id_val").val();
			//alert(url_name+"\r\n"+withdrawfee);
					...
					if(!data["amount_jsn"]){
						...
						setTimeout(function(){ 
							...
							if(withdrawfee>0 && data["withdrawfee_calc"]){
								...
								withdrawfee_calc = data["withdrawfee_calc"];
	...
	$('#bankList_id_ui option_div.bank_2').click(function(){
		var selectedThis = $(this);
		
       
        $('#bankList_id_ui option_div').removeClass('active');

        var thisVal = selectedThis.attr("data-value");
		...
		if(thisVal==='addNewBank'||thisVal==='addCryptoWallet'){
            ...
            $(this).addClass("active");
			bankCountNo=thisVal;
            $('#bankList_id_val').val(thisVal);	
		}else{
           
           // $('#bankList_id_ui option_div').attr("data-value",bankVal).addClass("active");
    ...
    $('#bankList_id_ui option_div.bank_1').click(function(){
		alert("We cannot process withdrawal for:\r\n \"" +$(this).text()+ "\" \r\nKindly contact to support for further information\r\n");
		return false;
	});
	
    $('#bankList_id_ui option_div.newBankAdd').click(function(){
        var retVal= confirm('Are you sure to '+$(this).text());
		if (retVal) {
			window.location.href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?action="+$(this).attr("data-value");
		    return false;
		}
	...
	<? if(isset($post['bank_back'])&&$post['bank_back']){ ?>
        set_option_div='1';
        console.log("=====post['bank_back']====<?=@$post['bank_back']?>");
		setTimeout(function(){ 
				$('#bankList_id option[value="<?=$post['bank_back']?>"]').prop('selected','selected').trigger("change");	
			}, 20);
	<? }elseif($data['bank_id']){?>
        set_option_div='2';
        console.log("=====data['bank_id']====<?=@$data['bank_id']?>");
		setTimeout(function(){ 
			$("#bankType_id").eq(0).trigger("click");
            //$('#bankList_id_ui option_div.bank_2').eq(0).trigger("click");
            //$('#bankList_id_ui .dropdown-menu').removeClass('show');
		}, 30);
	<? }elseif(isset($post['bank'])&&$post['bank']){ ?>
        set_option_div='3';
        console.log("=====post['bank']====<?=@$post['bank']?>");
		setTimeout(function(){ 
				$('#bankList_id option[value="<?=$post['bank']?>"]').prop('selected','selected').trigger("change");	
			}, 20);
	<? }?>
	
    //alert(set_option_div);

    console.log("=====set_option_div===="+set_option_div);
...
<form method="post"  <? if($post['step']>3){?> id="myFormReq" <? }?> >
  ...
  <?if($post['step']>2){?> 
	   <? if(isset($_POST['amount'])){?> 
			<input type="hidden" name="amount"  id="amount_id" value="<?=((isset($_POST['amount']) &&$_POST['amount'])?$_POST['amount']:'')?>" <?=@$input_readonly?> >
	   <? }?>
  ...
  <div class="breadcrumb" style="height:inherit;<?=(($data ['HideAllMenu'])?"display:block !important;":"")?>">
   <? 
	...
	if($data['ThisPageUrl']=='trans_withdraw-fund_v3_custom_settlement')
 	...
 	else  include("../include/summery_balance_link".$data['iex']);
...
<div class="mobileLayout col1 border col-sm-6 rounded"  style="background: linear-gradient(to bottom, <?=$_SESSION['background_gl5'];?> 0%, <?=$data['tc']['hd_b_l_9'];?> 100%);">   
 <!-- ///////////////////-->
  <div id="calculate_div_id" class="g calculate_div_id my-2 p-2 row">
	 ...
	 <div class="row col-12 p-0 m-0 border border-1 border-start-0 border-end-0 border-top-0">
	  <div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> Gross Settlement Amount : 
	  </div><div class="col p-1 text-end"><b><?=@$post['ab']['account_curr_sys']?></b><b id="totalAmtId" class="data-currency"><?=((isset($_POST['inputTotalOriginalAmt'])&&$_POST['inputTotalOriginalAmt']?number_formatf_2($_POST['inputTotalOriginalAmt']):number_formatf_2($post['summ_mature_amt'])));?>
      </b></div>
	</div>
	  
	   <? if($post['monthly_fee']>0){?>
		<div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> Monthly Maintenance Fee @<?=($gpost['payout']['per_monthly_fee']);?>*<?=($gpost['payout']['total_month_no']);?> : 
		 </div><div class="col p-1 text-end"> -<?=($post['ab']['account_curr_sys'].number_formatf_2($post['monthly_fee']));?>
		  </div>
		<? }?>
    ...
    <div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> Settlement Wire Fee : </div><div class="col p-1 text-end">-<?=($post['ab']['account_curr_sys'].number_formatf_2($post['settlement_fixed_fee']));?></div>

    <? }?>

   
    <? if((isset($post['total_gst_fee'])&&$post['total_gst_fee']?$post['total_gst_fee']:'') >0){?>
    <div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> GST Fee : </div><div class="col p-1 text-end">-<?=($post['ab']['account_curr_sys'].number_formatf2($post['total_gst_fee'],4));?></div>

    <? }?>
  
	<? if((isset($post['virtual_fee1'])&&$post['virtual_fee1']?$post['virtual_fee1']:'') >0){?>
    <div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> Virtual Terminal Fee : </div><div class="col p-1 text-end">-<?=($post['ab']['account_curr_sys'].number_formatf_2($post['virtual_fee1']));?></div>

    <? }?>
	
	
	
	<div class="row col-12 p-0 m-0 border-dark border border-1 border-start-0 border-end-0">
		<div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> Total : </div><div class="col p-1 text-end"><?=($post['ab']['account_curr_sys'].number_formatf_2($post['pay_amount']));?> </div>
	</div>
	
	
	<div id="withdrawFeeTextRowId" class="row col-12 p-0 m-0 border-dark border border-1 border-start-0 border-end-0 border-top-0" style=" <?=$withdrawFeeText_row;?> ">
	<div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> <?=$data['ThisPageLabel'];?>&nbsp;Fee <b id="withdrawFeeTextId"><? if(isset($_POST['inputWithdrawFee'])) echo $_POST['inputWithdrawFee']?> </b>% Applicable (After Deduction of Monthly Fee & Wire Fee) : </div><div class="col p-1 text-end">-<?=@$post['ab']['account_curr_sys']?><font id="withdrawFeeCalcTextId"><? if(isset($_POST['inputWithdrawFeeCalc'])) echo $_POST['inputWithdrawFeeCalc']?></font></div>
		
	</div>
	
	<div class="col-6 p-1"><i class="<?=$data['fwicon']['circle-right'];?>"></i> <?=$data['ThisPageLabel'];?>/Payout&nbsp;Amount : 	</div><div class="col p-1 text-end"><b><?=@$post['ab']['account_curr_sys']?></b><b id="selectedAmtId"><?=(number_formatf_2($vAmt));?></b>
	 </div>
  ...
  <div style="display:none;word-wrap: break-word;margin: 0 0 10px 0;float: left;color: #9e9b9b;background: #fff;font-weight: normal;line-height: 140%;" class="btn-action single">You can
    ...
    <?=$post['amount'];?>
          ...
          <span class="input-group-text" id="basic-addon1"><?=$post['ab']['account_curr_sys'];?></span>
            <input required type="text" name="amount" id="amount_id" class="form-control" value="<?=(isset($_POST['amount'])?number_formatf_2($_POST['amount']):number_formatf_2($post['amount']));?>" <?=@$input_readonly?>  />
             </div>
        </div>
	...
	$("input[type='radio'].bType").click(function() {
		...
		if($(this).is(':checked')) {
			if ($(this).val() == '992') {
				...
				$('.newBankAdd').html('<a href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?action=addCryptoWallet">Add New Crypto Wallet<\/a>');
				//$('.newBankAdd').val('addCryptoWallet');	
                $('.newBankAdd').attr("data-value","addCryptoWallet");		
				$('.fa_totalAmt .paySumAmt, .fa_selectedBank .s24').html('');

                $('#bankList_id_ui option_div.crypto_list.bank_2').eq(0).trigger("click");
                $('#bankList_id_ui .dropdown-menu').removeClass('show');

				$('#bankList_id_ui .crypto_list').show(500);
				$('#bankList_id_ui .bank_list').hide();
				...
				$('#bankList_id_ui').removeAttr('required');
                ...
                $('.newBankAdd').html('<a href="<?=$data['USER_FOLDER']?>/bank<?=$data['ex']?>?action=addNewBank">Add New Bank<\/a>');
				$('.newBankAdd').attr("data-value","addNewBank");

                $('#bankList_id_ui option_div.bank_list.bank_2').eq(0).trigger("click");
                $('#bankList_id_ui .dropdown-menu').removeClass('show');
                
				$('#bankList_id_ui .bank_list').show(500);
				$('#bankList_id_ui .crypto_list').hide();
				
				<? if($data['bank_id']){?>
                  
                    $('#bankList_id_ui option_div.val_<?=$data['bank_id']?>').addClass('active');
					$('#bankList_id_val').val("<?=$data['bank_id']?>");	
				<? }?>
...
<div class="hide bank_accord_div_textArea" style="float:left;width:100%;"></div>
  ...
  <? }?>
    ...
    <input type="hidden" name="bank" id="bankList_id_val" value="<?=((isset($_POST['bank']) &&$_POST['bank'])?$_POST['bank']:'')?>">

  <div class="my-2 btn btn-primary btn-sm text-white  mx-1" id="bankList_id_ui" data-bs-toggle="dropdown" aria-expanded="false" style="position:relative;min-width:99%;"> 
            <i class="fa-regular fa-solid fa-landmark mx-1 text-success"></i>
            <label  id="bankList_label" class="my-2 fw-bold" >Select </label>     
    <div class="dropdown-menu" aria-labelledby="bankList_id_ui" style="min-width:100% !important;">
        <div class="visiting_container  ">
            <div class="visiting_header hide">
                <h3 id="bankList_label_3" style="font-size: 14px !important;">Account Manager</h3>
            </div>
            <div class="visiting_details">
                <option_div data-value="" class="hide">Select</option_div>
                ...
                ?>
                <option_div data-value='<?=$val['id'];?>' data-id="<?=$val['id']?>bank" class="hideXXX <?=($iconAcc);?>_list bank_<?=@$val['primary']?> val_<?=$val['id'];?>"  ><?=(@$val['primary']==2?'<i class="fa-regular fa-circle-check mx-1 text-success"></i>':'')?><b style="font-weight:bold"><?=ucwords(strtolower(prntext($val['bname'])))?></b> (... <?=encode(decrypts_string($val['baccount']),6,0);?>)</option_div>
                <? }?>
                
            
            </div>
            <option_div data-value="" class="visiting_details newBankAdd" >Add New Bank</option_div>
        </div>
    </div>
  ...
  <? }elseif($post['step']==4){ //step4 ?>
	...
	<div class="modal_msg_body" style="width: 300px; height: 270px;"> <a class="modal_msg_close" onclick="document.getElementById('modal_msg').style.display='none';">×</a>
	  ...
	  <a class="modal_msg_close2" onclick="document.getElementById('modal_msg').style.display='none';">Close</a></div>
		...
		<?
		//echo "<br/><hr/><br/>withdraw_gmfa=>"; print_r($data['withdraw_gmfa']); echo "<br/><hr/><br/>";
		if($data['withdraw_gmfa'])
		{
