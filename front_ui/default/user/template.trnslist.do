...
<div class="widget widget-gray widget-body-white" >
  ...
  <? if(isset($data['NameOfFile']) && (isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&(strtolower($post['settlement_optimizer'])=='manually'||strtolower($post['settlement_optimizer'])=='custom'||strtolower($post['settlement_optimizer'])=='weekly'))){ ?>
  <div class="breadcrumb" id="withdraw_fund_for_css" style="height:inherit;<?=((isset($data ['HideAllMenu'])&&$data ['HideAllMenu'])?"display:block !important;":"")?>">
    <? 
    if(isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&(strtolower($post['settlement_optimizer'])=='manually'||strtolower($post['settlement_optimizer'])=='custom'||strtolower($post['settlement_optimizer'])=='weekly'))
 	  include("../include/trans_balance_ui_withdraw_v3_custom".$data['iex']);
 	  else  include("../include/trans_balance_ui".$data['iex']);
  ?>
    <div class="row w-100">
