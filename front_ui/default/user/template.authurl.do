...
<div class="containerMain processing">
	...
	<div class="my-2 fs-5 text-primary-emphasis"><?=get_currency($getresponse['bill_currency']);?><?=$getresponse['bill_amt']?></div>
	
	<? if(@$pay_type!="upi"){ ?>
	<div class="my-2 fs-6 fw-bold text-info-emphasis"> TransID : <?=$getresponse['transID']?></div>

	<? if(@$getresponse['reference']){ ?><div class="text-muted my-2 fs-6"> Reference : <?=$getresponse['reference']?></div><? } ?>
