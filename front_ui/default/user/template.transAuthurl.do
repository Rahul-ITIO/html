...
function nexturl_f($url){
	...
	if(($data['moto_vt']==true||$getresponse['order_status']==0)&&($_SESSION['s30_count_1']<1)&&(isset($getresponse['type'])&&!in_array($getresponse['type'],["27","28"]))){
...
<div class="containerMain processing">
	...
	<div class="text-header"><?=$data['header_msg']?></div>
	<div class="hr">&nbsp;</div>
	<? if($getresponse['trans_response']){?>
		<h4 class="text-reason">Trans Response  : <?=($getresponse['trans_response']?$getresponse['trans_response']:$getresponse['status']);?></h4>
	<? } ?>
	<h4 class="text-amt">Amount : <?=get_currency($getresponse['bill_currency']);?><?=$getresponse['bill_amt']?></h4>
	<div class="text-muted"> TransID : <?=$getresponse['transID']?></div>
