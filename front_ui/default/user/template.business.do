      ...
      <table id="display-large-screen" class="table table-hover bg-primary text-white">
        ...
        <tbody>
          ...
          <tr>
			...
			<? if($value['public_key']){ ?>
      <a data-value="<?=(@$value['public_key'])?>" title="Copy Public Key - <?=($value['public_key'])?>" onclick="copytext_f(this,'Public Key for TerNO - <?=($value['id'])?> ')" class="btn btn-icon btn-primary btn-sm show"><i class="<?=$data['fwicon']['copy'];?>"></i> </a>
        ...
        <div class="col-sm-8 text-end  my-2">
            <div>
              ...
              <!--//============ Private Key :: For Api Encoded ===========-->
                ...
                <a id="generate_private_key2_link" data-title="Copy Private Key for Encryption - " data-value='<?=@$_SESSION['generate_private_'.$_SESSION['uid']];?>' class="btn btn-icon btn-primary btn-sm <?=((isset($_SESSION['generate_private_'.$_SESSION['uid']])&&$_SESSION['generate_private_'.$_SESSION['uid']]<>"")?'show':'hide');?>" title="Copy Private Key for Encryption - <?=@$_SESSION['generate_private_'.$_SESSION['uid']];?>"  onclick="copytext_f(this,'Private Key for Encryption')" data-bs-toggle="tooltip" data-bs-placement="bottom" ><i class="<?=@$data['fwicon']['copy'];?>" ></i></a>
              ...
              <script>
                ...
                function confirm1(e){
                  ...
                  //alert();
                    ...
                    var generate_public_key_url='<?=$data['USER_FOLDER'];?>/<?=($data['PageFileName']);?><?=$data['ex']?>?bid=<? if(isset($post['bid'])) echo $post['bid'];?>&id=<?=$post['gid']?>&action=re_generate_public_key&ajax=1';
                ...
                function confirm2(e){
                  ...
                  if (retVal) 
                  {
                    ...
                    var generate_private_key_url='<?=$data['USER_FOLDER'];?>/<?=($data['PageFileName']);?><?=$data['ex']?>?bid=<? if(isset($post['bid'])) echo $post['bid'];?>&id=<?=$post['gid']?>&action=generate_private_key&ajax=1';
...
<link href="<?=$data['TEMPATH']?>/common/css/chosen/chosen.min.css" rel="stylesheet"/>
        ...
        <div class="px-1 col-sm-6">
          ...
          <textarea class="form-control" name="business_nature"  id="input_business_nature" title="Enter your Business Description in short" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off"><? if(isset($post['business_nature'])) echo $post['business_nature']?></textarea>

             <label for="input_business_nature">Business Description</label>
		  </div>
        </div>
