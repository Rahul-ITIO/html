...
.input-field select:focus+label, .input-field select:valid+label { position: absolute;top: -10px;left: 6px;font-size: 13px;}
...
<p class="text-center fw-bold my-2">Make a payment to <?=prntext($_SESSION['info_data']['company_name'])?></p>
<p class="shadow-none p-2 bg-light rounded fs-5"><?=$post['curr']?$post['curr']:$_SESSION['curr'];?> <?=($post['total']?$post['total']:$post['price'])?></p>

							
<div class="inputWithcheck mx-2"> <input class="form-control w-100 border border-2" style="background-color: #ffffff !important;width: 100% !important;" type="hidden" value="Pay <?=get_currency($post['curr']?$post['curr']:$_SESSION['curr']);?> <?=($post['total']?$post['total']:$post['price'])?> to <?=prntext($_SESSION['info_data']['company_name'])?>" readonly="">  </div>
	  ...
	  if(isset($post['nick_name_card'])&&!empty($post['nick_name_card'])){?>
		...
		?>
			...
			<input type="radio" value="<?=(isset($value['nick_name'])?$value['nick_name']:'');?>" id="<?=(isset($value['nick_name'])?$value['nick_name']:'');?>" required name="account_types" class="account_types" data-value="<?=(isset($value['nick_name'])?$value['nick_name']:'');?>" data-name="ecard" data-ccn="<?=(isset($_SESSION['b_'.$value['nick_name']]['account_custom_field_14'])?$_SESSION['b_'.$value['nick_name']]['account_custom_field_14']:'')?>" data-currency="<?=$pcrcy;?>" data-ewlist="<?=(isset($data['t'][$value['nick_name']]['name6']))?$data['t'][$value['nick_name']]['name6']:'';?>"  data-etype="<?=(isset($data['t'][$value['nick_name']]['name4']))?$data['t'][$value['nick_name']]['name4']:'';?>"  data-count='<? if(isset($_SESSION["b_".$value['nick_name']]['deCon'])) echo $_SESSION["b_".$value['nick_name']]['deCon'];?>' data-dnmc='<? if(isset($_SESSION["b_".$value['nick_name']]['donotmachcountries'])) echo $_SESSION["b_".$value['nick_name']]['donotmachcountries'];?>' style="display:none;" />
						...
						<span class=" float-start align-middle" style=""><i class="<?=$data['fwicon']['credit-card'];?> img-hover"></i></span>
                        <? if(isset($value['checkout_level_name'])&&$value['checkout_level_name']){ echo $value['checkout_level_name'];}else{?>
                        Pay by Card [
...
<form id="zts-echeck" method="post" action="<?=$data['Host'];?>/echeckprocess<?=$data['ex']?>" onSubmit="return validateForm(this);">
                    <div class="ecol2">
                      <div style="display:none">
						...
						<input type="hidden" name="gid" value="<?=(isset($_SESSION['clientid'])?$_SESSION['clientid']:'');?>"/>
						<input type="hidden" name="bid" value="<?=(isset($post['bid'])?$post['bid']:'')?>"/>
                        ...
                        <div class="front">
                          ...
                          <!--<div class="number">-->
							...
							<i id="input-card-icon" class="<?=$data['fwicon']['credit-card'];?> display-inner"></i>
                              <input id="number" type="tel" name="ccno" maxlength="19" placeholder="Card Number" value='<?=(isset($post['ccno'])&&$post['ccno'])?$post['ccno']:"";?>' class="iconvisa form-control " title="Card Number" pattern=".{16,21}" required  />                <label for="number">Card Number</label>
                            </div>
						  ...
						  <div class="form-group input-field mt-3">
							...
							<input type="text" class="form-control" title="Card Holder Name" value="<?=(isset($post['fullname'])&&$post['fullname']?prntext($post['fullname']).'':'');?>" placeholder="Card Holder Name" id="input_name_hidden" required />
                          ...
                          <option value="" disabled="disabled"  selected="selected" >yy</option>
                            ...
                            <div class="float-end input-field" style="width:70px;">
                              ...
                              <input id="cvc" type="password" name="ccvv" maxlength="4" size="30" placeholder="cvc" value="<?=(isset($post['ccvv'])&&$post['ccvv'])?$post['ccvv']:"";?>" class="form-control" required  title="Three digit CV code on the back of your card" >
...
<? if(isset($post['t_name6'])&&strpos($post['t_name6'],'SabPaisawalletList') !== false){?>
                  ...
                  <div class="row mt-1">
					...
					<div class="col-sm-4 border p-2 desImg 17">
					<div class="text-center"><img src="<?=$data['bankicon']['icici'];?>" title="ICICI" class="bank_img"/></div>
					<span class="bankNm txNm">ICICI</span>
					</div>
					
					<div class="col-sm-4 border p-2 desImg 26">
					<div class="text-center"><img src="<?=$data['bankicon']['kotak'];?>" title="Kotak" class="bank_img"/></div>
					<span class="bankNm txNm">Kotak</span>
					</div>
					
					<div class="col-sm-4 border p-2 desImg 8006">
					<div class="text-center"><img src="<?=$data['bankicon']['idfc'];?>" title="IDFC" class="bank_img"/></div>
					<span class="bankNm txNm">IDFC</span>
					</div>
					
					<div class="col-sm-4 border p-2 desImg 41">
					<div class="text-center"><img src="<?=$data['bankicon']['yes'];?>" title="Yes Bank" class="bank_img"/></div>
					<span class="bankNm txNm">Yes Bank</span>
					</div>
					
					<div class="col-sm-4 border p-2 desImg 18">
					<div class="text-center"><img src="<?=$data['bankicon']['idbi'];?>" title="IDBI Bank" class="bank_img"/></div>
					<span class="bankNm txNm">IDBI Bank</span>
					</div>
					
					<div class="col-sm-4 border p-2 desImg 30">
					<div class="text-center"><img src="<?=$data['bankicon']['pnb'];?>" title="PNB" class="bank_img"/></div>
					<span class="bankNm txNm">PNB</span>
					</div>
...
function cardvalidorf(){
    ...
    cardNumber2.on('keyup3 keypress change', function(e){
