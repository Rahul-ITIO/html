			...
			<div class="row d-flex77 justify-content-center customizecss" id="printarea">
				<div class="col-md-8" style="max-width:350px !important;">
					<div class="card">
						...
						$merchantId=$pn	= $json_value['siteid_get']['merchantId'];
						...
						if(isset($post['softpos_terNO'])&&$post['softpos_terNO'])
						{
							...
							//Dev Tech : 24-10-22 fetch value from admin save in soft via hard code value 
							...
							if($validate)
							{
								...
								$url = "upi://pay?pa=".$vpa."&pn=".$pn."&tr=".$sub_merchantId."&cu=".$curr."&mc=".$mcc_code;
								
								if(isset($_REQUEST['qp'])){
									echo "<br/><hr/>QR_CODE_URL=><br/>".$url;
								}

								$url = urlencode($url);

								if(isset($_REQUEST['qp'])){
									echo "<br/><hr/>final qr_code_url=><br/>".$url;
								}
