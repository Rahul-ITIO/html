  ...
  <form method="post">
    ...
    <div class="row m-2 border  rounded">
      ...
      <?if(isset($_REQUEST['action'])&&$_REQUEST['action']=='add'){?>
        <div class="col-sm-12 my-2">
          <div class="input-group"> <span class="input-group-text col-sm-3" id="basic-addon4">Template Key :</span>
          <input type="text" name="template_key" class="form-control"  required  value="<?=($post['template'])?>_ADD">
          </div>
        </div>
      <?}?>
      ...
      <div class="col-sm-12 my-2">
      <?if(isset($_REQUEST['action'])&&$_REQUEST['action']=='add'){?>
        <button class="btn btn-primary submit" type="submit" value="Submit" name="add"><i class="<?=$data['fwicon']['check-circle'];?>"></i> Add New</button>
        <?}else{?>
