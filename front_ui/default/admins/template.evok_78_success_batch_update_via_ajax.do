...
<div class="sm-col-12" style="text-align:left;">
...
<textarea name='trans_ids' placeHolder='Enter TransID with comma separated values' style='width:100%;height:96px;clear:both;display:none;'><?=@$post['trans_ids'];?></textarea>
...
valuesArray=<?=json_encode(@$_SESSION['transArrayList'])?>;
var subQuery_javascript = "<?=$post['subQuery_javascript']?>";
var subQuery_jsonEncode=<?=@$post['subQuery_jsonEncode']?>;

 var downloadHtml3_pre_approved_tr = '<?=implode(",",@$_SESSION['pre_approved_tr'])?>';
		...
		$('#modal_popup_iframe_div').html(my_value);
			...
			if (valuesArray == null || valuesArray == "") {
				...
				if(count>0)
				{
					...
					function load_f() {
						c=i+1;
                        transID = ary[i]['transID'];
                        console.log("===================ary 1=> "+ary[i]+"=====================================");
						if(wn){ alert("ary=>"+ary[i]+", i=>"+i+", c=>"+c); }

						console.log("===================transID=> "+transID+" | count=> "+count+" | i=> "+i+" | c=> "+c+"=====================================");
						...
						response="<center><br>In Process... "+c+" - "+ary[i]['transID']+"<br><br><b>Do not close the window. Please wait...</b></center><br>";
						...
						{
                            data=ary[i]['data_send'];
                            data_json = JSON.stringify(data);

                            console.log("===================data_send=> "+data+"=====================================");
                            console.log("===================data in Json=> "+data_json+"=====================================");
									...
									//if(data && (statusTxt == "success")) {
										...
										$("#trans_url_load_content_2").load(return_url, data, function(responseText_2, statusTxt_2, jqXHR_2){
											...
											if (c==count){
												...
												var html ='';
												html +=  "<==Already successful, no action taken==>\r\n"+downloadHtml3_pre_approved_tr+"\r\n\r\n";
