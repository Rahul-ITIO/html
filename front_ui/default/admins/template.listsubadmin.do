<? if(isset($data['ScriptLoaded'])){ 
	...
	if(isset($data['PCI_SCAN_DURING_ON'])&&@$data['PCI_SCAN_DURING_ON']=='Y') $rel_noreferrer=' rel="noopener noreferrer" ';
	else $rel_noreferrer='';
?>
<style>
...
<a title="SubAdmin Display Option" data-bs-toggle="tooltip" data-bs-placement="right" class="btn btn-primary btn-sm" onclick="view_next3(this,'')"><i class="<?=$data['fwicon']['transaction-display-option'];?>"></i></a>
...
<select id="transaction_display" data-placeholder="SubAdmin Display Option" title="SubAdmin Display Option" multiple class="chosen-select chosen-rtl1 form-select" name="transaction_display[]">
...
if(isset($_SESSION['subadmin_display_arr'])&&is_array($_SESSION['subadmin_display_arr']))
{
	...
	<td scope="col" valign="top" >
		...
		if(isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y')
		{
			...
			$acc_man_id=account_manager_fetch(@$value['account_manager_id'],1);

			if(isset($acc_man_id['manager_name']))

			
			{ ?>
				<div title="Account Manager : <?=@$acc_man_id['manager_name'];?>" class="my-2 btn btn-primary btn-sm text-white  mx-1" id="acc_sub" data-bs-toggle="dropdown" aria-expanded="false" style="position: relative;"> 
							<img src="../images/account_manager_icon_1.png" style="height:16px;" />
				
					<div class="dropdown-menu" aria-labelledby="acc_sub" style="min-width: 210px !important;">
						<div class="visiting_container  ">
							...
							<div class="visiting_profile">
								...
								<div class="visiting_info">
									<h3><?=@$acc_man_id['manager_name'];?></h3>
								</div>
							</div>
							<div class="visiting_details"><?=@$acc_man_id['am_icon'];?></div>
						</div>
					</div>
	...
	<div class="btn-group dropstart">
			...
			<? } ?>
				...
				<li><a href="<?=$data['Admins']?>/subAdminLogin<?=$data['ex']?>?bid=<?=$value['id']?>" target="_blank" <?=@$rel_noreferrer?> title="Click to Login" data-bs-toggle="tooltip" class="dropdown-item" style="color:<?=$themecolor?>;"><i class="<?=$data['fwicon']['login'];?> fa-fw"></i> Login</a></li> 
				
				<li><a href="<?=$data['Admins'];?>/history<?=$data['ex']?>?clients=<?=$value['id']?>&t=sadm" target="_blank" <?=@$rel_noreferrer?> 
	...
	<a href="<?=$link?>" target="_blank" <?=@$rel_noreferrer?>  title="Domain Url" style="color:<?=$themecolor?> !important;"><?=$value[$val]?></a></div>
	<? }else{ ?>
...
<!-- for chosen selected fields dragable -->
<script>
