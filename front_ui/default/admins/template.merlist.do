...
<script>
function settlement_optimizer_f(theVal){
	//alert(theVal);
	if(theVal=='weekly')
	   {
			//alert(settlement_optimizer);
			//$("#settlement_optimizer_weekly").show(100);
			$("#day_name_div").show(1000);
			$("#week_delay_div").show(1000);
			$("#pdf_report_div").show(1000);
			
	   }
	   else {
			//$("#settlement_optimizer_weekly").hide(100);
			$("#day_name_div").hide(100);
			$("#week_delay_div").hide(100);
			$("#pdf_report_div").hide(100);
	   }
}
...
<button type="submit" class="dropdown-item" name="login" value="login" style="background: none; border: none;color:<?=@$themecolor?>; cursor: pointer;" /><i class="<?=@$data['fwicon']['login'];?> fa-fw"></i> Login</button>
  ...
  <? }else{ ?>
  <div class=" my-2" role="alert" style="position:relative">
	<center style="position:relative">
		<div class="position-absolute top-50 start-50 translate-middle p-3 mb-2 bg-primary bg-gradient fs-4 rounded">
			<strong><?=ucwords(@$_GET['type']);?> Merchant Not Found</strong>
		</div>
		<img class=" img-fluid Ximg-thumbnail" src="<?=@$data['Host']?>/images/result_search.png" style="width:100%;" />
	</center> 
	</div>
  <? } ?>
  <? }
	elseif($post['action']=='detail'){
		...
		//if(isset($_SESSION['uid_'.$post['gid']]))unset($_SESSION['uid_'.$post['gid']]);
		
		//if(isset($_SESSION['uid_wv2'.$post['gid']])) unset($_SESSION['uid_wv2'.$post['gid']]);
...
#######################################################
    ...
    if(isset($_SESSION['uid_'.$post['gid']]['trans_detail_array'])) unset($_SESSION['uid_'.$post['gid']]['trans_detail_array']);
    if(isset($_SESSION['uid_'.$post['gid']]['ab'])) unset($_SESSION['uid_'.$post['gid']]['ab']);
    if(isset($_SESSION['uid_'.$post['gid']]['payout'])) unset($_SESSION['uid_'.$post['gid']]['payout']);
    if(isset($_SESSION['uid_'.$post['gid']]['deduction_array_ajax'])) unset($_SESSION['uid_'.$post['gid']]['deduction_array_ajax']);
    if(isset($_SESSION['uid_'.$post['gid']]['payout_array_ajax'])) unset($_SESSION['uid_'.$post['gid']]['payout_array_ajax']);
    ...
    //unset array value withdraw for v2

    if(isset($_SESSION['uid_wv2'.$post['gid']]['trans_detail_array'])) unset($_SESSION['uid_wv2'.$post['gid']]['trans_detail_array']);
    if(isset($_SESSION['uid_wv2'.$post['gid']]['ab'])) unset($_SESSION['uid_wv2'.$post['gid']]['ab']);
    if(isset($_SESSION['uid_wv2'.$post['gid']]['payout'])) unset($_SESSION['uid_wv2'.$post['gid']]['payout']);
    if(isset($_SESSION['uid_wv2'.$post['gid']]['deduction_array_ajax'])) unset($_SESSION['uid_wv2'.$post['gid']]['deduction_array_ajax']);
    if(isset($_SESSION['uid_wv2'.$post['gid']]['payout_array_ajax'])) unset($_SESSION['uid_wv2'.$post['gid']]['payout_array_ajax']);

    
    if(isset($_SESSION['uid_wv2'.$post['gid']])){
        unset($_SESSION['uid_wv2'.$post['gid']]);
    }
    
    if(isset($_SESSION['last_withdraw_micro_current_date_'.$post['gid']]))
        unset($_SESSION['last_withdraw_micro_current_date_'.$post['gid']]);
...
#######################################################
  ...
  <div class="mywrap-collabsible tog1">
	...
	{ ?>
		...
		<a onClick="closealltab(5)" href="<?=@$data['Admins'];?>/withdraw_id_date_admin<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Withdrawal Date Wise for match the payout and not initiate the Withdrawal. " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> Date Wise v2</a> 
		
	
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_wv3<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Withdrawal Date Wise for match the payout and not initiate the Withdrawal. " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> Real Time Payout v3</a> 
		
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_date_v3_custom<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Withdrawal V3 Date Wise for custom in Settlement Optimizer " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> V3 Custom ...</a> 
		
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_date_v3_custom_2<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Withdrawal V3 Date Wise for custom in Settlement Optimizer via sum_f  " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> V3 Custom 2...</a> 
		
	
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_from_date_v3_custom<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Withdrawal V3 - From and To Date Wise for custom in Settlement Optimizer " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> V3 Custom Date Wise</a> 
		
	
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_from_date_v3_custom<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>&im=y&ro=y" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Rolling Withdrawal V3 - From and To Date Wise for custom in Settlement Optimizer " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> Rolling V3 </a> 
	...
	<?if(isset($_SESSION['settlement_optimizer'])&&(strtolower($_SESSION['settlement_optimizer'])=='custom'||strtolower($_SESSION['settlement_optimizer'])=='weekly')&&isset($data['CUSTOM_SETTLEMENT_WD_V3'])&&$data['CUSTOM_SETTLEMENT_WD_V3']=='Y'){?>
		<a onClick="closealltab(5)" href="<?=@$data['Host'];?>/include/withdraw_date_v3_custom<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Reals Time Withdrawal Date Wise for custom in Settlement Optimizer " data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> V3 Custom :: <?=strtoupper(@$_SESSION['settlement_optimizer'])?></a> 
	<?}
	else
	{
	?>
		<a onClick="closealltab(5)" href="<?=@$data['USER_FOLDER'];?>/trans_withdraw-fund_system_v2<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['id'];?><?=(isset($_GET['qp'])?'&qp='.$_GET['qp']:'');?>" target="iframe_withdrawal" class="btn btn-primary  " title="Withdrawal v2" data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['withdrawal'];?>"></i> v2</a> 
...
<div class="float-end">
	...
	if((isset($_SESSION['login_adm']))||(isset($_SESSION[$mtype])&&$_SESSION[$mtype]==1))
	{
		if((isset($_SESSION['login_adm']))||(isset($_SESSION['clients_total_transaction'])&&$_SESSION['clients_total_transaction']==1)){ ?>
            ...
            <a  data-mid="<?=@$_GET['id']?>" onclick="return confirm('Are you sure to reset password?')" href="<?=@$data['Admins'];?>/passwordreset<?=@$data['ex']?>?bid=<?=@$_GET['id'];?>" class="sub_logins22 btn btn-icon btn-primary " target="hform" title="Reset Password" data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=@$data['fwicon']['reset-password'];?>"></i></a> 
      ...
      <div id="collapsible1_html" class="content-inner"  style="padding:0 !important;">
        ...
        <div class="row text-start row text-start rounded" style="clear:both;">
          ...
          <div class="col-sm-12 my-2 link-hover">
			...
			<? if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y'&&isset($post['MemberInfo']['account_manager_view']['manager_name']))&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1))){ ?>
				<div title="Account Manager : <?=@$post['MemberInfo']['account_manager_view']['manager_name'];?>" class="my-2 btn btn-primary btn-sm text-white  mx-1" id="acc_manag" data-bs-toggle="dropdown" aria-expanded="false"> 
					<img src="../images/account_manager_icon_1.png" style="height:20px;" />
				</div>
				<div class="dropdown-menu" aria-labelledby="acc_manag">
					<div class="visiting_container  ">
						...
						<div class="visiting_profile">
							...
							<div class="visiting_info">
								<h3><?=@$post['MemberInfo']['account_manager_view']['manager_name'];?></h3>
							</div>
						</div>
						<div class="visiting_details"><?=@$post['MemberInfo']['account_manager_view']['am_icon'];?></div>
					</div>
				</div>
			<? }?>	
...
<div class="aq_n1 row1 m-2 mt-3 hide1">
	  ...
	  ?>
		...
		<div class="row row-cols-2 row-cols-lg-4 g-2 g-lg-3">
			  ...
			  <? }
			  if(@$post['PayinInfo']['settlement_optimizer']=='weekly'){ ?>
				<div class="col-sm-4 my-2 px-2 card"> <strong>Day Name : </strong>
					<?=label_namef(@$post['PayinInfo']['day_name']);?>
				</div>
				<div class="col-sm-4 my-2 px-2 card"> <strong>Week Delay : </strong>
					<?=label_namef(@$post['PayinInfo']['week_delay']);?>
				</div>
				<div class="col-sm-4 my-2 px-2 card"> <strong>PDF Report : </strong>
					<?=label_namef(@$post['PayinInfo']['pdf_report']);?>
				</div>
          ...
          <div class="span12">
            <div class="bg_div1 row mt-1 ps-1">
              <div class="input_col_2 row vkg my-2 ps-0">
				...
				<!-- Dev Tech: 24-12-30 Account manager ID assing by Admin only	-->
                ...
                <div class="col-sm-4">
                  <label for="account_manager_id">Account Manager ID</label>
                  <input type="text" class="form-control w-100" id="account_manager_id" list="account_manager_list" name="account_manager_id" value="<?php if(isset($post['account_manager_id'])){ echo $post['account_manager_id'];}?>">
                
                <datalist id="account_manager_list">
                  <?//=showselect($post['account_manager_option'], (isset($post['account_manager_id'])?$post['account_manager_id']:0))?>
                  <?=@$post['account_manager_option'];?>
                </datalist>
               
                <script>$("#account_manager_id").val('<?=@$post['account_manager_id']?>').trigger("chosen:updated");</script>
				</div>
                <? } ?>
...
function UnCheckBox(){
  ...
  if ((document.getElementById('registered_address_bs') != null) && (document.getElementById('Same_Registered_Address') != null) && (document.getElementById('registered_address_bs') != null) && ((document.getElementById('registered_address_bs').value)==(document.getElementById('registered_address_bs').value)))
  {document.getElementById('Same_Registered_Address').checked=true;}
  else if(document.getElementById('Same_Registered_Address') != null) {document.getElementById('Same_Registered_Address').checked=false;}
</script>
  <? }else{ ?>
  ...
  <div class="table-responsive">
    ...
    <? }elseif($post['action']=='insert_bank'||$post['action']=='update_bank'){ 
		...
		if(isset($_SESSION['uid']))
		{
			...
			if(isset($_SESSION['uid'.$uid]['trans_detail_array'])) unset($_SESSION['uid'.$uid]['trans_detail_array']);
			if(isset($_SESSION['uid'.$uid]['ab'])) unset($_SESSION['uid'.$uid]['ab']);
			if(isset($_SESSION['uid'.$uid]['payout'])) unset($_SESSION['uid'.$uid]['payout']);
 ...
 <? } ?>
            ...
            <div class="col-sm-6  ps-0 mb-2">
				...
				<div class="my-1"><span class="form-label w-100" id="basic-addon4" title="Bank Document">Bank Document</span>
					...
					<input <?=@$ep;?> name="upload_logo" value="<? if(isset($post['bank_doc'])) echo $post['bank_doc']?>"
						type="hidden" class="">
					<div class="show_img_img w-25 float-start" id="show_img_<? if(isset($post['bid'])) echo $post['bid']?>">
						...
						<? }else{ ?>
						<a onclick="uploadfile_viewf(this)"> <img src="<?=@$data['Host']?>/images/No_image_available.svg"
								style="height:36px !important;" class="img-thumbnail" /></a>
						<? } ?>
					</div>
				</div>
			</div>
			...
			<div class="col-sm-6 ps-0 mb-2">
             <span class="form-label d-none d-sm-block" id="basic-addon4" title="Settlement Webhook Url">Settlement Webhook Url </span>
              <input type="text" class="form-control" name="settlement_webhook_url" id="settlement_webhook_url" placeholder="Settlement Webhook Url" title="Settlement Webhook Url" value="<? if(isset($post['settlement_webhook_url'])) echo $post['settlement_webhook_url']?>" >
            </div>
        ...
        <div class="p-0">
          ...
          <div class="row my-2 me-2">
			 ...
			 <div class="col-sm-6">
				...
				?>
                <script>
				$('#settlement_optimizer option[value="<?=@$post['settlement_optimizer']?>"]').prop('selected','selected').trigger('change');
			...
			<!-- start: settlement_optimizer_weekly	-->
			<div class="row hide" id="settlement_optimizer_weekly">	
			</div>
				<div class="col-sm-6 hide" id="day_name_div">
					<label for="day_name">Day: <i class="<?=@$data['fwicon']['star'];?>  text-danger"></i> <span data-bs-toggle="tooltip" title="" data-bs-original-title="Day name for weekly settlement"><i class="<?=@$data['fwicon']['circle-info'];?>" style="font-size:8px;"></i></span> </label>
					<select name="day_name" id="day_name" class="form-select w-100">
					...
					<?
					if(isset($post['day_name'])) 
					{
					?>
					<script>$('#day_name option[value="<?=@$post['day_name']?>"]').prop('selected','selected');</script>
					<?
					}?>                
				</div>	 
				
				<div class="col-sm-6 hide" id="week_delay_div">
					<label for="week_delay">Week Delay: <i class="<?=@$data['fwicon']['star'];?>  text-danger"></i> <span data-bs-toggle="tooltip" title="" data-bs-original-title="Week Delay for weekly settlement"><i class="<?=@$data['fwicon']['circle-info'];?>" style="font-size:8px;"></i></span> </label>
					<select name="week_delay" id="week_delay" class="form-select w-100">
						<option value="">Week Delay</option>
					...
					<?
					if(isset($post['week_delay'])) 
					{
					?>
					<script>$('#week_delay option[value="<?=@$post['week_delay']?>"]').prop('selected','selected');</script>
					<?
					}?>                
				</div>	 
				
				<div class="col-sm-6 hide" id="pdf_report_div">
					<label for="pdf_report">Week Delay: <i class="<?=@$data['fwicon']['star'];?>  text-danger"></i> <span data-bs-toggle="tooltip" title="" data-bs-original-title="Week Delay for weekly settlement"><i class="<?=@$data['fwicon']['circle-info'];?>" style="font-size:8px;"></i></span> </label>
					<select name="pdf_report" id="pdf_report" class="form-select w-100">
						<option value="">Select PDF Report</option>
						<option value="download">Download Link in Mer.</option>
						<option value="weekly_date_wise">Weekly Date Wise</option>
						<option value="weekly_save_json_wise">Weekly Save Json Wise </option>
					</select>
					<?
					if(isset($post['pdf_report'])) 
					{
					?>
					<script>$('#pdf_report option[value="<?=@$post['pdf_report']?>"]').prop('selected','selected')</script>
					<?
					}?>                
				</div>	 
				
						
			<!-- end: settlement_optimizer_weekly	-->
      ...
      <div class="row my-2 text-start vkg" style="#f7f7f7;overflow:hidden;">
        ...
        <div class="p-0">
          ...
          <div class="row text-start vkg border border-primary bg-vlight rounded" style="padding:5px 0;margin:10px 0;">
                ...
                <div class="col-sm-6">
                  <label for="sub_merchantId">Sub MerchantId: <i class="<?=@$data['fwicon']['star'];?> text-danger"></i></label>
                  <input type="text" name="sub_merchantId" id="sub_merchantId" class="form-control" value="<?=(isset($post['sub_merchantId'])&&$post['sub_merchantId']?$post['sub_merchantId']:'');?>"  required  />
                </div>
			
                <div class="col-sm-6">
                  <label for="currency">Currency: <i class="<?=@$data['fwicon']['star'];?> text-danger"></i></label>
                  <input type="text" name="currency" id="currency" class="form-control" value="<?=(isset($post['currency'])&&$post['currency']?$post['currency']:'');?>"  required  />
                </div>
		...
		<div class="col-sm-6">
			<input type="text" name="panNumber" id="panNumber" placeholder="PAN number" class=" form-control" maxlength="10" value="<?=(isset($post['panNumber'])?$post['panNumber']:'')?>" autocomplete="off" pattern="[a-zA-Z]{5}[0-9]{4}[a-zA-Z]{1}" <? if(isset($post['action'])&&$post['action']=='update'&&isset($post['bid'])&&$post['bid']) echo 'readonly';?> onblur="checkvpa()" required data-bs-toggle="tooltip" title="Please enter valid PAN number. E.g. AAAAA9999A" data-bs-placement="top" style="text-transform: uppercase;" />
		</div>
		...
		?>
			...
			<div class="col-sm-6">
				<input type="text" name="json_value" placeholder="json value" title="Profile Pic Url" class=" form-control" value="<?=(isset($post['json_value'])?$post['json_value']:'')?>" autocomplete="off" data-bs-toggle="tooltip" data-bs-placement="top" />
				<label class="form-text ms-2">e.g. https://www.abc.com/profile-pic.png</label>
			</div>
...
<?
            ...
            <div class="col-sm-10 my-1" >
				<?
					...
					{
						echo "<br/><hr/><br/> terNO post['bid']=>".$post['bid'];
						echo "<br/><br/>clientId post['gid'] from id=>".$post['gid'];
						echo "<br/><hr/><br/>";
					}
				?>

				<!--//============ Private Key :: For Api Encoded ===========-->
             
              	<a onclick="return confirm2(this);" class="btn btn-icon btn-primary btn-sm" title="Re-generate Private Key" data-bs-toggle="tooltip" data-bs-placement="bottom"><img src="<?=@$data['Host']?>/images/refresh_private_key.png" style="height:18px" /></i></a> 
			  
				
				<a id="generate_private_key2_link" data-title="Copy Private Key for Encryption - " data-value='<?=@$_SESSION['generate_private_'.$post['gid']];?>' class="btn btn-icon btn-primary btn-sm <?=((isset($_SESSION['generate_private_'.$post['gid']])&&$_SESSION['generate_private_'.$post['gid']]<>"")?'show':'hide');?>" title="Copy Private Key for Encryption - <?=@$_SESSION['generate_private_'.$post['gid']];?>"  onclick="copytext_f(this,'Private Key for Encryption')" data-bs-toggle="tooltip" data-bs-placement="bottom" ><i class="<?=@$data['fwicon']['copy'];?>" ></i></a>
				
				<!--//============ Public Key :: For Api Token ===========-->
				
				<a onclick="return confirm1(this);" class="btn btn-icon btn-primary btn-sm" title="Re-generate Public Key" data-bs-toggle="tooltip" data-bs-placement="bottom"><i class="<?=@$data['fwicon']['rotate'];?>"></i></a> 
				
				<a id="generate_public_key_link" data-title="Copy Public Key - " data-value='<?=(isset($post['public_key'])?$post['public_key']:'');?>'  class="btn btn-icon btn-primary btn-sm <?=((isset($post['public_key'])&&$post['public_key']<>"")?'show':'hide');?>"  title="Copy Public Key - <?=(isset($post['public_key'])?$post['public_key']:'');?>" onclick="copytext_f(this,'Public Key')" data-bs-toggle="tooltip" data-bs-placement="bottom" ><i class="<?=@$data['fwicon']['copy'];?>"></i></a>
		    ...
		    <script>
			function confirm1(e){
				var retVal= confirm('Are you sure to Re-generate Public Key!');
				if (retVal) {
				//alert();

					var generate_public_key_url='<?=@$data['Admins'];?>/<?=@$data['my_project']?><?=@$data['ex']?>?bid=<?=(isset($post['bid'])?$post['bid']:'');?>&id=<?=(isset($post['gid'])?$post['gid']:'');?>&action=re_generate_public_key&ajax=1';

					if(wn){
						window.open(generate_public_key_url+'&dtest=2','_blank');
						return false;
					}

					ajaxf3(e,generate_public_key_url,'#generate_public_key_link');
					
					$('#generate_public_key_link').css("display", "inline-block");
					return true;
					//e.preventDefault();
			...
			function confirm2(e){
				var retVal= confirm('Are you sure Re-generate Private Key!');
				if (retVal) 
				{
					var generate_private_key_url='<?=@$data['Admins'];?>/<?=@$data['my_project']?><?=@$data['ex']?>?bid=<?=(isset($post['bid'])?$post['bid']:'');?>&id=<?=(isset($post['gid'])?$post['gid']:'');?>&action=generate_private_key&ajax=1';

					if(wn){
						window.open(generate_private_key_url+'&dtest=2','_blank');
						return false;
					}
					ajaxf3(e,generate_private_key_url,'#generate_private_key2_link');

					$('#generate_private_key2_link').css("display", "inline-block");
					return true;
					//e.preventDefault();
	...
	?>
              <div class="row mx-2" id="mer_website_css">
                <? $c=0;if($post['AccountInfo']){foreach($post['AccountInfo'] as $key=>$value_ac){$c++;if($value_ac['acquirer_id']&&$value_ac['acquirer_processing_mode']!='3'){if($value_ac['acquirer_processing_currency']&&strpos($value_ac['acquirer_processing_currency'],' ')!==false){$curr_ex=explode(' ',$value_ac['acquirer_processing_currency']);$curr=$curr_ex[0];}else{$curr="$";}
					...
					@$value_ac['acquirer_id']=ltrim(@$value_ac['acquirer_id'],",");

					if(isset($_REQUEST['qp']))
					{
						echo "<br/>qp=>".@$value_ac['acquirer_id'];
					}
				 
					
				if(!is_array($sj)) 
				{
					$sj=[]; // Initialize as an empty array if $sj is not an array
				}
				
				 if(!isset($sj)&&!isset($sj['terNO_json'][$value_ac['acquirer_id']])) 
				  	@$sj['terNO_json'][$value_ac['acquirer_id']] = '';


					if(isset($_REQUEST['qp']))
					{
						echo "<br/>qp -2 =>".@$value_ac['acquirer_id'];
						echo "<br/>acquirer_processing_json=>".@$value_ac['acquirer_processing_json'];
						echo "<br/>terNO_json=>"; print_r(@$sj['terNO_json'][$value_ac['acquirer_id']]);
					...
					if(isset($_REQUEST['qp'])&&$_REQUEST['qp']==2)
					{
						$store_values_en=jsonencode1(@$value_ac['acquirer_processing_json']);
					}
					else
					{

						if(isset($sj)&&is_array($sj['terNO_json'])&&!empty(@$sj['terNO_json'][$value_ac['acquirer_id']]))	 
						$store_values_en=jsonencode1(@$value_ac['acquirer_processing_json'],@$sj['terNO_json'][$value_ac['acquirer_id']]);

						elseif(isset($value_ac['acquirer_processing_json'])&&@$value_ac['acquirer_processing_json'])	 
						$store_values_en=jsonencode1(@$value_ac['acquirer_processing_json']);
			 ...
			 if(isset($store_values_en)&&strpos($store_values_en,'||sm_')!==false){
...
<input type="text" name="ter_name" placeholder=" <?=($store_name);?> Name of your choice" class="form-control" value="<? if(isset($post['ter_name'])) echo $post['ter_name']?>" required>
            ...
            <div class="col-sm-6 my-2 ">
              <label for="business_nature"><strong>Business Description: : </strong></label>
			  <textarea name="business_nature" placeholder="Describe your Business Description in Short " class="form-control" ><? if(isset($post['business_nature'])) echo $post['business_nature']?></textarea>
            </div>
            ...
            <div class="col-sm-6 my-2">
              <label for="business_description" title="Admin Note for about of Business Describe ">Admin Note : </label>
...
<input type="hidden" name="StartPage" value="<?=(isset($post['StartPage'])?$post['StartPage']:'');?>">
    ...
    <div class="my-2">
      <div class="row m-1 text-start vkg border rounded " >
		...
		<?if($data['con_name']=='clk'||(isset($data['domain_server']['as']['gst_fee'])&&$data['domain_server']['as']['gst_fee'])||(isset($post['gst_rate'])&&$post['gst_rate']>0)){?>
		<div class="row col-sm-6 my-2 hide1">
          <div class="col-sm-4">
            <?=@$acc_arr[2];?>
            GST Rate(%):</div>
...
$( document ).ready(function() {
	...
	$('#sponsorsNam').on('change', function() {
	   var sponsors = $(this).val();
	   //alert(sponsors);
	   if(sponsors){
			$.ajax({
				type: "POST",
				url: "../signins/merlist<?=@$data['ex'];?>?action=account_manager_subadmin&id="+sponsors,
				data: { id: sponsors} 
			}).done(function(data){
				//alert(data);
				$("#account_manager_id").val("");
				$("#account_manager_list").html(data);
	...
	$('#settlement_optimizer').on('change', function() {
	   var settlement_optimizer = $(this).val();
	   //alert($(this).val());
	   settlement_optimizer_f($(this).val());
	   
	});

	settlement_optimizer_f('<?=@$post['settlement_optimizer']?>');
});
