...
.dialog_box2.glyphicons.remove_2 i:before {
    left: 0px;
    top: 1px;
}

.action_link .itxv {display:none;}
.action_link a {
    width: 40px;
    height: 30px;
    margin: 0;
...
<div  class="container border my-2 bg-white rounded">
	   ...
	   <? if(count($post['Transactions'])!=0){ ?>
        <thead>
          <tr>
            ...
            <th scope="col">Trans.<!--Amt.--><!--(<?=$_SESSION['default_currency']?> )--></th>
...
<div class="alert alert-secondary alert-dismissible fade show" role="alert">
  ...
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        ...
        <tbody>
          ...
          <tr>
            <td title="Payment ID" ><a class="hrefmodal text-wrap text-link" data-tid="<?=$value['transaction_id']?>" data-href="<?=$data['Host'];?>/transactions_get<?=$data['ex']?>?id=<?=$value['id']?><? if(isset($post['StartPage'])){ ?>&page=<?=$post['StartPage']?><? } ?>&action=details" style="white-space:nowrap;">
              <?=$value['transaction_id']?>
              </a></td>
            <td title="Order Id" ><?=$value['mrid']?></td>
            <td title="Customer Name"><?=$value['names']?></td>
            <td title="Order Amt." class="pricecolor"><?=$value['amount']?></td>
            <td title="Trans.Amt.-(<?=$_SESSION['default_currency']?>)" class="pricecolor"><?php echo $value['transaction_amt'];?></td>
            <?/*?><td title="Balance-(<?=$_SESSION['default_currency']?>)"><?=$value['available_balance']?></td> <?*/?>
            <td title="Date"><?=prndate($value['tdate'])?></td>
            
            <td title="Status"><?=$value['status']?></td>
            <td title="Action" > <!--class="action_link"--> <!--<div class="bg1"> </div>
              <div class="txt"> </div>-->
			  
			  <a class="hrefmodal" data-href="<?=$data['Host'];?>/transactions_get<?=$data['ex']?>?id=<?=$value['id']?><? if(isset($post['StartPage'])){ ?>&page=<?=$post['StartPage']?><? } ?>&action=details" data-tid="<?=$value['transaction_id']?>"  title="view"  ><i class="<?=$data['fwicon']['eye'];?>"></i><!--<span class='itxv'>View</span>--></a>
			  
              <?  if($value['trname']=="ch"){ ?>
              <a onclick="iframe_openfvkg(this);" class="glyphicons" data-mid="<?=$value['receiver']?>" data-href="<?=$idx;?>_toggle" data-tabname="" data-url="" data-ihref="<?=$data['USER_FOLDER'];?>/echeckvt<?=$data['ex']?>?actionuse=1&hideAllMenu=1&id=<?=$value['tableid']?>&action=update"  title="Duplicate Transaction"><i class="<?=$data['fwicon']['clone'];?>"></i></a>
              <? if($value['ostatus']==0){ ?>
              ...
              <a data-amount="<?=$value['oamount']?>" data-reason="Refund Request" data-href="<?=$data['USER_FOLDER']?>/<?=($data['FileName']);?>?id=<?=$value['id']?><? if($post['StartPage']){ ?>&page=<?=$post['StartPage']?><? } ?><? if($post['type']){ ?>&type=<?=$post['type']?><? } ?>&action=refund" class="dialog_refunded" title="Refund"><i class="<?=$data['fwicon']['rotate-right'];?> mx-1"></i><!--<span class='itxv'>Refund</span>--></a>
              <? } ?>
              ...
              <? if($value['canrefund']){ ?>
              <a href="<?=$data['USER_FOLDER']?>/<?=($data['FileName']);?>?id=<?=$value['id']?><? if(isset($post['StartPage'])){ ?>&page=<?=$post['StartPage']?><? } ?>&action=refund" onClick="return cfmform()" title="Refund"><!--<span class='itxv'>Refund</span>--></a>
              <? } ?>            </td>
			<? if($data['con_name']=='clk'){ ?>
			<td title="MDR"><?=$value['mdr_amt']?></td>
			<td title="GST"><?=$value['gst_fee']?></td>
			<td title="UPA"><?=$value['upa']?></td>
			<td title="Card Number"><?=bclf($value['ccno'],$value['bin_no']);?></td>
			<? } ?>
