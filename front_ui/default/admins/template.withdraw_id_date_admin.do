...
<body class='admins <?=@$data['PageFile']?> bnav'>
    ...
    <div class="container border rounded my-1 ">
        ...
        <form id="myFormReq" class="myFormReq" method="post"
            action="<?=@$data['USER_FOLDER'];?>/trans_withdraw-fund_system_v2<?=@$data['ex']?>?admin=1&bid=<?=@$_GET['bid'];?>">

            <?=((isset($data['is_admin_input_hide'])&&$data['is_admin_input_hide'])?$data['is_admin_input_hide']:'');?>

            <?=(isset($_GET['qp'])?'<input type="hidden" name="qp" value="1">':'')?>
