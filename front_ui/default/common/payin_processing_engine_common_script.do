...
function heightCalcf(theHeight='.processall',preStyle='max-width:375px;margin:0 auto;width:100% !important;'){
	...
	if((contentHeight > screenHeight)){
		//alert('111 contentHeight=>'+contentHeight+', screenHeight=>'+screenHeight);
		$(theHeight).attr('style',preStyle+'transform : translate(-50%,10px) !important;top:0px !important;');
	}
	else {
		//alert('222 contentHeight=>'+contentHeight+', screenHeight=>'+screenHeight);
...
//alert("fetch_trnsStatusf=> "+transID);
	...
	$.ajax({url: fetch_trnsStatus+"?actionurl=viasystem&transID="+transID, type: "GET", dataType: 'json', success: function(result){
		...
		else if((result["order_status"] && (result["order_status"] == 2 || result["order_status"] == 22 || result["order_status"] == 23 || result["order_status"] == 10)))
		{
		...
		if( (statusCheckStart=='1') && ((result["order_status"] && ( result["order_status"] == 1 || result["order_status"] == 9 ) ) || (failed_status && failed_status == "failed")) )
		{
			...
			if(result["mop"])
			{
				...
				if(success_status == "success") {
					...
					responce_box +='<div class="my-2 text-center text-light fs-8">' + result["response"] + '</div>';
				}
...
function fetch_trans_statusf(){
	if(typeof(transID) != 'undefined' && transID != null && transID !='')
	{
		console.log("========fetch_trans_statusf=========");
		fetch_trnsStatusf(transID);
	}
}
...
function clearIntervalf() {
  ...
  console.log("========clearIntervalf=========");
}

function clearIntervalf_all() {
	//alert('99');
	console.log("========clearIntervalf_all=========");
	setTimeout(function(){ 
		clearIntervalf();
	}, 100);
	setTimeout(function(){ 
...
function clearIntervalf_status() {
	//alert('99');
	console.log("========clearIntervalf_status=========");
	setTimeout(function(){ 
		clearInterval(clear_fetch_trans_statusf);
		console.log("========clearIntervalf_status 100=========");
	}, 100);
	setTimeout(function(){ 
		clearInterval(clear_fetch_trans_statusf);
		console.log("========clearIntervalf_status 500=========");
	}, 500);
	setTimeout(function(){ 
		clearInterval(clear_fetch_trans_statusf);
		console.log("========clearIntervalf_status 1500=========");
	}, 1500);
	setTimeout(function(){ 
		clearInterval(clear_fetch_trans_statusf);
		console.log("========clearIntervalf_status 10000=========");
	}, 10000);
}
...
function spinner_show_f(){
	...
	if(typeof heightCalcf_2 === 'function'){ heightCalcf_2();}
...
function generate_qr_codef(){
	...
	var acquirerValue= $('.account_types.active').attr('data-value');

	if( ($('.account_types').hasClass('active')) && (ewlistVar !== undefined) && (ewlistVar) && (ewlistVar.match('scanqrArray|scanqr2Array'))&&(acquirerValue)){

		$('.upi_qr_border_'+acquirerValue).addClass('active');
		$('.upi_qr_border_'+acquirerValue).html("<i class='<?=@$data['fwicon']['spinner'];?>  fs-1 fa-spin-pulse mt-5 text-info'></i><br>Generating QR");
		...
		success: function(data) {
			...
			if (typeof data === 'string') {
				...
				if(stringifyf(data['transID'])) { transID = stringifyf(data['transID']); }
				 ...
				 var html_redirect_url = stringifyf(data['html_redirect_url']);
			...
			if($DONE_AJAX=='DONE_AJAX')
			{
				...
				//alert(qrTransID);
				//alert(ReferenceNo);
				if( typeof(varReferenceNo) != 'undefined' && varReferenceNo != null && varReferenceNo !="" )
				{
					var ReferenceNo="Reference No : " + varReferenceNo;
					$('.qrTransID').html(ReferenceNo);
				}else if(typeof(transID) != 'undefined' && transID != ""){
				...
				if( typeof(html_redirect_url) != 'undefined' && html_redirect_url != null && html_redirect_url !="" )
				{
					//pendingCheckStartf('reStart');
					
					var html_redirect_url = stringifyf(data['html_redirect_url']);
					var html_class = stringifyf(data['html_class']);

					console.log("html_redirect_url=>"+html_redirect_url);
					console.log("html_class=>"+html_class);
					
					
					//$('.check_html_reset').html('');

					/* Dev Tech : 24-05-28 html response like btc qrcode */
					
					$(html_class).load(html_redirect_url, {"actionUrl":"html_response"}, function(responseTxt, statusTxt, jqXHR){
						
						if(statusTxt == "success"){
							$('.active .hide_sub_input').hide(20);
							$('.active '+html_class+'_show').slideDown(1500);
							//alert("success!");
						}
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(stringifyf(data['DONE_AJAX']))
			{
				...
				else if( typeof(html_redirect_url) != 'undefined' && html_redirect_url != null && html_redirect_url !="" )
				{
					//pendingCheckStartf('reStart');
