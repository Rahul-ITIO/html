...
<div class="scanqr_div scanqrArray_div ewalist hide">
  ...
  <div class="row border p-2 mb-2 rounded row upi_qr_row" id="qr_toggle" >
    <!--style="display:none;"-->
    <?if(isset($post['scan_qrcode_array'])&&@$post['scan_qrcode_array']&&count($post['scan_qrcode_array'])>0)
    {
      foreach($post['scan_qrcode_array'] as $ky1=>$va1)
      {
        //upi_qr_border_
      ?>
        <div class="col-sm-12 upi_qr_border_div <?=@$va1?> hide" style="overflowx:hidden;position:relative;z-index:5;height:auto;">
    ...
    <div class="col-sm-12 upi_qr ps-2 mt-2">
      ...
      <div class="my-2 d-flex justify-content-center"> 
          ...
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'BTC') !== false){?>
              <img class="BTC_div ewalist hide" src="<?=$data['Host']?>/bank/bitcoin_icon.png" title="Bitcoins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'bnbCoins') !== false){?>
              <img class="bnbCoins_div ewalist hide" src="<?=$data['Host']?>/bank/binance_icon.png" title="BNB Coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'ethCoins') !== false){?>
              <img class="ethCoins_div ewalist hide" src="<?=$data['Host']?>/bank/ethereum_icon.png" title="ETH coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'trxCoins') !== false){?>
              <img class="trxCoins_div ewalist hide" src="<?=$data['Host']?>/bank/tron_trc20_icon.png" title="TRX coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'xrpCoins') !== false){?>
              <img class="xrpCoins_div ewalist hide" src="<?=$data['Host']?>/bank/xrp_icon.png" title="XRP coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'tonCoins') !== false){?>
              <img class="tonCoins_div ewalist hide" src="<?=$data['Host']?>/bank/ton_icon.png" title="TON coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'eurCoins') !== false){?>
              <img class="eurCoins_div ewalist hide" src="<?=$data['Host']?>/bank/eurc_icon.png" title="EUR coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'ltcCoins') !== false){?>
              <img class="ltcCoins_div ewalist hide" src="<?=$data['Host']?>/bank/ltc_icon.png" title="LTC coins" style="display:inline-block;" />
          <?}?>
          <? if(isset($post['t_name6'])&&strpos($post['t_name6'],'usdCoins') !== false){?>
              <img class="usdCoins_div ewalist hide" src="<?=$data['Host']?>/bank/usdc_icon.png" title="USDC coins" style="display:inline-block;" />
          <?}?>
...
<div class="qrcodeadd_div ewalist hide">
  ...
  <div class="row border p-2 mb-2 rounded row upi_qr_row" id="qr_toggle" >
    <!--style="display:none;"-->
    <div class="col-sm-6 upi_qr_border check_html_reset" style="overflow:hidden;">
      <div class="px-2 my-3">
...
<? //tether Transport Protocol - tetherTransportProtocol - Start ?>
<?
/*
if(isset($post['t_name6'])&&strpos($post['t_name6'],'tetherTransportProtocol') !== false){?>
<div class="tetherTransportProtocol_div ewalist hide is_channel">


  <div class="row border p-2 mb-2 rounded row upi_qr_border_via_network_show hide " >
    <div class="col-sm-12 upi_qr_border_via_network check_html_reset " style="overflow:hidden;">
        <div class="px-2 my-3">
          <img src="<?=@$data['bankicon']['qr-button'];?>" title="QR" class="upi_qr_img"/> </div>
      </div>
      <div class="col-sm-12 upi_qr ps-2" style="padding-top: 0px;">
        <div class="px-1 upi_qr_text" style="font-size:10px !important;">Scan the QR using app of network payment on your phone.</div>
...
<? 
    ...
    <?
    if(isset($post['t_name6'])&&strpos($post['t_name6'],'usdcNetwork') !== false){?>
    <div class="usdcNetwork_div ewalist hide col-sm-12 row">

      <div class="row border p-2 mb-2 rounded row qr_code_usdc_show hide " >
        <div class="col-sm-12 qr_code_usdc check_html_reset " style="overflow:hidden;"> </div>
      </div>

      <div class="row hide_sub_input">
          <h4>Select protocol:</h4>
          
          <div class="row">
            <div class="col-sm-6 border p-2 desImg ETHEREUM Ethereum">
              <div class="text-center"><img src="<?=@$data['bankicon']['ethereum'];?>" title="ERC-20 (Ethereum)" class="bank_img"/></div>
              <span class="bankNm txNm">ERC-20 (Ethereum)</span> </div>
            <div class="col-sm-6 border p-2 desImg BINANCE_SMART_CHAIN">
              <div class="text-center"><img src="<?=@$data['bankicon']['binance_pro'];?>" title="BEP-20 (Binance Smart Chain)" class="bank_img"/></div>
              <span class="bankNm txNm">BEP-20 (Binance Smart Chain)</span> </div>
          </div>
          <select class="w94 dropDwn required form-select from-select-sm hide_sub_input" name="tetherTransportProtocol" id="tetherTransportProtocol" style="margin:5px 0;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Select protocol" placeHolder="Select protocol" data-required="required">
            <option value="" disabled="disabled">Select protocol</option>
            <option value="ETHEREUM">ERC-20 (Ethereum)</option><option value="BINANCE_SMART_CHAIN">BEP-20 (Binance Smart Chain)</option>
            ?>
            
          </select>
          <input class="w94" type="text" name="tetherTransportProtocol_text" style="margin:5px 0;display:none;" />
      </div>
    </div>
    <?}
    
    if(isset($post['t_name6'])&&strpos($post['t_name6'],'usdtNetwork') !== false){ ?>
    <div class="usdtNetwork_div ewalist hide col-sm-12">
      <div class="row border p-2 mb-2 rounded row qr_code_usdt_show hide " >
          <div class="col-sm-12 qr_code_usdt check_html_reset " style="overflow:hidden;"> </div>
      </div>

      <div class="row hide_sub_input">
        <h4>Select protocol:</h4>
        <div class="row">
          <div class="col-sm-4 border p-2 desImg ETHEREUM Ethereum">
            <div class="text-center"><img src="<?=@$data['bankicon']['ethereum'];?>" title="ERC-20 (Ethereum)" class="bank_img"/></div>
            <span class="bankNm txNm">ERC-20 (Ethereum)</span> </div>
          <div class="col-sm-4 border p-2 desImg TRON">
            <div class="text-center"><img src="<?=@$data['bankicon']['trc20'];?>" title="TRC-20 (TRON)" class="bank_img"/></div>
            <span class="bankNm txNm">TRC-20 (TRON)</span> </div>
          <div class="col-sm-4 border p-2 desImg BINANCE_SMART_CHAIN">
            <div class="text-center"><img src="<?=@$data['bankicon']['binance_pro'];?>" title="BEP-20 (Binance Smart Chain)" class="bank_img"/></div>
            <span class="bankNm txNm">BEP-20 (Binance Smart Chain)</span> </div>
          <div class="col-sm-4 border p-2 desImg MATIC">
            <div class="text-center"><img src="<?=@$data['bankicon']['polygon_pro'];?>" title="MATIC (Polygon)" class="bank_img"/></div>
            <span class="bankNm txNm">MATIC (Polygon)</span> </div>
          <div class="col-sm-4 border p-2 desImg SOLANA">
            <div class="text-center"><img src="<?=@$data['bankicon']['solana_sol_pro'];?>" title="SOL (Solana)" class="bank_img"/></div>
            <span class="bankNm txNm">SOL (Solana)</span> </div>
        </div>
          <select class="w94 dropDwn required form-select from-select-sm hide_sub_input" name="tetherTransportProtocol" id="tetherTransportProtocol" style="margin:5px 0;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Select protocol" placeHolder="Select protocol" data-required="required">
            <option value="" disabled="disabled">Select protocol</option>
            <?
              foreach($data['tetherTransportProtocol'] as $ke=>$va){
                echo "<option value='{$ke}'>{$va}</option>";
              }
            ?>
...
<div class="submit_div text-muted m-button my-2 clearfix">
  ...
  <button onclick="checkValidity_tr_f('hideCloseButton');" id="cardsend_submit" type="submit" name="send123" value="CHANGE NOW!"  class="paybutton submit_div submitbtn btn btn-slide next w-50 float-end b5556"><i></i><b class="contitxt">Continue</b></button>
