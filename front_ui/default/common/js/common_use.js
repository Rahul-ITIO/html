...
//copytext_f(this,'Pay URL');
//copytext_f(this,'Pay URL','data-value');
//copytext_f(this,'Pay URL','data-href','0');
function copytext_f(e,theLabel='',theAttr='data-value',prnt='1') {
	//alert($(e).attr('data-value')+"\n\nffff. ");
	
	var textField = document.createElement('textarea');
    textField.innerText = $(e).attr(theAttr);
	...
	if(prnt=='1'){
		alert($(e).attr(theAttr)+"\n\nCopied. "+theLabel);
	}else{
		alert("Copied. "+theLabel);
	}
}
function ctc_f(e,theLabel='',theAttr='data-value',prnt='0') {
	
	copytext_f(e,theLabel,theAttr,prnt);
...
function view_next3(e,theId=false,prev=false,label='') {
	if($(e).hasClass('active')){
			if(prev){
				$(e).prev().slideUp(200);
			}else{
...
function ajaxf3(e,theUrl,theIds='',theAttr='data-value'){
	//alert("ajaxf3=> "+theUrl);
	$.ajax({url: theUrl,success: function(result){
		if(theIds&&theAttr){
			$(theIds).attr(theAttr,result);
			var data_title = $(theIds).attr('data-title');
			if(data_title !== undefined){
				$(theIds).attr('title',data_title+result);
				$(theIds).attr('data-bs-original-title',data_title+result);
			}
		}
		else {
			$(e).html(result);
...
function ajaxf1(e,theUrl,theIds,active=true,pop='0'){
	//alert("114 ajaxf1=> "+theUrl);
	if(theIds&&theUrl){

		if(pop=='4'){
			active=false;
			$(e).removeClass('active_ajx');
		...
		if($(e).hasClass('active_ajx')){
		  ...
		  else if(pop=='2'||pop=='4'){loading_imgf($(theIds));}
		  $.ajax({url: theUrl,success: function(result){
				if(pop=='3'){
					$(e).html(result);
				}else{
...
function hrefmodal(theUrl,data_tid,eThis=''){	
	...
	$.ajax({url: theUrl,success: function(result){
		...
		*/
		...
		data_tid='<a style="position:relative;z-index:999;">'+data_tid+' <i class="fa-solid fa-copy" onclick="copytext_f(this,\'TransID\',\'data-value\')" data-value="'+data_tid+'" ></i></a>';
		//alert(data_tid);
...
function activeHerf()
{
    ...
    console.log('Current URL:', filename_a);

    // Match based on pathname and query parameters
    ...
    // Get the current path and trim trailing slashes
    var path = window.location.pathname.replace(/\/+$/, ''); // Remove trailing slashes
    path = path.split("/").pop(); // Get the last segment
    ...
    // Check if the path matches specific conditions
    if ((path !== undefined) && (path) && (path.match('trnslist|merlist|emails'))) {
        // Do nothing for these specific paths
    } else {
        // Use a more flexible selector to match the path
        $('#sidebar-wrapper a[href*="/' + path + '"]').addClass('active_a');
    }
