...
<body class='admins <?=@$data['PageFile']?> bnav'>
    ...
    <div class="container border rounded my-1 " style="position:relative;">
  ...
  <? } ?>
      ...
      <!-- trans_withdraw-fund_v3_custom_settlement -->
       ...
       if(isset($_REQUEST['url'])&&$_REQUEST['url']==1){
            $url_withd=@$data['Host'].'/include/withdraw_date_v3_custom'.$data['ex'];
       }
       else {
            $url_withd=@$data['USER_FOLDER'].'/trans_withdraw-fund_v3_custom_settlement'.$data['ex'];
       }
       ?>
        ...
        <form id="myFormReq" class="myFormReq hide1" method="post"
            ...
            <div class="row p-4  text-center">
                ...
                <div class="bg_div1 row pt-4">
                    ...
                    <?if(isset($post['last_withdraw_transID'])){?>
                        <div class="col text-nowrap">
                            Pre. Withdraw TransID:<br/> 
                            <? if(isset($post['last_withdraw_transID'])) echo 'W3 '.prntext($post['last_withdraw_transID'])?>
                        </div>
                   

                        <div class="col">
                            Pre. Withdraw from Date:<br/> 
                            <div class="text-nowrap"><? if(isset($post['withdraw_from_date'])) echo prntext($post['withdraw_from_date'])?></div>
                        </div>
                    <?
                    }
                    elseif(isset($post['first_success_transID'])){
                    ?>
                        <div class="col">
                            First Success TransID:<br/> 
                            <? if(isset($post['first_success_transID'])) echo prntext($post['first_success_transID'])?>
                        </div>
                   

                        <div class="col">
                             First Success - from Date:<br/> 
                            <div class="text-nowrap"><? if(isset($post['withdraw_from_date'])) echo prntext($post['withdraw_from_date'])?></div>
                        </div>
                    <?
                    }
                    ...
                    <? 
                    if((isset($data['withdraw_transID'])&&count($data['withdraw_transID'])>0)&&(isset($_REQUEST['da']))) 
                    {
                    ?>
                        <div class="col">
                            ...
                            <input type="text" class="form-control w-100" id="withdraw_previous_transID" list="withdraw_previous_transID_list" name="withdraw_previous_transID"
                                value="<?php if(isset($post['withdraw_previous_transID'])){ echo $post['withdraw_previous_transID'];}?>"  <?=((isset($data['withdraw_prev_transID'])&&count($data['withdraw_prev_transID'])>0)?"":"")?> >
...
$(document).ready(function() {
    ...
    $('#myFormReq').submit(function(e){ 
	  try {
        ...
        $('#error-message').text('');
         ...
         // If withdraw_from_date is empty, assign the current date and time
        if (!withdraw_from_date) {
            //withdraw_from_date = new Date().toISOString().slice(0, 26).replace('T', ' '); // Format: YYYY-MM-DD HH:MM:SS.ssssss
        }
        ...
        */
        ...
        else  if ( withdraw_from_date && datePattern.test(withdraw_from_date) && datePattern.test(withdraw_to_date) && ( new Date(withdraw_from_date) >= new Date(withdraw_to_date) ) ) {
            // Convert the date strings to Date objects for comparison
            $('#error-message').append('From Date must be less than To Date.<br>');
            $('.myFormReq input[name="withdraw_to_date"]').focus();
            return false; // Return false to stop further processing
        }
        else if (new Date(withdraw_to_date) > new Date() && !withdraw_from_date ) {
                $('#error-message').append('Withdraw To Date cannot be greater than the current date.<br>');
                $('.myFormReq input[name="withdraw_to_date"]').focus();
                return false; // Return false to stop further processing
        }
        else 
    ...
    // Form Validation 
    $('#myFormReq_XX').submit(function(e) { 
        ...
        try {
            ...
            // Check if withdraw_from_date is empty
            if (withdraw_from_date === '') {
                alert("Withdraw from Date cannot be blank! Please enter the Withdraw from Date.");
                $('.myFormReq input[name="withdraw_from_date"]').focus();
                return false;
            }

            // Check if withdraw_to_date is empty
            if (withdraw_to_date === '') {
                alert("Withdraw to Date cannot be blank! Please enter the Withdraw to Date.");
                $('.myFormReq input[name="withdraw_to_date"]').focus();
                return false;
            }

            // Validate date formats
            if (!datePattern.test(withdraw_from_date)) {
                $('#error-message').append('Invalid From Date format. Expected format: YYYY-MM-DD HH:MM:SS.ssssss<br>');
                $('.myFormReq input[name="withdraw_from_date"]').focus();
                return false;
            }

            if (!datePattern.test(withdraw_to_date)) {
                $('#error-message').append('Invalid To Date format. Expected format: YYYY-MM-DD HH:MM:SS.ssssss<br>');
                $('.myFormReq input[name="withdraw_to_date"]').focus();
                return false;
            }
            ...
            // Convert the date strings to Date objects for comparison
            var fromDateObj = new Date(withdraw_from_date);
            var toDateObj = new Date(withdraw_to_date);
            var currentDate = new Date(); // Get the current date

            // Check if withdraw_to_date is greater than the current date
            if (new Date(withdraw_to_date) > new Date()) {
                $('#error-message').append('Withdraw To Date cannot be greater than the current date.<br>');
                $('.myFormReq input[name="withdraw_to_date"]').focus();
                return false; // Return false to stop further processing
            }

            // Check if withdraw_from_date is greater than or equal to withdraw_to_date
