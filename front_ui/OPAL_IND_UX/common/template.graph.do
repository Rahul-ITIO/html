...
/*
if(isset($data['WITHDRAW_INITIATE_SYSTEM_WV2'])&&@$data['WITHDRAW_INITIATE_SYSTEM_WV2']=='Y') {
...
.col-sm-9{ width: 100% !important; }
		...
		$payout_array_ajax=[];
		if(isset($_SESSION['uid_wv2'.$uid]['payout_array_ajax'])) $payout_array_ajax=@$_SESSION['uid_wv2'.$uid]['payout_array_ajax'];
...
<div class="container ps-0">
  ...
  <? unset($_SESSION['action_success']); } ?>
	<script>
		//var readyToSettle = $('#read_to_settle_3')
		//var settlementProcessed = $('#settlementProcessed_3');
	</script>
    ...
    <div class="col-sm-9" style="height:100vh;">
	  <div class="row px-2">
	    <div class="col mb-2 text-center px-0">
		  ...
		  <div class="card-body cursor-pointer" onClick="ajaxf1(this,'<?=@$data['Host']?>/include/trans_balance_merchant_opal_ind_ux_ajax<?=@$data['ex'];?>?a=v2&actionAjax=session&actionUI=readyToSettle_settlementProcessed','#read_to_settle_3','0','4')" title="Click here for view the Ready to Settle" data-bs-toggle="tooltip" data-bs-placement="top" >
              <div class="text-card cursor-pointer">
			  	<h3 class="cursor-pointer">Ready to Settle</h3>
					<h6 id="read_to_settle_3">
						<?if(isset($payout_array_ajax['summ_mature'])&&trim($payout_array_ajax['summ_mature'])){
							echo $payout_array_ajax['summ_mature'];
						}else{?>
        ...
        <div class="col mb-2 text-center px-0">
			<div class="card card1 rounded-tringle pull-up mx-1 bg-white text-dark">
				<div class="card-body cursor-pointer" onClick="ajaxf1(this,'<?=@$data['Host']?>/include/trans_balance_merchant_opal_ind_ux_ajax<?=@$data['ex'];?>?a=v2&actionAjax=session&actionUI=readyToSettle_settlementProcessed','#settlementProcessed_3','1','4')" title="Click here for view the Settlement Processed" data-bs-toggle="tooltip" data-bs-placement="top" >
					<div class="text-card cursor-pointer">
						<h3 class="cursor-pointer">Settlements</h3>
						<h6 id="settlementProcessed_3">
							<?if(isset($payout_array_ajax['summ_withdraw'])&&trim($payout_array_ajax['summ_withdraw'])){
								echo $payout_array_ajax['summ_withdraw'];
							}else{?>
								<i class="<?=@$data['fwicon']['view'];?>"></i>
							<?}?>
						</h6>
        ...
        <div class="col mb-2 text-center px-0">
          <div class="card card1 rounded-tringle pull-up mx-1 bg-white text-dark">
            <div class="card-body cursor-pointer" onClick="ajaxf1(this,'<?=@$data['Host']?>/include/trans_balance_merchant_opal_ind_ux_ajax<?=@$data['ex'];?>?a=v1&actionAjax=session&actionUI=refundAmount','#total_refund_amount','1','2')" title="Click here for view the Refund Amount" data-bs-toggle="tooltip" data-bs-placement="top" >
              <div class="text-card cursor-pointer">
			  	<h3 class="cursor-pointer">Refund Amount</h3>
				<h6 id="total_refund_amount">
					<?if(isset($deduction_array_ajax['total_amt_refunded'])&&trim($deduction_array_ajax['total_amt_refunded'])){
						echo $deduction_array_ajax['total_amt_refunded'];
					}else{?>
						<i class="<?=@$data['fwicon']['view'];?>"></i>
					<?}?>
				</h6>
...
$(document).ready(function() {
	...
	*/
	...
	<?if(!isset($payout_array_ajax['summ_mature'])){  ?>
		ajaxf1(this,'<?=@$data['Host']?>/include/trans_balance_merchant_opal_ind_ux_ajax<?=@$data['ex'];?>?a=v2&actionAjax=session&actionUI=readyToSettle_settlementProcessed','#read_to_settle_3','0','4');					
	<? } ?>
