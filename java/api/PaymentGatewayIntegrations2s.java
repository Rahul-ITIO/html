...
public class PaymentGatewayIntegrations2s {
    public static void main(String[] args) {
        try {
            ...
            // Parse JSON response and redirect to authurl if present
            if (responseCode == 200) {
                JSONObject jsonResponse = new JSONObject(response.toString());
                if (jsonResponse.has("authurl")) {
                    String authUrl = jsonResponse.getString("authurl");
                    System.out.println("Redirecting to: " + authUrl);

                    // Redirect to authUrl (simulated in Java)
                    java.awt.Desktop.getDesktop().browse(new URI(authUrl));
                } else {
                    System.out.println("authurl not found in the response.");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
