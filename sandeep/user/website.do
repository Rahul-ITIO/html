...
$data['PageName']='Store';
$data['PageFile']='store';
$data['PageFileName']='store';
###############################################################################
include('../config.do');
$data['PageTitle'] = 'My Store - '.$data['domain_name']; 
##########################Check Permission#####################################
...
function templatesf($pst){
	...
	foreach($tid as $valuet){
		...
		if($account_count>0){
			...
			$bgt_slc=db_rows(
				...
				" WHERE `account_no`={$valuet} LIMIT 1",0
			);
	...
	*/
	...
	$pro_slc=db_rows(
		"SELECT templates_log FROM `{$data['DbPrefix']}products`".
		" WHERE `id`={$pst['gid']} LIMIT 1",0
	);
	$pLog=$pro_slc[0]['templates_log'];
	...
	" SET `active`={$pst['active']}, `midcard`='{$pst['midcard']}', `templates`={$pst['tid']}, `templates_log`='{$t_log}'  WHERE `id`={$pst['gid']}",0);
...
if(isset($post['send'])&&$post['send']){
	...
	if(isset($post['step'])&&$post['step']==1){
		...
		if(!$post['name']){
			...
			$data['Error']='Describe your Business Nature in Short';
		}else{
			if(!$post['gid']){
				...
				header('location:'.$data['USER_FOLDER']."/store".$data['ex']);
				exit;
			}else{ 
				if(isset($_SESSION['store_active'])&&$_SESSION['store_active']==1){
...
}elseif(isset($post['action'])&&$post['action']=='re_generate_api_token'){
	$api_token_store=api_token_store($uid,$post['gid']);
	
	if(isset($_REQUEST['ajax'])&&$_REQUEST['ajax']){
		echo $api_token_store;exit;
	}
...
if(isset($post['action'])&&($post['action']=='update'||$post['action']=='view')){
	...
	foreach($product[0] as $key=>$value)if(!isset($post[$key]) || !$post[$key])$post[$key]=$value;
	$post['active']=$product[0]['active'];
	
	if(isset($post['templates'])&&$post['templates']){
		$pt=db_rows(
			"SELECT * FROM `{$data['DbPrefix']}acquirer_group_template`".
			" WHERE `id`={$post['templates']} LIMIT 1"
		);
		$post['templates_name']=$pt[0]['templates_name'];
	}
	
	$_SESSION['store_active']=$product[0]['active'];
	$_SESSION['store_name']=$product[0]['name'];
	$_SESSION['store_bussiness_url']=$product[0]['bussiness_url'];
	$_SESSION['store_transaction_currency']=$product[0]['transaction_currency'];
	$_SESSION['store_business_nature']=$product[0]['business_nature'];
	
	if($is_admin&&isset($post['website_mcc_code'])&&trim($post['website_mcc_code'])){
		$website_mcc_code=merchant_categoryf(0,0,$post['website_mcc_code']);
		if($website_mcc_code&&$post['website_mcc_code'])
		$mcc_codes_list=[];
		if(isset($data['mcc_codes_list'])&&!empty($data['mcc_codes_list'])) $mcc_codes_list=$data['mcc_codes_list'];

		$post['website_mcc_code']=implode(" , ",$mcc_codes_list);
		//echo "<br/>website_mcc_code=>".$post['website_mcc_code'];
	}
...
if(isset($post['step'])&&$post['step']==1){
	...
	$data['Products']=select_products($uid,0,0,0,0,' AND (`active` NOT IN (2) ) ');
	$data['result_count']=sizeof($data['Products']);
}
//echo $post['step'];
###############################################################################
display('user');
