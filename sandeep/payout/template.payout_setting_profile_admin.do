...
<div class="col m-col">
	<? if((isset($_SESSION['login_adm']))||(isset($_SESSION['show_payout_status'])&&$_SESSION['show_payout_status']==1)){?>
	
	<label for="vtid">Payout Status: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	<!--oninvalid="alert('Please Select Payout Request');"-->
	<select name="payout_status" id="payout_status" class="form-select w-100" title="Payout Request" >
		<option value="">Payout Gateway</option>
		<option value="1">Live</option>
		<option value="2">Test</option>
		<option value="3">Inactive</option>
	...
	<script>$('#payout_status option[value="<?=$post['payout_status']?>"]').prop('selected','selected');</script>
	<? } ?>
 <? } ?>
</div>
...
<div class="my-2" id="Payout_Scrubbed_Period">
	<div class="row text-start vkg border border-primary bg-vlight rounded" >
		<div class="row col-sm-12 my-2">
			...
			<div class="row col-sm-6 my-2" >
				<div class="col-sm-4">
					<label for="payout_fixed_fee" style="clear:left;">Payout Fixed fee: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
				 </div>
				<div class="col-sm-8">
					<div class="input-group mb-2">
						<input type="text" name="payout_fixed_fee" id="payout_fixed_fee" class="form-control"  value="<?php if(isset($post['payout_fixed_fee'])&&$post['payout_fixed_fee']){echo number_formatf_2(stf($post['payout_fixed_fee']));}?>" />
		...
		<? if((isset($_SESSION['login_adm']))||(isset($_SESSION['show_payout_account'])&&$_SESSION['show_payout_account']==1)){?>				
			<div class="row col-sm-6 my-2" >
				<div class="col-sm-4">
					<label for="gst_fee" style="clear:left;">Payout A/c: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
				 </div>
				<div class="col-sm-6">
					<div class="input-group mb-2">
						...
						<? if(isset($post['payout_account'])) { ?>
							<script>$('#payout_account option[value="<?=@$post['payout_account']?>"]').prop('selected','selected');</script>
						<? } ?>
					</div>
				</div>
				<? if((isset($_SESSION['login_adm']))||(isset($_SESSION['show_payout_keys'])&&$_SESSION['show_payout_keys']==1)){?>
				  <div class="col-sm-2">
					...
					<a class="btn btn-primary text-start" title="Payout Secret Key Generated" style="margin-top:0px !important;"><i class="far fa-check-circle"></i></a>
					<?
								  if(isset($_SESSION['login_adm'])&&$_SESSION['login_adm']&&$post['payout_secret_key']){?>
					<a class="btn btn-primary text-start" onclick="return confirm_remove(this, <?=$post['gid']?>);" title="Remove Payout Secret Key" style="margin-top:0px !important;"><i class="fas fa-times-circle"></i></a>
					<? }
								 }else{ ?>
					<a class="btn btn-primary text-start" title="Payout Secret Key Not Generated" style="margin-top:0px !important;"><i class="far fa-times-circle"></i></a>
					<? } ?>
					...
					<b id="generate_api_token" class="hide"><?php if(isset($post['payout_token'])&&$post['payout_token']){echo $post['payout_token'];}else{echo $payout_token;}?></b>
					 ...
					 <a class="btn btn-primary float-end text-start" title="Copy Payout API Token - <?php if(isset($post['payout_token'])&&$post['payout_token']){echo $post['payout_token'];}else{echo $payout_token;}?>" onclick="CopyToClipboard2('#generate_api_token')" style="margin-top:0px !important;"> <i class="fas fa-copy"></i> </a>
					<? }else{ ?>
					<a class="btn btn-primary float-end mx-1" title="Payout Token Not Generated" style="margin-top:0px !important;"><i class="fas fa-file-excel"></i></a>
					<? } ?>
				  </div>
			  <? } ?>
	...
	<div class="row col-sm-6 my-2" >
	  <div class="col-sm-4">
		<label title="Scrubbed Period">Scrubbed Period: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	  </div>
	  <div class="col-sm-8">
	...
	<div class="row col-sm-6 my-2">
	  <div class="col-sm-4">
		<label title="Min Trxn Limit:">Min Trxn Limit: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	  </div>
	  <div class="col-sm-8">
	...
	<div class="row col-sm-6 my-2">
	  <div class="col-sm-4">
		<label title="Max Trxn Limit">Max Trxn Limit: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	  </div>
	  <div class="col-sm-8">
	...
	<div class="row col-sm-6 my-2">
	  <div class="col-sm-4">
		<label title="Min. Success Count">Min Success Count: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	  </div>
	  <div class="col-sm-8">
	...
	<div class="row col-sm-6 my-2" >
	  <div class="col-sm-4">
		<label title="Min. Failed Count">Min. Failed Count: <i class="<?=$data['fwicon']['star'];?> text-danger"></i></label>
	  </div>
	  <div class="col-sm-8">
