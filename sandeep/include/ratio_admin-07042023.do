...
function update_ratio_info($uid, $card_ratio1, $check_ratio1 ){
	...
	db_query(
		"UPDATE `{$data['DbPrefix']}members` SET ". $upd_str. " WHERE `id`='{$uid}'",0
	);
}
