...
if(isset($_REQUEST['bid'])&&$_REQUEST['bid']>0){
	...
	$post['row'] = $result_select[0];
}

$mid = $_REQUEST['mid'];
$memb=select_tablef("`id`='{$mid}'",'clientid_table',0,1,"`company_name`");
$post['row']['company_name']=isset($memb['company_name'])&&$memb['company_name']?$memb['company_name']:'';
...
showpage("common/template.payout_transactions_details".$data['iex']);exit;
