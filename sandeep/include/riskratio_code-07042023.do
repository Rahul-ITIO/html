...
$_SESSION['notification_to']="";
foreach($post['vtAccountInfo'] as $key=>$value){
  //echo $value['acquirer_processing_mode']; echo $value['view_settelement_report'];exit;

	$_SESSION['account_type_all'].=$value['acquirer_id'].",";
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Check') !== false) && ($value['acquirer_processing_mode']!="3") && ($value['vt']=="1")){$_SESSION['m_check21_vt']=true;}
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Card') !== false) && ($value['acquirer_processing_mode']!="3") && ($value['vt']=="1")){$_SESSION['m_moto_vt']=true;}
	
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Check') !== false)&&($value['acquirer_processing_mode']!="3") ){$_SESSION['account_type_check'].=$value['acquirer_id'].",";}
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Card') !== false)&&($value['acquirer_processing_mode']!="3") ){$_SESSION['account_type_card'].=$value['acquirer_id'].",";}
	
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Check') !== false) && ($value['view_settelement_report']=="1" || empty($value['view_settelement_report']))){$_SESSION['m_check21_pdfreport']=true;}
	if((strpos(@$data['t'][$value['acquirer_id']]['name2'],'Card') !== false) && ($value['view_settelement_report']=="1" || empty($value['view_settelement_report']))){$_SESSION['m_card_pdfreport']=true;}
	
	if($value['notification_to']){$_SESSION['notification_to'].=$value['notification_to'].",";}
	
}
if(isset($_SESSION['notification_to'])){$_SESSION['notification_to']=implode(',',array_unique(explode(',',$_SESSION['notification_to'])));}
...
if(!empty($_SESSION['account_type_card'])){  echo "============>";exit;
