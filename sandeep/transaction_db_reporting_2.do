...
function withdraw_max_prev2($uid=0,$tid=0,$clk_status=0){
	global $data;$qprint=0;
	$result=array();
	$not_status=" AND ( NOT( `status`=2 ) ) ";
	//if($clk_status){ $not_status=" ";}
	
	$c_withdraw_max=db_rows(
		"SELECT `id`, `transaction_id`, `tdate`, `created_date`".
		" FROM `{$data['DbPrefix']}transactions`".
		" WHERE ( `receiver`={$uid} OR `sender`={$uid} ) AND ( `id`={$tid} ) ".
		" LIMIT 1",$qprint
	); 
	
	$c_created_date=$c_withdraw_max[0]['created_date'];
	
	$result=max_settelement_period($uid,$c_created_date);
	
	$result["c_id"]=$c_withdraw_max[0]['id'];
	$result["c_transaction_id"]=$c_withdraw_max[0]['transaction_id'];
	$result["c_tdate"]=$c_withdraw_max[0]['tdate'];
	$result["c_created_date"]=$c_withdraw_max[0]['created_date'];
	
	$t_withdraw_max=db_rows(
		"SELECT (`id`) AS `id`, (`transaction_id`) AS `transaction_id`, (`tdate`) AS `tdate`".
		" FROM `{$data['DbPrefix']}transactions`".
		" WHERE ( `sender`={$uid} ) AND ( `type`=2 ) {$not_status} AND ( `id`<{$tid} ) ".
		" ORDER BY `id` DESC LIMIT 1",$qprint
	);
	
	
	
	$result["previous_wd_transaction_id"]=$t_withdraw_max[0]['transaction_id'];
	
	if($t_withdraw_max[0]['id']==null){
		$t_withdraw_max=db_rows(
			"SELECT  `id`,`transaction_id`, `tdate`".
			" FROM `{$data['DbPrefix']}transactions`".
	...
	$result["transaction_id"]=$t_withdraw_max[0]['transaction_id'];
	$result["tdate"]=$t_withdraw_max[0]['tdate'];
	
	
	
	$remaining_balance_prev=db_rows(
		"SELECT  `amount`,`json_value` ".
		" FROM `{$data['DbPrefix']}transactions`".
		" WHERE ( `transaction_id`={$result["transaction_id"]} AND `id`={$result["id"]} )  ".
		" ORDER BY `id` ASC LIMIT 1",$qprint
	);
	$remaining_balance_prev=$remaining_balance_prev[0];
	$json_value_de=json_decode($remaining_balance_prev['json_value'],1);
	
	$result["remaining_balance_prev"]=number_formatf2((double)$json_value_de['mature_fund']-(double)str_replace_minus($remaining_balance_prev['amount']));
...
$withdraw_fee=$wd_json_value['withdraw_fee'];
$withdrawfee_calc=$wd_json_value['withdrawfee_calc'];
...
$remaining_balance_tr=number_formatf2($mature_fund-str_replace_minus($order_amount));
...
if (date('Ymd',strtotime($wd_created_date))<date('Ymd',strtotime($wd_created_date_prev))) {
	$wd_created_date=$wd_created_date_prev;
}
