...
if($post['action']=='confirm'){	
	...
	if((isset($data['PRO_VER']))&&($data['PRO_VER']==3)&&(isset($data['Database_2'])&&$data['Database_2'])){
		...
		$trans_result=select_tablef("`id` IN ({$_REQUEST['id']})",$data['MASTER_TRANS_TABLE'],0,1);
		...
		$memers_row = select_tablef("`id` IN ({$trans_result['merID']})",'clientid_table',0,1," `payout_request`,`payout_account`");


		$payout_request= $memers_row['payout_request'];		//payout request
		$payout_account= $memers_row['payout_account'];		//mapped payout account
...
//=================New Condation
	...
	if(isset($tst['json_value'])&&@$tst['json_value']){
		...
		$hostUrl=$json_value_gt['hostUrl'];
	}
	
	if(isset($hostUrl)&&trim($hostUrl)) $host_path=$hostUrl;
	else $host_path=$data['Host'];
	
	$r2=''; $subQuery="&destroy=2&actionurl=admin_direct";
	
	$data_send = strip_tags_d($data_send);
	
	if($data_send['acquirer_status_code']==2){ //success
		$return_url = $host_path."/return_url{$data['ex']}?transID=$transID&action=redirect".$subQuery;
	}
	elseif($data_send['acquirer_status_code']==1){ //pending
		//$return_url = $host_path."/trans_processing{$data['ex']}?transID=$transID&action=redirect".$subQuery;
		$return_url = $host_path."/return_url{$data['ex']}?transID=$transID&action=redirect".$subQuery;
	}
	else{ //failed or expired or Cancelled
		$return_url = $host_path."/return_url{$data['ex']}?transID=$transID&action=redirect".$subQuery;
	...
	//echo "<br/><br/>use_curl=>"; print_r($use_curl); exit;
...
//			$where[]=" (t.sender={$post['bid']} OR t.merID={$post['bid']})";
	...
	// Note: keyname array defined in function_raj.do
		...
		//print_r($where);
		//exit;
		//if($post['status']==1)$reportdate.=" OR `trans_status`=7 ";
	...
	//condition for trans_type
		...
		if((isset($_SESSION['sub_admin_id']))&&($_SESSION['get_mid']!='M. All')){
			...
			//$reportdate.=" AND ( `merID` IN ({$get_mid})  OR  `sender` IN ({$get_mid} ) ) ";
		}
		...
		if(isset($_REQUEST['payment_status']) && $_REQUEST['payment_status']){
			...
			if($payment_status>=0){
				//echo "<br/><br/>payment_status=>".$payment_status."<br/><br/>";
		...
		if(isset($_REQUEST['search_key']) && isset($_REQUEST['key_name']) && $_REQUEST['search_key']&&$_REQUEST['key_name']){
			...
			elseif($kn=='transID'||$kn=='bearer_token'||$kn=='reference'||$kn=='acquirer'||$kn=='trans_status'||$kn=='merID'||$kn=='terNO'||$kn=='channel_type'||$kn=='trans_type'||$kn=='settelement_delay'||$kn=='rolling_delay'||$kn=='remark_status'){
				//$search_key2=impf($search_key1);
			...
			elseif($kn=='descriptor'||$kn=='json_value'||$kn=='acquirer_response'){
				...
				$search_key2=impf($search_key1,1);
				$que3= "`t`.`".$kn."` {$sort}IN ({$search_key2})  ";
				
			}
			//$que3=queryArrayf($_REQUEST['search_key'],$_REQUEST['key_name'],$sort.'LIKE',$sortImp,';');
			$reportdate .= " AND ({$que3}) ";
			$where[]	= " ({$que3}) ";
		}
...
if(!isset($post['StartPage']) || !$post['StartPage']) $post['StartPage'] =0;
	//fetch the all transaction via query created in above (function adm_trans_list() defined in function_raj) 
