...
<a name="send" id="sub_remark" value="CONTINUE"  class="add_remark_submit btn btn-primary btn-sm mb-2"><i class="<?=$data['fwicon']['circle-plus'];?>"></i> Submit</a>
	...
	<div class="col-sm-12 border rounded mb-2">
	   ...
	   <?
$getimg=$post['TransactionDetails']['mop'];		
if ($getimg=='visa'){$fwimg=1; $fwicon=$data['fwicon']['visa']; $img="visacard.png";$txt='Visa Card';}
elseif ($getimg=='WA'){$fwimg=1; $fwicon=$data['fwicon']['wallet'];}
elseif ($getimg=='UPI'){$fwimg=0; $img="upi.png";}
elseif ($getimg=='jcb'){$fwimg=1; $fwicon=$data['fwicon']['jcb']; $img="jcb.png";$txt='JCB Card';}
elseif ($getimg=='mastercard'){$fwimg=1; $fwicon=$data['fwicon']['mastercard'];$img="master.png";$txt='Master Card';}
elseif ($getimg=='discover'){$fwimg=1; $fwicon=$data['fwicon']['discover'];$img="discover.png";$txt='Discover';}
elseif ($getimg=='rupay'){$img="rupay.jpg";$txt='Rupay';}
elseif ($getimg=='amex'){$fwimg=1; $fwicon=$data['fwicon']['amex'];$img="amex.png";$txt='American Express';}
elseif (empty($getimg)){$fwimg=1; $fwicon=$data['fwicon']['nocc'];$img="nocc.png";$txt='No Card Available';}
else{
	$fwimg=0;
...
<? if(isset($img)&&trim($img)&&$fwimg==0){ ?>
	  <img src="<?=$data['Host']?>/images/<?=$img?>" style="max-height:18px;" class="img-thumbnail55" alt="<?=(isset($txt)&&trim($txt)?$txt:'')?>" title="<?=(isset($txt)&&trim($txt)?$txt:'')?>">
<? }else{ echo $fwicon;} ?>
