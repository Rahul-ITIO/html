...
if(isset($data['TransactionsList'])&&$data['TransactionsList'])
{
	...
	for($i=0;$i<count($data['TransactionsList']);$i++)
	{
		...
		$email_add	= $transactions_detail['bill_email']; // change from $transactions_detail['email_add'];
		...
		if($email_add)
		{
			if(isset($emailAMT[$email_add])){ 
				$emailAMT[$email_add] = $emailAMT[$email_add]+($amount);
				$emailCNT[$email_add]++;
			}
			else{ 
				$emailAMT[$email_add] = ($amount);
				$emailCNT[$email_add] =1;
			}
		}
		//group by bin_no
		if($bin_no)
...
<?
				...
				foreach($emailAMT as $x => $x_value) {
					...
					$email_name = '<a href="'.$tr_url.'&key_name=email_add&search_key[]='.$email_name.'" target="_blank">'.$email_name.'</a>';
					?>
