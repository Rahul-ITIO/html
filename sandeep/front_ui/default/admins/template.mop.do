...
<h4 class="my-2"><i class="<?=$data['fwicon']['merchant-category'];?>"></i> MOP (Mode of Payment) <a data-ihref="<?=$data['Admins']?>/json_log_all<?=$data['ex']?>?tablename=mop_table&mode=json_log" title="View Json Log History" onclick="iframe_open_modal(this);"><i class="fa-solid <?=$data['fwicon']['circle-info'];?> text-danger fa-fw"></i></a></h4>
</div>
	...
	?>
        <tr valign="top" class="rounded">
          <td><?=ucwords(strtolower($value['mop_name']))?></td>

          <td>
			<span class="btn btn-abv-search text-start"><?=$value['assign_id'];?></span>          
		  </td>
		  ...
		  <div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action"><i class="<?=$data['fwicon']['circle-down'];?> text-link"></i></a>
                <ul class="dropdown-menu dropdown-menu-icon pull-right text-center" >
                  ...
                  <li> <a class="dropdown-item" href="<?=$data['Admins'];?>/<?=$data['PageFile']?><?=$data['ex']?>?id=<?=$value['id']?>&action=update"><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a></li>

                  <li> <a class="dropdown-item"  href="<?=$data['Admins'];?>/<?=$data['PageFile']?><?=$data['ex']?>?id=<?=$value['id']?>&action=delete" onclick="return confirm('Are you Sure to Delete');" title="Delete"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a></li>
...
?>
    ...
    <h4 class="my-2"><i class="<?=$data['fwicon']['recentorder'];?>"></i> <?php if(isset($post['gid'])&&$post['gid']){ ?> Edit <? } else { ?> Add New <? } ?> MOP (Mode of Payment) <?php if(isset($post['gid'])&&$post['gid']){ ?> 
          ...
          <select id="mop_code" data-placeholder="Start typing the MOP Symbols " multiple class="chosen-select form-control" name="mop_code[]" style="clear:right;width:100%;" >
			...
			<?
				...
				foreach($iconsList as $img_Name){
					$imgname=str_replace(['../images/icons/','../bank/'],'',$img_Name);
