...
//this function will be used for transaction status on based transaction id
	...
	{
		$_SESSION['notifybridge_url_mer']=trim($t_status['return_url']);
	}
	...
	$status_array["trans_response"]=$t_status['trans_response'];
	$status_array["id_order"]=$t_status['reference'];
	if($t_status['mop']){
...
if(!empty($webhook_url))
{
	...
	$status_array['webhook_via']="s2s_base_webhook";	
	$chs = curl_init();
...
elseif((empty($webhook_url))&&($notify_failed_source='')){
	...
	$status_array['webhook_via']="host_base_notify";	
}
