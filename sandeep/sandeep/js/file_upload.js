...
function add_more_files(e,theInputs){
	$(e).before($("<div/>", {id: 'filediv'}).fadeIn('slow').append(
		$("<input/>", {name: theInputs, type: 'file', onchange: 'imageIsChange(this)', id: 'file'}),        
		$("<br/><br/>")
	));
}
...
$('body').bind('change', '#formdiv #file', function(){
	if (this.files && this.files[0]) {
		...
		$(this).hide();
		$("#formdiv #abcd"+ abc).append($("<img/>", {id: 'img', src: templateUrl+'/images/x.png', alt: 'delete'}).click(function() {
		$(this).parent().parent().remove();
		}));
	}
});

function imageIsChange(e) {
	//alert(e.name);
		...
		$(e).before("<div id='abcd"+ abc +"' class='abcd'><span onclick='uploadfile_viewf(e)'><img id='previewimg" + abc + "' src=''/></span></div>");
