...
//end: class	
		...
		$header 		= array('Payment ID', 'Name', 'Amount', 'Discount Rate', 'Transaction Fee', 'Holdback Reserve', 'Total');
		...
		foreach($post['pdfsettledtrans'] as $key=>$value){
			...
			if($value['status']==1 || $value['status']==4 || $value['status']==7){ // completed
				$dis_rate 	= ($value['transaction_amt']/100)*$accounts[0]['transaction_rate'];
				$rol_fee	= ($value['transaction_amt']/100)*$accounts[0]['rolling_fee'];
				$this_total	= ($value['transaction_amt'])-($dis_rate+$accounts[0]['txn_fee']+$rol_fee);
				
				$total_txn_fee = $total_txn_fee+$accounts[0]['txn_fee'];
				
				$total_dis_rate = $total_dis_rate + $dis_rate;
				$total_rol_fee	= $total_rol_fee + $rol_fee;
				
				$sales_volume	= $sales_volume+$value['transaction_amt'];
				
				$completed_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".bcadd(0,$dis_rate,2).";".bcadd(0,$accounts[0]['txn_fee'],2).";".bcadd(0,$rol_fee,2).";".bcadd(0,$this_total,2)."";
			}
			if($value['status']=="2" || $value['status']==2){ // canceled
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_total	= bcadd(0,$accounts[0]['txn_fee'],2);
				...
				if($value['trname']=="ch"){
					...
					$cancelled_data[] .= $value['transaction_id'].";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";".$accounts[0]['txn_fee'].";".$rol_fee.";-".$this_total."";
		...
		foreach($post['pdftrans_rtn'] as $key=>$value){
			...
			if($value['status']=="11" || $value['status']==11){ // cbk1 
				$cbk1_fee=$accounts[0]['cbk1'];
				
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_tr_fee= $cbk1_fee;
				
				$this_total	= bcadd(0,$value['transaction_amt']+$cbk1_fee,2); // 45
			...
			if($value['status']==3 || $value['status']==12){ // Refunded
				...
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_total	= bcadd(0,$value['transaction_amt']+15,2);
				
				$total_refund_fee = bcadd(0,$total_refund_fee+15,2);
				
				$refund_volume	= bcadd(0,$refund_volume+$value['transaction_amt'],2);
				
				$refunded_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";15;".$rol_fee.";-".$this_total."";
			}
			if($value['status']=="5" || $value['status']==5){ // Chargeback 
				...
				$this_total	= bcadd(0,$value['transaction_amt']+$charge_back_fee,2); // 45
				
				$total_returns_fee = bcadd(0,$total_returns_fee+$charge_back_fee,2); //45  as per risk ratio and account 1 from 3
