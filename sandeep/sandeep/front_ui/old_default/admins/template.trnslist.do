...
$(document).ready(function(){
	...
	$(".actionlnk_xxx").click(function(){
		...
		var subparameter2 = "<?=$data['Admins'];?>/<?=$data['trnslist'];?><?=$data['ex']?>?action="+$(this).attr('data-action')+"&trange="+valuesArray+"<?php echo "&bid=".$get_bid."&type=".$get_type."&status=".$get_status."&page=".$get_page."&order=".$get_order;?>";
		var promptTxt="";
			...
			if (promptmsg == null || promptmsg == "") {
				...
				function f() {
					...
					if(data_action==='sendtobank' || data_action==='utrnstatus'){
						var subparameter2 = "<?=$data['Host'];?>/nodal/"+data_action+"<?=$data['ex']?>?action="+data_action+"&orderset="+ary[i]+"<?php echo "&bid=".$get_bid."&type=".$get_type."&status=".$get_status."&page=".$get_page."&order=".$get_order?>&admin=1&actionurl=admin_direct";
					}else{
						var subparameter2 = "<?=$data['Admins'];?>/<?=$data['trnslist'];?><?=$data['ex']?>?action="+data_action+"&trange="+ary[i]+"<?php echo "&bid=".$get_bid."&type=".$get_type."&status=".$get_status."&page=".$get_page."&order=".$get_order?>";
					}
...
function check_statusf(e,statusUrl='') {
	var orderset=$(e).attr('data-orderset');
...
function hkipstatus(e,theValue='') {
	...
	if(acctype==22||acctype==21){
		...
		subqry="?orderset="+mhoids+"&actionurl="+actionurl+"&type="+acctype+"&admin=1";
		...
		subqry="?orderset="+mhoids+"&actionurl="+actionurl+"&type="+acctype+"&admin=1&limit1=1&pop=2";
		popuploadig();
...
<div class="container border bg-white my-1 rounded" >
                      ...
                      <div class="" style="margin-top: -2px;">
                        ...
                        <div id="transaction_display_divid" class="hide" style="float:right;right:0;">
                          <select id="transaction_display" data-placeholder="Transaction Display Option" title="Transaction Display Option" multiple class="chosen-select chosen-rtl1 form-select" name="transaction_display[]">
                            ...
                            <option value="transID" data-placeholder="Payment ID" title="TransID">TransID</option>
							...
							<? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['bearer_token'])&&$_SESSION['bearer_token']==1)){?>
                            <option value="bearer_token" data-placeholder="Reference" title="Bearer Token">Bearer Token</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_fullname'])&&$_SESSION['tr_fullname']==1)){?>
                            <option value="fullname" data-holder="Customer Name" title="Customer Name">Customer Name</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_amount'])&&$_SESSION['tr_amount']==1)){?>
                            <option value="amount" data-holder="Order Amount" title="Order Amount">Trans Amt</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_transaction_amt'])&&$_SESSION['tr_transaction_amt']==1)){?>
                            <option value="trans_amt" data-holder="Trans.Amt." title="Trans.Amt.">Trans.Amt.</option>
                            ...
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mop'])&&$_SESSION['tr_mop']==1)){?>
                            <option value="mop" data-holder="Visa;MasterCard" title="Visa,MasterCard">Source - Card Type</option>
                            ...
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_merID'])&&$_SESSION['tr_merID']==1)){?>
                            <option value="merID" data-holder="Receiver" title="Receiver">Receiver</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_acquirer_response'])&&$_SESSION['tr_acquirer_response']==1)){?>
                            <option value="acquirer_response" data-holder="Bank Response" title="Bank Response">Bank Response</option>
                            ...
                            <option value="bill_currency" data-holder="Order Currency" title="Order Currency">Order Currency</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_remark'])&&$_SESSION['tr_remark']==1)){?>
                            <option value="mer_note" title="Note Merchant">Note Merchant</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_support_note'])&&$_SESSION['tr_support_note']==1)){?>
                            ...
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_orderset'])&&$_SESSION['tr_orderset']==1)){?>
                            <option value="orderset" title="Orderset">Orderset</option>
                            ...
                            <option value="buy_mdr_amt" title="Discount Rate">Discount Rate</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_txtfee_amt'])&&$_SESSION['tr_mdr_txtfee_amt']==1)){?>
                            <option value="buy_txnfee_amt" title="Transaction Fee">Transaction Fee</option>
                            ...
                            <option value="settelement_date" title="Payout Date">Payout Date</option>
                            ...
                            <option value="acquirer_ref" data-placeholder="Bank Txn ID" title="Bank Txn ID">RefNo. Bank Txn ID</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_trans_type'])&&$_SESSION['tr_trans_type']==1)){?>
                            <option value="trans_type" data-holder="wd;wr;dp;cn;ch" title="wd;wr;dp;cn;ch">Payment Type</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_email'])&&$_SESSION['tr_bill_email']==1)){?>
                            <option value="bill_email" data-holder="Email for Transaction" title="Email for Transaction">Email</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_phone'])&&$_SESSION['tr_bill_phone']==1)){?>
                            <option value="bill_phone" data-holder="Phone for Transaction" title="Phone for Transaction">Phone</option>
                            ...
                            <option value="bill_address" data-holder="Address" title="Address">Address</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_city'])&&$_SESSION['tr_city']==1)){?>
                            <option value="bill_city" data-holder="City" title="City">City</option>
                            <? }if((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_state'])&&$_SESSION['tr_state']==1)){?>
                            <option value="bill_state" data-holder="State" title="State">State</option>
                            ...
                            <option value="bill_zip" data-holder="Zip" title="Zip">Zip</option>
                            ...
                            <option value="bill_ip" data-holder="IP" title="IP">IP</option>
                            ...
                            <option value="gst_amt" data-holder="GST Fee" title="GST Fee">GST Fee</option>
                            <? } ?>
                            <? } ?>
                  ...
                  <div class="rowd2 row"> <span class="mywrap-collabsible tog1" >
                    ...
                    <div id="collapsible3_content_html" class="collapsible-content border bg-light rounded text-start" style="float:left;width:100%;overflow:inherit;max-height:inherit;display:none;">
                      <div id="collapsible3_html" class="content-inner" style="height:auto;float:left;width:97.6%;display:block;">
                        <!-- start Content-->
                        <div class="m-2"> <b style="margin:0 0 0 0px;">Transaction For Account:</b> <a <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['merchant_action_view'])&&$_SESSION['merchant_action_view']==1)){?> href="<?=$data['Admins']?>/<?=$data['MER']?><?=$data['ex']?>?id=<?=$post['MemberInfo']['id']?>&action=detail" <? }?> >
...
if(((isset($_SESSION['login_adm']))||(isset($_SESSION['transaction_payout_manual'])&&$_SESSION['transaction_payout_manual']==1))&&($transaction_payout_manual)){?>
					 ...
					 ?>
                              <li class="dropdown"> <a data-toggle="dropdown" class="glyphicons hand_down payoutpdf0" onclick="filteraction0(this)"><?php echo date("D d-m-Y",strtotime($value)); ?> </a>
                                <ul class="dropdown-menu pull-right">
                                  ...
                                  <? }?>
                                  <li><a onclick="filteraction(this)" target="pdfreport" href="<?=$data['Host']?>/<?=$report_file_name?><?=$data['ex']?>?bid=<?php echo $_GET['bid'];?>&pdate=<?php echo date("Y-m-d",strtotime($value));?>&type=<?php echo $_GET['acquirer'];?>" class="payoutpdf pdfreportcl"><i class="<?=$data['fwicon']['pdf'];?>"></i><span>PDF Report</span></a></li>
                                  <li><a onclick="filteraction(this)" target="pdfreport_1" href="<?=$data['Host']?>/<?=$report_file_name?>_1<?=$data['ex']?>?bid=<?php echo $_GET['bid'];?>&pdate=<?php echo date("Y-m-d",strtotime($value));?>&type=<?php echo $_GET['acquirer'];?>" class="payoutpdf pdfreportcl_1"><i class="<?=$data['fwicon']['pdf'];?>"></i><span>PDF Report</span></a></li>
  ...
  checkboxes[i].checked = source.checked;
          ...
          <table id="content22"  class="table table-hover" >  <!--frame22 echektran22-->
              <thead>
                <tr>
				  ...
				  <th scope="col"  class="text-start">
                    ...
                    <a class="trPaymentId text-link" href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? }?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=id" title="TransID" data-bs-toggle="tooltip" data-bs-placement="top">TransID</a> 
					
					
					</th>
                  <? }?>
                  <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['transaction_action_all'])&&$_SESSION['transaction_action_all']==1)){ ?>
                  <th scope="col" title="Action" data-bs-toggle="tooltip" data-bs-placement="top"><i class="<?=$data['fwicon']['circle-info'];?> text-link"></i></th>
                  <? }?>
                  <? if(in_array("reference",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mrid'])&&$_SESSION['tr_mrid']==1))){?>
                  <th scope="col"><a title="Merchant Reference" data-bs-toggle="tooltip" data-bs-placement="top" href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=reference" class="text-link">Reference</a></th>
                  <? }?>
				  <? if(in_array("bearer_token",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mrid'])&&$_SESSION['tr_bearer_token']==1))){?>
                  <th scope="col"><a title="Bearer Token" data-bs-toggle="tooltip" data-bs-placement="top" href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=bearer_token" class="text-link">B.Token</a></th>
                  <? }?>
                  <? if(in_array("fullname",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_fullname'])&&$_SESSION['tr_fullname']==1))){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=fullname" class="text-link" title="Customer Name" data-bs-toggle="tooltip" data-bs-placement="top">Customer&nbsp;Name</a></th>
                  <? }?>
                  ...
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=amount" class="text-link" title="Order Amount" data-bs-toggle="tooltip" data-bs-placement="top">Bill&nbsp;Amt</a></th>
                  <? }?>
                  <? if(in_array("trans_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_transaction_amt'])&&$_SESSION['tr_transaction_amt']==1))){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=trans_amt" class="text-link" title="Transaction Amount" data-bs-toggle="tooltip" data-bs-placement="top">Trans.&nbsp;Amount</a></th>
                  <? }?>
                  ...
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=available_balance" title="Balance Amount" data-bs-toggle="tooltip" data-bs-placement="top" class="text-link">Balance</a></th>
                  <? }?>
                  <? if(in_array("tdate",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_tdate'])&&$_SESSION['tr_tdate']==1))){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=tdate" class="text-link" title="Transaction Date Time" data-bs-toggle="tooltip" data-bs-placement="top">Timestamp </a></th>
                  <? }?>
                  <? if(in_array("mop",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mop'])&&$_SESSION['tr_mop']==1))){?>
                  <th scope="col" title="Source of Transaction" data-bs-toggle="tooltip" data-bs-placement="top">So.</th>
                  <? }?>
                  ...
                  <? if($post['acquirer']<0){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=type" class="text-link">Acquirer</a></th>
                  <? }?>
                  <? }?>
                  <? }?>
                  ...
                  <? if($post['status']<0){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=status" class="text-link">Status</a></th>
                  <? }?>
                  <? }?>
                  <? if(in_array("merID",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_merID'])&&$_SESSION['tr_merID']==1))){?>
                  <th scope="col"><a href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>page=<?=$curr_pg?>&type=<?=$post['acquirer']?>&status=<?=$post['status']?>&action=select&order=merID" class="text-link">Receiver</a></th>
                  <? }?>
                  <? if(in_array("upa",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_upa'])&&$_SESSION['tr_upa']==1))){?>
                  <th scope="col" title="UPA" data-bs-toggle="tooltip" data-bs-placement="top">UPA</th>
                  <? }?>
                  <? if(in_array("ccno",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_ccno'])&&$_SESSION['tr_ccno']==1))){?>
                  <th scope="col" title="Card Number" data-bs-toggle="tooltip" data-bs-placement="top">Card&nbsp;Number</th>
                  <? }?>
                  <? if(in_array("bill_ip",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_ip'])&&$_SESSION['tr_ip']==1))){?>
                  <th scope="col" title="IP" data-bs-toggle="tooltip" data-bs-placement="top">IP</th>
                  <? }?>
                  <? if(in_array("terNO",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_terNO'])&&$_SESSION['tr_terNO']==1))){?>
                  <th scope="col" title="TerNO" data-bs-toggle="tooltip" data-bs-placement="top">TerNO</th>
                  <? }?>
                  <? if(in_array("acquirer_ref",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_txn_id'])&&$_SESSION['tr_txn_id']==1))){?>
                  <th scope="col" title="Ref No. of Bank Response" data-bs-toggle="tooltip" data-bs-placement="top">Ref&nbsp;No.</th>
                  <? }?>
                  <? if(in_array("acquirer_response",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_acquirer_response'])&&$_SESSION['tr_acquirer_response']==1))){?>
                  <th scope="col" title="Bank Response" data-bs-toggle="tooltip" data-bs-placement="top">Bank&nbsp;Response</th>
                  <? }?>
                  ...
                  <th scope="col" title="Post Josn">Post&nbsp;Josn</th>
                  <? }?>
                  <? if(in_array("mer_note",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_remark'])&&$_SESSION['tr_remark']==1))){?>
                  <th scope="col" title="Note Merchant" data-bs-toggle="tooltip" data-bs-placement="top">Note&nbsp;Merchant</th>
                  <? }?>
                  <? if(in_array("support_note",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_support_note'])&&$_SESSION['tr_support_note']==1))){?>
                  <th scope="col" title="Note Support" data-bs-toggle="tooltip" data-bs-placement="top">Note&nbsp;Support</th>
                  <? }?>
                  ...
                  <th scope="col" title="Note System" data-bs-toggle="tooltip" data-bs-placement="top">Note&nbsp;System</th>
                  <? }?>
                  ...
                  <th scope="col" title="Source Url" data-bs-toggle="tooltip" data-bs-placement="top">Source&nbsp;Url</th>
                  <? }?>
                  <? if(in_array("webhook_url",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_notify_url'])&&$_SESSION['tr_notify_url']==1))){?>
                  <th scope="col" title="Webhook Url" data-bs-toggle="tooltip" data-bs-placement="top">Webhook&nbsp;Url</th>
                  <? }?>
                  <? if(in_array("return_url",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_success_url'])&&$_SESSION['tr_success_url']==1))){?>
                  <th scope="col" title="Return Url" data-bs-toggle="tooltip" data-bs-placement="top">Return&nbsp;Url</th>
                  <? }?>
                 
                  <? if(in_array("buy_mdr_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_amt'])&&$_SESSION['tr_mdr_amt']==1))){?>
                  <th scope="col" title="Discount Rate" data-bs-toggle="tooltip" data-bs-placement="top">Disc.&nbsp;Rate</th>
                  <? }?>
                  <? if(in_array("gst_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_gst_fee'])&&$_SESSION['tr_gst_fee']==1))&&($data['con_name']=='clk')){?>
                  <th scope="col" title="GST Fee" data-bs-toggle="tooltip" data-bs-placement="top">GST&nbsp;Fee</th>
                  <? }?>
				  
				  <? if(in_array("rrn",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_rrn'])&&$_SESSION['tr_rrn']==1))){?>
                  <th scope="col" title="RRN" data-bs-toggle="tooltip" data-bs-placement="top">RRN</th>
                  <? }?>
				  
                  <? if(in_array("buy_txnfee_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_txtfee_amt'])&&$_SESSION['tr_mdr_txtfee_amt']==1))){?>
                  <th scope="col" title="Transaction Fee" data-bs-toggle="tooltip" data-bs-placement="top">Transaction&nbsp;Fee</th>
                  <? }?>
                  ...
                  <th scope="col" title="Rolling Fee" data-bs-toggle="tooltip" data-bs-placement="top">Rolling&nbsp;Fee</th>
                  <? }?>
                  ...
                  <th scope="col" title="Chargeback Fee" data-bs-toggle="tooltip" data-bs-placement="top">CBK&nbsp;Fee</th>
                  <? }?>
                  ...
                  <th scope="col" title="Predispute Fee" data-bs-toggle="tooltip" data-bs-placement="top">Predispute&nbsp;Fee</th>
                  <? }?>
                  ...
                  <th scope="col" title="Refund Fee" data-bs-toggle="tooltip" data-bs-placement="top">Refund&nbsp;Fee</th>
                  <? }?>
                  ...
                  <th scope="col" title="Payout Amount" data-bs-toggle="tooltip" data-bs-placement="top">Pay.&nbsp;Amt.</th>
                  <? }?>
                  <? if(in_array("settelement_date",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_payout_date'])&&$_SESSION['tr_payout_date']==1))){?>
                  <th scope="col" title="Payout Date" data-bs-toggle="tooltip" data-bs-placement="top">Payout&nbsp;Date</th>
                  <? }?>
                  ...
                  <th scope="col" title="Created Date" data-bs-toggle="tooltip" data-bs-placement="top">Created&nbsp;Date</th>
                  <? }?>
                  ...
                  <th scope="col" title="Risk Ratio" data-bs-toggle="tooltip" data-bs-placement="top">Risk&nbsp;Ratio</th>
                  <? }?>
                  ...
                  <th scope="col" title="Transaction Period" data-bs-toggle="tooltip" data-bs-placement="top">Transaction&nbsp;Period</th>
                  <? }?>
                  ...
                  <th scope="col" title="Bank Processing Amount" data-bs-toggle="tooltip" data-bs-placement="top">B.&nbsp;Proc.&nbsp;Amount</th>
                  <? }?>
                  ...
                  <th scope="col" title="Bank Processing Currency" data-bs-toggle="tooltip" data-bs-placement="top">B.&nbsp;Proc.&nbsp;Currency</th>
                  <? }?>
                  <? if(in_array("bill_currency",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_currname'])&&$_SESSION['tr_currname']==1))){?>
                  <th scope="col" title="Order Currency" data-bs-toggle="tooltip" data-bs-placement="top">Order&nbsp;Currency</th>
                  <? }?>
                  <? if(in_array("orderset",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_orderset'])&&$_SESSION['tr_orderset']==1))){?>
                  <th scope="col" title="Orderset" data-bs-toggle="tooltip" data-bs-placement="top">Orderset</th>
                  <? }?>
                  <? if(in_array("trans_type",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_trans_type'])&&$_SESSION['tr_trans_type']==1))){?>
                  <th scope="col" title="Payment Type" data-bs-toggle="tooltip" data-bs-placement="top">Payment&nbsp;Type</th>
                  <? }?>
                  <? if(in_array("bill_email",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_email'])&&$_SESSION['tr_bill_email']==1))){?>
                  <th scope="col" title="Email" data-bs-toggle="tooltip" data-bs-placement="top">Email</th>
                  <? }?>
                  <? if(in_array("bill_phone",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_phone'])&&$_SESSION['tr_bill_phone']==1))){?>
                  <th scope="col" title="Phone" data-bs-toggle="tooltip" data-bs-placement="top">Phone</th>
                  <? }?>
                  ...
                  <th scope="col" title="Product Name" data-bs-toggle="tooltip" data-bs-placement="top">Product&nbsp;Name</th>
                  <? }?>
                  <? if(in_array("bill_address",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_address'])&&$_SESSION['tr_address']==1))){?>
                  <th scope="col" title="Address" data-bs-toggle="tooltip" data-bs-placement="top">Address</th>
                  <? }?>
                  <? if(in_array("bill_city",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_city'])&&$_SESSION['tr_city']==1))){?>
                  <th scope="col" title="City" data-bs-toggle="tooltip" data-bs-placement="top">City</th>
                  <? }?>
                  <? if(in_array("bill_state",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_state'])&&$_SESSION['tr_state']==1))){?>
                  <th scope="col" title="State" data-bs-toggle="tooltip" data-bs-placement="top">State</th>
                  <? }?>
                  ...
                  <th scope="col" title="Country" data-bs-toggle="tooltip" data-bs-placement="top">Country</th>
                  <? }?>
                  <? if(in_array("bill_zip",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_zip'])&&$_SESSION['tr_zip']==1))){?>
                  <th scope="col" title="Zip" data-bs-toggle="tooltip" data-bs-placement="top">Zip</th>
                  <? }?>
                  ...
                  <th scope="col" title="Descriptor" data-bs-toggle="tooltip" data-bs-placement="top">Descriptor</th>
                  <? }?>
                </tr>
              </thead>
              ...
              <tr valign="top" >
                <? if(in_array("transID",$_SESSION['transaction_display_arr'])){?>
                <td>				<? if((isset($_SESSION['login_adm']))||(isset($_SESSION['t_multiple_check_box'])&&$_SESSION['t_multiple_check_box']==1)){ ?>
                  <input type="checkbox" name="echeckid[]" class="echeckid  float-start form-check-input" data-value="<?=$value['transID']?>" value="<?=(($value['typenum']==2)?$value['transID']."_".$value['id']:$value['id']);?>" style="display:none;">
                  <? }?></td>
				<td data-bs-toggle="tooltip" data-bs-placement="top" title="<?=$value['transID'];?>">

                  <a class="hrefmodal ms-1 text-link" data-tid="<?=$value['transID']?>" data-href="<?=($data['Host']."/{$data['trnslist']}_get".$data['ex']."?id=".$value['id']."&uid=".((isset($post['bid'])&&$post['bid']>0)?$post['bid']:$value['merID'])."&bid=".((isset($post['bid'])&&$post['bid']>0)?$post['bid']:$value['merID'])).(isset($_GET['keyname'])?"&keyname={$_GET['keyname']}":"").(isset($_GET['bid'])?"&mview=admin":"");?>&action=details&admin=1&tempui=<?=$data['frontUiName']?>" title="<?=$value['transID'];?>" >
                  <? 
			
			if($value['typenum']==2){ echo 'W'; }
			elseif($value['ostatus']=="3" || $value['ostatus']=="5" || $value['ostatus']=="6" || $value['ostatus']=="11" || $value['ostatus']=="12"){ echo 'R'; }?><?=substr($value['transID'],0,14);?>
                  </a>
  ...
  <li><a class="iframe_open11 dropdown-item" data-mid="<?=$value['merID']?>" data-href="<?=$idx;?>_toggle" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/include/edit_trans<?=$data['ex']?>?id=<?=$value['id']?>&admin=1" title="Edit|Delete|Cancelled Trans." onClick="iframe_open_modal(this);" ><i class="<?=$data['fwicon']['edit'];?>"></i> Edit Trxn.</a></li>
  <? }?>
  <? 
  ...
  <?
										...
										?>
  <li class='g_validate 18'><a data-mhoid="<?=$value['transID'];?>_<?=$value['id'];?>" data-type="<?=$b_val['payout_id'];?>" onClick="post_Withdraw(this);" class="hkip_status_id dropdown-item" title="<?=$payout_json['checkout_level_name'];?> Post"><i class="<?=$data['fwicon']['bank'];?>"></i>
    <?=$payout_json['checkout_level_name'];?>
    Post</a></li>
  <?
										}
  ...
  <li><a data-amount="<?=$value['oamount']?>" data-reason="<?=$value['acquirer']?> Approved" data-label="<?=$value['acquirer']?> Approved" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?bid=<?=$value['merID']?>&id=<?=$value['id']?>&type=<?=$value['typenum']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?>&action=confirm" data-cond="1" class="dialog_refunded dropdown-item withdraw type_<?=$value['typenum']?> "><i class="<?=$data['fwicon']['rotate-right'];?>"></i> Accept</a></li>
  <? }?>
  <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['t_withdraw_reject'])&&$_SESSION['t_withdraw_reject']==1)){?>
  <li><a data-reason="<?=$value['acquirer']?> Request Rejected" data-label="<?=$value['acquirer']?> Reason" onClick="confirm2(this);" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?bid=<?=$value['merID']?>&id=<?=$value['id']?>&type=<?=$value['typenum']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?>&action=cancel" class="ajxstatus dropdown-item"><i class="<?=$data['fwicon']['reject'];?>"></i> Reject</a></li>
  <? }?>
  ...
  <? }?>
  <? if($value['ostatus']==1 || $value['ostatus']==4  || $value['ostatus']==6 || $value['ostatus']==8){?>
  <? if($value['trans_type']=="cn"){?>
  <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['t_status_chargeback'])&&$_SESSION['t_status_chargeback']==1)){?>
  <li><a data-reason="Chargeback Reason" data-label="Chargeback" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['id']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?><? if($post['acquirer']){?>&type=<?=$post['acquirer']?><? }?>&action=chargeback" class="dialog_open dropdown-item"><i class="<?=$data['fwicon']['chargeback'];?>"></i> Chargeback</a></li>
  <? }?>
  <? }?>
  <? }?>
  <? if($value['ostatus']!=1&&(isset($_SESSION['login_adm']))||(isset($_SESSION['t_a_require'])&&$_SESSION['t_a_require']==1)){?>
  <li><a data-reason="Transaction documents has been requested from the merchant" data-label="System will send the email to merchant to request documents" onClick="confirm2(this);" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['id']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?><? if($post['acquirer']){?>&type=<?=$post['acquirer']?><? }?>&action=action_require" class="ajxstatus dropdown-item"><i class="<?=$data['fwicon']['retweet'];?>"></i> A. Require</a></li>
  <? }?>
  <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['t_add_remark'])&&$_SESSION['t_add_remark']==1)){?>
  <li><a data-href="<?=$idx;?>_toggle" data-tabname="addremarkform_tab" class="atablink dropdown-item"><i class='fa-solid fa-comments' ></i> Add Remark</a></li>
  <? }?>
  <? if($value['typenum']!=2&&((isset($_SESSION['login_adm']))||(isset($_SESSION['t_cs_trans'])&&$_SESSION['t_cs_trans']==1))){?>
  <li><a class="iframe_open99 dropdown-item" data-mid="<?=$value['merID']?>" data-href="<?=$idx;?>_toggle" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/include/similar_trans<?=$data['ex']?>?id=<?=$value['id']?>&admin=1" title="Create Similar Transaction" onClick="iframe_open_modal(this);"><i class="<?=$data['fwicon']['database'];?>"></i>&nbsp;C. S. Trxn.</a></li>
  <? }?>
  <? 
  ...
  <li><a data-reason="Test Transaction" data-label="Do you want to Test this Transaction" onClick="confirm2(this);" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['id']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?><? if($post['acquirer']){?>&type=<?=$post['acquirer']?><? }?>&action=testransaction" class="ajxstatus dropdown-item"><i class="<?=$data['fwicon']['test'];?>"></i> Test</a></li>
  <? }?>
  ...
  <li><a data-reason="UnFlag Reason" data-label="UnFlag Reason" onClick="confirm2(this);" data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['id']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?><? if($post['acquirer']){?>&type=<?=$post['acquirer']?><? }?>&action=unflagransaction" class="ajxstatus dropdown-item"><i class="<?=$data['fwicon']['flag'];?>"></i> Unflag</a></li>
  <? }?>
  <? } ?>
  <? if($value['ostatus']==1 || $value['ostatus']==4  || $value['ostatus']==6 || $value['ostatus']==8){?>
  <? if($value['trans_type']=="cn"){?>
  <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['t_status_pre_dispute'])&&$_SESSION['t_status_pre_dispute']==1)){?>
  <li><a data-reason="Pre Dispute Reason" data-value="Pre-Dispute Alert Received"  data-href="<?=$data['Admins']?>/<?=$data['trnslist'];?><?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['id']?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? }?><? if($post['acquirer']){?>&type=<?=$post['acquirer']?><? }?>&action=cbk1" class="dialog_open dropdown-item"><i class="<?=$data['fwicon']['rotate-right'];?>"></i> Pre Dispute</a></li>
  <? }?>
  <? }?>
  <? }?>
  ...
  <?php /*?><li class='g_validate 13'><a data-orderset="<?=$value['orderset'];?>_<?=$value['id'];?>" data-type="<?=$value['typenum'];?>" onClick="check_statusf(this,'<?=($data['Host']);?>/<?=($value['json_value_de']['status_'.$value['typenum']]);?>&actionurl=by_admin&admin=1&type=<?=($value['typenum']);?>');" class="hkip_status_id dropdown-item"><i class="<?=$data['fwicon']['retweet'];?>"></i> <?=$data['acquirer_name'][$value['typenum']];?> Status</a></li><?php */?>
  

  
  <li><a class="hrefmodal dropdown-item" data-tid="<?=$data['acquirer_name'][$value['typenum']];?> Status - <?=$value['transID']?>" data-mid="<?=$value['merID']?>" data-href="<?=($data['Host']);?>/<?=($value['json_value_de']['status_'.$value['typenum']]);?>&actionurl=by_admin&admin=1&type=<?=($value['typenum']);?>" title="<?=$data['acquirer_name'][$value['typenum']];?> Status" ><i class="<?=$data['fwicon']['retweet'];?>"></i> <?=$data['acquirer_name'][$value['typenum']];?> Status</a></li>
  
    
  <? }else{?>
  <? if($value['typenum']>4){?>
  <li class='g_validate 14'><a data-mhoid="<?=$value['orderset'];?>_<?=$value['id'];?>" data-type="<?=$value['typenum'];?>" onClick="hkipstatus(this);" class="hkip_status_id dropdown-item"><i class="<?=$data['fwicon']['retweet'];?>"></i>
    <?=$data['acquirer_name'][$value['typenum']];?>
    Status</a></li>
  <? }?>
  <? }?>
  <? if($value['mop']=='amex'){?>
  <li class='g_validate 15'><a data-mhoid="<?=$value['orderset'];?>_<?=$value['id'];?>" data-type="16" onClick="hkipstatus(this);" class="hkip_status_id dropdown-item"><i class="<?=$data['fwicon']['retweet'];?>"></i> Amex Status</a></li>
  <? }?>
  <? }?>
  <? }?>
</ul>
                ...
                <? }?>
                <? }?>
                <? if(in_array("reference",$_SESSION['transaction_display_arr'])){?>
                <td align="center"><div title="<?=$value['reference']?>" data-bs-toggle="tooltip" data-bs-placement="top" style="width:90px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">
                    <?=$value['reference']?>
                  </div></td>
                <? }?>
				<? if(in_array("bearer_token",$_SESSION['transaction_display_arr'])){?>
                <td align="center"><div style="width:120px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">  
					<?if(isset($value['json_log_history'])&&trim($value['json_log_history'])){
						$acquirer_response1=jsonencode1($value['json_log_history'],'',1);
					?>
						<a onClick="load_popup_jsonf('<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>','<?=encryptres($acquirer_response1);?>','jsonLogHisotry');" class="nomid btn btn-outline-danger btn-sm my-2"  title="Json Log History" id="acqresp"><i class="<?=$data['fwicon']['eye-solid'];?>"></i></a>
					<?}?>
					
					
                    <a class=""  data-mid="<?=$value['merID']?>" data-ihref="<?=$data['Admins']?>/json_log_all<?=$data['ex']?>?tablename=<?=$data['MASTER_TRANS_TABLE']?>&clientid=<?=$value['merID']?>&action_name=<?=$value['bearer_token']?>" onclick="iframe_open_modal(this);" style="cursor:pointer;" title="<?=$value['bearer_token']?>" data-bs-toggle="tooltip" data-bs-placement="bottom"  ><?=$value['bearer_token']?></a>
					
					
					
                  </div></td>
                <? }?>
                <? if(in_array("fullname",$_SESSION['transaction_display_arr'])){?>
                <td align="center" nowrap title="Full Name: <?=prntext($value['fullname']);?>" data-bs-toggle="tooltip" data-bs-placement="top" nowrap><?=prntext(lf($value['fullname'],15,1));?>                </td>
                <? }?>
                <? if(in_array("amount",$_SESSION['transaction_display_arr'])){?>
                <td align="center" class="pricecolor" nowrap><?=$value['bill_amt']?></td>
                <? }?>
                <? if(in_array("trans_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_transaction_amt'])&&$_SESSION['tr_transaction_amt']==1))){?>
                <td align="center" class="pricecolor" nowrap><?=$value['trans_amt']?></td>
                <? }?>
                <? if(in_array("available_balance",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_available_balance'])&&$_SESSION['tr_available_balance']==1))){?>
                <td align="center" class="pricecolor" nowrap><?=$value['available_balance']?></td>
                <? }?>
                <? if(in_array("tdate",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_tdate'])&&$_SESSION['tr_tdate']==1))){?>
                <td align="center" nowrap><?=prndate($value['tdate'])?></td>
                <? }?>
                <? if(in_array("mop",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mop'])&&$_SESSION['tr_mop']==1))){?>
	   ...
	   if(isset($data['t'][$value['typenum']]['logo'])&&$data['t'][$value['typenum']]['logo']){
			...
			$txt=$data['acquirer_name'][$value['typenum']];
	   ...
	   elseif ($value['mop']=='rupay'){$img="rupay.jpg";$txt='Rupay';$sort="&ccard_type=rupay";}
	  ...
	  if(isset($img)&&trim($img)&&$img=="nocc.png"){ $fwimg=1; $fwicon=$data['fwicon']['nocc']; }
	   ...
	   ?>
                  <a href="<?=$sort;?>" title="<?=$value['mop']?> <?=(isset($txt)&&trim($txt)?' - '.$txt:'')?>" data-bs-toggle="tooltip" data-bs-placement="top">
                  <? if(isset($img)&&trim($img)&&$fwimg==0){ ?>
                ...
                <? }?>
                <? if(in_array("reason",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_reason'])&&$_SESSION['tr_reason']==1))){?>
                <td align="center"><div title="<?=$value['trans_response']?>" data-bs-toggle="tooltip" data-bs-placement="top" style="width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                ...
                <td align="center" nowrap><?=$value['acquirer']?></td>
               
                <? }?>
                <? }?>
                <? if(in_array("status",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_status'])&&$_SESSION['tr_status']==1))){?>
               
                <td align="center" nowrap><? if(isset($data['wdata1']) && $data['wdata1']=='5'){?>
                  <span style="color:#00F">Pending</span>
                  <? }else {echo $value['trans_status'];}?>                </td>
              
                <? }?>
                <? if(in_array("merID",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_merID'])&&$_SESSION['tr_merID']==1))){?>
                <td align="center" nowrap><? if($value['merID']<0){?>
                  <?=$value['recvuser']?>
                  <? }else{?>
                  <a <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['merchant_action_view'])&&$_SESSION['merchant_action_view']==1)){?> class="collapsea atablink mprofile text-link" data-mid="<?=$value['merID']?>" data-href="<?=$idx;?>_toggle" data-tabname=""  data-url="<?=$data['Host'];?>/include/riskratioall<?=$data['ex']?>?uid=<?=$value['merID']?>"   data-mhref="clients<?=$data['ex']?>?<? if(isset($post['bid']) && $post['bid']){?>bid=<?=$post['bid']?>&<? } ?>id=<?=$value['merID']?>&action=detail" <? }?> data-bs-toggle="tooltip" data-bs-placement="top" title="View Merchant Details" >
                  <?=$value['recvuser']?>
                  </a>
                  <? }?>                </td>
                <? }?>
                <? if(in_array("upa",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_upa'])&&$_SESSION['tr_upa']==1))){?>
                <td align="center" nowrap title='UPA: <?=$value['upa']?>' data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['upa']?></td>
                <? }?>
                <? if(in_array("ccno",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_ccno'])&&$_SESSION['tr_ccno']==1))){?>
                <td align="center" nowrap title="Card Number: <?=bclf($value['ccno'],$value['bin_no']);?>" data-bs-toggle="tooltip" data-bs-placement="top"><?=bclf($value['ccno'],$value['bin_no']);?></td>
                <? }?>
                <? if(in_array("bill_ip",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_ip'])&&$_SESSION['tr_ip']==1))){?>
                <td align="center" nowrap><a onclick="iframe_open_modal(this);" class="dotdot nomid" data-ihref='<?=$data['Host'];?>/include/ips<?=$data['ex']?>?remote=<?=$value['bill_ip']?>' title='IP: <?=prntext($value['bill_ip']);?>' data-bs-toggle="tooltip" data-bs-placement="top">
                  <?=prntext(lf($value['bill_ip'],15,1));?>
                  </a></td>
                <? }?>
                <? if(in_array("terNO",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_terNO'])&&$_SESSION['tr_terNO']==1))){?>
                <td align="center" nowrap><?=$value['terNO']?></td>
                <? }?>
                <? if(in_array("acquirer_ref",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_txn_id'])&&$_SESSION['tr_txn_id']==1))){?>
                <td align="center" nowrap title="RefNo ID of Bank Response" ><?=$value['acquirer_ref']?></td>
                <? }?>
                <? if(in_array("acquirer_response",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_acquirer_response'])&&$_SESSION['tr_acquirer_response']==1))){?>
                <td nowrap align="center" title="<?=encryptres($value['acquirer_response']);?>" data-label="Bank Response" data-bs-toggle="tooltip" data-bs-placement="top"><a  class="dotdot nomid text-link modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['acquirer_response']);?>' >
                  <?=prntext(lf($value['acquirer_response'],15,1));?>
                  </a></td>
                <? }?>
                <? if(in_array("json_value",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_json_value'])&&$_SESSION['tr_json_value']==1))){?>
                <td nowrap align="center" data-label="Post Josn" data-bs-toggle="tooltip" data-bs-placement="top"><a class="dotdot nomid text-link modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['json_value']);?>' >
                  <?=prntext(lf($value['json_value'],15,1));?>
                </a></td>
                <? }?>
                <? if(in_array("mer_note",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_remark'])&&$_SESSION['tr_remark']==1))){?>
                <td align="center" nowrap title='<?=prntext($value['mer_note']);?>'><a onclick="iframe_open_modal(this);" class="dotdot nomid text-link" data-ihref='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['mer_note']);?>' style="height:20px;" >
                  <?=prntext($value['mer_note']);?>
                  </a></td>
                <? }?>
                <? if(in_array("support_note",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_support_note'])&&$_SESSION['tr_support_note']==1))){?>
                <td align="center" nowrap><a onclick="iframe_open_modal(this);" class="dotdot nomid text-link" data-ihref='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?strip=skip&json=<?=encryptres($value['support_note']);?>' style="height:20px;" >Note Support</a></td>
                <? }?>
                <? if(in_array("system_note",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_system_note'])&&$_SESSION['tr_system_note']==1))){?>
                <td align="center" nowrap><a onclick="iframe_open_modal(this);" class="dotdot nomid" data-ihref='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?strip=skip&json=<?=encryptres($value['system_note']);?>' style="height:20px;overflow:hidden;" >Sys.Note</a></td>
                <? }?>
                <? if(in_array("source_url",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_source_url'])&&$_SESSION['tr_source_url']==1))){?>
                <td align="center" nowrap title="Source Url: <?=prntext($value['source_url']);?>" data-label="Source Url" data-bs-toggle="tooltip" data-bs-placement="top"><a class="dotdot nomid text-link modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?strip=skip&json=<?=encryptres($value['source_url']);?>' >
                  <?=prntext(lf($value['source_url'],15,1));?>
                  </a></td>
                <? }?>
                <? if(in_array("webhook_url",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_notify_url'])&&$_SESSION['tr_notify_url']==1))){?>
                <td align="center" nowrap title="Webhook Url: <?=prntext($value['webhook_url']);?>" data-label="Webhook Url"  data-bs-toggle="tooltip" data-bs-placement="top"><a class="dotdot nomid text-link modal_for_iframe" ihref='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?strip=skip&json=<?=encryptres($value['webhook_url']);?>' style="height:20px;overflow:hidden;" >
                  <?=prntext(lf($value['webhook_url'],15,1));?>
                  </a></td>
                <? }?>
                <? if(in_array("return_url",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_success_url'])&&$_SESSION['tr_success_url']==1))){?>
                <td align="center" nowrap title="Return Url: <?=prntext($value['return_url']);?>" data-label="Return Url"  data-bs-toggle="tooltip" data-bs-placement="top"><a class="dotdot nomid text-link modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?strip=skip&json=<?=encryptres($value['return_url']);?>' style="height:20px;overflow:hidden;" >
                  <?=prntext(lf($value['return_url'],15,1));?>
                  </a></td>
                <? }?>
                
				
                <? if(in_array("buy_mdr_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_amt'])&&$_SESSION['tr_mdr_amt']==1))){?>
                <td align="center" nowrap title="<?=$value['buy_mdr_amt']?>" data-bs-toggle="tooltip" data-bs-placement="top" ><?=$value['buy_mdr_amt'];?></td>
                <? }?>
                <? if(in_array("gst_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_gst_fee'])&&$_SESSION['tr_gst_fee']==1))&&($data['con_name']=='clk')){?>
                <td align="center" nowrap title="GST Fee : <?=(isset($value['gst_amt'])&&trim($value['gst_amt'])?$value['gst_amt']:'');?>" data-label="GST Fee" data-bs-toggle="tooltip" data-bs-placement="top"><?=(isset($value['gst_amt'])&&trim($value['gst_amt'])?$value['gst_amt']:'');?></td>
                <? }?>
				<? if(in_array("rrn",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_rrn'])&&$_SESSION['tr_rrn']==1))){?>
                <td align="center" nowrap title="RRN : <?=$value['rrn'];?>" data-label="RRN" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['rrn'];?></td>
                <? }?>
                <? if(in_array("buy_txnfee_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_txtfee_amt'])&&$_SESSION['tr_mdr_txtfee_amt']==1))){?>
                <td align="center" nowrap title="Transaction Fee : <?=remove_string($value['buy_txnfee_amt']);?>" data-label="Transaction Fee " data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['buy_txnfee_amt'];?></td>
                <? }?>
                <? if(in_array("rolling_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_rolling_amt'])&&$_SESSION['tr_rolling_amt']==1))){?>
                <td align="center" nowrap title="Rolling Fee : <?=$value['rolling_amt'];?>" data-label="Rolling Fee" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['rolling_amt'];?></td>
                <? }?>
                <? if(in_array("mdr_cb_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_cb_amt'])&&$_SESSION['tr_mdr_cb_amt']==1))){?>
                <td align="center" nowrap title="Fee : <?=$value['mdr_cb_amt'];?>" data-label="Fee" data-bs-toggle="tooltip" data-bs-placement="top" ><?=$value['mdr_cb_amt'];?></td>
                <? }?>
                <? if(in_array("mdr_cbk1_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_cbk1_amt'])&&$_SESSION['tr_mdr_cbk1_amt']==1))){?>
                <td align="center" nowrap title="Predispute Fee : <?=$value['mdr_cbk1_amt'];?>" data-label="Predispute Fee"  data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['mdr_cbk1_amt'];?></td>
                <? }?>
                <? if(in_array("mdr_refundfee_amt",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_mdr_refundfee_amt'])&&$_SESSION['tr_mdr_refundfee_amt']==1))){?>
                <td align="center" nowrap title="Refund Fee : <?=$value['mdr_refundfee_amt'];?>" data-label="Refund Fee" data-bs-toggle="tooltip" data-bs-placement="top" ><?=$value['mdr_refundfee_amt'];?></td>
                <? }?>
                <? if(in_array("payable_amt_of_txn",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_payable_amt_of_txn'])&&$_SESSION['tr_payable_amt_of_txn']==1))){?>
                <td align="center" nowrap title="Payout Amount : <?=$value['payable_amt_of_txn'];?>" data-label="Payout Amount" data-bs-toggle="tooltip" data-bs-placement="top" ><?=$value['payable_amt_of_txn'];?></td>
                <? }?>
                <? if(in_array("settelement_date",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_payout_date'])&&$_SESSION['tr_payout_date']==1))){?>
                <td align="center" nowrap title="Payout Date : <?=prndate($value['settelement_date']);?>" data-label="Payout Date" data-bs-toggle="tooltip" data-bs-placement="top"><?=prndate($value['settelement_date']);?></td>
                <? }?>
                <? if(in_array("created_date",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_created_date'])&&$_SESSION['tr_created_date']==1))){?>
                <td align="center" nowrap title="Created Date : <?=prndate($value['created_date']);?>" data-label="Created Date" data-bs-toggle="tooltip" data-bs-placement="top"  ><?=prndate($value['created_date']);?></td>
                <? }?>
                <? if(in_array("risk_ratio",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_risk_ratio'])&&$_SESSION['tr_risk_ratio']==1))){?>
                <td align="center" nowrap title="Risk Ratio : <?=$value['risk_ratio'];?>" data-label="Risk Ratio" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['risk_ratio'];?></td>
                <? }?>
                <? if(in_array("transaction_period",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_transaction_period'])&&$_SESSION['tr_transaction_period']==1))){?>
                <td align="center" nowrap title="Transaction Period : <?=$value['transaction_period'];?>" data-label="Transaction Period" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['transaction_period'];?></td>
                <? }?>
                <? if(in_array("bank_processing_amount",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bank_processing_amount'])&&$_SESSION['tr_bank_processing_amount']==1))){?>
                <td align="center" nowrap title="Bank Processing Amount : <?=$value['bank_processing_amount'];?>" data-label="Bank Processing Amount"  data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['bank_processing_amount'];?></td>
                <? }?>
                <? if(in_array("bank_processing_curr",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bank_processing_curr'])&&$_SESSION['tr_bank_processing_curr']==1))){?>
                <td align="center" nowrap title="Bank Processing Currency : <?=$value['bank_processing_curr'];?>" data-label="Bank Processing Curr" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['bank_processing_curr'];?></td>
                <? }?>
                <? if(in_array("bill_currency",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_currname'])&&$_SESSION['tr_currname']==1))){?>
                <td align="center" nowrap title="Order Currency : <?=$value['bill_currency'];?>" data-label="Order Currency" data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['bill_currency'];?> </td>
                <? }?>
                <? if(in_array("orderset",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_orderset'])&&$_SESSION['tr_orderset']==1))){?>
                <td align="center" nowrap title="Orderset: <?=$value['orderset'];?>" data-label="Orderset" data-bs-toggle="tooltip" data-bs-placement="top"><? if($value['orderset']){ ?>
                  <a class="dotdot nomid modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['orderset']);?>' ><?=prntext(lf($value['orderset'],15,1));?></a>
                  
                  
                  <? } ?></td>
                <? } ?>
                <? if(in_array("trans_type",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_trans_type'])&&$_SESSION['tr_trans_type']==1))){?>
                <td align="center" nowrap title="CN/CH/WD : <?=$value['trans_type'];?>" data-label="CN/CH/WD"  data-bs-toggle="tooltip" data-bs-placement="top"><?=$value['trans_type'];?></td>
                <? }?>
                <? if(in_array("bill_email",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_email'])&&$_SESSION['tr_bill_email']==1))){?>
                <td align="center" nowrap title="Email : <?=prntext($value['bill_email']);?>" data-label="Email" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_email']);?></td>
                <? }?>
                <? if(in_array("bill_phone",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_bill_phone'])&&$_SESSION['tr_bill_phone']==1))){?>
                <td align="center" nowrap title="Phone No. : <?=prntext($value['bill_phone']);?>" data-label="Phone No."  data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_phone']);?></td>
                <? }?>
                <? if(in_array("product_name",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_product_name'])&&$_SESSION['tr_product_name']==1))){?>
                <td align="center" nowrap title="Product Name: <?=prntext($value['product_name']);?>" data-label="Product Name" data-bs-toggle="tooltip" data-bs-placement="top"><a class="dotdot nomid modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['product_name']);?>' ><?=prntext(lf($value['product_name'],15,1));?></a>
                  </td>
                <? }?>
                <? if(in_array("bill_address",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_address'])&&$_SESSION['tr_address']==1))){?>
                <td align="center" nowrap title="Address: <?=prntext($value['bill_address']);?>" data-label="Address"><a class="dotdot nomid modal_for_iframe" href='<?=$data['Admins'];?>/json_pretty_print<?=$data['ex']?>?json=<?=encryptres($value['bill_address']);?>' data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext(lf($value['bill_address'],15,1));?></a></td>
                <? }?>
                <? if(in_array("bill_city",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_city'])&&$_SESSION['tr_city']==1))){?>
                <td align="center" nowrap title="City : <?=prntext($value['bill_city']);?>" data-label="City" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_city']);?></td>
                <? }?>
                <? if(in_array("bill_state",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_state'])&&$_SESSION['tr_state']==1))){?>
                <td align="center" nowrap title="State : <?=prntext($value['bill_state']);?>" data-label="State" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_state']);?></td>
                <? }?>
                <? if(in_array("country",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_country'])&&$_SESSION['tr_country']==1))){?>
                <td align="center" nowrap title="Bill Country : <?=prntext($value['bill_country']);?>" data-label="Country" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_country']);?></td>
                <? }?>
                <? if(in_array("bill_zip",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_zip'])&&$_SESSION['tr_zip']==1))){?>
                <td align="center" nowrap title="Zip Code : <?=prntext($value['bill_zip']);?>" data-label="Zip Code" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['bill_zip']);?></td>
                <? }?>
                <? if(in_array("descriptor",$_SESSION['transaction_display_arr'])&&((isset($_SESSION['login_adm']))||(isset($_SESSION['tr_descriptor'])&&$_SESSION['tr_descriptor']==1))){?>
                <td align="center" nowrap title="Descriptor : <?=prntext($value['descriptor']);?>" data-label="Descriptor" data-bs-toggle="tooltip" data-bs-placement="top"><?=prntext($value['descriptor']);?></td>
                <? }?>
              </tr>
...
?>
          ...
          <table class="frame" width="400" border="0" cellspacing="1" cellpadding="2">
            ...
            <tr>
              <td>Comments:</td>
              <td><?=$post['TransactionDetails']['tech_comments']?></td>
            </tr>
            <tr>
              <td>Details:</td>
