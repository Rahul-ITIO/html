...
<?
	...
	$data['payout_types']=[1=>'Payout',2=>'NodalBank Payout',3=>'3D Card Payment',4=>'Wallets Payment',5=>'Click Zep Project',6=>'UPI Payment',7=>'Net Banking Payment',8=>'Nodal Account Payment',9=>'Coins Payment',9=>'Network Payment',99=>'Other Payment'];
      ...
      <table class="table table-hover">
        <thead>
          <tr>
			...
			<th scope="col" class="hide-768">Bank Test URL</th>
          ...
          <td data-label="Action - " class="text-wrap" ><? if(isset($_SESSION['login_adm'])){ ?>
            <div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action"><i class="<?=$data['fwicon']['action'];?> text-link"></i> </a>
                <ul class="dropdown-menu dropdown-menu-icon pull-right" >
                  <li> <a class="dropdown-item" href="<?=$data['Admins'];?>/bank_payout<?=$data['ex']?>?id=<?=$value['id']?>&action=update" title="Edit" ><i class="<?=$data['fwicon']['edit'];?> text-success fa-fw float-start"></i> <span class="action_menu">Edit</span></a></li>
				  
                  <li> <a class="dropdown-item" href="<?=$data['Admins'];?>/bank_payout<?=$data['ex']?>?id=<?=$value['id']?>&action=duplicate" title="Duplicate" onclick="return confirm('Are you Sure to Create Duplicate');"><i class="<?=$data['fwicon']['copy'];?>  fa-fw float-start"></i> <span class="action_menu">Create Duplicate</span></a></li>

                  <li> <a class="dropdown-item"  href="<?=$data['Admins'];?>/bank_payout<?=$data['ex']?>?id=<?=$value['id']?>&action=delete" onclick="return confirm('Are you Sure to Delete');" title="Delete"><i class="<?=$data['fwicon']['delete'];?> text-danger  fa-fw float-start"></i> <span class="action_menu">Delete</span></a></li>
...
placeholder='value1,value2,value3' ><? if(isset($post['acq']['br_success'])) echo $post['acq']['br_success'];?>
      ...
      <div class="row border bd-blue-100 my-2 text-start py-2 vkgclr rounded">
        <div class="btn btn-outline-light text-dark fw-bold mb-2">Bank Transaction</div>
        <div class="col-sm-6">
...
<? } ?>
    ...
    <div class="my-2 text-center row p-0">
      <div class="col-sm-12 my-2 remove-link-css">
        ...
        <a href="<?=$data['Admins']?>/bank_payout<?=$data['ex']?>" class="btn btn-icon btn-primary"><i class="<?=$data['fwicon']['back'];?>"></i> Back</a> </div>
    </div>
    <? }?>
  </form>
</div>
