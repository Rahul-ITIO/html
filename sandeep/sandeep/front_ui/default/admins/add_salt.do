...
if(!empty($post["tid"])) {
  $rs = db_rows("SELECT * FROM zt_acquirer_table WHERE `acquirer_id`='{$post["tid"]}' ORDER BY id DESC LIMIT 1",0);
  //echo $id=$rs[0]['id'];
  $rs = $rs[0];
  
  $acquirer_json=$rs['acquirer_json'];
  $jssiteidon=$rs['siteid'];
 
  //$json_value=json_decode($bank_json,true);
  
  $ac=jsondecode($rs['acquirer_json'],true);
  $bank_salt=$ac['bank_salt']; // 1
  $hkip_siteid=$ac['hkip_siteid']; // 2
  $bank_json=$rs['bank_json']; // 3
  ...
  $hash_code=$rs['hash_code']; // 4.4
	...
	if($bank_salt<>''){
		...
		$json_value=createJsonf('hkip_siteid',$hkip_siteid);
	}elseif($bank_json<>''){
		$json_value=createJsonf('bank_json',$bank_json);
	}else{
		if($bank_merchant_id<>''){
	...
	function isJson($string) {
	 ...
	 // echo $getgid;
	  ...
	  $row = db_rows("SELECT * FROM zt_salt_management WHERE id='" . $getgid . "'");
	...
	$json_array_key=array();$json_arr=array();
	$json_de=$json_value;
	$json_arr['saltJson']=$json_de;$json_array_key=array_merge(['saltJson'],(array_keys($json_arr['saltJson'])),['saltJson']);
	$de_josn=is_multi_arrayf($json_arr, $json_array_key);
...
<script>
	...
	$( document ).ready(function() {
		...
		<?if($data['t'][$post["tid"]]['name1']){?>
		 $('fieldset').find('legend:contains("1. saltJson")', this).html("<?=$post['tid']?> <?=$data['t'][$post["tid"]]['name1']?>");
		 $('fieldset').find('legend:contains("1. Salt Json")', this).html("<?=$post['tid']?> <?=$data['t'][$post["tid"]]['name1']?>");
		//$('fieldset > legend:first-child').html("<?=$data['t'][$post["tid"]]['name1']?>");
		<?}?>
