...
if($data['frontUiName']=="default"){ //default_UX

if((isset($post['fname'])&&$post['fname']) && (isset($post['designation'])&&$post['designation']) && (isset($post['phone'])&&$post['phone']) &&(isset($post['company_name'])&&$post['company_name']) && (isset($post['business_contact'])&&$post['business_contact']) && (isset($post['business_address'])&&$post['business_address'])){
}else{
...
if((in_array('withdraw',$data['gmfa']))&&($post['google_auth_access']==1||$post['google_auth_access']==3)&&($post['google_auth_code'])){
...
if(isset($front_ui_Left_Panel) && $front_ui_Left_Panel==0){
	if(strpos($_SESSION['themeName'],'LeftPanel')!==false){
		...
		$trans_details=create_graphf($post_form);
			...
			*/
			...
			if(!isset($data['PRO_VER']))
			{
				$last_ticketid=db_rows(
					...
					" WHERE `clientid`={$uid} AND `status`=91 ".
...
$post['products']=select_products($uid,0,0,0,1);
	$data['store_size']=0;
	if(isset($post['products']) && $post['products']){
		foreach($post['products'] as $key=>$value){
			if(strpos($value['notification_to'],'007')!==false){
				$data['store_size']++;
			}
		}
	}
...
if(isset($front_ui_Left_Panel) && $front_ui_Left_Panel==1){
	...
	if(!isset($_SESSION['tr_sum_count']))
	{
		...
		$noOfTransaction=db_rows(
			"SELECT SUM(`transaction_amt`) AS `summ`, COUNT(`id`) AS `count` FROM `{$data['DbPrefix']}transactions`".
			" WHERE  ( `owner_id`={$uid} )  AND ( `status` IN (1,7) ) AND ( `trname` IN ('cn','ch') )  {$where_pred}  LIMIT 1 ",0
		...
		$noOfTotalTransactions=db_rows(
			"SELECT COUNT(`id`) AS `count` FROM `{$data['DbPrefix']}transactions`".
			" WHERE  ( `owner_id`={$uid} )  AND ( `status` IN (1,2,7) ) AND ( `trname` IN ('cn','ch') )  {$where_pred}  LIMIT 1 ",0
		);
		...
		$refundAmount=db_rows(
			"SELECT SUM(`transaction_amt`) AS `summ`, COUNT(`id`) AS `count` FROM `{$data['DbPrefix']}transactions`".
			" WHERE  ( `owner_id`='{$uid}' )  AND ( `status` IN (3) )  AND ( `trname` IN ('cn','ch') ) LIMIT 1 ",0
		);
		$_SESSION['tr_sum_count']['refundAmount']=$post['refundAmount']=number_formatf($refundAmount[0]['summ']);

		$settlements=db_rows(
			"SELECT SUM(`transaction_amt`) AS `summ`, COUNT(`id`) AS `count` FROM `{$data['DbPrefix']}transactions`".
			" WHERE  ( `sender`={$uid} )  AND ( `status` IN (1,13) )  AND ( `type` IN (2) ) LIMIT 1 ",0
		);
		$_SESSION['tr_sum_count']['settlements']=$post['settlements']=number_formatf($settlements[0]['summ']);

		
		$_SESSION['mNotificationCount']=$trans_reply_counts=trans_reply_counts($uid,'1,2');
...
/*if((isset($_REQUEST['act'])&&$_REQUEST['act']) && ($_REQUEST['act']=='2' || $_REQUEST['act']=='3' || $_REQUEST['act']=='1')){
$today=date("Y-m-d");
$before7days=date('Y-m-d', strtotime($today. '-7 day'));
$before30days=date('Y-m-d', strtotime($today. '-30 day'));
$before365days=date('Y-m-d', strtotime($today. '-365 day'));

    if($_REQUEST['act']=='1'){
		$qrs1=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before7days'  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today'  AND (`status` IN (2)) LIMIT 1  ";
		$_SESSION['failed_trans_new']=get_transaction_count_where_pred($qrs1);

	    $qrs2=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before7days'  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today' AND (`status` IN (1,7)) LIMIT 1 ";
	    $_SESSION['success_trans_new']=get_transaction_count_where_pred($qrs2);  
	    $_SESSION['total_trans_new']=$success_trans_new + $failed_trans_new;
	}

	if($_REQUEST['act']=='2'){
		$qrs1=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND ( `trname` IN ('cn','ch') )  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before30days'        AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today'   AND (`status` IN (2)) LIMIT 1 ";
		$_SESSION['failed_trans_new']=get_transaction_count_where_pred($qrs1);

	    $qrs2=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before30days'  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today'  AND (`status` IN (1,7)) LIMIT 1 ";
	    $_SESSION['success_trans_new']=get_transaction_count_where_pred($qrs2);  
	    $_SESSION['total_trans_new']=$success_trans_new + $failed_trans_new;
	}
	
		if($_REQUEST['act']=='3'){
		$qrs1=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before365days'  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today' AND (`status` IN (2)) LIMIT 1 ";
		$_SESSION['failed_trans_new']=get_transaction_count_where_pred($qrs1);

	    $qrs2=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND DATE_FORMAT(`tdate`,'%Y-%m-%d') >=  '$before365days'  AND DATE_FORMAT(`tdate`,'%Y-%m-%d') <=  '$today' AND (`status` IN (1,7)) LIMIT 1";
	    $_SESSION['success_trans_new']=get_transaction_count_where_pred($qrs2);  
	    $_SESSION['total_trans_new']=$success_trans_new + $failed_trans_new;
	}

   if($_REQUEST['act']=='4'){
		$qrs1=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND (`status` IN (2))  LIMIT 1";
		$_SESSION['failed_trans_new']=get_transaction_count_where_pred($qrs1);

	    $qrs2=" `owner_id` IN ('$uid') AND ( `trname` IN ('cn','ch') ) AND (`status` IN (1,7)) LIMIT 1";
	    $_SESSION['success_trans_new']=get_transaction_count_where_pred($qrs2);  
	    $_SESSION['total_trans_new']=$success_trans_new + $failed_trans_new;
	}
}*/	

if(($data['frontUiName']=="quartz") || ($data['frontUiName']=="opal" ) || ($data['frontUiName']=="OPAL_IND" )){
