...
<div class="container border border-light border-3 px-0">
  <div class="row my-2 me-1 border border-primary rounded border-1">
    <div class="col-sm-6 my-2">
      <p class="ms-2 text-white"><strong>Copy This Code And Paste Into Your Page:</strong></p>
    </div>
    <div class="col-sm-6 text-end"><? if($_POST['store_id']==""){ ?> <a href="<?=$data['USER_FOLDER'];?>/generate<?=$data['ex']?>?action=store<? if(isset($get_q)) echo $get_q;?><?=(isset($_GET['hide_comment']))?"":"&hide_comment=1";?>&id=<?=$_GET['id'];?>" class="nopopup btn btn-primary btn-sm my-2">
      <?=(isset($_GET['hide_comment']))?"<i class='".$data['fwicon']['eye-solid']."'></i> Show Comment":"<i class='".$data['fwicon']['eye-slash']."'></i> Remove Comment";?>
      </a> <? } ?><a href="<?=$data['Host'];?>/<?=($developers);?><?=$data['ex']?>" target="_blank" class="nopopup btn btn-primary btn-sm  my-2 mx-2"><i class="<?=$data['fwicon']['circle-plus'];?>"></i> Developer Api</a></div>
  </div>
  <table class="table">
    <tr>
      <td class="capl">
    ...
    <tr>
      <td class="text-center text-white mt-2 mb-2"><strong>CODE #1 - Using POST method:</strong> <a class="nopopup btn btn-sm my-2"  onclick="CopyToClipboard2('#postMethod')"><i class="<?=$data['fwicon']['copy'];?> text-white" title="Copy code"></i></a>
        <textarea id="postMethod" class="textAreaAdjust form-control nst bg-primary text-white" onkeyup="textAreaAdjust(this)" rows="10"  readonly>&lt;?php
...
$(document).ready(function(){
    ...
    $('#paymentForm').click(function(){
	   ...
	   $('#paymentFormTd input[name="id_order"]').val('Order Id');
    });
