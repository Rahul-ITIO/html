...
//echo $domain_server['members']['payout_request'];
//echo $domain_server['members']['qrcode_gateway_request'];
...
//for permission
$str1 = trim(((isset($_SESSION['m_member_role'])&&$_SESSION['m_member_role'])?$_SESSION['m_member_role']:''));
$grole=explode(',',$str1);
...
include($data['Path'].'/include/header_color_new'.$data['iex']); 
...
include($data['Path'].'/include/color_font_adjustment_new'.$data['iex']);
...
$_SESSION['m_member_type']=((isset($_SESSION['m_member_type'])&&$_SESSION['m_member_type'])?$_SESSION['m_member_type']:'');
$_SESSION['m_member_role']=((isset($_SESSION['m_member_role'])&&$_SESSION['m_member_role'])?$_SESSION['m_member_role']:'');
?>
...
<a type="button" data-bs-toggle="dropdown" aria-expanded="false">
    <vgg class="theme-icon-active"><use class="fa-regular fa-sun" href="fa-regular fa-sun text-white"></use></vgg>
  </a>
  <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="bd-theme" style="--bs-dropdown-min-width: 8rem;">
              <li>
                <button type="button" class="dropdown-item d-flex align-items-center change_color active" data-bs-theme-value="light">
                  <vgg class="bi me-2 opacity-50 theme-icon"><use class="fa-regular fa-sun" href="fa-regular fa-sun text-white"></use></vgg>
                  Light
                </button>
              </li>
              <li>
                <button type="button" class="dropdown-item d-flex align-items-center change_color" data-bs-theme-value="dark">
                  <vgg class="bi me-2 opacity-50 theme-icon"><use class="fa-solid fa-moon" href="fa-solid fa-moon text-white"></use></vgg>
                  Dark
                </button>
              </li>
              <li>
                <button type="button" class="dropdown-item d-flex align-items-center change_color" data-bs-theme-value="auto">
                  <vgg class="bi me-2 opacity-50 theme-icon"><use class="fa-solid fa-circle-half-stroke" href="fa-solid fa-circle-half-stroke text-white"></use></vgg>
                  Auto
                </button>
              </li>
            </ul>
...
<div class="switcher-box">
  <div class="hiddenbox light-backg border" title="Adjust Font Sizing" data-bs-toggle="tooltip" data-bs-placement="left"><i class="<?=$data['fwicon']['a'];?>"></i></div>
  <div class="colorbox-container vkg" style="display: none;">
    
	
	<div class="head"><button aria-label="Increase Font Size ++" id="font-plus2" data-code="plus2" class="btn btn-primary btn-sm change_font_size" title="Increase Font Size ++"><i class="<?=$data['fwicon']['a'];?>" style="font-size:20px;"></i></button></div>
	
    <div class="head"><button aria-label="Increase Font Size +" id="font-plus" data-code="plus" class="btn btn-primary btn-sm change_font_size" title="Increase Font Size +"><i class="<?=$data['fwicon']['a'];?>"  style="font-size:16px;"></i></button></div>
	
	<div class="head"><button aria-label="Default Font Size" id="font-minus" data-code="minus" class="btn btn-primary btn-sm change_font_size font_active" title="Default Font Size"><i class="<?=$data['fwicon']['a'];?>" style="font-size:12px;"></i></button></div>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light5 bg-primary text-white">
  <div class="container">
    ...
    <a href="<?=$data['Members']?>/index<?=$data['ex']?>" class="img-fluid"><img src="<?=encode_imgf($domain_server['LOGO']);?>" style="height:30px;padding-right: 40px;"></a>
    <? } ?>
   ...
   ?>
        ...
        <?
			...
			?>
        <? }else{ ?>
        <li class="nav-item"> <a class="nav-link text-white active menu-summary" aria-current="page" href="<?=$data['Members']?>/dashboard<?=$data['ex']?>">Dashboard</a> </li>
        <? if(((in_array(7, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li class="nav-item"> <a class="nav-link text-white menu-summary" href="<?=$data['Members']?>/bank<?=$data['ex']?>">Account</a> </li>
        <? } ?>
        <? if(((in_array(8, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li class="nav-item"> <a class="nav-link text-white menu-summary" href="<?=$data['Members']?>/website<?=$data['ex']?>">Website</a> </li>
        <? } ?>
        <li class="nav-item dropdown"> <a class="nav-link text-white dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false"> Transactions </a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
            <? if(((in_array(3, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/transactions<?=$data['ex']?>">All Transaction</a></li>
            <? } ?>
            <? if(((in_array(4, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/block_transactions<?=$data['ex']?>">Block Transaction</a></li>
            <? } ?>
            <? if(((in_array(15, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/test_transactions<?=$data['ex']?>">Test Transaction</a></li>
            <? } ?>
            <? if(((in_array(10, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/recent_order<?=$data['ex']?>">Recent Order</a></li>
            <? } ?>
            <li>
              <hr class="dropdown-divider">
            </li>
            <? if(((in_array(14, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/statement<?=$data['ex']?>">Statement</a></li>
            <? } ?>
            <? if(((in_array(16, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
            <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/withdraw-fund-list<?=$data['ex']?>">Withdraw Fund</a></li>
            <? } ?>
          </ul>
        </li>
        <? } ?>
...
?>
      ...
      <div class="dropdown"> <a class="btn btn-primary dropdown-toggle55 border  header_menu_link"  data-bs-toggle="dropdown"> <i class="<?=$data['fwicon']['circle-user'];?>" aria-hidden="true" title="Welcome <?=ucwords(strtolower(prntext($username)));?>" ></i><span class="text-hide-mobile"> Welcome
        ...
        <?php 
		if((isset($_SESSION['m_member_name'])&&$_SESSION['m_member_name'])?$_SESSION['m_member_name']:''){
		echo "(".$_SESSION['m_member_name'].")";
		}
		
		?>
        </span> </a>
        <ul class="dropdown-display dropdown-menu" id="VDisplayleft" style="width: 190px;">
		...
		<li class="d-block d-sm-none"><hr class="dropdown-divider"></li>
          ...
          <? if($_SESSION['dashboard_type']!="payout-dashboar"){ ?>
          <? if(((in_array(9, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/profile<?=$data['ex']?>" ><i class="<?=$data['fwicon']['profile'];?> fa-fw"></i> Profile</a> </li>
          <? } ?>
          <? if(((in_array(1, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/emails<?=$data['ex']?>"><i class="<?=$data['fwicon']['email'];?> fa-fw"></i> Emails</a> </li>
          <? } ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/summary-account<?=$data['ex']?>"><i class="<?=$data['fwicon']['account-summary'];?> fa-fw"></i> Account Summary</a> </li>
          <? if(((in_array(2, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/account-security<?=$data['ex']?>"><i class="<?=$data['fwicon']['shield'];?> fa-fw"></i> Account Security</a> </li>
          <? } ?>
          <?php if($data['themeName']!='LeftPanel'){ ?>
          <? if(((in_array(5, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/manage-user<?=$data['ex']?>"><i class="<?=$data['fwicon']['manage-user'];?> fa-fw"></i> User Management</a> </li>
          <? } ?>
          <? if(((in_array(17, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/stats<?=$data['ex']?>"><i class="<?=$data['fwicon']['chart'];?> fa-fw"></i> Success Ratio</a> </li>
          <? } ?>
          <? } ?>
          <li> <a class="dropdown-item menu-summary" href="<?=$data['Members']?>/myblacklist<?=$data['ex']?>"><i class="<?=$data['fwicon']['ban'];?> fa-fw"></i> Black List</a> </li>
          <li>
            <hr class="dropdown-divider">
          </li>
          <? } ?>
          <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/logout<?=$data['ex']?>"><i class="<?=$data['fwicon']['signout'];?> text-danger fa-fw"></i> Sign Out</a></li>
        </ul>
      </div>
      <? } else { ?>
      User
...
?>
    ...
    <?
			...
			?>
    <? } else { ?>
    <li><a class="text-white text-decoration-none menu-summary" href="<?=$data['Members']?>/dashboard<?=$data['ex']?>" title="Dashboard"><i class="<?=$data['fwicon']['home'];?> fa-fw"></i> <span class="menu_title">Dashboard</span></a></li>
    <li class="nav-item dropdown"> <a class="nav-link text-white dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" > <i class="<?=$data['fwicon']['transaction'];?> fa-fw"></i> <span class="menu_title">Transactions</span> </a>
      <ul class="dropdown-menu text-white" aria-labelledby="navbarDropdown" style="">
        <? if(((in_array(3, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/transactions<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['transaction'];?> fa-fw"></i>&nbsp;All Transaction</a></li>
        <? } ?>
        <? if(((in_array(4, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/block_transactions<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['blocktransaction'];?> fa-fw"></i>&nbsp;Block Transaction</a></li>
        <? } ?>
        <? if(((in_array(15, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/test_transactions<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['testtransaction'];?> fa-fw"></i>&nbsp;Test Transaction</a></li>
        <? } ?>
        <? if(((in_array(10, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/recent_order<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['recentorder'];?> fa-fw"></i>&nbsp;Recent Order</a></li>
        <? } ?>
        <? if(((in_array(14, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/statement<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['mystatement'];?> fa-fw"></i>&nbsp;My Statement</a></li>
        <? } ?>
        <? if(((in_array(11, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a class="dropdown-item menu-summary" href="<?=$data['Members']?>/requests<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['requestfund'];?> fa-fw"></i>&nbsp;Request Funds
          <?php /*?><?=$post['default_currency_sys'];?><?php */?>
          </a></li>
        <? } ?>
        <? if(((in_array(16, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
        <li><a  class="dropdown-item menu-summary" href="<?=$data['Members']?>/withdraw-fund-list<?=$data['ex']?>">&nbsp;&nbsp;<i class="<?=$data['fwicon']['settlement'];?> fa-fw"></i>&nbsp;Withdrawal Funds
          <?php /*?> <?=$post['default_currency_sys'];?><?php */?>
          </a></li>
        <? } ?>
      </ul>
    ...
    <? if(((in_array(8, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
    <li><a class="text-white text-decoration-none menu-summary" href="<?=$data['Members']?>/website<?=$data['ex']?>"><i class="<?=$data['fwicon']['website'];?> fa-fw"></i><span class="menu_title">&nbsp;My Website</span></a></li>
    <? } ?>
    <? if(((in_array(7, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
    <li><a class="text-white text-decoration-none menu-summary" href="<?=$data['Members']?>/bank<?=$data['ex']?>"><i class="<?=$data['fwicon']['bank'];?> fa-fw"></i><span class="menu_title">&nbsp;My Bank Accounts</span></a></li>
    <? } ?>
    <? if(((in_array(14, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
    <li><a  class="text-white text-decoration-none menu-summary" href="<?=$data['Members']?>/stats<?=$data['ex']?>"><i class="<?=$data['fwicon']['chart'];?> fa-fw"></i><span class="menu_title">&nbsp;Success Ratio</span></a></li>
    <? } ?>
    <? if(((in_array(5, $grole)) && ($_SESSION['m_member_type']=='Sub Member')) || ($_SESSION['m_member_type']=='')){ ?>
    <li><a class="text-white text-decoration-none menu-summary" href="<?=$data['Members']?>/manage-user<?=$data['ex']?>"><i class="<?=$data['fwicon']['manage-user'];?> fa-fw"></i><span class="menu_title">&nbsp;User Management</span></a></li>
    <? } ?>
    <? } ?>
  </ul>
</nav>
...
<div>
  <nav class="navbar navbar-expand-lg navbar-light bg-light5 bg-primary text-white">
    <div class="container">
      <? if(($data['hdr_logo'])&&($domain_server['LOGO'])){ ?>
      <a href="<?=$data['Members']?>/index<?=$data['ex']?>" class="img-fluid"><img src="<?=$domain_server['LOGO'];?>" style="height:30px;padding-right: 10px;"></a>
      <? } ?>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"> <span class="navbar-toggler-icon"></span> </button>
      <div class="collapse navbar-collapse bg-primary text-white" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item"> <a class="nav-link text-white active" aria-current="page" href="<?=$data['Members']?>/dashboard<?=$data['ex']?>">Dashboard</a> </li>
          <? if(isset($_SESSION['login'])&&$_SESSION['login']){ 
        if($_SESSION['m_username']){ $username=$_SESSION['m_username'];
		}elseif($_SESSION['username']){ $username=$_SESSION['username']; }
?>
          <li class="nav-item">
          <li><a class="nav-link text-white" href="<?=$data['Members']?>/logout<?=$data['ex']?>">Sign Out</a></li>
          </li>
          <? } else { ?>
          <li class="nav-item"> <a href="<?=$data['Host']?>/signup<?=$data['ex']?>" class="nav-link text-white"><i></i>Sign Up</a> </li>
          <li class="nav-item"> <a href="<?=$data['Host']?>" class="nav-link text-white"><i></i>Login</a> </li>
        ...
        <div class="dropdown"> <a class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown"> <i class="<?=$data['fwicon']['user'];?>" aria-hidden="true" ></i> Welcome
          <?=ucwords(strtolower(prntext($username)));?>
          <? if($_SESSION['m_member_name']<>""){ echo "(".$_SESSION['m_member_name'].")"; } ?>
          </a>
          <ul class="dropdown-menu">
            <li>
              <hr class="dropdown-divider">
            </li>
            <li><a class="nav-link text-dark" href="<?=$data['Members']?>/logout<?=$data['ex']?>"><i class="<?=$data['fwicon']['signout'];?>"></i> Sign Out</a></li>
          </ul>
        </div>
        <? } else { ?>
...
$header_title_search = array("processall", "test3dsecureauthentication", "success", "failed", "developer", "docs", "transaction_processing", "device_confirmations", "forgot", "login", "logout", "signup", "password","confirm","two-factor-authentication","bank","chart");
$header_title_center = array("login", "forgot", "signup","confirm","password","confirm","confirm","logout","device_confirmations");

$hide_color_box = array("processall","success","failed","transaction_processing", "forgot", "login", "logout", "signup", "password");
...
if((in_array($data['PageFile'], $hide_color_box)) || ($_REQUEST['admin']==1)){
?>
