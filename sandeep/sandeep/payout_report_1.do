...
if(!$_SESSION['adm_login'] && !$_SESSION['login']){
        header("Location:{$data['USER_FOLDER']}/login.htm");
        echo('ACCESS DENIED.');
        exit;
}
...
$newdata['from_date']=$wd_json_value['wd_date_from'];
$newdata['to_date']=$wd_json_value['wd_date_to'];
...
if($post['step']==1){
			...
			//$ratio=riskratio($uid,$nick_name_card); // account_type
			$ratio=get_riskratio_trans($uid);
			$post['total_lead']=$ratio['total_ratio'];
			
			$pdftrans_rtn=db_rows(
				"SELECT * FROM `{$data['DbPrefix']}transactions`".
				" WHERE (`receiver`={$uid} OR `sender`={$uid} ) ".$account_type_whr.
				//" AND ( tdate between '".$newdata['from_date_rtn_db']."'  AND  '".$newdata['to_date_rtn_db']."' ) ".
				" ORDER BY `id` ASC",0
			);
			
			$post['pdftrans_rtn']=$pdftrans_rtn;

			$pdfsettledtrans=db_rows(
				"SELECT * FROM `{$data['DbPrefix']}transactions`".
				" WHERE (`receiver`={$uid} OR `sender`={$uid} ) ".$account_type_whr.
				//" WHERE `receiver`={$uid}".
				//" AND ( tdate between '".$newdata['from_date_db']."' AND '".$newdata['to_date_db']."' ) ".
				" ORDER BY `id` ASC",0
			);
			
			$post['pdfsettledtrans']=$pdfsettledtrans;
			
						
			$pdf_download=false;
			
				
			//$virtual_terminal_fee=$post['AccountInfo'][0]['virtual_fee']; //Virtual Terminal Fee
			//if(empty($virtual_terminal_fee)){$virtual_terminal_fee=25;}
			//$min_settelement_amt=$post['AccountInfo'][0]['settled_amt']; //Min. Settelement Amt
			//if(empty($min_settelement_amt)){$min_settelement_amt=1500;}
			$vt=$post['AccountInfo'][0]['vt']; 
			if(empty($vt)){$vt="2";}
			$charge_back_fee=0;
			if($post['total_lead']>0 && $post['total_lead']<=1){$charge_back_fee=$post['AccountInfo'][0]['charge_back_fee_1'];}
			if($post['total_lead']>1 && $post['total_lead']<=3){$charge_back_fee=$post['AccountInfo'][0]['charge_back_fee_2'];}
			if($post['total_lead']>3 && $post['total_lead']<100){$charge_back_fee=$post['AccountInfo'][0]['charge_back_fee_3'];}
			...
			foreach($post['Banks'] as $key=>$value){
				if(!empty($value['required_currency']) && $value['primary']=="2"){
...
//end: class	
		...
		foreach($post['pdfsettledtrans'] as $key=>$value){
			...
			if($value['status']==1 || $value['status']==4 || $value['status']==7){ // completed
				$dis_rate 	= (double)$value['mdr_amt'];
				$rol_fee	= (double)$value['rolling_amt'];
				$this_total	= (double)$value['payable_amt_of_txn'];//($value['transaction_amt'])-($dis_rate+$accounts[0]['txn_fee']+$rol_fee);
				...
				$sales_volume	= (double)$sales_volume+(double)$value['transaction_amt'];
				
				$completed_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".bcadd(0,$dis_rate,2).";".bcadd(0,$value['mdr_txtfee_amt'],2).";".bcadd(0,$rol_fee,2).";".bcadd(0,$this_total,2)."";
			}
			if($value['status']=="2" || $value['status']==2){ // canceled
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_total	= bcadd(0,$value['mdr_txtfee_amt'],2);
				
				if($value['trname']=="ch"){
					//$total_txn_fee = bcadd(0,$total_txn_fee+$accounts[0]['txn_fee'],2);
					$total_txn_fee = bcadd(0,$total_txn_fee+$value['mdr_txtfee_amt'],2);
					
					$cancelled_data[] .= $value['transaction_id'].";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";".$value['mdr_txtfee_amt'].";".$rol_fee.";-".$this_total."";
				}else{
					
					//$sales_volume	= bcadd(0,$sales_volume+$value['transaction_amt'],2);
				
					$cancelled_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";0;".$rol_fee.";0";
		...
		foreach($post['pdftrans_rtn'] as $key=>$value){
			//$pdf->Cell(0,10,$value['transaction_id']."       ".$value['transaction_amt'],0,1);
			...
			if($value['status']=="11" || $value['status']==11){ // cbk1 
				$cbk1_fee=$value['mdr_cbk1_amt'];//$accounts[0]['cbk1'];
				
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_tr_fee= $cbk1_fee;
				
				$this_total	= bcadd(0,$value['payable_amt_of_txn'],2); // 45
				
				$total_cbk1_fee = bcadd(0,$total_cbk1_fee+$cbk1_fee,2); //
				
				$cbk1_volume	= bcadd(0,$cbk1_volume+$value['transaction_amt'],2);
				
				$cbk1_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";".bcadd(0,$this_tr_fee,2).";".$rol_fee.";".$this_total."";
			}
			
			if($value['status']==3 || $value['status']==12){ // Refunded
			
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_total	= bcadd(0,$value['payable_amt_of_txn'],2);
				
				$total_refund_fee = bcadd(0,$total_refund_fee+$value['mdr_refundfee_amt'],2);
				
				$refund_volume	= bcadd(0,$refund_volume+$value['transaction_amt'],2);
				
				$refunded_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";".$value['mdr_refundfee_amt'].";".$rol_fee.";".$this_total."";
			}
			if($value['status']=="5" || $value['status']==5){ // Chargeback 
			
				$dis_rate 	= "0";
				$rol_fee	= "0";
				$this_tr_fee= $value['mdr_cb_amt'];
				$this_total	= bcadd(0,$value['payable_amt_of_txn'],2); // 45
				
				$total_returns_fee = bcadd(0,$total_returns_fee+$value['mdr_cb_amt'],2); //45  as per risk ratio and account 1 from 3
				
				$returns_volume	= bcadd(0,$returns_volume+$value['transaction_amt'],2);
				
				$returned_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".bcadd(0,$value['transaction_amt'],2).";".$dis_rate.";".bcadd(0,$this_tr_fee,2).";".$rol_fee.";".$this_total."";
			}
...
$header_total 	= array(' ', 'Total', ' ', bcadd(0,$total_dis_rate,2), bcadd(0,$total_txn_fee,2), bcadd(0,$total_rol_fee,2), '');
...
$datas_seles 	= array("Sales Volume;".bcadd(0,$sales_volume,2).";$account_cur");

$datas_over 	= array();
	$datas_over[] 	.= "Discount Rate;".bcadd(0,$total_dis_rate,2).";$account_cur";
	$datas_over[] 	.= "Transaction Fee;".bcadd(0,$total_txn_fee,2).";$account_cur";
	$datas_over[] 	.= "Refund;".bcadd(0,$refund_volume,2).";$account_cur";
	$datas_over[] 	.= "Chargeback;".bcadd(0,$returns_volume,2).";$account_cur";
	
	if($cbk1_volume>0){$datas_over[] 	.= "CBK1;".bcadd(0,$cbk1_volume,2).";$account_cur"; } // by Transaction List
	
	

	$datas_over[] 	.= "Refund Fee;".bcadd(0,$total_refund_fee ,2).";$account_cur";
	//if($total_returns_fee>0){}
	$datas_over[] 	.= "Chargeback Fee;".bcadd(0,$total_returns_fee,2).";$account_cur"; 
	if($total_cbk1_fee>0){$datas_over[] 	.= "CBK1 Fee;".bcadd(0,$total_cbk1_fee ,2).";$account_cur"; } // get by Account ex 45
	//$datas_over[] 	.= "Other Fees;0.00;$account_cur";
	$datas_over[] 	.= "Holdback Reserve;".bcadd(0,$total_rol_fee,2).";$account_cur";
...
$reverse 		= bcadd(0,bcadd(0,$refund_volume,2)+bcadd(0,$returns_volume,2)+bcadd(0,$total_dis_rate,2)+bcadd(0,$total_txn_fee,2)+bcadd(0,$total_refund_fee,2)+bcadd(0,$total_returns_fee,2)+bcadd(0,$total_rol_fee,2)+$mmf+$mvtf,2)+bcadd(0,$total_cbk1_fee,2)+bcadd(0,$cbk1_volume,2);
$sales_volume	= bcadd(0,$sales_volume,2);
$tpa			= $sales_volume-$reverse;

/*
echo "<br/>sales_volume=>".$sales_volume;
echo "<br/>reverse=>".$reverse;
echo "<br/>tpa=>".$tpa;
echo "<br/>min_settelement_amt=>".$min_settelement_amt;
*/
...
$datat = array("Gross Payout Amount ;".bcadd(0,$tpa,2).";$account_cur");
$data = $pdf->LoadData($datat);
$pdf->ImprovedThreeColTable($data,'',0,true);
$data = $pdf->LoadData(array("Settlement Wire Fee;".bcadd(0,$swf,2).";$account_cur"));
$pdf->ImprovedThreeColTable($data,'',0,false,1);
if(!empty($currency_rate) && ($available_payout>0)){
	//$pdf_download=true;
	/*
	$available_payout=bcadd(0,$available_payout,2);
	$currency_conversion=(($available_payout*3)/100);
	$currency_conversion=bcadd(0,$currency_conversion,2);
...
$data = $pdf->LoadData(array("Available Payout ".$nextselt." ;".bcadd(0,$available_payout,2).";$account_cur")); //
$pdf->ImprovedThreeColTable($data,'',0,true);
if(!empty($currency_rate) && ($available_payout>0)){
	...
	$available_payout_exchange=bcadd(0,$available_payout_exchange,2);
...
$html = '<p>Error of the system generated statement should be reported by the merchant asap. MSP hold the right to adjust the difference manually.</p>';
		...
		//$pdf->Image('logo.png',10,50,30,0,'',$data['Host']);
...
// end of 1st page
	...
	$pdf->ln(-8);
	$pdf->Cell(40,10,'Over All Summary');

		$data_c = $pdf->LoadData($completed_data);
		if(!empty($data_c)){
			$pdf->ImprovedTable($header,$data_c,'COMPLETE TRANSACTION',10);
		}
		...
		if(!empty($datarf)){
			$pdf->ImprovedTable($header,$datarf,'REFUNDED TRANSACTION ('.date('F d,Y',strtotime($newdata['from_date_rtn'])).' to '.date('F d,Y',strtotime($newdata['to_date_rtn'])).')',10);
		}
		$datar = $pdf->LoadData($returned_data);
		if(!empty($datar)){
			$pdf->ImprovedTable($header,$datar,'CHARGEBACK TRANS. ('.date('F d,Y',strtotime($newdata['from_date_rtn'])).' to '.date('F d,Y',strtotime($newdata['to_date_rtn'])).')',10);
		}
