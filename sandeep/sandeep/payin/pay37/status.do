<?php 
	...
	$apiUrl			= $bank_status_url;
	$certFile		= $data['Path'].'/payin/pay'.$siteid_get['default_mid'].'/'.$siteid_get['mode'].'_indusapi.indusind.com-sscert.pem';
	$pgMerchantId	= $siteid_get['pgMerchantId'];
	...
	$Payload = [
		...
		'pageNumber' => "",
		'custRefNo' => "$txn_id",
		'npciTranId' => "$transaction_id",
		'pspRefNo' => "$transaction_id",
	];
