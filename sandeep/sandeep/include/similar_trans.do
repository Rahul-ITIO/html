...
if(isset($post['send']) && $post['send'] && $is_admin){
		...
		if(!isset($post['merID'])){
			...
			if(isset($data['MASTER_TRANS_ADDITIONAL'])&&$data['MASTER_TRANS_ADDITIONAL']=='Y'){
				...
				if(isset($data['MASTER_TRANS_ADDITIONAL'])&&$data['MASTER_TRANS_ADDITIONAL']=='Y'){
					...
					"('{$transactions_newid}','{$transID}' ".$additional_data.")",0);
	...
	if(isset($data['qp'])&&!empty($data['qp'])){
		...
		@db_query(
                "UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`".
                " SET `trans_status`='{$post['trans_status']}',`tdate`='{$tdate}',`transID`='{$post['transID']}',`merID`='{$post['merID']}',`bill_amt`='{$post['bill_amt']}',`acquirer`='{$post['acquirer']}',`fullname`='{$post['fullname']}',`bill_email`='{$post['bill_email']}',`mop`='{$post['mop']}',`bill_currency`='{$post['bill_currency']}',`transaction_flag`='{$post['transaction_flag']}',`buy_mdr_amt`='{$post['buy_mdr_amt']}',`buy_txnfee_amt`='{$post['buy_txnfee_amt']}',`rolling_amt`='{$post['rolling_amt']}',`mdr_cb_amt`='{$post['mdr_cb_amt']}',`mdr_cbk1_amt`='{$post['mdr_cbk1_amt']}',`mdr_refundfee_amt`='{$post['mdr_refundfee_amt']}',`available_rolling`='{$post['available_rolling']}',`available_balance`='{$post['available_balance']}',`payable_amt_of_txn`='{$post['payable_amt_of_txn']}',`fee_update_timestamp`='{$post['fee_update_timestamp']}',`trans_amt`='{$post['trans_amt']}',`remark_status`='{$post['remark_status']}',`bill_ip`='{$post['bill_ip']}',`bank_processing_amount`='{$post['bank_processing_amount']}',`bank_processing_curr`='{$post['bank_processing_curr']}',`terNO`='{$post['terNO']}'  ".$update_append.$master_update.
                " WHERE `id`='{$id}'",$data['qp']
        );
