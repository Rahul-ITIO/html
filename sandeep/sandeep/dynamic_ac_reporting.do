...
$remaining_balance=prnsum2($wd_json_value['remaining_balance']);
$withdraw_fee=prnsum2($wd_json_value['withdraw_fee']);
$withdrawfee_calc=prnsum2($wd_json_value['withdrawfee_calc']);
...
//end: class	
		...
		foreach($post['payout_mature'] as $key=>$value){
			...
			if(($value['status']==1 || $value['status']==4 || $value['status']==7 || $value['status']==8)&&($value['trname']=='cn'||$value['trname']=='ch')){ // completed
                ...
                $dis_rate 	= number_formatf(((double)$value['transaction_amt']/100)*(double)$accounts[0]['transaction_rate']);
				$rol_fee	= number_formatf(((double)$value['transaction_amt']/100)*(double)$accounts[0]['rolling_fee']);
				...
				$this_total	= number_formatf2(((double)$value['transaction_amt'])-($dis_rate+(double)$accounts[0]['txn_fee']+$rol_fee));
				
				$total_txn_fee = $total_txn_fee+(double)$accounts[0]['txn_fee']+$gst_mdr_txtfee_amt;
				
				$total_dis_rate = $total_dis_rate + $dis_rate;
				$total_rol_fee	= $total_rol_fee + $rol_fee;
				
				$sales_volume	= number_formatf2($sales_volume+(double)$value['transaction_amt']);
