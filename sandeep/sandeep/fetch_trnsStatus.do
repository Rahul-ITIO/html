...
if((isset($_REQUEST['actionurl']))&&($_REQUEST['actionurl']=='validate'||$_REQUEST['actionurl']=='fetch_trnsStatus')){
	...
	$tr_upd_validate['system_note']='<b>Source - validate</b> | Status validate by Merchant | Current Status : <b>'.$jsonarray_all["status"].'</b> trans_status';
...
if((isset($_REQUEST['actionurl']))&&($_REQUEST['actionurl']=='validate'||$_REQUEST['actionurl']=='fetch_trnsStatus')){
	...
	if(!empty($merchantUrl)){
		...
		if(url_f1($merchantUrl)){
			...
			$tr_upd_merchanturl['system_note']='<b>Source - validate</b> | Merchant url redirect | Current Status : <b>'.$jsonarray_all["status"].'</b> trans_status as redirect on '.$merchantUrl;
	...
	elseif(isset($redirect_validate_status_url)&&trim($redirect_validate_status_url))
	{
		...
		$tr_upd_merchanturl['system_note']='<b>Source - validate</b> | Merchant url not found | Current Status : <b>'.$jsonarray_all["status"].'</b> trans_status as redirect on '.$redirect_validate_status_url;
