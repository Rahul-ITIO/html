...
$apJson['mid']=(isset($data['apJson']['mid'])?$data['apJson']['mid']:'');
$apJson['orderId']=(isset($data['apJson']['orderId'])?$data['apJson']['orderId']:'');
...
//exit;
	...
	$results = $res;
	  $res['body']['resultInfo']['resultCode'];
  //exit;
	//applied condition according to the status response for fail success and pending 
	if (isset($res) && count($res)>0)
	{
		$status = $res['body']['resultInfo']['resultCode'];
		$_SESSION['acquirer_transaction_id'] = $res['body']['txnId'];
		$_SESSION['acquirer_descriptor'] = $res['body']['gatewayName'];
		
		if($qp){
			echo "<br/><br/><=status=>".$status;
		}

		if(isset($res['body']['resultInfo']['resultMsg'])&&$res['body']['resultInfo']['resultMsg']){
			$message	= $res['body']['resultInfo']['resultMsg'];
		}else{
			$message	= $res['message'];
		}
...
//Comes from Acquirer Status Page
...
$_SESSION['responseAmount']=$res['body']['txnAmount']; // for Test amount fetch from status 
