...
function payout_trans_newf($uid=0,$type=2)
{
	...
	if($qprint==1){
		echo "<hr/>tran_id=> ".$result['tran_id'];
	}
	...
	for($i=0;$i<count($trans_detail_array);$i++)
	{
		...
		if($temp_trans_arr['id']=="$t_withdraw_max")	//if withdraw max id is match with current id then set start date with current created date and break loop
		{
...
function m_bal_update_trans($uid=0,$tid=0,$cond=''){
	...
	*/
	...
	$pre_tra=last_available_balance_and_rolling($tid,$tdate,$uid,$qprint);
	...
	if(isset($_GET['promptmsg'])&&$_GET['promptmsg']){
	...
	if($qprint){
		...
		echo "pre_tra available_balance=>".$last_tr_bal; echo "<br/>payable_amt_of_txn=>".@$c_tra[0]['payable_amt_of_txn']; echo "<br/>Start available_balance=>".$available_balance; echo "<hr/>Selected transID =>".@$c_tra[0]['transID']; echo "<hr/>pre_tra transID=>".@$pre_tra['last_transID']; echo "<br/><br/><br/>".$form."<br/><br/><br/>";
...
function get_trans_graph($post,$grfType=1,$subAdminId=''){
	...
	//acquirer type
	if(isset($post['acquirer_type'])&&$post['acquirer_type']){
		$post['acquirer_type']=implodef($post['acquirer_type']);	//array to string
		$where_pred.=" AND ( `t`.`acquirer` {$sort}IN ({$post['acquirer_type']}) ) ";
	}else{
	...
	if(isset($_REQUEST['search_key'])&&isset($_REQUEST['key_name'])&&$_REQUEST['search_key']&&$_REQUEST['key_name']){
		...
		elseif($kn=='payable_amt_of_txn'||$kn=='bill_amount'||$kn=='trans_amt'||$kn=='buy_mdr_amt'||$kn=='buy_txnfee_amt'||$kn=='rolling_amt'||$kn=='mdr_cb_amt'||$kn=='mdr_cbk1_amt'||$kn=='mdr_refundfee_amt'||$kn=='bank_processing_amount'||$kn=='available_balance'){
			$que= "`t`.`".$kn."` {$sort}IN ({$search_key2}) ";
		}else{
			$que=queryArrayf($search_key,$kn,$sort.'LIKE',$sortImp,';');	//create query
		}
		$where_pred .= " AND ({$que}) ";
	}
	...
	*/
	...
	//Filter clients list for seleced sponsor - MID
		if ((isset($_REQUEST['mid']))&& ($_REQUEST['mid']!='')){
			$where="WHERE m.sponsor=".$_REQUEST['mid']." AND (m.active='{$active}')  AND m.sub_client_id IS NULL"; //add AND m.sub_client_id IS NULL By Vikash
			$active= '';
		}else {
		  if($active==5){   // condation Added By Vikash
		  $active="WHERE 1 AND m.sub_client_id IS NOT NULL"; //add AND m.sub_client_id IS NULL By Vikash
		  }elseif($active==6){   // condation Added By Vikash
		  $active="WHERE 1 "; //add AND m.sub_client_id IS NULL By Vikash
		  }elseif($active==7){   /// condation for Live Payout Merchant Added By Vikash 08-09-2022
		  $active="WHERE 1 AND m.payout_request=1 "; 
		  }elseif($active==8){   // condation for Test Payout Merchant Added By Vikash 08-09-2022
		  $active="WHERE 1 AND m.payout_request=2 "; 
		  }elseif($active==9){   // condation for Inactive Payout Merchant Added By Vikash 08-09-2022
		  $active="WHERE 1 AND m.payout_request=3 "; 
		  }else{
			$active="WHERE (m.active='{$active}') AND m.sub_client_id IS NULL"; //add AND m.sub_client_id IS NULL 
			}
		}
...
function common_trans_detail($id, $uid){
	...
	$trans=db_rows(
		"SELECT * FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` {$join_additional}  WHERE `id`='{$id}' LIMIT 1",0
	);
	if(isset($data['Database_3'])&&$data['Database_3']&&function_exists('db_connect_3')&&count($trans)==0){
		$trans=db_rows(
			"SELECT * FROM `{$data['Database_3']}`.`{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` {$join_additional}  WHERE `id`='{$id}' LIMIT 1",0
		);
	}
	$trans=$trans[0];
	if($trans){
...
function update_trans_ranges($uid, $status, $trange='', $paydate='',  $update_tr=true,  $adm_login=true, $json_value=''){
	...
	foreach($tresult as $key=>$value){
		...
		if($adm_login){
			...
			if(isset($data['MASTER_TRANS_ADDITIONAL'])&&$data['MASTER_TRANS_ADDITIONAL']=='Y')
			{
				...
				if(isset($data['MASTER_TRANS_ADDITIONAL'])&&$data['MASTER_TRANS_ADDITIONAL']=='Y'&&$transactions_newid>0){
					...
					"('{$transactions_newid}','{$transactioncode}' ".$additional_data.")",0);
		...
		if($status==512||$status==513||$status==611||$status==612||$status==412||$status==712||$status==8){
			$calculation_tr_fee_upd=false;
		}
	  ...
	  if(empty($json_value)){
		if($tresult){
			...
			*/
			...
			if(trim($terminal_info['bussiness_url'])&&trim($value['descriptor'])){
	...
	foreach($post['select_transactions'] as $key=>$value){		//execute all trans row by row
		if($value){
			...
			if($merchant_setting)
			{
				...
				$accounts=jsondecode($acqTbl['mer_setting_json'],1);
...
function mer_settings($uid, $id=0, $single=false, $acquirer_id=''){
	...
	$account=db_rows(
		...
		($id?" AND `id`='{$id}'":'').($single?" LIMIT 1":' ORDER BY acquirer_display_order IS NULL ASC, acquirer_display_order ASC, acquirer_id ASC '),0
	);
	$result=array();
...
function create_mer_setting($post, $uid, $notify=true){
	...
	db_query(
			...
			"'{$post['monthly_fee']}','{$post['assignee_type']}',{$post['sponsor']},'{$post['checkout_label_mobile']}','{$post['settelement_delay']}','{$post['reserve_delay']}','{$post['txn_fee_failed']}','{$post['tr_scrub_success_count']}','{$post['tr_scrub_failed_count']}','{$post['acquirer_display_order']}','{$scrubbed_json}'{$insert_qr_2})",0
	);
	$newid=newid();
...
function create_new_trans($merID, $bill_amt, $acquirer, $trans_status,$fullname='', $bill_address='', $bill_city='', $bill_state='', $bill_zip='', $bill_email='', $ccno='', $bill_phone='', $product_name='', $source_url=''){
	...
	try {
		...
		if(isset($_SESSION['rolling_delay'])){
			$rolling_delay=$_SESSION['rolling_delay'];
		}else{
			if($data['con_name']=='clk'){
				$rolling_delay=0;
			}else{
		...
		if(isset($data['BIN_EX_DATA']) && $data['BIN_EX_DATA']==1)
		{
			...
			if(isset($_SESSION['ex_month'])&&$_SESSION['ex_month'])
			{
				$bin_fld	.= ",`ex_month`";
				$bin_data	.= ",'".exp_encrypts256($_SESSION['ex_month'])."'"; 
			}
			if(isset($_SESSION['ex_year'])&&$_SESSION['ex_year'])
			{
				$bin_fld	.= ",`ex_year`";
...
function select_terminals($uid, $id=0, $single=false, $active=0,$where_pred=''){
	...
	$terminals=db_rows(
		...
		($id?" AND `id`='{$id}'":'').($active?" AND `active`='{$active}'":'').($single?" LIMIT 1":''),0
	);
	$result=array();
...
function get_all_clients_new($Id=0)
{
	...
	$result = db_rows(
		"SELECT `c`.`id`,`c`.`username`,`c`.`registered_email`,`c`.`payout_request`,`c`.`default_currency`, `p`.`available_balance`,`p`.`available_rolling` ". 
		...
		" $where_clause  ORDER BY `c`.`id`",$qprint
	);
	$clients_list = array();
	foreach ($result as $key => $val) {
...
function get_clients_username($uid){
	...
	$result=db_rows(
		"SELECT `username` FROM `{$data['DbPrefix']}clientid_table`".
...
// Added and updated By Vikash from old backup  on 05042023
...
// Added and updated By Vikash from old backup  on 11042023
//function to generate transaction id
...
function acquirer_tablef($list=0){
	...
	$select_pt=db_rows(
		"SELECT `acquirer_status`,`acquirer_id`,`acquirer_name`,`channel_type`,`acquirer_refund_policy` FROM `{$data['DbPrefix']}acquirer_table` WHERE `acquirer_status` NOT IN (0) ",0
	);
