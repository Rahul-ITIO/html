...
if(isset($_REQUEST['public_key'])&&!empty($_REQUEST['public_key'])){
		...
		$decryptres_api = decryptres($post['public_key']);
		$apiToken=explode('_',$decryptres_api);
		if(@$qp)echo "<hr/>apiToken=".$decryptres_api;
...
//Dev Tech: 25-02-04 Decode transID 
if(isset($_REQUEST['key'])&&!empty($_REQUEST['key'])){
		
		$decryptres_key = decode64f($_REQUEST['key']);
		$decryptres_key_de=jsondecode($decryptres_key,1,1);
		$_REQUEST['transID']=@$decryptres_key_de['transID'];
		if(@$qp){
			echo "<hr/>decryptres_key=".$decryptres_key;
			echo "<hr/>decryptres_key_de="; print_r($decryptres_key_de);
		}
...
if(isset($_REQUEST['transID'])&&!empty($_REQUEST['transID'])){
	$transID=transIDf($_REQUEST['transID'],0);
	$where_pred =" (`t`.`transID`={$transID}) AND ";
}
...
$trans_status_get=db_rows(
	...
	" ORDER BY `t`.`id` DESC {$limit_set}",$qp //DESC ASC
);
...
$jsonarray_all["order_status"]=$trans_status;
if(!empty($message)){
...
if(isset($_REQUEST['reference'])&&!empty($_REQUEST['reference'])){
	...
	if(isset($_REQUEST['a'])&&$_REQUEST['a']=='e') $trans_status=0;
	...
	if(isset($trans_status_get[0]['json_value'])&&$trans_status_get[0]['json_value']&&$trans_status==0){
		...
		if(!empty($authurl_redirect)){
			if(url_f1($authurl_redirect)){
				...
				$tr_upd_authurl['system_note']='<b>Source - AuthURL Sent</b> | <b>'.$jsonarray_all["status"].'</b> trans_status redirect on '.$authurl_redirect;
...
if($trans_status_get[0]['trans_status']==0){	
	...
	$data['footer_msg'] = "<p>Slow connectivity while establishing a connection with the bank server.</p><br/><p>The transaction status will be notified via email soon.</p><br/><p>Please do not attempt a new transaction for the next four hours.</p><br/><br/>";
