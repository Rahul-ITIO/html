...
//print_r($txn);exit;
	...
	//	$postData['BnfNickName']=substr($bnameacc,0,4).substr($beneId,-3);
		...
		$txn_value['bene_response'] = isset($response_arr)&&$response_arr?$response_arr:(isset($newsource)&&$newsource?$newsource:$raw_response);

		if(isset($response_arr['Response'])&&$response_arr['Response']=='SUCCESS')
		{
			$BNF_ID = $response_arr['BNF_ID'];
			$sqlStmt = "UPDATE `{$data['DbPrefix']}banks` SET `bank_bene_id`='{$BNF_ID}' WHERE `id`='{$beneId}' ";
			db_query($sqlStmt);
			
			$txn_value['message'] = "Beneficiary added successfully with Beneficiary Id $BNF_ID. Fund transfer to the newly added Beneficiary will not be allowed immediately. You can send fund after 30 Minutes";
		}
		else
		{
			$txn_value['message'] =$response_arr['Message'];
		}
	}
...
//	if($new_beneficiary==false)
	if($bank_bene_id)
	{
		...
		if(isset($_REQUEST['method'])&&strtolower($_REQUEST['method'])=='neft')	// create payload if method is NEFT
		{
			...
			$sendRequest['narration1']		="Pay for $transaction_id";	//"Test";	//to be discuss
		...
		elseif(isset($_REQUEST['method'])&&strtolower($_REQUEST['method'])=='rtgs')	// create payload if method is RTGS
		{
			...
			$sendRequest['REMARKS']			="Pay for $transaction_id";	//"Test";	//to be discuss
...
elseif(($is_mer) || ($verify_by_admin)){
	...
	$sendRequest['narration1']		="Pay for $transaction_id";		//to be discuss
	$sendRequest['crpId']			=$crpId;
	$sendRequest['crpUsr']			=$crpUsr;
	$sendRequest['aggrId']			=$aggrId;
	$sendRequest['urn']				=$urn;
...
if(empty($txn_id))
{
	if($bank_bene_id&&($status==13 || ($is_mer) || ($verify_by_admin)))
	{
		...
		//$txn_value['response'] = isset($responseParam)&&$responseParam?$responseParam:(isset($newsource)&&$newsource?$newsource:$raw_response);
		$txn_value['response'] = $responseParam;
...
//		echo "<div style='display:none'>LOG=".nl2br($log)."</div>";
		...
		if(isset($_REQUEST['method'])&&$_REQUEST['method'])
			$txn_value['payment_method'] = $_REQUEST['method'];
		...
		elseif(isset($response_arr['STATUS'])&&$response_arr['STATUS']=='PENDING')
		{
			...
			update_transaction_ranges(-1, 0, $td['id']);	//FOR PENDING
		}
		else
		{
