...
if($_SERVER["HTTP_HOST"]=='localhost'||$_SERVER["HTTP_HOST"]=='localhost:8080'||isset($_SESSION["http_host_loc"])){	//check localhost or not, if yes then define localhost in data and set true
	$localhosts=true; 
...
if($data['localhosts']==true)	//if you browse in local system then execute following section
{
	...
	$db_database='ipgdb'; // ipgdb	ipglivepgdb31 
	$db_username='postgres';
		
	//5 ipg
	$data['connection_type']='PSQL';
	$db_database='ipglivepgdb31'; // ipgdb	ipglivepgdb31 
	$db_username='pgslquser';
...
//define customize number format with comma seprate 
...
//The function number_formatf_3() return value in RED color if value is negative, return in GREEN, comma seprate 
function number_formatf_3($summ, $splus=true){
	global $data;
	if($summ<0)$color='red';else $color='green';
	return
		"<font color='{$color}'>".number_format_comma($summ).
		'</font>'
	;
}
...
function use_curl($url, $post=null){
	...
	if(isset($post)&&is_array($post)&&(@$post['action']=='admin_direct'||@$post['actionurl']=='admin_direct'||@$post['cron_tab']=='cron_tab'||@$post['error_skip']=='error_skip')){
