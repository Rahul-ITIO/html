...
elseif(isset($_POST['amount'])&&isset($_SESSION['uid_'.$uid]['s_amount'])&&$_SESSION['uid_'.$uid]['s_amount']>=$_POST['amount']){
	$convertamtAction=1;
}
...
if((isset($_POST['action'])&&$_POST['action']=="convertamt")&&($convertamtAction)&&(isset($_POST['amount']))&&(isset($_SESSION['uid_'.$uid]['s_amount']))){
	...
	if($withdraw_fee>0){
		/*
		$json["withdraw_fee_on_amount_jsn"]=$withdraw_fee_on_amount=number_formatf_2($amount_get-$_SESSION['uid_'.$uid]['s_cal_total_fee']); // Gross Settlement Amount - all fee for amount of withdraw fee
...
if(isset($data['settlement_optimizer_available'])&&$data['settlement_optimizer_available']=='Y'){
	if((isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&$post['settlement_optimizer']=='manually')||($is_admin&&isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&($post['settlement_optimizer']=='manually'||$post['settlement_optimizer']=='no_settlement'))){
...
if(!isset($_SESSION['uid_'.$uid]['payout']))
{
	...
	$_SESSION['uid_'.$uid]['payout']=$payout = payout_trans_newf($uid,$post['type']);
...
if($uid_session_true)
{
	...
	//$_SESSION['uid_'.$uid]['ab']=$post['ab']=ms_trans_balance_calc_new($uid);
	
	$trans_detail_array = fetch_trans_balance($uid);
	$_SESSION['uid_'.$uid]['ab']=$post['ab']=trans_balance_newac($uid,"","",$trans_detail_array);	
...
if((isset($post['send'])&&$post['send'])||(isset($post['action'])&&$post['action']=="convertamt")){
	...
	if ($post['step'] == 1||$post['step'] == 4) {
		...
		$upa=@$post['BankType'];
			...
			$additional_fld=$master_fld=",`acquirer_ref`,`acquirer_response`,`product_name`,`support_note`,`source_url`,`system_note`,`json_value`,`trans_response`,`upa` " ;
			
			$additional_data=$master_data=",'{$acquirer_ref}','{$acquirer_response}','{$product_name}','{$remark_upd}','{$source_url}','{$system_note}','{$json_value}','{$trans_response}','{$upa}' " ;
			...
			*/
			...
			$_SESSION['action_success']='Success : We will be process the '.$data ['ThisTitle'].' within 4 working days';
