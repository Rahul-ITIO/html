...
if($post['action']=='select'){
		...
		*/
		...
		if(isset($_REQUEST['date_1st']) && isset($_REQUEST['date_2nd']) && $_REQUEST['date_1st']&&$_REQUEST['date_2nd']){
			...
			$reportdate .=" AND ( `t`.`tdate` BETWEEN '{$date_1st}' AND '{$date_2nd}' )  "; 
									
			$where[]	= " (  `t`.`tdate` BETWEEN '{$date_1st}' AND '{$date_2nd}' )  "; 
...
*/
	...
	//$maxRowsByPage=50;
	if( $data['last_record'] < $maxRowsByPage && isset($data['DB_CON']) && !isset($_GET['DB_CON']) && function_exists('config_db_more_connection') )
	{
		...
		*/
		...
		if(isset($_REQUEST['dtest']))echo "<br/>connection_type=>".$data['connection_type']."<br/>";
		//print_r($db_con_arr);
