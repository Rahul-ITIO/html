   ...
   (int)$_SESSION['attempts']++;   	
	...
	if(($_SESSION['attempts']<$data['PassAtt'])&&($status>30)){
		...
		*/
			...
			$result=db_rows("SELECT `username`,`google_auth_access`,`google_auth_code`,`sub_client_id`,`sub_client_role`,`registered_email`,`fullname`,`company_name` FROM `{$data['DbPrefix']}clientid_table` WHERE `username`='".$post['username']."'",0);
			$result=$result[0];
			//$phone=$result['phone'];
