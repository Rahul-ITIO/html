...
//check validation and execute script according conditions
...
if($data['localhosts']==true)	//if you browse in local system then execute following section
{
	...
	$db_password='';			// mysql db_password for db1 || psql pass: 
	...
	$data['DbPort']='3307';	// if not db port 3306 than assing new db port
...
if(isset($_REQUEST['a'])&&$_REQUEST['a']=='cm') $_SESSION['con_more']='/thirdpartyapp'; 
...
switch ($data['connection_type']) {
	case 'PDO':
		...
		break;
	case 'MYSQLI':
		if (!@include('config_mysqli'.$data['iex'])){
			echo '<center><b>Error!</b> Connection File not Found';
			die();
		}
		break;
	case 'PSQL':
		if (!@include('config_psql'.$data['iex'])){
			echo '<center><b>Error!</b> Connection File not Found';
			die();
		}
		break;
...
if(isset($data['Database_2'])&&$data['Database_2']){
...
function send_attchment_message($email_to,$email_to_name,$email_subject,$email_message,array $post=NULL,$email_from='',$email_reply=''){
	...
	if($mgun)
	{
		...
		if(isset($mail_migomta_api)&&trim($mail_migomta_api)){
...
function send_email($key, $post, $sponsorJsn=0){
	...
	//Dev Tech : 23-03-13 SEND_EMAIL_ARR is dynamic get 
	if(isset($data['SEND_EMAIL_ARR'])&&$data['SEND_EMAIL_ARR']){
		foreach($data['SEND_EMAIL_ARR'] as $v9){
			if(isset($post[$v9])&&trim($post[$v9])&&!empty($post[$v9])){
			...
			elseif(isset($post[$v9])&&empty($post[$v9])){
				if(strpos($text,"[$v9]")!==false){
...
function json_log_upd($tableId=0,$tableName='json_log',$action_name='Update',array $log=[],$clientid='',$bearer='',$transID='',$reference='',$bill_amt='',$bill_email=''){
...
function json_log_view1($log='',$action_name='View Json Log',$tableId=0,$tableName='json_log',$clientid=''){
	...
	if((isset($_SESSION['login_adm']))||(isset($_SESSION['json_log_view'])&&$_SESSION['json_log_view']==1)){
		...
		if($log){	//if json log exists then display 
		?>
...
function use_curl($url, $post=null){
	...
	if(@$post['action']=='admin_direct'||@$post['actionurl']=='admin_direct'||@$post['cron_tab']=='cron_tab'||@$post['error_skip']=='error_skip'){
...
function last_available_balance_and_rolling($id,$tdate,$uid,$qp=0){
	global $data; $result=array();
			
	$tdate_current_tr=date('Y-m-d H:i:s',strtotime($tdate));	//transaction date
	
	//$tdate_current_tr=date('Y-m-d H:i:s'.substr((string)microtime(), 1, 6));
	...
	$pre_tra=db_rows(
		...
		" AND (`available_balance` IS NOT NULL AND `available_balance` != '') ".
		" AND ((STR_TO_DATE(`tdate`, '%Y-%m-%d %H:%i:%s.%f')) < (STR_TO_DATE('{$tdate_current_tr}', '%Y-%m-%d %H:%i:%s.%f'))) ".
...
function available_balance_and_rolling($id,$tdate,$uid,$qp=0){
	...
	if(isset($data['SKIP_AVAILABLE_BALANCE_AND_ROLLING'])&&$data['SKIP_AVAILABLE_BALANCE_AND_ROLLING']=='Y'){
		...
		$current_tra=db_rows(
			"SELECT (`id`) AS `id`, (`transID`) AS `transID`, (`tdate`) AS `tdate` , `available_balance` , `available_rolling`".
			" FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`".
			" USE INDEX(tdate_desc) WHERE ( `merID`='{$uid}' )".
			" AND ( `id` NOT IN ({$id}) ) ".
			" AND ( `trans_status` NOT IN (0,9,10) ) ".
			" AND (`available_balance` IS NOT NULL AND `available_balance` != '') ".
			" ORDER BY tdate DESC LIMIT 1",$qp
