...
if($post['bid']){
	...
	$trans_detail_array = fetch_balance($post['bid']);

	$monthly_fee	= payout_trans_new($post['bid']);
	...
	{
		$post['mbt']	= account_trans_balance_calc_new($post['bid'], $trans_detail_array, $ptdate);
		$post['mbt_d']	= account_trans_balance_calc_d_new($post['bid'],$ptdate,0,$trans_detail_array);
	}
	else
	{
		$post['ab']	= account_balance_newac($post['bid'],"",$ptdate, $trans_detail_array);	
	}
}
###############################################################################
...
?>
	...
	<a class="restartfa" onclick="ajaxf1(this,'<?=$data['Admins'];?>/transaction_list_calc_new<?=$data['ex'];?>?<?=$sub_quer;?>&compare=1','#direct_account_calculation2','1','2')">Direct Account Calculation <font class="icons glyphicons restart "><i></i></font></a>
	<? }?>
	</div>
