...
if($data['con_name']=='clk'&&$post['send']=='signupbycurl'){
//if($data['con_name']=='clk'){
	$json_action=true;
...
if(isset($post['send'])&&$post['send']){
	...
	*/
		...
		if(isset($_SESSION['sponsor_id'])&&$_SESSION['sponsor_id']&&$_SESSION['sponsor_username']){
			...
			$post['newques']=jsonencode($get_sponsor);
		}
		
		$post['password']=((isset($post['password'])&&$post['password'])?$post['password']:'');
		$post['newques']=((isset($post['newques'])&&$post['newques'])?$post['newques']:'');
		$post['newansw']=((isset($post['newansw'])&&$post['newansw'])?$post['newansw']:'');
		$post['newcompany']=((isset($post['newcompany'])&&$post['newcompany'])?$post['newcompany']:'');
		$post['newregnum']=((isset($post['newregnum'])&&$post['newregnum'])?$post['newregnum']:'');
		
		$post['newdrvnum']=((isset($post['newdrvnum'])&&$post['newdrvnum'])?$post['newdrvnum']:'');
		$post['newregnum']=((isset($post['newregnum'])&&$post['newregnum'])?$post['newregnum']:'');
		$post['newdrvnum']=((isset($post['newdrvnum'])&&$post['newdrvnum'])?$post['newdrvnum']:'');
		$post['newaddress']=((isset($post['newaddress'])&&$post['newaddress'])?$post['newaddress']:'');
		$post['newcity']=((isset($post['newcity'])&&$post['newcity'])?$post['newcity']:'');
		$post['newcountry']=((isset($post['newcountry'])&&$post['newcountry'])?$post['newcountry']:'');
		$post['newstate']=((isset($post['newstate'])&&$post['newstate'])?$post['newstate']:'');
		$post['newzip']=((isset($post['newzip'])&&$post['newzip'])?$post['newzip']:'');
		$post['newphone']=((isset($post['newphone'])&&$post['newphone'])?$post['newphone']:'');
		$post['newfax']=((isset($post['newfax'])&&$post['newfax'])?$post['newfax']:'');
		
		/*
		if(isset($_POST['fullname'])&&count($namearr=explode(' ',$_POST['fullname'])) > 0){
			$post['newlname']=trim(str_replace($namearr[0],"",$_POST['fullname']));
			$post['newfname']=$namearr[0];
		}
		*/
		//print_r($post);
		//exit;
