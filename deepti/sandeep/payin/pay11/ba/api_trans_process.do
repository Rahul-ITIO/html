...
##############################################################
	...
	if($post['integration-type']=="s2s"){
		...
		elseif(isset($generate_qr_code) && !empty($generate_qr_code) && isset($post['actionajax']) && $post['actionajax']=='ajaxQrCode') {
			...
			echo "<img src=\"https://chart.googleapis.com/chart?chs=150x150&cht=qr&chl={$generate_qr_code}&choe=UTF-8\" title=\"{$orderCurrencySymbol}{$total_payment} paying to {$dba} \" />";exit;
		}
