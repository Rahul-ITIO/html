...
if(isset($_REQUEST['api_token'])&&!empty($_REQUEST['api_token'])){
		...
		$where_pred.=" (`merID`={$merID}) AND (`store_id`={$store_id}) AND ";
		
}

if(isset($_REQUEST['id_order'])&&($_REQUEST['id_order'])&&($merID)){
	$where_pred.=" (`reference`='{$_REQUEST['id_order']}') AND ";
}


if(isset($_REQUEST['transID'])&&!empty($_REQUEST['transID'])){
	$where_pred =" (`transID`={$_REQUEST['transID']}) AND ";
}
...
$trans_status_get=db_rows(
	"SELECT `id`,`acquirer`,`trans_status`,`bill_amt`,`transID`,`descriptor`,`tdate`,`bill_currency`,`merID`,`webhook_url`,`return_url`,`orderset`,`trans_response`,`mop`,`reference`,`source_url`,`json_value`,`terNO`,`channel_type`,`authurl`,`authdata` ". 
	" FROM `{$data['DbPrefix']}master_trans_table`".
	" WHERE  ".$where_pred.
	" ORDER BY `id` DESC {$limit_set}",0 //DESC ASC
);
...
if(isset($_REQUEST['reference'])&&!empty($_REQUEST['reference'])){
	...
	if(($trans_status_get[0]['trans_status']==0)&&(isset($is_test)&&$is_test=="9")){
		if($trans_status_get[0]['channel_type']=='2' || $trans_status_get[0]['channel_type']=='3'){
			$redirecturl = $data['Host']."/test3dsecureauthentication".$data['ex']."?orderset=".$trans_status_get[0]['orderset'];
		}else{
			$redirecturl = $data['Host']."/success".$data['ex']."?orderset=".$trans_status_get[0]['orderset'];
		}
...
if($trans_status_get[0]['trans_status']==0){	
	...
	$jsonarray_all["orderset"]=$trans_status_get[0]['orderset'];
