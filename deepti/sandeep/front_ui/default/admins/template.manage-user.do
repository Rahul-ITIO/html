...
<div id="zink_id" class="container bg-white px-0">
  <form method="post" name="data">
    ...
    <div class="container my-2 py-2 border rounded bg-white" >
      <div class="row vkg">
        <div class="col-sm-12 ">
          <h4 class="float-start"><i class="<?=$data['fwicon']['user-plus'];?>"></i> User Management</h4>
          <button class="btn btn-primary btn-sm float-end my-1" type="submit" name="send" value="ADD NEW USER!" title="Add A New Sub Merchant"><i class="<?=$data['fwicon']['circle-plus'];?>"></i></button>
        </div>
      </div>
	  <div id="display-small-screen"></div>
<div class="table-responsive-sm">
<table id="display-large-screen" class="table table-hover">
  <thead>
    <tr>
      <th scope="col">#</th>
...
if($value['active']==1){ $user_status="<span class='text-success fw-bold'>Active</span>"; } else { $user_status="<span class='text-danger fw-bold'>Suspended</span>"; }

$data_div="<div class='row table border rounded p-2'>";
	 $data_div.="<div class='col-sm-8 fw-bold'>Sub Merchant Details</div><div class='col-sm-4 text-end px-2 fw-bold' onclick='dataclose()'>X</div>";
	 $data_div.="<div class='divider bg-dark' style='padding-bottom: 0.1rem!important;'></div>";
	 $data_div.="<div class='col-sm-4'>User ID</div><div class='col-sm-8'>: ".$value['username']."</div>";
	 $data_div.="<div class='col-sm-4'>Name</div><div class='col-sm-8'>: ".ucwords($value['fname']." ".$value['lname'])."</div>";
	 $data_div.="<div class='col-sm-4'>Email</div><div class='col-sm-8'>: ".encrypts_decrypts_emails($value['email'],2)."</div>";
	 $data_div.="<div class='col-sm-4'>Status</div><div class='col-sm-8'>: ".$user_status."</div>";
	 $data_div.="<div class='col-sm-4'>Assign Work</div><div class='col-sm-8'>".$rolestring."</div>";
	 $data_div.="</div>";
	 
?>
      <tr>
	  <th scope="row"><div class="content"><i class="<?=$data['fwicon']['display'];?> data_display" title="View details"></i><input type="hidden" id="data_div" name="data_div" value="<?=$data_div;?>"/></div></th>
	  <td><?=$value['username']?></td>
	  <td><?php echo ucwords($value['fname']." ".$value['lname']);?></td>
	  <td class="text-truncate"><?php echo encrypts_decrypts_emails($value['email'],2);?></td>
	  <td><div class="btn-group dropstart"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action"><i class="<?=$data['fwicon']['circle-down'];?>"></i></a>
                <ul class="dropdown-menu dropdown-menu-icon pull-right" >
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/manage-user<?=$data['ex']?>?id=<?=$value['id']?>&action=update" title="Edit" ><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a></li>
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/password<?=$data['ex']?>?id=<?=$value['id']?>&action=password"  title="Change Password" onclick="return confirm('Do you want to change Password?');"><i class="<?=$data['fwicon']['unlock'];?>"></i></a></li>
                  <? if($value['active']==1){ ?>
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/manage-user<?=$data['ex']?>?id=<?=$value['id']?>&action=suspend"  title="Suspend" onclick="return confirm('Do you want to suspend?');"><i class="<?=$data['fwicon']['ban'];?> text-danger"></i></a></li>
                  <? } ?>
                  <li> <a class="dropdown-item"  href="<?=$data['USER_FOLDER']?>/manage-user<?=$data['ex']?>?id=<?=$value['id']?>&action=delete"  title="Delete" onclick="return confirm('Do you want to Delete?');"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a></li>
                  <? if($value['active']==0){ ?>
                  <li> <a class="dropdown-item" href="<?=$data['USER_FOLDER']?>/manage-user<?=$data['ex']?>?id=<?=$value['id']?>&action=active" title="Activate" onclick="return confirm('Do you want to activate?');"><i class="<?=$data['fwicon']['check-circle'];?>"></i> </a></li>
                  <? } ?>
                </ul>
              </div>
            </td>
  ...
  ?>
    ...
    <div class="container mt-2 mb-2 border rounded bg-white vkg" >
      <h4 class="my-2"><i class="<?=$data['fwicon']['user-plus'];?>"></i> User Management - <?=$post['add_titileName'];?> Sub Merchant</h4>

      <div class="mb-3 row bg-vlight p-1 rounded">
        <div class="col-sm-6 p-1">
          <label for="Full Name" class="form-label">Full Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
          <input type="text" class="form-control" name="fname" placeholder="Enter full name for sub merchant" value="<?=((isset($post['subclientsuserfullname']) &&$post['subclientsuserfullname'])?$post['subclientsuserfullname']:'')?>"  title="Enter full name for sub merchant" data-bs-toggle="tooltip" data-bs-placement="bottom"  autocomplete="off" required />
        </div>
        <div class="col-sm-6 p-1">
          <label for="Email Address" class="form-label">Email <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
          <input type="email" class="form-control" name="email"  placeholder="Enter email address for sub merchant" value="<?=((isset($post['subclientsuseremail']) &&$post['subclientsuseremail'])?encrypts_decrypts_emails($post['subclientsuseremail'],2):'')?>" autocomplete="off" title="Enter email address for sub merchant" data-bs-toggle="tooltip" data-bs-placement="bottom" required />
        </div>
        <div class="col-sm-6 p-1">
          <label for="Sub UserName" class="form-label">Sub User Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
          <input type="text" class="form-control" name="uname" placeholder="Enter Username For sub merchant" value="<?=((isset($post['subclientsusername']) &&$post['subclientsusername'])?$post['subclientsusername']:'')?>" title="Enter username for sub merchant" data-bs-toggle="tooltip" data-bs-placement="bottom" autocomplete="off" required />
        </div>
        <?/*?>
        <div class="col-sm-6 p-1" style="display: <?=$post['field_disable'];?>;" >
		...
		<?*/?>
        <div class="col-sm-12">
          <div class="input-group my-2 row" id="manageuserforcss">
<?
...
echo ">";
echo "<label class='form-check-label' for='flexCheckChecked'>".$mrtitle."&nbsp;</label></div>" ;
}
}
?>
          </div>
