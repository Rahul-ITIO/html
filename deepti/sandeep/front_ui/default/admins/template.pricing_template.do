...
// for show / hide dive for display details 
    ...
    <div class="container row text-end px-0 title_addnew_common">
      <div class="col-sm-6 my-2 ps-0 text-start">
        <div class="container vkg px-0">
          <h4 class="my-2"><i class="<?=$data['fwicon']['pricing-template'];?>"></i> Pricing Template </h4>
          <div class="vkg-main-border"></div>
        </div>
      </div>
      <div class="col-sm-6 my-2 ps-0 text-end">
        <button type="submit" name="send" value="Add A New Pricing Template!"  class="btn btn-primary" title="Add A New Pricing Template"><i class="<?=$data['fwicon']['circle-plus'];?>"></i></button>
      </div>
    </div>
    <div class="container table-responsive-sm">
      <table class="table table-hover">
		...
		<td><a data-bs-toggle="modal" data-count="<?=prntext($value['id'])?>" class="tr_open_on_modal text-decoration-none" data-bs-target="#myModal"><i class="<?=$data['fwicon']['display'];?> text-link" title="View details"></i></a></td>
          <td><?=$value['id'];?></td>
          <td title="<?=$value['assign_id'];?>" data-bs-toggle="tooltip" data-bs-placement="top"><span class="d-inline-block text-truncate" style="max-width: 150px;" ><?=$value['assign_id'];?></span></td>
		  ...
		  <div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false"  title="Action"><i class="<?=$data['fwicon']['action'];?> text-link"></i></a>
			    ...
			    <? if(isset($value['json_log_history'])&&$value['json_log_history']){?>
			      <li> 
			<a class="dropdown-item" onclick="popup_openv('<?=$data['Host']?>/include/json_log<?=$data['ex']?>?tableid=<?=$value['id'];?>&tablename=pricing_template')" title="View Json History">
      ...
      <div class="container vkg px-0">
        <h4 class="my-2">
          ...
          <i class="<?=$data['fwicon']['circle-info'];?> text-info fa-fw" 
			onclick="popup_openv('<?=$data['Host']?>/include/json_log<?=$data['ex']?>?tableid=<?=$post['id'];?>&tablename=pricing_template')" title="View Json History"></i>
          <? } ?>
          <? } ?>
        </h4>
        <div class="vkg-main-border"></div>
