...
if(isset($_SESSION['merchant_logo'])&&$_SESSION['merchant_logo']){
			$_SESSION['pg_logo']=$_SESSION['merchant_logo'];
}elseif(isset($_SESSION['merchant_logo_url'])&&$_SESSION['merchant_logo_url']){
			$_SESSION['pg_logo']=$_SESSION['merchant_logo_url'];
}elseif(isset($data['ADMIN_LOGO'])&&$data['ADMIN_LOGO']&&encode_imgf($data['ADMIN_LOGO'])){ 
			$_SESSION['pg_logo']=encode_imgf($data['ADMIN_LOGO']);
}elseif(isset($_SESSION['domain_server']['LOGO'])&&$_SESSION['domain_server']['LOGO']){ 
			$_SESSION['pg_logo']=$_SESSION['domain_server']['LOGO'];
} 
...
<div class="clients_pro my-2">
  <? if((isset($_SESSION['login']))&&($post['action']=="vt")){?>
  <? $c_style="style='float:none;'"; ?>
  <ul class="breadcrumb" style="text-align:left;">
    <li><a href="<?=$data['USER_FOLDER']?>/index<?=$data['ex']?>" ><i class="<?=$data['fwicon']['home'];?>"></i> <?=prntext($data['SiteName'])?></a></li>
    <li class="divider"></li>
    <li>Moto</li>
  </ul>
  <? } ?>
...
/* Dev Tech : 23-02-09 host base url and @user */
...
<script language="javascript" type="text/javascript">
/******* Vikash : 23-04-26 for dynamic qr-code generate *****************/
	...
	*/
	...
	$.ajax({
		...
		success: function(data) {
			//alert(data);
			if(data.match('googleapis|img')){
				$('.paybutton').hide();
				$('.paid_qrcode_link').show();
				$('.paid_qrcode_button').show();
				$('.paid_qrcode_button').html('<i class="fa-solid fa-spinner fa-spin-pulse"></i>');
...
function generate_intent_urlf(){
  ...
  <? if(!empty($post['acquirer_id_check'])){?>
  <script language="javascript" type="text/javascript"> 
	$(document).ready( function(){
...
<style>
html .modal_msg_body {top:50% !important;} 
...
.cname .inputype.the_icon.active {background:var(--color-4);display:block;}
...
.bg-vlight {   background:#c0defb !important; }
.cname .inputype.the_icon.active {background:#c0defb !important;}
<? } ?>
...
.img-hover { font-size:14px;}

.input-field.select label{top:-9px !important;font-size: 10px!important;} 
.input-field select:focus+label, .input-field select:valid+label { position: absolute;top: -10px;left: 6px;font-size: 10px;padding: 0px 2px;background-color: var(--bs-body-bg) !important;color: var(--bs-body-color) !important;}
 ...
 /* Start New CSS by vikash on 24-01-2023*/
	...
	.inputWithIcon i { position: absolute;right: 6px;top:5px;color: #aaa;transition: 0.3s;font-size: 24px;}
...
.clients.processall #credit_cards img {width:18.6%;}
.credit-card-box .logo_card_icon {position:absolute;top: 17px;right: 8px;width:256px; text-align: right;}
...
.cname label {font-size:12px;width:100%;white-space:nowrap;display:inline-block;line-height:18px; cursor:pointer;}

.account_types.active {color:orange !important;}
label.active {color:#000000 !important;font-weight:bold;}
.active label.active {color:#ffffff !important;font-weight:bold;}
...
.banksDiv, .walletsDiv, .upisDiv {line-height:40px;float:left;cursor:pointer; padding-left:8px}	
...
.b-example-divider {height: 2rem !important; background-color: #2a82d7 !important;color: #ffffff !important;border-width:unset !important; background: var(--background-1)!important;}
.text-shadow{ text-shadow: var(--background-1) 1px 1px 2px;}
...
@media (max-width:1049px){
	...
	.clients.processall #credit_cards img {
  ...
  <div class="processall container my-2">
    <div class="content_top" style="">
      <div class="capl hide bg-white" colspan="2" style="">
       ...
       <? if(isset($_SESSION['pg_logo'])&&$_SESSION['pg_logo']){ ?>
        <div class="col-sm-12 text-center"><img src="<?=($_SESSION['pg_logo']);?>" class="img-fluid" alt="Logo" style="max-width: 100px;"/> </div>
        <? } ?>

        <h4><?=prntext($_SESSION['info_data']['company_name'])?></h4>
	 ...
	 ?>
        ...
        <div id="MainDivvkg" class="row slidePaymentDiv" <?=$c_style2;?> >
          ...
          <div class="col-sm-6 bg-vlight px-2 rounded-payment">
		    <div class="row">
            <? if(isset($data['re_post'])&&$data['re_post']){?>
            <div class="translateDiv col-sm-12 m-0 bg-vlight ms-2 mt-2">
...
<? if(isset($_SESSION['bearer_token_id'])&&trim($_SESSION['bearer_token_id'])){?>
	<div class="text-center my-2 text-shadow">
		Bearer Token : <?=($_SESSION['bearer_token_id']);?>
	</div>
<? } ?>

<? if(isset($_SESSION['info_data']['company_name'])&&trim($_SESSION['info_data']['company_name'])){?>
<div class="text-center fw-bold my-2">Make a payment to <?=prntext($_SESSION['info_data']['company_name'])?></div>
<? } ?>

<p class="shadow-none bg-light rounded fs-5"><?=$post['curr']?$post['curr']:$_SESSION['curr'];?> <?=(isset($post['total'])&&$post['total']?$post['total']:(isset($post['bill_amt'])?$post['bill_amt']:''))?></p>

							
<div class="inputWithcheck mx-2"> <input class="form-control w-100 border border-2" style="background-color: #ffffff !important;width: 100% !important;" type="hidden" value="Pay <?=get_currency($post['curr']?$post['curr']:$_SESSION['curr']);?> <?=(isset($post['total'])&&$post['total']?$post['total']:(isset($post['bill_amt'])?$post['bill_amt']:''))?> to <?=prntext($_SESSION['info_data']['company_name'])?>" readonly="">  </div>
...
<div>
<?=(isset($post['bill_email'])&&$post['bill_email']?'<i class="'.$data['fwicon']['email'].' px-1 fa-fw"></i> '.$post['bill_email'].'':'');?></div>
<? } ?>					
...
<? if(isset($post['chooseStore'])&&$post['chooseStore']){?>
	<div class="col-sm-12 px-2">
	  <label class="col-sm-12 p-2"><strong>Choose Business Type</strong></label>
	  <select name="storeType" id="storeType" class="form-select" >
		...
		<?=$val['bussiness_url']?>
		<? if($data['localhosts']==true){?>|<?=$val['ter_name']?>:<?=$val['public_key']?><? } ?>
		</option>
		<? } ?>
	  </select>
...
<div class="row roubded text-start vkg">
			...
			<? if(isset($post['status_mem'])&&$post['status_mem']!=0 && $post['status_mem']!=1 ){ ?>
              <form method="post" id="validateformid" onSubmit="return validateThisForm(this)" class="needs-validation was-validated" novalidate="">
                <div style="display:none">
                  ...
                  <input type="hidden" name="acquirer" value="<?=(isset($post['acquirer'])?$post['acquirer']:'');?>"/>
                  <input type="hidden" name="bill_country_name" id="bill_country_name" value="<?=(isset($post['bill_country_name'])?$post['bill_country_name']:'');?>"/>
                  <input type='hidden' name='clients' value="<?=(isset($post['clients'])?$post['clients']:'');?>"/>
                ...
                <div id="card" class="showfp text-center" style="max-width:500px;margin:auto;">
                  ...
                  <div class="alert alert-secondary" role="alert"> <a href='<?=$data['Host'];?>/checkout<?=$data['ex']?>?action=vt' type="button" class="close" data-dismiss="alert">&times;</a> <strong>Payment Successful! Transaction Id: <?=$_SESSION['transID'];?>
                    and Bill Amount: <?=$_SESSION['bill_amt'];?> </strong> <br/>
                  ...
                  <div class="alert alert-success"  role="alert"> <a href='<?=$data['Host'];?>/checkout<?=$data['ex']?>?action=vt' type="button" class="close" data-dismiss="alert">&times;</a> <strong>Payment Failed.</strong>
...
<? if(isset($_SESSION['pg_logo'])&&$_SESSION['pg_logo']){ ?>
			  <div class="merchantLogo text-center mt-1"> <img src="<?=($_SESSION['pg_logo']);?>" class="img-fluid" alt="Logo" style="max-height:50px;"/></div>
<? } ?>
			  
<? if(isset($post['product_name'])&&$post['product_name']){ ?>			
<p class="text-center fw-bold my-2" style="text-shadow: pink 1px 1px 2px;">Payment for <?=$post['product_name'];?></p>
<? } ?>				  
				...
				<div class="step_1 my-2">
					...
					<?
						...
						if(!isset($post['bill_amt'])||empty($post['bill_amt'])||$post['bill_amt']=="0.00"){
							unset($post['bill_amt']);
							$post['bill_amt']='';
						}
						?>
                          <? if(empty($post['bill_amt'])||$post['bill_amt']=="0.00"){?>
                          <div class="ecol4">
                            <label style="font-weight:normal;padding-left:3px;">Bill Amount</label>
                            <input type="text" name="bill_amt" placeholder='Bill Amount' value="<?=$post['bill_amt']?>" required />
                          </div>
                          <? } ?>
                        </div>
...
<input placeholder='Full Name' type='text' name='fullname' id="validationCustom01" class="form-control" value='<?=(isset($post['fullname'])&&$post['fullname']?prntext($post['fullname']):'');?>' title="Enter your full name" data-bs-toggle="tooltip" data-bs-placement="bottom" required>
</div>
<? }else{ ?>
<input placeholder='Full Name' type='hidden' name='fullname' value='<?=prntext($post['fullname']);?>' required  >
<? } ?>

<? if(!isset($post['bill_email'])||empty($post['bill_email'])){?>
<div class="ecol4 col-sm-12 my-1">

<input placeholder="Email ID" type="bill_email" name="bill_email" class="form-control" value="<?=(isset($post['bill_email'])&&$post['bill_email']?prntext($post['bill_email']):'');?>" title="Enter your email id" data-bs-toggle="tooltip" data-bs-placement="bottom" required>
</div>
<? }else{ ?>
<input placeholder="Email ID" type="hidden" name="bill_email" value="<?=(isset($post['bill_email'])&&$post['bill_email']?prntext($post['bill_email']):'');?>" required  >
<? } ?>
<? if(!isset($post['bill_phone'])||empty($post['bill_phone'])){?>
<div class="ecol4 col-sm-12 my-1">
<input placeholder="Phone" type="text" name="bill_phone" class="form-control" value="<?=(isset($post['bill_phone'])&&$post['bill_phone']?prntext($post['bill_phone']):'');?>" title="Enter your phone number" data-bs-toggle="tooltip" data-bs-placement="bottom" required>
</div>
<? }else{ ?>
<input placeholder="Phone" type="hidden" name="bill_phone" value="<?=(isset($post['bill_phone'])&&$post['bill_phone']?prntext($post['bill_phone']):'');?>" required>
<? } ?>
<? if(!isset($post['bill_address'])||empty($post['bill_address'])){?>
<div class="ecol4 col-sm-12 my-1">
<input placeholder="Enter your bill address" type="text" name="bill_address" class="form-control" value="<?=(isset($post['bill_address'])&&$post['bill_address']?prntext($post['bill_address']):'');?>" title="Enter your bill address" data-bs-toggle="tooltip" data-bs-placement="bottom" required>
</div>
<? }else{ ?>
<input placeholder="Enter your bill address" type="hidden" name="bill_address" value="<?=(isset($post['bill_address'])&&$post['bill_address']?prntext($post['bill_address']):'');?>">
<? } ?>
<? /*if(!isset($post['bill_street_2'])||empty($post['bill_street_2'])){?>
<div class="ecol4 col-sm-12 my-1">
<input placeholder="Street 2" type="text" name="bill_street_2" class="form-control" value="<?=(isset($post['bill_street_2'])&&$post['bill_street_2']?prntext($post['bill_street_2']):'');?>" title="Enter your address street 2" data-bs-toggle="tooltip" data-bs-placement="bottom" >
</div>
<? }else{ ?>
<input placeholder="Street 2" type="hidden" name="bill_street_2" value="<?=(isset($post['bill_street_2'])&&$post['bill_street_2']?prntext($post['bill_street_2']):'');?>">
<? } */?>
...
<? }else{ ?>
<input placeholder="Bill State" type="hidden" name="bill_state" value="<?=prntext($post['bill_state']);?>">
<? } ?>						  
	...
	if($('input[id="state_list"]')){
		$('#state_list').val("<?=(isset($post['state_two'])&&$post['state_two']?$post['state_two']:'')?>");
	}else{
		$('#state_list option[value="<?=(isset($post['state_two'])&&$post['state_two']?$post['state_two']:'')?>"]').prop('selected','selected');
	} 
...
<? } ?>
                      ...
                      <div class="capc text-center my-2"><button id="cardsend_submit" type="submit" name="integration-type" value="Encode-Checkout"  class="submit btn btn-icon btn-primary w-100 my-2"><i class="<?=$data['fwicon']['check-circle'];?>"></i> <b class="contitxt">Continue</b></button></div>
...
<?php /*?><p class="text-center fw-bold my-2" style="font-size:16px;">Payment for <?=prntext($post['product'])?></p><?php */?>
<? } ?>	
			  ...
			  <div class="payment_option">
				 ...
				 <a class="change hide" id="paymentMethodChange" style="display: none;"><i class="<?=$data['fwicon']['return'];?> fa-xl fa-fw" title="Change Payment Method"></i></a>
				  
                  <h4 class="avai_h4 text-primary my-2 text-center w-100" style="text-shadow: pink 1px 1px 2px;">How would you like to pay?<span id='result_view_chk' class='result_view'></span> <span id='result_view' class='result_view'> </span></h4>
				  
                  <? if(!empty($post['acquirer_id_check'])){ ?>
                  <div class="cname" >
                    <div class="inputgroup">
                      <? foreach($_SESSION['AccountInfo'] as $key=>$value){if(($value['acquirer_id']) &&  (strpos($data['t'][$value['acquirer_id']]['name2'],'Check') !== false) && (in_array($value['acquirer_id'],$post['acquirer_id_check_arr'])) && ($value['acquirer_processing_mode']!="3")  ){
		...
		?>
                      <div class="inputype">
                        <input type="radio" value="<?=$value['acquirer_id']?>" id="<?=$value['acquirer_id']?>" required name="account_types" class="account_types" data-value="<?=$value['acquirer_id']?>" data-name="echeck" data-currency="<?=$pcrcy;?>" />
                        <label for="<?=$value['acquirer_id']?>">
                        <? if(isset($value['checkout_label_web'])&&$value['checkout_label_web']){echo $value['checkout_label_web'];}else{?>
                        Pay by Check
                        <? } ?>
                        </label>
                      </div>
	  ...
	  if(isset($post['acquirer_id_card'])&&!empty($post['acquirer_id_card'])){?>
		...
		foreach($_SESSION['AccountInfo'] as $key=>$value){if((isset($value['acquirer_id'])&&$value['acquirer_id']) && isset($_SESSION['b_'.$value['acquirer_id']]['channel_type']) && (in_array($value['acquirer_id'],$post['acquirer_id_card_arr'])) && ($value['acquirer_processing_mode']!="3") && ($_SESSION['b_'.$value['acquirer_id']]['acquirer_status']>0)  && (!in_array($value['acquirer_id'],$outputArray_1)) ) {
		   ...
		   if(isset($_SESSION['curr'])&&$_SESSION['curr']){$pcrcy=$currency_smbl;}else{$pcrcy=$_SESSION['curr_tr_sys'];}
		?>
			<div class="inputype the_icon <?=countryCodeMatch2($post['country_two'],(isset($_SESSION["b_".$value['acquirer_id']]['countries'])?$_SESSION["b_".$value['acquirer_id']]['countries']:''),(isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])?$_SESSION["b_".$value['acquirer_id']]['processing_countries']:''),isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])?$_SESSION["b_".$value['acquirer_id']]['block_countries']:'')?> box-shadow my-1 w-100">
			<input type="radio" value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" id="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" required name="account_types" class="account_types" data-value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" data-name="ecard" data-ccn="<?=(isset($_SESSION['b_'.$value['acquirer_id']]['mop'])?@$_SESSION['b_'.$value['acquirer_id']]['mop']:'')?>" data-currency="<?=$pcrcy;?>" data-ewlist="<?=acquirer_popup_msg_f($value['acquirer_id'])?>"  data-etype="2D"  data-count='<? if(isset($_SESSION["b_".$value['acquirer_id']]['deCon'])) echo $_SESSION["b_".$value['acquirer_id']]['deCon'];?>' data-dnmc='<? if(isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['block_countries'];?>' style="display:none;" />
                        <label for="<?=$value['acquirer_id']?>">
						<p class="text-start fw-bold">
						<span class=" float-start align-middle h-icon" ><i class="<?=$data['fwicon']['credit-card'];?>"></i></span>
                        <? if(acquirer_label_f(@$value['checkout_label_web'],@$value['checkout_label_mobile'],$value['acquirer_id'])){echo acquirer_label_f(@$value['checkout_label_web'],@$value['checkout_label_mobile'],$value['acquirer_id']);}else{?>
                        Pay by Card [
...
<? $cardarray=explode(",",trim($_SESSION['b_'.$value['acquirer_id']]['mop']));
//print_r($cardarray);
foreach($cardarray as $key=>$value){
if($value=="visa"){ ?><i class="<?=$data['fwicon']['visa_code'];?> text-info img-hover" title="visa" id="visa"></i> <? }
elseif($value=="mastercard"){ ?><i class="<?=$data['fwicon']['mastercard_code'];?> text-danger img-hover" title="mastercard" id="mastercard"></i> <? }
elseif($value=="amex"){ ?><i class="<?=$data['fwicon']['amex_code'];?> text-info img-hover" title="amex" id="amex"></i> <? }
elseif($value=="jcb"){ ?><i class="<?=$data['fwicon']['jcb_code'];?> text-success img-hover" title="jcb" id="jcb"></i> <? }
elseif($value=="discover"){ ?><i class="<?=$data['fwicon']['jcb_code'];?> text-success img-hover" title="jcb" id="jcb"></i> <? }
else{}
...
?>
				<!--<i class="<?=$data['fwicon']['visa_code'];?> text-info img-hover" title="visa" id="visa"></i>
				<i class="<?=$data['fwicon']['mastercard_code'];?> text-danger img-hover" title="mastercard" id="mastercard"></i>
				<i class="<?=$data['fwicon']['amex_code'];?> text-info img-hover" title="amex" id="amex"></i>
				<i class="<?=$data['fwicon']['jcb_code'];?> text-success img-hover" title="jcb" id="jcb"></i>
				<i class="<?=$data['fwicon']['discover_code'];?> text-secondary img-hover" title="discover" id="discover"></i>
				<img src="<?=$data['Host']?>/images/rupaycard.jpg" id="rupay" class="rounded img-hover" title="rupay" height="14px" 
				...
				<div class="cname netBankEdiv">
					...
					?>
                      <div class="inputype the_icon  <?=countryCodeMatch2($post['country_two'],isset($_SESSION["b_".$value['acquirer_id']]['countries'])?$_SESSION["b_".$value['acquirer_id']]['countries']:'',isset($_SESSION["b_".$value['acquirer_id']]['processing_countries'])?$_SESSION["b_".$value['acquirer_id']]['processing_countries']:'',isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])?$_SESSION["b_".$value['acquirer_id']]['block_countries']:'')?> box-shadow my-1 w-100">
                        <input type="radio" value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" id="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" required name="account_types" class="account_types" data-value="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" data-name="<?=(isset($dataname)?$dataname:'');?>" data-currency="<?=$pcrcy;?>" data-submitmsg="intent_submitMsg" data-ewlist="<?=acquirer_popup_msg_f($value['acquirer_id'])?>" data-etype="3D" data-count='<?=(isset($_SESSION["b_".$value['acquirer_id']]['deCon'])?$_SESSION["b_".$value['acquirer_id']]['deCon']:'');?>' data-dnmc='<? if(isset($_SESSION["b_".$value['acquirer_id']]['block_countries'])) echo $_SESSION["b_".$value['acquirer_id']]['block_countries'];?>' style="display:none;" <?=(isset($data_ccn)?$data_ccn:'');?> />
                        <label for="<?=(isset($value['acquirer_id'])?$value['acquirer_id']:'');?>" class="text-start">
						...
						?>
							<?php /*?><img src="<?=$data['Host']?>/images/<?=acquirer_logo_f($value['acquirer_id']);?>_logo.png" title="<?=$value['checkout_label_web'];?>" class="img-hover 44"  /><?php */?>
						<? //} 
						if($pmt_type=="wa"){ 
						echo '<i class="'.$data['fwicon']['wallet_code'].' text-dark h-icon" title="UPI Wallets"></i>';
						}elseif($pmt_type=="nb"){ 
						echo '<i class="'.$data['fwicon']['net_banking'].' text-info h-icon" title="Net Banking"></i>';
						}elseif($pmt_type=="qr"){ 
						echo '<i class="'.$data['fwicon']['qrcode'].' text-info h-icon" title="Qrcode"></i>';
						}elseif($pmt_type=="upi"){ 
						?>
						<img src="<?=$data['bankicon']['upi-icon'];?>" title="upi" class="h-icon"  />
                  ...
                  <? if(((!empty($post['acquirer_id_ewallets']))||(!empty($post['acquirer_id_net_banking'])))&&(!isset($_SESSION['mISO2']))){?>
...
<i class="<?=$data['fwicon']['credit-card'];?> mx-2"></i> All Other Payment Method</a> <a class="change hide" id="goBackPaymentMethodLink" style="display: none;color: var(--color-1)!important;"><i class="<?=$data['fwicon']['circle-left'];?> mx-2"></i> Go Back</a> </div>
			...
			if(isset($post['bill_address'])){
				$common_input .="
				<div style=display:none>
				<input type=hidden name=bill_address value='".$post['bill_address']."'>
				</div>"; 
			}
			if(isset($post['bill_street_2'])){
				$common_input .="
				<div style=display:none>
                      ...
                      <div class="flip">
                        <div class="front">
                          ...
                          <!--<div class="number">-->
                            ...
                            <div class="form-group mt-3 input-icons-left99 input-field inputWithIcon" id="card-number-field">
							
							<i id="input-card-icon" class="<?=$data['fwicon']['credit-card'];?> display-inner"></i>
                              <input id="number" type="tel" name="ccno" maxlength="19" value='<?=(isset($post['ccno'])&&$post['ccno'])?$post['ccno']:"";?>' class="iconvisa form-control " title="Card Number" pattern=".{16,21}" required  />                <label for="number">Card Number</label>
                            </div>
						  ...
						  <div class="form-group input-field mt-3">
							...
							<?php /*?><!--<div class="form-control " title="Card Holder Name" value="333333333"><?=(isset($post['fullname'])&&$post['fullname']?prntext($post['fullname']).'':'');?></div>--><?php */?>
							
							<input type="text" class="form-control" title="Card Holder Name" value="<?=(isset($post['fullname'])&&$post['fullname']?prntext($post['fullname']).'':'');?>" id="input_name_hidden" required />
							<label for="input_name_hidden">Card Holder Name</label>
                            </div>
                          ...
                          <div class="clearfix my-2">
						    ...
						    <div class="float-start input-field" style="width:70px;">
							
                           <select id="expMonth" name="month" class="form-select " required title="Select Month of Card Expiry" >
							...
							<div class="float-start input-field ps-1" style="width:70px;">
							
                            <select id="expYear" name="year" class="form-select " required title="Select Year of Card Expiry">
                          <option value="" disabled="disabled"  selected="selected" >yy</option>
                            ...
                            <div class="float-end input-field" style="width:70px;">
							
							
                              <input id="cvc" type="password" name="ccvv" maxlength="4" size="30" value="<?=(isset($post['ccvv'])&&$post['ccvv'])?$post['ccvv']:"";?>" class="form-control" required  title="Three digit CV code on the back of your card" >
	...
	function goback(){	
		...
		window.location.href="<?=$data['Host'];?>/checkout<?=$data['ex']?>?action=vt";
	}
	</script>
                    </div>
                  </form>
...
<?//start walletlist list----  ?>
						  ...
						  <div class="walletLogo_div ewalist hide">
							...
							<div class="walletsDiv desImg 4008"><img src="<?=$data['Host']?>/images/amazonpay_logo.png" alt="AmazonPay" title="AmazonPay" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">AmazonPay</span></div>
							<div class="walletsDiv desImg 4009"><img src="<?=$data['Host']?>/images/phonepe.png" alt="PhonePe" title="PhonePe" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">PhonePe</span></div>
						  </div>
						  <?}?>
						...
						if(isset($post['t_name6'])&&strpos($post['t_name6'],'upiAppListForIntent') !== false){?>
							...
							<div class="walletsDiv desImg in.amazon.mShop.android.shopping"><img src="<?=$data['Host']?>/images/amazonpay_logo.png" alt="AmazonPay" title="AmazonPay" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">AmazonPay</span></div>
							<div class="walletsDiv desImg com.phonepe.app"><img src="<?=$data['Host']?>/images/phonepe.png" alt="PhonePe" title="PhonePe" style="display:block;width:95%;margin:3px auto;max-height: 90%;"><span class="wallets txNm hide">PhonePe</span></div>
							</div>
						<? }
						...
						if(isset($post['t_name6'])&&strpos($post['t_name6'],'upiIntentInputList') !== false){?>
							<div class="upiIntentInputList_div ewalist hide">
								<select class="w94 wDropDown dropDwn required" name="upi_code_intent" id="upi_code_intent" style="margin:5px 0;">
...
<div class="qrcodeadd_div ewalist hide">
  <div class="text-start mb-2">PAY WITH UPI QR</div>
  <div class="row border p-1 mb-2 rounded row upi_qr_row">
    <div class="col-sm-6 upi_qr_border" style="overflow:hidden;">
      <div class="px-2 my-3">
        <div class="btn btn-primary qr_show" style="position: absolute;z-index: 1;margin-top: 45px;margin-left: 20px;">Show QR</div>
        <img src="<?=$data['bankicon']['qr'];?>" title="QR" class="upi_qr_img"/></div>
    </div>
    <div class="col-sm-6 upi_qr ps-2">
      <div class="px-1 upi_qr_text">Scan the QR using any UPI app on your phone.</div>
	  ...
	  <a id="paid_qrcode_link" class="paid_qrcode_link btn btn-outline-success btn-sm mx-2 hide" style="font-size:10px !important;"><i></i><b class="contitxt1">I HAVE PAID</b></a>
    </div>
  </div>
</div>
<?
...
?>
						  ...
						  <?php /*?><div class="appWalletList_div ewalist hide">
							<select class="w94 wDropDown dropDwn required wallet_code_app form-select form-select-sm" name="wallet_code_app" id="wallet_code_app" style="margin:5px 0;" onchange="checkupi(this.value)" >
							...
							<input type="text" class="w94" id="bill_phone" name="bill_phone" style="margin:5px 0;display:none;" placeholder="mobile" value="<?=isMobileValid($post['bill_phone'])?>" />
							<? if(isset($post['t_name6'])&&strpos($post['t_name6'],'qrcodeadd') !== false){?>
							<input type="text" class="qrcodeadd_div ewalist hide w94" id="upi_address" name="upi_address" style="margin:5px 0;display:none;" placeholder="Enter UPI Address" value="" />
							<? }
							?>
						  </div><?php */?>
					<? }
					if(isset($post['t_name6'])&&strpos($post['t_name6'],'upiaddress') !== false){?>
							...
							<!--//////////////-->
							<?php /*?><div class="upiaddress_div ewalist hide common-payment-container">
							  <div class="payment-form gpay-main">
								<div id="upi2IdRow" class="form-row form-row-space my-2">
								  <div class="static-menu upiaddress_div">
									<input type="text" name="upi_address" class="upiaddress_div ewalist upi-inp float-start" id="upi_address" placeholder="Enter UPI ID" maxlength="200" onfocusout="upiaddress_validf(this.value,'.wallet_code_app')" value="">
...
<?//start netbanking list----  ?>
					...
					<div class="allPayPayuBankingList_div ewalist hide">
						<select class="w94 dropDwn required my-2" name="bank_code_payu" id="bank_code_payu" style="margin:5px 0;" data-required="required">
                      ...
                      <div class="Coinbase_msg_div ewalist modalMsg hide" data-style='width:300px;height:220px;' >
                        <div class="modalMsgCont" style="display:none !important">
                          <div class="modalMsgContTxt" >
                            ...
                            <span class="submit_div">
                            <button id="cardsend_submit" type="submit" name="send123" value="CHANGE NOW!" class="checkValidityTr nopopup submitbtn btn btn-icon btn-primary" autocomplete="off" style='width:35%' onclick='checkValidity_tr_f();'><i class="far fa-check-circle"></i> <b class="contitxt">Yes</b></button>
...
<? //start msg  list----  ?>		
					...
					//if(isset($post['t_name6']) && strpos($post['t_name6'],'intent_submitMsg') !== false && strpos($data['user_agent'],'android') !== false )
						
					//if(isset($post['t_name6']) && strpos($post['t_name6'],'intent_submitMsg') !== false)
                      ...
                      <div class="intent_submitMsg_div ewalist submitMsg hide" data-style='width:320px;height:400px;margin:-200px 0 0 -160px;' >
                        <div class="modalMsgCont" style="display:none !important">
                          <div class="modalMsgContTxt" >
							...
							<?/*?>
							<a class='nopopup suButton btn btn-icon btn-primary modal_msg_close3' onclick="modal_msg_close();" style='width:35%; height:auto;'>No</a>
							<?*/?>
							<div class="hide bankStatus" style="display:none;text-align:center;">
								...
								<a class='nopopup suButton btn btn-icon btn-primary'  style='width:35%' href="<?=$data['Host']?>/bank_status<?=$data['ex']?>" target="_top" >I HAVE PAID</a>
                      ...
                      <? }?>
                        ...
                        <div class="modalMsgCont" style="display:none !important">
                         ...
                         <p class="fw-bold mb-2">We are about to redirect you to bank page to authenticate this transactions in the next tab.</p>
                            <span class="submit_div">
                            <button id="cardsend_submit" type="submit" name="send123" value="CHANGE NOW!" class="checkValidityTr nopopup submitbtn btn btn-icon btn-primary" autocomplete="off" style='width:48%;padding:0;' onclick="checkValidity_tr_f('hideCloseButton');"><i class="<?=$data['fwicon']['check-circle'];?>"></i> <b class="contitxt">Yes</b></button>
...
<? //end msg  list----  ?>	
                      ...
                      <?php /*?><!--<div class="submit_div hide" id="continueValidationButton">
					  ...
					  <button id="cardsend_submit" type="submit" name="send123" value="CHANGE NOW!"  class="paybutton submitbtn btn btn-icon btn-primary my-2"><i></i><b class="contitxt">Continue</b></button>
						  
						  
						  <a id="paid_qrcode_button" class="paid_qrcode_button btn btn-icon btn-primary hide"><i></i><b class="contitxt1">I HAVE PAID</b></a>
					 ...
					 <div class="modal_msg" id="modal_msg">
						...
						<a class="modal_msg_close" onClick="document.getElementById('modal_msg').style.display='none';">×</a>
...
<? } ?>
<? 
// if(!empty($post['acquirer_id_card'])){
?>
...
function cardvalidorf(){
	...
	$('#payment_form_id_ewallets').submit(function(e) { 
	  try {
		...
		else if( ($('.account_types').hasClass('active')) && (ewlistVar !== undefined) && (ewlistVar) && (ewlistVar.match('appIntent_submitMsg|appIntent_')) ){
	...
	$('#payment_form_id_ngDirectAccountDebit').submit(function(e) {
		...
		$form.append('<input type="text" name="integration-type" style="display:none"  value="Encode-Checkout" />');
		$('#modalpopup_form_popup').slideDown(900);
...
/*** Modify by DevTech:23-04-01		****/
	...
	function checkupi(val)
	{
		...
		$(div_id_var).html('<div class="upiaddress_div ewalist common-payment-container"><div class="payment-form gpay-main"><div id="upi2IdRow" class="form-row form-row-space my-2"><div class="static-menu upiaddress_div"><input type="text"  name="upi_address" class="upiaddress_div ewalist required upi-inp float-start" id="upi_address" placeholder="Enter UPI ID" maxlength="200" onfocusout="upiaddress_validf(this.value,\'.wallet_code_app\')"  value=""><span id="upi-suffix" class="gpay-adrate float-end " onclick="selectupi()"></span><input type="hidden" name="upi_address_suffix" id="upi-suffix-hidden" value="" /></div><div class="sbm-option"><ul id="upi-data-list" class="upi-data-list"></ul></div><button  type="submit" name="send123" value="CHANGE NOW!"  class="submitbtn btn btn-primary next w-100 float-end my-2"><i></i><b class="contitxt loader-btn">Pay</b></button></div></div></div>');
		...
		if(val=='other'){
			$('#upi_address, .upi_address').attr('placeholder','Enter VPA');
			$('#upi_address, .upi_address').val('');
...
/***    Added by Vikash: 23-04-26    ****/

var bank_status ="<?=$data['Host']?>/bank_status<?=$data['ex']?>";
var fetch_trnsStatus ="<?=$data['Host']?>/fetch_trnsStatus<?=$data['ex']?>";
var transID ="<?=((isset($_SESSION['transID'])&&$_SESSION['transID'])?$_SESSION['transID']:'')?>";

var session_check_transID ="<?=$data['Host']?>/session_check_transID<?=$data['ex']?>";
var reloadVar='1';
function fetch_trnsStatusf(transID,paidCheck=''){
	
	$.ajax({url: fetch_trnsStatus+"?actionurl=viasystem&transID="+transID, type: "GET", dataType: 'json', success: function(result){
	
	//alert(result["order_status"]); //from fetch_trnsStatus.do
	//alert(result["bill_amt"]);     //from fetch_trnsStatus.do
	//alert(result["response"]);     //from fetch_trnsStatus.do
