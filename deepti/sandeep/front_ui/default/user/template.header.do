...
<nav class="navbar navbar-expand-lg navbar-light bg-light5 bg-primary text-white">
  <div class="container">
	...
	<? if(isset($_SESSION['DB_CON_NAME'])&&$_SESSION['DB_CON_NAME']){ ?>
...
more_db_conf('menu-summary','');?> 
 ...
 <? 
//Dev Tech : 23-11-23 switch for more db instance 
if(isset($data['DB_CON'])) 
{
	if(strpos($data['urlpath'],"?")!==false) $switch_location=$data['urlpath']."&DB_CON=";
	else $switch_location=$data['urlpath']."?DB_CON=";
		
	foreach($data['DB_CON'] as $k4 => $v4){ 
	 //print_r($v4);echo "<br/><br/>";
	?>
	<li><a class="dropdown-item menu-summary" href="<?=$switch_location?><?=$k4;?>"><i class="fa-solid fa-database"></i>  <?=$v4['NAME'];?></a></li>
<? }
} ?>
