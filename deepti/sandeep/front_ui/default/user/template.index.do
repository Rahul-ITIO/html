...
if(!empty($join_additional)) $mts="`ad`";
      ...
      $result_select=db_rows(
			...
			" {$join_additional} WHERE `t`.`acquirer`='{$_SESSION['uid']}' AND `t`.`trans_status` in (1) AND {$mts}.`bill_country`!=''   GROUP BY {$mts}.`bill_country` HAVING COUNT(`t`.`id`) > 0",0
		);
