...
?>
  <div class="text-end">
    ...
    <a href="<?=$data['Admins'];?>/merchant<?=$data['ex']?>?id=<?=$post['MemberInfo']['id']?>&action=insert_stores&type=<?=$post['type']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>" class="btn btn-icon btn-primary" title=" Add A New <?=($store_name);?>" ><i class="<?=$data['fwicon']['circle-plus'];?>"></i></a>
    <? } ?>
  </div>
  <? if($data['Products']){ ?>
    ...
    <? $idx=0;$k=0;foreach($data['Products'] as $value){$k++; ?>
    <tr>
	  ...
	  ?>
	  <a href="<?=$data['USER_FOLDER']?>/<?=($store_url);?><?=$data['ex']?>?admin=1&mid=<?=$post['MemberInfo']['id']?>&id=<?=$value['id']?>&action=view&bid=<?=$post['MemberInfo']['id']?>" title="View <?=($store_name);?> Details" class="modal_for_website badge rounded-pill bg-primary text-white"><?=($value['id'])?></a>
	  <? } else { ?>
	  <a title="View <?=($store_name);?> Details" class="badge rounded-pill bg-primary text-white"><?=($value['id'])?></a>
	  <? } ?>
	  </td>
	  
      <td><? if($value['api_token']<>""){ ?><a data-href="<?=($value['api_token'])?>" title="Copy API Token - <?=($value['api_token'])?>" id="apic_<?=$k;?>"><i class="<?=$data['fwicon']['copy'];?>" onclick="ctcf('#apic_<?=$k;?>','data-href')"></i> </a><? } ?></td>
...
if(((isset($_SESSION['login_adm']))||(isset($_SESSION['q_website_sold'])&&$_SESSION['q_website_sold']==1)) ){
?>
      <td data-label="Sold : " nowrap><a class="restartfa" onClick="ajaxf1(this,'<?=$data['Admins']?>/merchant<?=$data['ex'];?>?action=transcount&mid=<?=$post['MemberInfo']['id']?>&id=<?=$value['id']?>','#tracount_<?=$value['id']?>')" ><i class="<?=$data['fwicon']['rotate'];?>"></i></a>
...
<? if((isset($_SESSION['login_adm']))||(isset($_SESSION['merchant_action_add_stores'])&&$_SESSION['merchant_action_add_stores']==1)){ ?>
        <li><a href="<?=$data['Admins'];?>/merchant<?=$data['ex']?>?bid=<?=$value['id'];?>&id=<?=$post['MemberInfo']['id']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>&action=update_stores&type=<?=$post['type']?>" title="Edit" class="dropdown-item"><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a></li>
		
		
		<li><a href="<?=$data['Admins'];?>/merchant<?=$data['ex']?>?bid=<?=$value['id'];?>&id=<?=$post['MemberInfo']['id']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>&action=delete_stores&type=<?=$post['type']?>" onclick="return cfmform()" title="Delete" class=" dropdown-item"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a></li>
	  
	  <li><a href="<?=$data['USER_FOLDER']?>/generate<?=$data['ex']?>?admin=1&bid=<?=$post['MemberInfo']['id']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>&id=<?=$value['id']?>&action=store" title="Generate Code" class="modal_for_website dropdown-item"><i class="<?=$data['fwicon']['code'];?>"></i></a></li>
        <? } ?>
        ...
        <? if((isset($_SESSION['login_adm']))||(isset($_SESSION['merchant_action_add_stores'])&&$_SESSION['merchant_action_add_stores']==1)){ ?>
        <a href="<?=$data['Admins'];?>/merchant<?=$data['ex']?>?bid=<?=$value['id'];?>&id=<?=$post['MemberInfo']['id']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>&action=update_stores&type=<?=$post['type']?>" title="Edit" class="mx-1"><i class="<?=$data['fwicon']['edit'];?> text-success"></i></a>&nbsp;<a href="<?=$data['Admins'];?>/merchant<?=$data['ex']?>?bid=<?=$value['id'];?>&id=<?=$post['MemberInfo']['id']?><?=((isset($data['is_admin_link'])&&$data['is_admin_link'])?$data['is_admin_link']:'');?>&action=delete_stores&type=<?=$post['type']?>" onclick="return cfmform()" title="Delete" class="mx-1"><i class="<?=$data['fwicon']['delete'];?> text-danger"></i></a>&nbsp;
...
<!--  /////////// Start Modal Trans /////////////////-->
<div class="modal" id="myModal_web">
