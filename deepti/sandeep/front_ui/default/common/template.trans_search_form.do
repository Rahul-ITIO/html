...
<input type="text" name="searchkey" class="searchkey_adv form-control form-control-sm my-1 bg-primary text-white" placeholder="Search.." value='<?=@$post['searchkey']?>'></div>
...
if(isset($data['NameOfFile']) && (isset($post['settlement_optimizer'])&&trim($post['settlement_optimizer'])&&$post['settlement_optimizer']=='manually')){
	?>
		 ...
		 { ?>
			<a href="<?=$data['USER_FOLDER']?>/<?=$data['FileName']?>?balance_check=re_check"   class="settle_withdraw btn btn-primary my-1 text-white btn-sm float-end ms-2" style="" title="Refresh for Account Balance">Refresh</a>
			
			<a href="<?=$data['USER_FOLDER']?>/trans_withdraw-fund<?=$data['ex']?>" onClick="javascript:$('#modalpopup_form_popup').show(200);"  class="settle_withdraw btn btn-primary my-1 text-white btn-sm float-end" style="" title="Withdraw Fund">Withdraw</a>
          <? }else{ ?>
          <a onClick="confirm_2mfa('You are required to Activate 2 Factor Authentication (2FA) to access the withdrawal section. Click on OK button to setup 2 Factor Authontication (2FA) now.','<?=$data['USER_FOLDER']?>/two-factor-authentication<?=$data['ex']?>');" class="settle_withdraw2 btn btn-primary btn-sm text-white my-1" style="background-color: #00b517 !important;color:#fff!important;float:right;display:inline-block;margin:2px 10px 0 0;" title="Withdraw Fund">Withdraw</a>
          <? } ?>
<? }?>
