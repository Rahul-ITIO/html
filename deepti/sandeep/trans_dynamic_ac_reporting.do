<?
$data['PageName']	= 'PDF STATEMENTS - Fee Dynamic from Account';
$data['PageFile']	= '';
###############################################################################
include('config.do');
###############################################################################
	...
	foreach($post['msInfo'] as $key=>$value){
		if(($value['acquirer_processing_mode']==1)&&($value['acquirer_id'])) {
			...
			if($value['virtual_fee']){
				$virtual_terminal_fee=prnsum2($virtual_terminal_fee)+prnsum2($value['virtual_fee']);
			}
...
$settelement_delay=$post['AccountInfo'][0]['settelement_delay'];
...
$wmp=withdraw_max_prevf($uid,$post['gid'],1);
if($qprint){
	print_r($wmp);
}
//print_r($wmp['transID']);
...
if($post['step']==1){
			...
			*/
			...
			//Select Data from master_trans_additional
			$join_additional2=join_additional('i');
			if(!empty($join_additional2)) $mts="`ad`";
			else $mts="`t`";


			$payout_mature=db_rows(
				"SELECT `t`.* , {$mts}.`acquirer_json` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` AS `t`".
				" {$join_additional2}  WHERE (`merID`={$uid} )  AND (trans_status NOT IN (0,9,10)) ".$account_type_whr.
			...
			if(empty($terNO)){
				$transactions_dis=db_rows(
					"SELECT GROUP_CONCAT(DISTINCT `terNO`) AS `terNO` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}`".
			...
			$moto_status=$post['AccountInfo'][0]['moto_status']; 
...
//end: class	
		...
		foreach($post['payout_mature'] as $key=>$value){
			...
			$acquirer_json=jsondecode($value['acquirer_json'],1,1);
			...
			$acMS=@$ms_get[0];
			if((!isset($acMS)||!is_array($acMS))&&(isset($acquirer_json)&&is_array($acquirer_json)&&count($acquirer_json)>0)){
				$acMS=$acquirer_json;
			}
			elseif((!isset($acMS)||!is_array($acMS))){
				$acqTbl=select_tablef("`acquirer_id` IN ({$value['acquirer']})",'acquirer_table',0,1,'`mer_setting_json`');
				$acMS=jsondecode(@$acqTbl['mer_setting_json'],1);
			}
			
			//print_r($acMS); echo $value['transID']; exit;
			...
			if(($value['trans_status']==1 || $value['trans_status']==4 || $value['trans_status']==7 || $value['trans_status']==8)&&($value['trans_type']==11)){ // completed
                ...
                $dis_rate 	= number_formatf(((double)$value['trans_amt']/100)*(double)$acMS['mdr_rate']);
				$rol_fee	= number_formatf(((double)$value['trans_amt']/100)*(double)$acMS['reserve_rate']);
				
				
				if($is_gst_fee&&isset($value['gst_amt'])&&$value['gst_amt']&&$dis_rate){
				...
				$gst_mdr_txtfee_amt=(double)$value['buy_txnfee_amt'];
				$this_total	= number_formatf2(((double)$value['trans_amt'])-($dis_rate+(double)$acMS['txn_fee_success']+$rol_fee));
				
				$total_txn_fee = $total_txn_fee+(double)$acMS['txn_fee_success']+$gst_mdr_txtfee_amt;
				
				$total_dis_rate = $total_dis_rate + $dis_rate;
				$total_rol_fee	= $total_rol_fee + $rol_fee;
				
				$sales_volume	= number_formatf2($sales_volume+(double)$value['trans_amt']);
				
				$completed_data[] .= $value['transID']."/".$trans_type.";".$value['fullname'].";".number_formatf2($value['trans_amt']).";".number_formatf2($dis_rate).";".number_formatf2($gst_mdr_txtfee_amt).";".number_formatf2($rol_fee).";".number_formatf2($this_total)."";
			}
			if(($value['trans_status']==2)&&($value['trans_type']==11)){ // canceled
				
                $count_failed=$count_failed+1;

                $dis_rate 	= "0";
				$rol_fee	= "0";
				//$this_total	= number_formatf($acMS['txn_fee_success']);
				
				if($acMS['txn_fee_failed']){
					$this_total=($acMS['txn_fee_failed']);
				}else{
					$this_total=($acMS['txn_fee_success']);
				}
				
				$total_txn_fee = number_formatf($total_txn_fee+$acMS['txn_fee_success']);
				
				$cancelled_data[] .= $value['transID'].";".$value['fullname'].";".number_formatf2($value['trans_amt']).";".$dis_rate.";".$this_total.";".$rol_fee.";-".number_formatf($this_total)."";
			...
			if(($value['trans_status']!=2)&&($value['trans_type']==15)){ // canceled
				...
				$withdraw_data[] .= $value['transID'].";".$value['fullname'].";".number_formatf2($value['bill_amt']).";".$dis_rate.";".$value['buy_txnfee_amt'].";".$rol_fee.";".number_formatf2($value['bill_amt'])."";
			...
			*/
			...
			if($value['trans_status']==3 || $value['trans_status']==12){ // Refunded and Partial Refund
				...
				$refund_fee=number_formatf($acMS['refund_fee']);
				
				$dis_rate 	= "0";
				$rol_fee	= number_formatf(($value['trans_amt']/100)*$acMS['reserve_rate']);
				$this_tr_fee= $refund_fee;
				
				
				$this_total	= number_formatf2($value['trans_amt']-($refund_fee-$rol_fee)); // 45
			...
			if($value['trans_status']==11){ // cbk1 
				...
				$cbk1_fee=number_formatf($acMS['cbk1']);
			...
			if($value['trans_status']==5){ // Chargeback 
				...
				$this_total	= number_formatf2($value['trans_amt']-($charge_back_fee-$rol_fee)); // 45
			...
			if($value['trans_status']==6){ // Returned 
				...
				$this_total	= number_formatf2($value['trans_amt']-($return_back_fee-$rol_fee)); // 45
...
$data_profie 	= array();
//if($terNO){
	$data_profie[] 	.= "  TerNO;:  ".$terNO."; Withdrawal ID;:  ".$wd_tran[0]['transID']."";
//}
...
if($monthly_maintance_fee){
	...
	$total_fees	= number_formatf2($total_dis_rate+$total_txn_fee+$total_refund_fee+$total_chargeback_fee+$total_returns_fee+$total_cbk1_fee+$monthly_maintance_fee+$virtual_terminal_fee+$settelement_wire_fee);
/*	
if($is_gst_fee){
	$gst_total_fees_amt=number_formatf2((($total_fees*$gst_fee)/100),4);
}
*/
$sum_other_deductions=number_formatf2($refund_volume+$chargeback_volume+$returns_volume+$cbk1_volume+$total_rol_fee);
...
if(empty($ptdate)){
	if($wd_status==2){
		...
		$data['Error']="Withdrawal trans type is missing";
