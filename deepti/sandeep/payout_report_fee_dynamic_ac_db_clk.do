...
$wmp=withdraw_max_prev($uid,$post['gid'],1);
...
if($data['con_name']=='clk'||$wd_json_value['gst_fee_applicable']){
	$is_gst_fee=1;
...
if($wd_json_value['min_settelement_period']<1){
	$time_from=" H:i:s";
	$time_to=" H:i:s";
}else{
	$time_from=" 00:00:00";
...
$wd_created_date_prev=date("Y-m-d{$time_from}",strtotime("0 day",strtotime($wmp['tdate'])));
...
$previous_wd_transaction_id=$wd_json_value['previous_wd_transaction_id'];
$remaining_balance =$wd_json_value['remaining_balance '];
...
$wd_created_date=date("Y-m-d{$time_to}",strtotime("0 day",strtotime($wd_created_date)));
...
if($post['step']==1){
			...
			$payout_mature=db_rows(
				"SELECT * FROM `{$data['DbPrefix']}transactions`".
				" WHERE (`receiver`={$uid} OR `sender`={$uid} )  AND (status NOT IN (0,9,10)) ".$account_type_whr.
...
//end: class	
		...
		foreach($post['payout_mature'] as $key=>$value){
			...
			if(($value['status']==1 || $value['status']==4 || $value['status']==7 || $value['status']==8)&&($value['trname']=='cn'||$value['trname']=='ch')){ // completed
                ...
                $dis_rate 	= number_formatf(($value['transaction_amt']/100)*$accounts[0]['transaction_rate']);
				$rol_fee	= number_formatf(($value['transaction_amt']/100)*$accounts[0]['rolling_fee']);
				
				
				$gst_mdr_txtfee_amt=(double)$value['mdr_txtfee_amt'];
				$this_total	= number_formatf2(($value['transaction_amt'])-($dis_rate+$accounts[0]['txn_fee']+$rol_fee));
				
				$total_txn_fee = $total_txn_fee+$accounts[0]['txn_fee']+$gst_mdr_txtfee_amt;
				...
				$completed_data[] .= $value['transaction_id']."/".$trname.";".$value['names'].";".number_formatf2($value['transaction_amt']).";".number_formatf2($dis_rate).";".number_formatf2($gst_mdr_txtfee_amt).";".number_formatf2($rol_fee).";".number_formatf2($this_total)."";
			}
			if(($value['status']==2)&&($value['trname']=='cn'||$value['trname']=='ch')){ // canceled
                ...
                $dis_rate 	= "0";
				$rol_fee	= "0";
				//$this_total	= number_formatf($accounts[0]['txn_fee']);
				
				if($accounts[0]['txn_fee_failed']){
					$this_total=($accounts[0]['txn_fee_failed']);
				}else{
					$this_total=($accounts[0]['txn_fee']);
				}
				
				$total_txn_fee = number_formatf($total_txn_fee+$accounts[0]['txn_fee']);
			...
			if($value['status']==3 || $value['status']==12){ // Refunded and Partial Refund
				...
				$refund_fee=number_formatf($accounts[0]['refund_fee']);
			...
			if($value['status']==11){ // cbk1 
				...
				$cbk1_fee=number_formatf($accounts[0]['cbk1']);
...
if($monthly_maintance_fee){
	$data_row_fee[] 	.= "Monthly Maintance Fee @{$profile[0]['monthly_fee']}x{$wd_json_value['wd_total_month_no']} ;$monthly_maintance_fee;$account_cur"; // Fist Week of Month only 
...
if($is_gst_fee){
	$gst_total_fees_amt=(double)(($total_fees*$gst_fee)/100);
}
...
$tpa			= ($sales_volume)-($total_fees+$gst_total_fees_amt+$sum_other_deductions);
...
if($is_gst_fee){
	$data_gst = $pdf->LoadData(array("GST Fee {$total_fees}@{$gst_fee}%;".number_formatf2($gst_total_fees_amt).";$account_cur"));
...
if($previous_wd_transaction_id){
	...
	$data = $pdf->LoadData(array("Due from Withdrawal ID  ".$previous_wd_transaction_id." ;".$available_payout_amt_only.";$account_cur")); 
	$pdf->ImprovedThreeColTable($data,'',0,true);
...
$html = '<p>Error of the system generated statement should be reported by the merchant asap. MSP hold the right to adjust the difference manually.</p>';
		...
		//$pdf->Image('logo.png',10,50,30,0,'',$data['SiteName']);
...
if($data['Error']){
	...
	if($json_access==false){
		...
		}else{ //echo"<script>alert('Fail to download the report please email support@{$data['Host']} for assistance.');</script>";
		}
		//$pdf->Output();
	}else{
