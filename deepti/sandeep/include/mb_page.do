<? /* if($post['mb']){ ?>	
<div class="row100 funda row" style="height:inherit;padding:10px 0 5px 0;">
	<h6 style="clear:both;">Total Batch Balance <span style="margin: 0;top:-11px;" class="btn-action single glyphicons circle_question_mark" data-toggle="tooltip" data-placement="top" data-original-title="It runs the calculation through CRON job and runs one time in 24 hs. Call the calculations are done and saved in a table. It saves the one record for one date, ie. sum of a date in one record"><i></i></span><?if(isset($_SESSION['login_adm'])){?> <a href="<?=$data['Admins'];?>/daily_trans_statement.do?action=merchant_date_wise&mid=<?=$post['bid']?>&update=1" target="hform" class="btn btn-icon btn-primary circle_plus" style="padding: 0 10px;">Update </a><?}?> </h6>
		<a href="" class="span3 col-sm-6 my-2" title="Ready to Withdraw Fund - You can request a fund">Mature Fund <b><?=$post['mb']['count_mature'];?> = <?=$post['mb']['summ_mature'];?></b></a>
		<a href="" class="span3 col-sm-6 my-2" title="Total Available Balance (Sum of Mature and Immature Fund)">Account Balance <b><?=$post['mb']['count_total'];?> = <?=$post['mb']['summ_total'];?></b>
		</a>
		<a href="" class="span3 col-sm-6 my-2" title="This fund will be available to withdraw after the maturity">Immature Fund <b><?=$post['mb']['count_immature'];?> = <?=$post['mb']['summ_immature'];?></b></a>
		<span class="span3 col-sm-6 my-2"><a <? if(isset($_SESSION['login_adm'])){?> href="transactions.do?<? if($post['bid']){?>bid=<?=$post['bid']?><? } ?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? } ?>&type=2&status=-1&action=select" <? } ?> title="Sum of All Previous Withdraw" >Total Withdraw Made</a> <b><?=$post['mb']['count_withdraw'];?> = <a <? if(isset($_SESSION['login_adm'])){?>  onclick="iframe_openf(this);" data-mid="<?=$_GET['bid']?>" data-href="" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/user/withdraw.do?admin=1" <? } ?> title="Withdraw Fund" class="glyphicons database_plus"  ><?=$post['mb']['summ_withdraw'];?></a></b></span>
	</div>
	<div class="row100 funda row" style="width:100%;padding:0px 0 5px 0;">
		...
		<span title="MB (Sum of Mature Balance) = Sum of Total Payable Amount Till Date">
			MB = <?=$post['mb']['summ_mature_1'];?>
		</span>
		|
		<span title="MF (Sum of Monthly Fee) = Fee applicable by 1st of every month ">
		MF = <?=$post['mb']['count_monthly_vt_fee'];?> X <?=$post['mb']['monthly_vt_fee_max'];?> =<?=$post['mb']['monthly_vt_fee'];?> 
		</span>
		|
		...
		<?$post['mb']['mf_ab']=($post['mb']['summ_mature_1']+$post['mb']['summ_received_fund_amt'])-($post['mb']['summ_send_fund_amt']+$post['mb']['monthly_vt_fee']+$post['mb']['summ_withdraw_amt_1']+$post['mb']['manual_adjust_balance']);?>
		MF = (<?=$post['mb']['summ_mature_1'];?> + <?=$post['mb']['summ_received_fund_amt'];?>) - (<?=$post['mb']['summ_send_fund_amt'];?> +  <?=$post['mb']['monthly_vt_fee'];?> + <?=$post['mb']['summ_withdraw_amt_1'];?> + <?=$post['mb']['manual_adjust_balance'];?>)  = <b><?=number_formatf2($post['mb']['mf_ab']);?></b>
		</span>
		| 
		<span title="AB (Account Balance) = MF (Mature Fund) + IF (Immature Fund)    ">
		AB = <?=number_formatf2($post['mb']['mf_ab']);?> + <?=number_formatf2($post['mb']['summ_immature_amt']);?>    = <b><?=number_formatf2($post['mb']['mf_ab']+$post['mb']['summ_immature_amt']);?></b>
		</span>
	...
	<div class="row100 funda row" style="padding:0px 0 5px 0;">
		<a href="" class="span3 col-sm-6 my-2" title="Ready to Withdraw Rolling Fund">Mature Rolling Fund <b><?=$post['mb']['count_mature_roll'];?> = <?=number_formatf2($post['mb']['summ_mature_roll']);?></b></a>
		<a href="" class="span3 col-sm-6 my-2" title="Total Available Rolling Balance (Sum of Mature and Immature Rolling)">Rolling Balance <b><?=$post['mb']['count_total_roll'];?> = <?=number_formatf2($post['mb']['summ_total_roll']);?></b>
		<a href="" class="span3 col-sm-6 my-2" title="This fund will be available to withdraw after the Rolling Maturity">Immature Rolling Fund <b><?=$post['mb']['count_immature_roll'];?> = <?=number_formatf2($post['mb']['summ_immature_roll']);?></b></a>
		<span class="span3 col-sm-6 my-2"><a <? if(isset($_SESSION['login_adm'])){?>  href="transactions.do?<? if($post['bid']){?>bid=<?=$post['bid']?><? } ?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? } ?>&type=3&status=-1&action=select" <? } ?> title="Sum of All Previous Rolling Withdraw">Withdraw Rolling Fund</a> <b><?=$post['mb']['count_withdraw_roll'];?> = <a <? if(isset($_SESSION['login_adm'])){?>  onclick="iframe_openf(this);"  data-mid="<?=$_GET['bid']?>" data-href="" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/user/withdraw_rolling.do?admin=1" <? } ?> title="Withdraw Fund" class="glyphicons database_plus" ><?=number_formatf2($post['mb']['summ_withdraw_roll']);?></a></b></span>
	</div>
...
<? /*if($post['mbt_dt']){?>	
<div class="row100 funda row" style="clear:both;padding:3px 0 5px 0;border-top:1px dashed #ccc;margin-top:3px;">
<h6 style="clear:both;">Total Trans. Balance - Fee Dynamic with Trans.  <span style="margin: 0;top:-11px;" class="btn-action single glyphicons circle_question_mark" data-toggle="tooltip" data-placement="top" data-original-title="(Calculation done at run time. It fetches only the Transaction amount from Transaction table and fetches all respective values from the account setting, calculate related values and show the sum of all values accordingly)"><i></i></span></h6>
		<a href="" class="span3 col-sm-6 my-2" title="Ready to Withdraw Fund - You can request a fund">Mature Fund <b><?=$post['mbt_dt']['count_mature'];?> = <?=$post['mbt_dt']['summ_mature'];?></b></a>
		<a href="" class="span3 col-sm-6 my-2" title="Total Available Balance (Sum of Mature and Immature Fund)">Account Balance <b><?=$post['mbt_dt']['count_total'];?> = <?=$post['mbt_dt']['summ_total'];?></b>
		</a>
		<a href="" class="span3 col-sm-6 my-2" title="This fund will be available to withdraw after the maturity">Immature Fund <b><?=$post['mbt_dt']['count_immature'];?> = <?=$post['mbt_dt']['summ_immature'];?></b></a>
		<span class="span3 col-sm-6 my-2"><a <? if(isset($_SESSION['login_adm'])){?> href="transactions.do?<? if($post['bid']){?>bid=<?=$post['bid']?><? } ?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? } ?>&type=2&status=-1&action=select" title="Sum of All Previous Withdraw" <? } ?>  >Total Withdraw Made</a> <b><?=$post['mbt_dt']['count_withdraw'];?> = <a <? if(isset($_SESSION['login_adm'])){?> onclick="iframe_openf(this);" data-mid="<?=$_GET['bid']?>" data-href="" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/user/withdraw.do?admin=1" <? } ?> title="Withdraw Fund" class="glyphicons database_plus" ><?=$post['mbt_dt']['summ_withdraw'];?></a></b></span>
	</div>
	<div class="row100 funda row" style="width:100%;padding:0px 0 5px 0;">
		<a class="span3 col-sm-6 my-2" style="clear:left;width:100%;float:left;">
		<span title="MB (Sum of Mature Balance) = Sum of Total Payable Amount Till Date">
			MB = <?=$post['mbt_dt']['summ_mature_1'];?>
		</span>
		|
		<span title="MF (Sum of Monthly Fee) = Fee applicable by 1st of every month ">
		MF = <?=$post['mbt_dt']['count_monthly_vt_fee'];?> X <?=$post['mbt_dt']['monthly_vt_fee_max'];?> =<?=$post['mbt_dt']['monthly_vt_fee'];?> 
		</span>
		|
		<span title="MF (Mature Fund) = (MB (Sum of Mature Balance) + RF (Sum of Received Fund))  - (SF (Sum of Send Fund) + MF (Sum of Monthly Fee) + WA (Sum of Withdraw Amount) + ABA (Admin  Balance Adjust))  ">
		<?$post['mbt_dt']['mf_ab']=($post['mbt_dt']['summ_mature_1']+$post['mbt_dt']['summ_received_fund_amt'])-($post['mbt_dt']['summ_send_fund_amt']+$post['mbt_dt']['monthly_vt_fee']+$post['mbt_dt']['summ_withdraw_amt_1']+$post['mbt_dt']['manual_adjust_balance']);?>
		MF = (<?=$post['mbt_dt']['summ_mature_1'];?> + <?=$post['mbt_dt']['summ_received_fund_amt'];?>) - (<?=$post['mbt_dt']['summ_send_fund_amt'];?> +  <?=$post['mbt_dt']['monthly_vt_fee'];?> + <?=$post['mbt_dt']['summ_withdraw_amt_1'];?> + <?=$post['mbt_dt']['manual_adjust_balance'];?>)  = <b><?=number_formatf2($post['mbt_dt']['mf_ab']);?></b>
		</span>
		| 
		<span title="AB (Account Balance) = MF (Mature Fund) + IF (Immature Fund)    ">
		AB = <?=number_formatf2($post['mbt_dt']['mf_ab']);?> + <?=number_formatf2($post['mbt_dt']['summ_immature_amt']);?>    = <b><?=number_formatf2($post['mbt_dt']['mf_ab']+$post['mbt_dt']['summ_immature_amt']);?></b>
		</span>
	...
	<div class="row100 funda row" style="padding:0px 0 5px 0;">
		<a href="" class="span3 col-sm-6 my-2" title="Ready to Withdraw Rolling Fund">Mature Rolling Fund <b><?=$post['mbt_dt']['count_mature_roll'];?> = <?=number_formatf2($post['mbt_dt']['summ_mature_roll']);?></b></a>
		<a href="" class="span3 col-sm-6 my-2" title="Total Available Rolling Balance (Sum of Mature and Immature Rolling)">Rolling Balance <b><?=$post['mbt_dt']['count_total_roll'];?> = <?=number_formatf2($post['mbt_dt']['summ_total_roll']);?></b>
		<a href="" class="span3 col-sm-6 my-2" title="This fund will be available to withdraw after the Rolling Maturity">Immature Rolling Fund <b><?=$post['mbt_dt']['count_immature_roll'];?> = <?=number_formatf2($post['mbt_dt']['summ_immature_roll']);?></b></a>
		<span class="span3 col-sm-6 my-2"><a <? if(isset($_SESSION['login_adm'])){?> href="transactions.do?<? if($post['bid']){?>bid=<?=$post['bid']?><? } ?><? if($post['StartPage']){?>&page=<?=$post['StartPage']?><? } ?>&type=3&status=-1&action=select" <? } ?>  title="Sum of All Previous Rolling Withdraw" >Withdraw Rolling Fund</a> <b><?=$post['mbt_dt']['count_withdraw_roll'];?> = <a <? if(isset($_SESSION['login_adm'])){?>  onclick="iframe_openf(this);"  data-mid="<?=$_GET['bid']?>" data-href="" data-tabname="" data-url="" data-ihref="<?=$data['Host'];?>/user/withdraw_rolling.do?admin=1" <? } ?> title="Withdraw Fund" class="glyphicons database_plus" ><?=number_formatf2($post['mbt_dt']['summ_withdraw_roll']);?></a></b></span>
	</div>
<?
}*/?>
