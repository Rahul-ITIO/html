<?
###############################################################################
$data['PageName']='QR Transactions';
$data['PageFile']='qr_transactions';
$data['PageFileName']='QR Transactions';
###############################################################################
include('../config.do');
$data['PageTitle'] = 'QR Code Statement - '.$data['domain_name']; 
##########################Check Permission#####################################
...
if(isset($post['step'])&&$post['step']==1){
	...
	if((isset($_REQUEST['status'])&&$_REQUEST['status'])&&$_REQUEST['status']!='-1'){
		...
		$sql_query.=" AND `trans_status` = '$search_status' ";		
	}
	...
	if(isset($_REQUEST['time_period'])&&$_REQUEST['time_period']==4){
		...
		$sql_query .=" AND `tdate` like '%{$date_today}%' "; 
	}
	...
	if(isset($_REQUEST['time_period'])&&$_REQUEST['time_period']==1){
		...
		$sql_query .=" AND ( `tdate` BETWEEN (DATE_FORMAT('{$date_1st}', '%Y%m%d%H%i%s')) AND (DATE_FORMAT('{$date_2nd}', '%Y%m%d%H%i%s')) )  ";
	}
...
//		$date_2nd=(date('Y-m-d H:i:s',strtotime($date_2nd. ' + 1 days')));
	...
	else $sql_query.=" AND `acquirer` ='' ";

	
	
	
	$data['ct_query'] = ($sql_query?$sql_query:"");
		
	////////////// for paging /////////////////
	$sqlStmt	= "SELECT count(`id`) AS `ct` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE $sql_query";
	$result_ct	= db_rows($sqlStmt,0);
	$data['total_record']= $result_ct[0]['ct'];
	////////////// End paging /////////////////

    $sqlStmt = "SELECT * FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE $sql_query ORDER BY $p_order_by $limit";
	$result_select= db_rows($sqlStmt,0);
