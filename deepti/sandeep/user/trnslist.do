...
if(basename($_SERVER['PHP_SELF'])=="{$data['trnslist']}.do"){
       if(!clients_page_permission('3',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
}

if(strpos($urlpath,"recent_order")!==false){

if(!clients_page_permission('10',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
...
if(!clients_page_permission('4',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
...
if(!clients_page_permission('15',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
...
if(!clients_page_permission('14',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
...
if(!clients_page_permission('16',$_SESSION['m_clients_role'],$_SESSION['m_clients_type'])){ header("Location:{$data['Host']}/index".$data['ex']);exit; }
	...
	$current_date_time=date('YmdHis', strtotime("-5 minutes"));
...
if($post['action']=='search'){
	...
	update_transaction_ranges($uid, 8, $post['gid'], '', false, false);
}elseif($post['action']=='refund'){
	update_transaction_ranges($uid, 8, $post['gid'], '', true, false);
	$post['action']='select';
}elseif($post['action']=='reminders_range'){
	$trange = $_GET['trange'];
	update_transaction_ranges($uid, 412, $trange, '', false, false);
...
if($post['action']=='select'){
		...
		*/
		...
		$where[]=" (`t`.`merID` IN ({$uid})) AND (  `t`.`related_transID` IS NULL OR `t`.`related_transID`='' OR `t`.`related_transID` LIKE '%\"recent_failed_tid\":%'  OR `t`.`trans_status` NOT IN (2) ) ";
		 
		$reportdate = " AND (  `t`.`related_transID` IS NULL OR `t`.`related_transID`='' OR `t`.`related_transID` LIKE '%\"recent_failed_tid\":%' OR `t`.`trans_status` NOT IN (2)  ) "; 
		...
		if( (isset($_REQUEST['searchkey'])&&isset($_REQUEST['key_name'])&&$_REQUEST['searchkey']&&isset($_REQUEST['key_name'])&&$_REQUEST['key_name'])&&(!isset($_REQUEST['keyname'])||!$_REQUEST['keyname']) ){
			...
			if($kn=='bill_phone'||$kn=='product_name'||$kn=='upa'||$kn=='rrn'||$kn=='acquirer_ref'||$kn=='acquirer_response'||$kn=='descriptor'||$kn=='bin_no'||$kn=='ccno'||$kn=='ex_month'||$kn=='ex_year'||$kn=='trans_response'||$kn=='bill_phone'||$kn=='bill_address'||$kn=='bill_city'||$kn=='bill_state'||$kn=='bill_country'||$kn=='bill_zip'){
				...
				$reportdate .= " AND ( {$mts}.`{$kn}` IN ('{$searchkey}') )";
				$where[]	= " ( {$mts}.`{$kn}` IN ('{$searchkey}') ) ";
			}
			...
			elseif($kn=='transID'||$kn=='bearer_token'||$kn=='reference'||$kn=='acquirer'||$kn=='terNO'||$kn=='channel_type'||$kn=='remark_status'){
				$reportdate .= " AND ( `t`.`{$kn}` IN ({$search_key1}) )";
				$where[]	= " ( `t`.`{$kn}` IN ({$search_key1}) ) ";
			}
			else{
				$searchkey=impf($search_key1,1);
		...
		*/
		...
		if(isset($_REQUEST['date_1st']) && isset($_REQUEST['date_2nd']) && $_REQUEST['date_1st']&&$_REQUEST['date_2nd']){
			...
			if(isset($_REQUEST['settelement'])&&@$_REQUEST['time_period']==5){
			...
			$reportdate .=" AND ( t.tdate BETWEEN (DATE_FORMAT('{$date_1st}', '%Y%m%d%H%i%s')) AND (DATE_FORMAT('{$date_2nd}', '%Y%m%d%H%i%s')) )  "; 
									
			$where[]	= " ( t.tdate BETWEEN (DATE_FORMAT('{$date_1st}', '%Y%m%d%H%i%s')) AND (DATE_FORMAT('{$date_2nd}', '%Y%m%d%H%i%s')) )  "; 
...
*/
	...
	if(isset($_REQUEST['downloadcvs'])){
		$csv_qr="";
		if($post['status']) $csv_qr="  AND ( `t`.`trans_status`={$post['status']} )";
		//echo "<br/>ct_query=>".$data['ct_query'];
		$_SESSION['merchant_csv_qr']=$csv_qr.$reportdate;
		//echo "<br/>merchant_csv_qr=>".$_SESSION['merchant_csv_qr'];
		header("Location:../include/merchant_csv_data".$data['ex']);exit;
	}
	

	$post['Transactions']=mer_trans_list(
		$uid,
		'both',
		-1,
		$post['status'],
