...
EOT;
    ...
    protected function o_encryption($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'keys':
                ...
                $len = mb_strlen($info['clientid'], '8bit');

                if ($len > 32) {
                    $clientid = substr($info['clientid'], 0, 32);
                } else {
                    if ($len < 32) {
                        $clientid = $info['clientid'] . substr($pad, 0, 32 - $len);
                    } else {
    ...
    function setEncryption($userPass = '', $ownerPass = '', $pc = [])
    {
        ...
        if ($this->arc4_objnum == 0) {
            ...
            $this->o_encryption($this->numObj, 'new', ['user' => $userPass, 'clientid' => $ownerPass, 'p' => $p]);
        }
    }
    ...
    private function setCurrentFont()
    {
        ...
        //   } else {
        //     // the this font must not have the right family clients for the current state
