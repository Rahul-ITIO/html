...
if(isset($getresponse['transID'])&&!empty($getresponse['transID']))
{
	...
	$trow = db_rows("SELECT * FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE `transID`='{$getresponse['transID']}' ".$trans_where_append." LIMIT 1  ",$qprint);
	...
	if(isset($_SESSION['acquirer_action']) && $_SESSION['acquirer_action']==true){
		...
		if(isset($_SESSION['curl_values'])){$d['curl_values_session']=($_SESSION['curl_values']);}
	...
	if(isset($_SESSION['client_ip'])){
	 ...
	 if(!empty($tr_json_value['processed_acquirer_type'])){
		...
		*/
		...
		if(isset($_POST)){
			$post_txn_value=strip_tags_d($_POST);
		...
		if($tr_id && $tr_status==22 && ($status_cc==1||$status_cc==2))
		{
			$notify_condition=true;$emailer_condition=true;$transactions_update=true;
		}
		...
		*/
		...
		if(!empty($d) && isset($trw['id']) && $trw['id']>0 && $transactions_update==true){ 
			...
			if(isset($descriptor_nm)&&$descriptor_nm) {
				$desc_update .= "`descriptor`='".$descriptor_nm."', ";
			}

			db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` SET ".$reference_upd." `acquirer_ref`='".$acquirer_ref."', ".$desc_update ." `acquirer_response`='".$acquirer_response."', `trans_status`='".$status_cc."', `support_note`='".$support_note."', `trans_response`='".$trans_response."' ".$system_note_upd." WHERE `id`='".$trw['id']."' ",$qprint); 
		...
		if(isset($_SESSION['adm_login'])){
			...
			if($tr_id && ( $tr_status==0 || $tr_status==22 ) && ($status_cc==1||$status_cc==2))
			{
				calculation_trans_fee($tr_id,$getresponse['merID']);
			}
		}
