<?
###############################################################################
$data['PageName']='vSTATISTIC';
$data['PageFile']='graphv';
//$data['graph']=true;
###############################################################################
include('../config.do');
...
if(isset($_REQUEST['action'])&&$_REQUEST['action']=='getaccountid'){
	...
	$post['websites']=getstoreid($_REQUEST['mid'],3);
	echo showselect($post['websites'], $post['storeid']);
	exit;
}
...
if(isset($_REQUEST['merchant_details'])&&$_REQUEST['merchant_details']){
	...
	if($merchant_req_count==1){
		$memb=select_tablef("`id`='{$merchant_req[0]}'","clients",0,1,"`default_currency`");
