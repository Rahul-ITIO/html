...
if($post['step']==1){
	...
	$sql_query=ltrim($sql_query,"AND");
	if(isset($sql_query)&&trim($sql_query))$sql_query=' WHERE '.$sql_query;
	
	//$data['cqp']=1;
	
	$sqlStmt	= "SELECT COUNT(*) AS `ct` FROM `{$data['DbPrefix']}payout_transaction`  $sql_query";
	$result_ct	= db_rows_2($sqlStmt,$data['cqp']);
	$data['total_record']= $result_ct[0]['ct'];

	$sqlStmt ="SELECT * FROM `{$data['DbPrefix']}payout_transaction`   $sql_query ORDER BY `transaction_date` DESC $limit";
	
	$result_select= db_rows_2($sqlStmt,$data['cqp']);
