...
<? $convert_qrcode_from_url=$data['Host']."/@".prntext(@$post['username']).""; ?>
  <div class="row vkg row clearfix_ice">
	...
	<div class="ro col-sm-12 row float-end"> <span class="float-end d-none d-sm-block text-end" style="width:calc(100% - 125px);"> <a href="<?=@$convert_qrcode_from_url;?>" target="_blank" title="Direct Checkout (UserId and Amount):" class="btn btn-primary my-rpnsive-btn text-break ">Payments Links: <?=@$convert_qrcode_from_url;?></a></span>
	  ...
	  <button title="Your QR code"  data-href="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=<?=urlencode(@$convert_qrcode_from_url);?>&choe=UTF-8" data-value='<?=urlencode(@$convert_qrcode_from_url)?>' class="btn btn-primary my-rpnsive-btn modal_from_url_qrcode mx-1" style="width:35px;"><i class="<?=@$data['fwicon']['qrcode'];?>"></i></button>
...
<div class="btn-group dropstart short-menu-auto-main"> <a data-bs-toggle="dropdown" aria-expanded="false" title="Action" ><i class="<?=@$data['fwicon']['action'];?> text-link"></i></a>
 <ul class="dropdown-menu dropdown-menu-icon pull-right" >
   ...
   <li> <a class="dropdown-item modal_from_url_qrcode" title="Your QR code"  data-href="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=<?=@$convert_qrcode_list;?>&choe=UTF-8" data-value='<?=$convert_qrcode_list?>' copyid="apic<?=@$j;?>" style="width:40px;"><i class="<?=@$data['fwicon']['qrcode'];?>  float-start text-white"></i> <span class="action_menu">QR code</span></a></li>
