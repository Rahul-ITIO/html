...
<div class="container vkg">
	<h1 class="my-2"><i class="fas fa-chart-bar"></i> Analytic</h1>
</div>
...
<? } ?>
		...
		<?php
//echo "<hr/><br/>post=>";print_r($post); echo "<hr/><br/>post2=>";print_r($post2);
		
		if(isset($post['transaction_amount'])&&is_array($post['transaction_amount'])){
			$s_amount_new="[";
	...
	//for merchant wise
	if(isset($post1['transaction_amount'])&&is_array($post1['transaction_amount'])){
		$s_amount_new_mer="[";
	...
	//for store wise
	if(isset($post2['transaction_amount'])&&is_array($post2['transaction_amount'])){
		$s_amount_new_store="[";
...
var activeGraph="Analytic | No. of Result: <?=$post['count_result'];?> | Total Amount: <?=$post['total_amount'];?> | No. of Trans.: <?=$post['ids'];?>";
...
var activeGraph="Analytic (Merchant wise) | No. of Result: <?=$post1['count_result'];?> | Total Amount: <?=$post1['total_amount'];?> | No. of Trans.: <?=$post1['ids'];?>";
...
<script>
var activeGraph="Analytic (Business wise) | No. of Result: <?=$post2['count_result'];?> | Total Amount: <?=$post2['total_amount'];?> | No. of Trans.: <?=$post2['ids'];?>";
...
Highcharts.chart('container_pie', {
  ...
  title: {
    text: 'Analytic (Merchant wise) | Total Merchant: <?=@$post1['count_result'];?> | No. of Trans.: <?=@$post1['ids'];?>',
    align: 'left'
  },
  subtitle: {
    text: '',
...
<a class="btn btn-primary my-1" href="<?=@$data['urlpath'];?>&SEARCH=SEARCH" title="Click for display all Records">View All</a>
<? } ?>
