...
// Store session For Display Payout Menu Added on 07-09-2022 by Vikash
...
?>
     ...
     <div class="dropdown mx-2 mb-1"> <a class="btn btn-primary dropdown-toggle33 border-watch  header_menu_link" data-bs-toggle="dropdown" title="Welcome, <?=@$_SESSION['admin_fullname'];?> [<?=@$_SESSION['sub_username'];?>]"> <i class="<?=$data['fwicon']['circle-user'];?>" aria-hidden="true" ></i> <? if(isset($_SESSION['sub_admin_id']) && $_SESSION['sub_admin_id']){?> <?=@$_SESSION['sub_admin_fullname'];?>  <? }else{ ?> Admin </span><? }?> </a>
...
<li><a class="dropdown-item" href="<?=$data['Admins']?>/glossary<?=$data['ex']?>"><i class="<?=$data['fwicon']['book'];?>  px-1 fa-fw"></i>  Glossary</a></li>
...
<div class="accordion accordion-flush " id="accordionFlushExample">
 <form class='hide1' name='search_bar' id='search_bar' action="<?=$search_form_action;?>" <?=$trans_target;?> novalidate >
  <div class="accordion-item  ">
    <h2 class="accordion-header row btn-primary p-0 pb-1" >
		...
		<div class="collapse multi-collapse col col-sm-10" id="flush-collapseOne2">
			<div  class="col-sm-12 text-start ps-0 pt-2" >
				<div class="start-float" style="font-size: 10px;">Showing Date Range :
					<input type="text" class="form-control col" name="date_range" id="date_range_top" placeholder="Custom Date" value="<?=@$_REQUEST['date_range'];?>" style="width:300px;" >
			...
			<?}?>
				...
				$(document).ready(function(){
				  ...
				  $("#searchkeyname").change(function(){
					...
					<?if(isset($_REQUEST['search_key'])&&is_array($_REQUEST['search_key'])&&(count($_REQUEST['search_key'])>0)&&isset($_REQUEST['SEARCH'])){?>
...
<div style=" width: calc(100% - 40px);float:left;" class="pe-1">
<button class="btn btn-primary w-100" type="submit"  id="SEARCH" name="SEARCH" value="SEARCH"><!--<i class="<?=$data['fwicon']['search'];?> text-white"></i>--> Search</button>
</div>
