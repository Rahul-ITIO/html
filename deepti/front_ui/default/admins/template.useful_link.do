...
<div class="container border my-1 vkg rounded">
	...
	<? if ((isset($data['Error'])&& $data['Error']) && (!$post['change2way'])){?>
		...
		<? if($data['MYWEBSITE']||$_REQUEST['a']){?>	
			...
			<a href="<?=$data['Host']?>/plugins/pay-woocommerce-plugin.zip" target="_blank" title="WP Plugins Download for <?=$data['Host']?> | pay-woocommerce-plugin.zip">pay-woocommerce-plugin.zip</a> 
			<hr />
			<h4>WHMCS Plugins (<?=($data['SiteName']);?>) Download </h4> 
			<a href="<?=$data['Host']?>/plugins/paynext-whmcs.zip" target="_blank" title="WHMCS Download for <?=$data['Host']?> | paynext-whmcs.zip">paynext-whmcs.zip</a> 
			<hr />

			<h4>PrestaShop 1.7 Plugins (<?=($data['SiteName']);?>) Download </h4> 
			<a href="<?=$data['Host']?>/plugins/PaydirectPrestaShop.zip" target="_blank" title="PrestaShop 1.7 Download for <?=$data['Host']?> | PaydirectPrestaShop.zip">PaydirectPrestaShop.zip</a>
	
		<? }?>
	<hr/>
	<h3>Pay via @user with Amount:</h3> 
	<a class="nopopup  mt-2 mb-3 cursor-pointer" onclick="ctcf('#paymeId','href')" style="cursor:pointer;"><i class="<?=$data['fwicon']['copy'];?>"></i></a>
