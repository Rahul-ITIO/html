...
<script src="<?=$data['TEMPATH']?>/common/js/common_use.js"></script>
...
if(isset($_GET['theme'])&&$_GET['theme']){$_SESSION['admin_theme_color']=$_GET['theme'];}
if(isset($_SESSION['admin_theme_color'])&&$_SESSION['admin_theme_color']){$theme_color=$_SESSION['admin_theme_color'];}else{$theme_color="orange";}
...
$heading_bg_color="url('".$data['TEMPATH']."/common/images/banner720.png')";
$heading_text_color="linear-gradient(95deg, #059dff 15%, #6549d5 45%, #e33fa1 75%, #fb5343 100%) 98%/200% 100%;";
...
//echo "<hr/>admin_theme_color=> ".$_SESSION['admin_theme_color'];
?>
...
<input type="hidden" name="unique_id" value="<? if(isset($post['unique_id'])) echo $post['unique_id']?>">
				...
				<div class="widget-head">
					<h4 class="heading glyphicons list"><i></i>Please enter transaction information/details </h4>
				</div>
...
<label for="transaction_id">Transaction Id <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="transaction_id" id="transaction_id" placeholder="" class="form-control <?=$no_input;?>" value="<?=prntext($post['transaction_id'])?>"  />
</div>
...
<? if($data['ThisPageLabel'] == 'Edit Transaction'){ ?>
	<div class="col-sm-6 p-1">
	<label for="sender">Sender <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="sender" id="sender" placeholder="Sender ID" class="form-control" value="<?=prntext($post['sender'])?>"  />
	</div>
<? }?>
<div class="col-sm-6 p-1">
<label for="owner_id">Owner ID <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="owner_id" id="owner_id" placeholder="Owner ID" class="form-control" value="<?=prntext($post['owner_id'])?>"  />
</div>
<div class="col-sm-6 p-1">
<label for="receiver">Receiver/MID <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="receiver" id="receiver" placeholder="Receiver ID" class="form-control" value="<?=prntext($post['receiver'])?>"  />
</div>
...
<label for="type">Account ID <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<select name="type" id="accountType" class="feed_input1 form-select"  required>
<option value=0>--Account ID-- </option>
	<? if($post['type']){ ?>
		<option value="<?=$post['type']?>"><?=$post['type']?> <?=$data['t'][$post['type']]['name1'];?></option>
	<? }?>
<? if($post['AccountInfo']){ foreach($post['AccountInfo'] as $key=>$value){if(($value['nick_name'])){ ?>
<option value="<?=$value['nick_name']?>"><?=$value['nick_name']?> <?=$data['t'][$value['nick_name']]['name1'];?></option>	
<? }}}?>
</select>
<script>
...
<div class="col-sm-6 p-1">					
<label for="accountStatus">Account Status <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<select name="status" id="accountStatus" class="feed_input1 form-select"  required>
...
<script>
$('#accountStatus option[value="<?=prntext($post['status'])?>"]').prop('selected','selected');
</script>
</div>
<? }?>
...
<? if(@$account_type!="check"){?>
	...
	<div class="col-sm-6 p-1">
	<label for="amount">Trxn Amt <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="amount" id="amount" placeholder="$19.99" class="form-control" value="<?=prntext($post['amount'])?>"  />
	</div>
	
	<div class="col-sm-6 p-1">					
	<label for="currname">Currency Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<select name="currname" id="currname" class="feed_input1 form-select"  required>
	<option value="" disabled="" selected="">Currency Name</option>
	...
	<script>
	$('#currname option[value="<?=prntext($post['currname'])?>"]').prop('selected','selected');
	</script>
    </div>
	<div class="col-sm-6 p-1">
	<label for="names">Full Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="names" id="names" placeholder="Full Name" class="form-control" value="<?=prntext($post['names'])?>"  />
	</div>
	<div class="col-sm-6 p-1">
	
	<label for="cardtype">Card Type <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="cardtype" id="product_name" placeholder="Enter Card Type" class="form-control" value="<?=prntext($post['cardtype'])?>"  />
	</div>
...
<? if($data['ThisPageLabel'] == 'Edit Transaction'){ ?>
	...
	<div class="col-sm-6 p-1">
	<label for="related">Related <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="related" id="related" placeholder="" class="form-control" value="<?=prntext($post['related'])?>"  />
	</div>
	<div class="col-sm-6 p-1">
	<label for="fees">Fees <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="fees" id="fees" placeholder="" class="form-control" value="<?=prntext($post['fees'])?>"  />
    </div>
<? }?>

<? if(isset($account_type)&&$account_type=="check"){ ?>
<div class="col-sm-6 p-1">		
<label for="FirstName">First Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="fullname" placeholder="john" class="form-control" value="<? if(isset($post['fullname'])&&$post['fullname']) echo $post['fullname'];
	else prntext($post['fname']).' '.prntext($post['lname']);?>" />
</div>
<?php /*?><div class="col-sm-6 p-1">
<label for="LastName">Last Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="lname" placeholder="joi" class="form-control" value="<?=prntext($post['lname'])?>"  />
</div><?php */?>

<? }?>							
							
<div class="col-sm-6 p-1">
<label for="Address">Address <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="address" placeholder="john joi 3225 NW 23th street Rd  " class="form-control" value="<?=prntext($post['address'])?>"   />
</div>
<div style="display:none;">
<div class="col-sm-6 p-1">
...
<? if(isset($account_type)&&$account_type!="check"){ ?>	
<div class="col-sm-6 p-1">
<label for="country_list">Country <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<div class="ecol2 country_listdiv">
	<select name="country" id="country_list"  class="form-select" autocomplete="false" > 
...
<div class="col-sm-6 p-1">
<label for="state_list">State <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>	
<div id="state_input_divid" class="ecol2">
	<select name="state" id="state_list" class="form-select"  > 
...
<script>
	...
	//alert($(this).val()+"\r\n"+$(this).find('option:selected').text()+"\r\n"+$(this).find('option:selected').attr('data-val'));
	 ...
	 $('#state_input_divid').html('<input type="text" class="form-control" placeholder="State" type="text" id="state_list" name=state size=100 maxlength=128 value="" >');
	 var state_selected='<select name="state" id="state_list" style="font-size:14px" ><option value="" disabled="disabled" selected="selected">Select State</option>';
	...
	$('#country_list option[data-val*="<?=$post['country']?>"]').prop('selected','selected');
	$("#country_list").trigger("change");
	
	setTimeout(function(){
		$('#state_list option[value*="<?=$state_iso2;?>"]').prop('selected','selected');
	}, 1000);
...
<div class="col-sm-6 p-1">
<label for="City">City <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="city" placeholder="Maimi Florida" class="form-control" value="<?=prntext($post['city'])?>"  />
</div>
<? if(isset($account_type)&&$account_type=="check"){ ?>
<?
$pur_state 			= explode('-',$post['state']);	
?>

<div class="col-sm-6 p-1">
<label for="State">State <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<select name="state" id="purchaser_state" class="feed_input1 form-select" >
...
<script>
$('#purchaser_state option[value="<?=prntext($post['state'])?>"]').prop('selected','selected');
</script>

<? }?>
<div class="col-sm-6 p-1">
<label for="PostalCode">Zip Code <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="zip"  placeholder="33124" class="form-control" value="<?=prntext($post['zip'])?>"   />
</div>	
<div class="col-sm-6 p-1">
<label for="Phone">Buyer Phone <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="phone_no" placeholder="425-666-2222"class="form-control" value="<?=prntext($post['phone_no'])?>"   />	
</div>
<div class="col-sm-6 p-1">
<label for="email">Buyer Email <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="email" name="email_add" placeholder="johnjio@gmail.com" class="form-control" value="<?=$post['email_add']?>"   />	
</div>
<? if(isset($account_type)&&$account_type=="check"){ ?>
<div class="col-sm-6 p-1">
<label for="routing">9 Digit ABA No. <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="routing" id="routing" placeholder="987654321" class="form-control" value="<?=$post['routing']?>"   />	
</div>
<div class="col-sm-6 p-1">
<label for="bank_name">Bank Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="bank_name" id="bank_name"  placeholder="Bank of America" class="form-control" value="<?=prntext($post['bank_name'])?>"   />
</div>
<div class="col-sm-6 p-1">
<label for="bank_address">Bank Address </label>
<input type="text" name="bank_address" id="bank_address" placeholder="560 halwkauwila street" class="form-control" value="<?=prntext($post['bank_address'])?>"   />
</div>
<div class="col-sm-6 p-1">
<label for="bank_city">Bank City <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="bank_city" id="bank_city" placeholder="Charlotte" class="form-control" value="<?=prntext($post['bank_city'])?>"   />

<?
$bank_state = explode('-',$post['bank_state']);	
?>
</div>
<div class="col-sm-6 p-1">
<label for="bank_name">Bank State <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<select name="bank_state" id="bank_state" class="feed_input1 form-select" >
<option>--Select Bank State--</option>
<option value="AL-Alabama">Alabama</option>
<option value="AK-Alaska">Alaska</option>
<option value="AZ-Arizona">Arizona</option>
<option value="AR-Arkansas">Arkansas</option>
...
<script>
$('#bank_state option[value="<?=($post['bank_state'])?>"]').prop('selected','selected');
</script>
</div>
<div class="col-sm-6 p-1">
<label for="bank_zipcode">Bank Zip <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="bank_zipcode" id="bank_zipcode" pattern="[0-9]{4,5}" placeholder="66775" class="form-control" value="<?=$post['bank_zipcode']?>"   />	
</div>


<div class="col-sm-6 p-1">
<label for="bankaccount">Account No. <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="bankaccount" id="bankaccount"  placeholder="340123456789" class="form-control" value="<?=($post['bankaccount']);?>"   />	
</div>
<div class="col-sm-6 p-1">
<label for="echecknumber">Check No. <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="echecknumber" id="echecknumber"  placeholder="051000017" class="form-control" value="<?=$post['echecknumber']?>"   />	
</div>
...
<div class="col-sm-6 p-1">
<label for="currname">Currency Name <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<select name="currname" id="currname" class="feed_input1" style="margin:5px 0px;" >
	<option value="" disabled="" selected="">Currency Name</option>
...
<div class="col-sm-6 p-1">
<label for="descriptor" style="clear: left;">Descriptor <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="descriptor" id="descriptor" placeholder="1(800)-000-0000" class="form-control" value="<?=prntext($post['descriptor'])?>"   />
<span style="margin: 0;" class="btn-action single glyphicons circle_question_mark" data-toggle="tooltip" data-placement="top" data-original-title="Your Company Name / 1(800)-000-0000 customer service number"><i></i></span>
</div>
...
<input type="text" name="company_name" id="company_name"  placeholder="IBM" class="form-control" value="<?=prntext($post['company_name'])?>"   />	
</div>

<div class="col-sm-6 p-1">
<label for="employee_number">Employee No. </label>
<input type="text" name="employee_number" id="employee_number"  placeholder="IBM-380300" class="form-control" value="<?=prntext($post['employee_number'])?>"   />	
</div>
<div class="col-sm-6 p-1">
<label for="client_location">Client Location <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="client_location" id="client_location"  placeholder="" class="form-control" value="<?=prntext($post['client_location'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="payable_to">Payable To <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="payable_to" id="payable_to"  placeholder="Company to appear as the receiver of the funds" class="form-control" value="<?=prntext($post['payable_to'])?>"   />
<span style="margin: 0;" class="btn-action single glyphicons circle_question_mark" data-toggle="tooltip" data-placement="top" data-original-title="Company to appear as the receiver of the funds"><i></i></span>
</div>

<div class="col-sm-6 p-1">
<label for="middle_initial">Middle Initial</label>
<input type="text" name="middle_initial" id="middle_initial"  placeholder="" class="form-control" value="<?=prntext($post['middle_initial'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="other_phone_number">Other Ph. No.</label>
<input type="text" name="other_phone_number" id="other_phone_number"  placeholder="" class="form-control" value="<?=prntext($post['other_phone_number'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="bank_phone">Bank Phone</label>
<input type="text" name="bank_phone" id="bank_phone"  placeholder="" class="form-control" value="<?=prntext($post['bank_phone'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="bank_fax">Bank Fax</label>
<input type="text" name="bank_fax" id="bank_fax" placeholder="" class="form-control" value="<?=prntext($post['bank_fax'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="nsf">NSF</label>
<input type="text" name="nsf" id="nsf"  placeholder="" class="form-control" value="<?=prntext($post['nsf'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="tax_id">Tax ID</label>
<input type="text" name="tax_id" id="tax_id"  placeholder="" class="form-control" value="<?=prntext($post['tax_id'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="date_of_birth">Date of Birth</label>
<input type="text" name="date_of_birth" id="date_of_birth"    placeholder="" class="form-control" value="<?=prntext($post['date_of_birth'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="id_number">ID No.</label>
<input type="text" name="id_number" id="id_number"  placeholder="" class="form-control" value="<?=prntext($post['id_number'])?>"   />
</div>

<div class="col-sm-6 p-1">
<label for="id_state">ID State</label>
<input type="text" name="id_state" id="id_state"  placeholder="" class="form-control" value="<?=prntext($post['id_state'])?>"   />
</div>
...
<? if($is_admin==true){ ?>
	...
	<div class="col-sm-6 p-1">
	<label for="notify_url">Notify URL <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="notify_url" id="notify_url" placeholder="Enter Notify URL" class="form-control" value="<?=prntext($post['notify_url'])?>"  />
	</div>
	
	<div class="col-sm-6 p-1">	
	<label for="success_url">Success URL <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="success_url" id="success_url" placeholder="Enter Success URL" class="form-control" value="<?=prntext($post['success_url'])?>"  />
	</div>
	
	<div class="col-sm-6 p-1">	
	<label for="failed_url">Failed URL <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="failed_url" id="failed_url" placeholder="Enter Failed URL" class="form-control" value="<?=prntext($post['failed_url'])?>"  />
	</div>
   
    <div class="col-sm-6 p-1">	
<label for="comments">Comments <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
<input type="text" name="comments" id="comments" placeholder="Enter Comments" class="form-control" value="<?=prntext($post['comments'])?>" />
</div>
...
<input type="text" name="rrn" id="rrn" placeholder="Enter RRN" class="form-control" value="<?=prntext($post['rrn'])?>"/>
	...
	<div class="col-sm-6 p-1">			
	<label for="txn_id">Trxn Id <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="txn_id" id="txn_id" placeholder="Enter Trxn Id" class="form-control" value="<?=prntext($post['txn_id'])?>"  />
	</div>
	
	<div class="col-sm-6 p-1">		
	<label for="txn_value">Trxn Value <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<input type="text" name="txn_value" id="txn_value" placeholder="Enter Value" class="form-control" value='<?=($post['txn_value'])?>'  />
	</div>
	...
	<input type="text" name="product_name" id="product_name" placeholder="Enter Value" class="form-control" value="<?=prntext($post['product_name'])?>"  />
	    ...
	    <div class="col-sm-6 p-1">	
		<label for="mdr_amt">MDR Amt <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="mdr_amt" id="mdr_amt" placeholder="" class="form-control" value="<?=prntext($post['mdr_amt'])?>"  />
		</div>
		
	<? if(isset($data['con_name'])&&$data['con_name']=='clk'){ ?>
		<div class="col-sm-6 p-1">	
		<label for="gst_fee">GST FEE <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="gst_fee" id="gst_fee" placeholder="" class="form-control" value="<?=prntext($post['gst_fee'])?>"  />
		</div>
	<? } ?>	
	
		<div class="col-sm-6 p-1">
		<label for="mdr_txtfee_amt">MDR Trxn Fee Amt <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="mdr_txtfee_amt" id="mdr_txtfee_amt" placeholder="" class="form-control" value="<?=prntext($post['mdr_txtfee_amt'])?>"  />
		</div>
		...
		<div class="col-sm-6 p-1">
		<label for="payout_date">Payout Date <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="date" name="payout_date" id="payout_date" placeholder="" class="form-control" value="<?=date('Y-m-d',strtotime($post['payout_date']));?>"   />
		</div>
		...
		<div class="col-sm-6 p-1">
		<label for="transaction_amt">Trxn Amt <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="transaction_amt" id="transaction_amt" placeholder="" class="form-control" value="<?=prntext($post['transaction_amt'])?>"  />
		</div>
		...
		<div class="col-sm-6 p-1">
		<label for="remark_status">Active <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="active" id="active" placeholder="" class="form-control" value="<?=prntext($post['active'])?>"  />
		</div>
		
		<div class="col-sm-6 p-1">
		<label for="ip">IP <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="ip" id="ip" placeholder="" class="form-control" value="<?=prntext($post['ip'])?>"  />
		</div>
		
		<div class="col-sm-6 p-1">
		<label for="ip">Bank Processing Amount <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		...
		<div class="col-sm-6 p-1">
		<label for="ip">Bank Processing Currency <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
...
<? }?>
<div class="col-sm-6 p-1">
<label for="ip">Table ID <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="tableid" id="tableid" placeholder="" class="form-control" value='<?=($post['tableid'])?>'  />
</div>

<div class="col-sm-6 p-1">
<label for="ip">Json Value <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="json_value" id="json_value" placeholder="" class="form-control" value='<?=($post['json_value'])?>'  />
</div>

<div class="col-sm-6 p-1">	
<label for="ip">Reason <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
		<input type="text" name="reason" id="reason" placeholder="" class="form-control" value='<?=($post['reason'])?>'  />
</div>
		
<div class="col-sm-12 p-1">
<label for="remark" style="clear:left;">Remark/Merchant Note <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<textarea name="remark" id="remark" class="jqte-test form-control" placeholder="Enter Merchant Note"><?=stripslashes($post['remark']);?></textarea>
</div>
<div class="col-sm-12 p-1">
...
<div class="col-sm-12 p-1">
<label for="reply_remark">Support Note <i class="<?=$data['fwicon']['star'];?>  text-danger"></i></label>
	<textarea name="reply_remark" id="reply_remark" class="jqte-test form-control" rows="5" placeholder="Enter Support Note" ><?=stripslashes($post['reply_remark']);?></textarea>
</div>
...
<button type="submit" name="delete" value="CONTINUE"  class="btn btn-primary my-2"  onClick="return confirm('Are you Sure to DELETE this Transaction id is <?=$post['transaction_id'];?>');" ><i class="fa fa-times"></i> Delete</button>
