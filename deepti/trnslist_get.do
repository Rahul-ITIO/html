...
//if $post['action'] is set and has value details
if(isset($post['action']) && $post['action']=='details'){
    ...
    $post['TransactionDetails']=common_trans_detail($post['gid'], $uid);//`common_trans_detail` that fetches transaction details based on the provided `$id` and `$uid`
	...
	if($post['TransactionDetails']['merID']==$uid&&$post['TransactionDetails']['typenum']==2){
		$wmp=withdraw_max_prevf($uid,$post['gid']);//Fetch the last withdrawal detail of a merchant.
