...
if(empty($acquirer_refund_url)){
	$acquirer_refund_url="https://portal.finvert.io/api/refund";
}

if($tr_st['acquirer']==46){
	
	$params['apiKey']=$apc_get['apiKey'];
...
if(isset($qp)&&$qp)
{
	...
	echo "<br/><br/>apc_get via => ";print_r($apc_get);echo "<br/><br/>";
	//exit;
}
...
$params['customer_order_id']=$tr_st['acquirer_ref'];
...
$dataRefund = [
'customer_order_id' => $tr_st['transID'],
'api_key' => $params['apiKey']
];
$requesRefund = json_encode($dataRefund);
			
echo "<br/><hr/>requesRefund=>".$requesRefund; 
//exit;
			...
			curl_setopt($curl,CURLOPT_POSTFIELDS, $requesRefund);
		...
		$refund_qry_db=true;
		//Array ( [status] => success [message] => Refund Updated Successfully! )
		...
		if($refund_qry_db==true){	
			db_query(
				"UPDATE `{$data['DbPrefix']}master_trans_table`".
