...
function makePaymentUpdate($paymentId, $clientReferenceCode) {
    global $merchant_id,$transaction_key,$shared_secret,$profile_id,$account_id;
    $url = "https://apitest.cybersource.com/pts/v2/payments/{$paymentId}"; // Update URL
    $ch = curl_init();
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_CUSTOMREQUEST => "PATCH", // Set the request type to PATCH
        CURLOPT_POSTFIELDS => json_encode($updateData), // Encode the update data as JSON
        CURLOPT_HTTPHEADER => [
            'Content-Type: application/json', // Set the content type to JSON
            'v-c-merchant-id: ' . $merchant_id, // Merchant ID
            'v-c-date: ' . gmdate('D, d M Y H:i:s T'), // Generate the current date
            'digest: ' . generateDigest($updateData), // Generate digest
            'signature: ' . generateSignature($updateData) // Generate signature
        ],
    ]);
...
// Example usage
$paymentId = $transaction_key; // Replace with the actual payment ID
$clientReferenceCode = 'TC50171_3'; // Example reference code

$responseUpdate = makePaymentUpdate($paymentId, $clientReferenceCode);
