...
{ // all pending to be expired 
		...
		db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` SET `trans_status`=22 WHERE `trans_status` IN (0); ",1);
...
{ // acquirer table wise
	...
	db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` t  SET `trans_status` = '22' FROM `{$data['DbPrefix']}acquirer_table` a WHERE t.`trans_status` IN (0)  AND  t.`acquirer` = a.`acquirer_id`  AND  t.`tdate` <= now() - (a.trans_auto_expired * INTERVAL {$qr_interval_2});",1);
...
{ // minute wise 
		...
		db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` SET `trans_status`=22 WHERE `trans_status` IN (0) AND `tdate` <= now() - interval {$qr_interval}; ",1);
...
if((isset($_REQUEST['notAcquirer']))&&($_REQUEST['notAcquirer']))
{
	$notAcquirer=','.$_REQUEST['notAcquirer'];
...

{
	...
	if($rootPhp==""){
		...
		$acquirer_distinct=$acquirer_all[0]['acquirer'];
		if($pq) echo "<br/><br/><hr/><br/><b>ALL ACQUIRER</b>=><br/>".@$acquirer_distinct."<br/><br/><br/><br/>";
		
		$mid_query .= "  AND  ( `acquirer` IN ({$acquirer_distinct}) )  ";
	...
	if($pq) echo "<hr/>count=>".count($slct)."<br/><br/>";
		...
		foreach($slct as $key=>$value){
			...
			if(isset($_REQUEST['notify'])&&$_REQUEST['notify']&&$update_condition==1){
