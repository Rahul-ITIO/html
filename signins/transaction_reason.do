<?
$data['PageName']	= 'Reason Of Transaction';
...
elseif($post['action']=='delete_reasontrs'){
  ...
  json_log_upd($post['gid'],'reason_table','Delete',$dresult,'');
		...
		if($data['connection_type']=='PSQL'){
			db_query(
				"SELECT setval('{$data['DbPrefix']}reason_table_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}reason_table`)+1);",0
			);
		}
