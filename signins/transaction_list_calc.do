...
?>
 <?/* if(((isset($_SESSION['login_adm']))||(isset($_SESSION['clients_risk_ratio_bar'])&&$_SESSION['clients_risk_ratio_bar']==1))&&(!isset($_GET['compare']))){?>

<div class="row100">
  <div id="risk_ratio_id_admin" class="w50" style="width:100% !important">
	
		 
				<? //include("../include/risk_ratio_template".$data['iex']);?>
				
				<a class="restartfa" onClick="ajaxf1(this,'<?=$data['Host']?>/include/ratio_admin<?=$data['ex'];?>?gid=<?=$post['bid'];?>&sponsor=<?=$_GET['sponsor'];?>&dtrange=5','#risk_ratio_id_admin')" style="display: block;text-align:center;">Ratio Realtime <font class="icons glyphicons restart " ><i></i></font></a>
...
?>
	...
	<a class="restartfa" onclick="ajaxf1(this,'<?=$data['Admins'];?>/transaction_list_calc<?=$data['ex'];?>?<?=$sub_quer;?>&compare=1','#direct_account_calculation','1','2')">Direct Account Calculation <font class="icons glyphicons restart "><i></i></font></a>
	<?}?>
	</div>
