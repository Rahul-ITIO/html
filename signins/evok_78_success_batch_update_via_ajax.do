...
#########################################################################
$data['PageName']='78 Evok - Status Update by TransID via ajax';
$data['PageFile']='evok_78_success_batch_update_via_ajax'; 
##########################################################################
...
$data['PageTitle'] = 'Status Update one by one via TransID with comma separated values  - '.$data['domain_name']; 
...
if(isset($_REQUEST['pq'])&&$_REQUEST['pq']) $qp=$_REQUEST['pq'];
 ...
 ########################################################
    ...
    @$admin_id='dev test trans_id_wise_check_status'.@$admin_id;
    
######################################################################

if(isset($_REQUEST['downloadHtml_1'])&&$_REQUEST['downloadHtml_1'])
{
    
    @$_REQUEST['downloadHtml_1']=str_replace(["<br/>","<hr/>"],["\r\n",""],@$_REQUEST['downloadHtml_1']);
    $_REQUEST['downloadHtml']=str_replace(["<br/>","<hr/>"],["\r\n",""],$_REQUEST['downloadHtml']);

    $download_date=(new DateTime())->format('YmdHisu');
    $fileName = $data['SiteName']."_transid_status_log_{$download_date}.txt";
    //echo '<br/>fileName=>'.$fileName;exit;
    header('Content-Encoding: UTF-8');
    header('Content-Disposition: attachment; filename='.$fileName);
    echo @$_REQUEST['downloadHtml_1']."\r\n\r\n\r\n".@$_REQUEST['downloadHtml'];
    //print_r($_REQUEST);
    exit;
...
// subquery url for statu url in javascript 
$subQuery_array=[];
//$subQuery_array['tdateSkip']='tdateSkip';
$subQuery_array['action']='webhook';
$subQuery_array['cron_tab']='cron_tab';
$subQuery_array['cron_host_response']=@$admin_id;
//$subQuery_array['destroy']='2';
$subQuery_array['session_destroy']='N';
    $subQuery_array['admin']='1';
    $subQuery_array['actionurl']='admin_direct';
    $subQuery_array['acquirer_retrun_json_response']='Y';
    $subQuery_array['no_merchant_redirect']='Y';


@$post['subQuery_javascript']='&'.http_build_query($subQuery_array);
@$post['subQuery_jsonEncode']=json_encode($subQuery_array);
...
if(isset($_POST['submit']))
{
    ...
    /*
    if(isset($_REQUEST['trans_ids'])&&trim($_REQUEST['trans_ids']))
    {
        $trans_ids_exp=explode(",",$_REQUEST['trans_ids']);

        foreach($trans_ids_exp as $val){
            if(trim($val)){
                $transID=preg_replace("/[^0-9.]/", "",$val);

                @$_SESSION['t_id_all'][]=@$t_id[]=@$transID;
               // @$_SESSION['transArrayList'][$transID]=@$transID;

                $post['csv_file_on']=1;

                if(isset($transID)&&trim($transID)){
                    db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` SET `trans_status`='22'  WHERE `transID`={$transID} AND `trans_status` NOT IN (1)  ",$qp);
                }
            }
            ...
            while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
            {
                ...
                else
                {
                    ...
                    $pramPost=array();
                        ...
                        $pramPost['amount']	= $transArray['amount']= $filesop[1];
                    $pramPost['rrn']=$transArray['rrn']=@$rrn=$filesop[2]; // rrn
                    $pramPost['transID']=$transArray['transID']=@$transID=preg_replace("/[^0-9.]/", "",$filesop[3]); //Ext Id is our transID 
                    ...
                    $pramPost['txn_id']=$transArray['txn_id']=@$acquirer_ref=@$acquirer_transaction_id=$filesop[10]; // acquirer ref via txn id
                    ...
                    { // not success then update transaction for successfull 
                        ...
                        */
                        ...
                        $return_url_include = "return_url{$data['ex']}?transID={$transID}&action=webhook&session_destroy=N".@$subQuery;
                        ...
                        $jsonArray=[];
                        $jsonArray['transID']=$transID;
                        $jsonArray['return_url']=$return_url;
                        $jsonArray['data_send']=$data_send;
                       // $jsonArray['data_send']=json_encode($data_send);

                        @$_SESSION['transArrayList'][]=@$jsonArray;
                        //@$_SESSION['transArrayList'][]=json_encode(@$jsonArray);
...
if(isset($post['csv_file_on'])&&trim($post['csv_file_on']))
{
     ...
     $post['downloadHtml_1']="<br/><hr/><br/><==Total TransID==><br/>".implode(",",@$_SESSION['t_id_all']);


     $downloadHtml.= $post['downloadHtml_1'];
