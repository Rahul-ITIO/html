...
}elseif($post['action']=='delete'||@$_REQUEST['a']=='r'){

	if(@$_REQUEST['a']=='r'){

		if($data['connection_type']=='PSQL'){
			db_query(
				"SELECT setval('{$data['DbPrefix']}acquirer_table_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}acquirer_table`)+1);",1
			);
		}
		exit;
	}
...
if($post['step']==1){
	...
	if(isset($_GET['active_filter']) && $_GET['active_filter']=="ac"&& isset($_GET['se']) && !empty($_GET['se'])){
		$sponsor_qr .= " ( `acquirer_id` IN ( {$_GET['se']} ) ) ";
		...
		$sponsor_qr .= " ( `select_mcc` LIKE ('%{$_GET['se']}%')  )  ";
	}elseif(isset($_GET['active_filter']) && $_GET['active_filter']=="mcc_code"){
		$sponsor_qr .= " ( CONVERT(`mer_setting_json` USING utf8) LIKE '%{$_GET['active_filter']}%' ) ";
	}elseif(isset($_GET['active_filter']) && $_GET['active_filter']!="ALL"){
		$sponsor_qr .= " ( `acquirer_status` IN ( {$_GET['active_filter']} ) ) ";
	}
