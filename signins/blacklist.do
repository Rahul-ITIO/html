...
if(isset($post['action'])&&($post['action']=='insert_blkdata')){
	...
	{	
		//remove space, tab and new line from json encode value 
		if(!empty($post['blacklist_type'])&&!in_array($post['blacklist_type'],["Country","City"])&&@$post['blacklist_value']) {
			$post['blacklist_value']=str_replace(" ","",$post['blacklist_value']);
			$post['blacklist_value'] = preg_replace('~[\r\n\t]+~', '', $post['blacklist_value']);
		}
		

		$result=get_blacklist_details(1, 'addnew', $post);
		
		if($result=='SUCCESS')
		$_SESSION['action_success']="Your data has been addedd successfully :: ".@$result;
		else 
			$_SESSION['action_error']=$result;
		if(isset($_REQUEST['dtest'])&&$_REQUEST['dtest']==1) exit;
