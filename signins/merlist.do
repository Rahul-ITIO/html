...
if(isset($post['action'])&&$post['action']=='account_manager_subadmin'&&@$post['id']>0)
{
	$account_manager_id_get=select_tablef("`id`='{$post['id']}'","subadmin",0,1,"`account_manager_id`");
	$account_manager_id=$account_manager_id_get['account_manager_id'];
	//echo "DEV==>".$post['id'].", account_manager_id==>".$account_manager_id;
	//Dev Tech: 24-12-30 List for Account manager ID 
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y'&&@$account_manager_id>0)&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1)))
	{
		 $account_manager_option=account_manager_id($account_manager_id,2);
		 echo $account_manager_option;
	}
	exit;
}
...
function templatesf($pst){
	...
	foreach($tid as $valuet){
		...
		$account_count=db_rows(
			"SELECT COUNT(*) AS `count` FROM `{$data['DbPrefix']}mer_setting`".
...
elseif(isset($post['action'])&&($post['action']=='insert'||$post['action']=='update')){ //inser/update clients info
	...
	*/
	...
	if(isset($post['action'])&&$post['action']=='update'){
		...
		//Dev Tech: 24-12-30 List for Account manager ID 
		if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y'&&@$post['account_manager_id']>0)&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1))){
			$post['account_manager_option']=account_manager_id(@$post['account_manager_id'],2);
		}
...
}elseif(isset($post['action'])&&$post['action']=='generate_private_key'){	//generate secret key as private key for encrytion method for s2s api 
	//Dev Tech : 25-01-28 bug fix required bid not gid
	if(isset($_REQUEST['dtest'])){
		echo "<br/><hr/><br/>bid=>".$post['bid'];
		echo "<br/><br/>gid=>".$post['gid'];
		echo "<br/><hr/><br/>";
	}

	$generate_private_key=generate_private_key($post['gid']);	//generate secret key
	if(isset($_REQUEST['ajax'])&&$_REQUEST['ajax']){
...
elseif(isset($post['action'])&&$post['action']=='update_payin_setting'){
	...
	if(!isset($payin_setting_get['id'])) {
		//echo "<br/>Not found payin_setting";
		db_query("INSERT INTO `{$data['DbPrefix']}payin_setting`(`clientid` )VALUES({$id})",$data['cqp']);
		$newid=newid();
		//echo "<br/>payin_setting id=>".$newid."<br/>";
	}
	
	//print_r($payin_setting_get);exit;
	...
	if(isset($post['send'])&&$post['send']){
		...
		}elseif(@$post['settlement_optimizer']=='weekly'&&(!isset($post['week_delay'])||!$post['week_delay'])){
			$data['Error']='Please Select Week Delay';
		
		}else{
			
			$upd_set_paying="";
			if($post['settlement_optimizer']=='weekly')
				$upd_set_paying=", `day_name`='{$post['day_name']}', `week_delay`='{$post['week_delay']}', `pdf_report`='{$post['pdf_report']}'";
			
			
			db_query(
				"UPDATE `{$data['DbPrefix']}payin_setting`".
				" SET `settlement_optimizer`='{$post['settlement_optimizer']}',`payin_theme`='{$post['payin_theme']}',`settlement_fixed_fee`='{$post['settlement_fixed_fee']}', `settlement_min_amt`='{$post['settlement_min_amt']}', `monthly_fee`='{$post['monthly_fee']}', `frozen_balance`='{$post['frozen_balance']}'  ".$upd_set_paying.
				" WHERE `clientid`='".$id."'  ",$data['cqp']
			);
...
elseif(isset($post['action'])&&$post['action']=='update_softpos_setting'){
	...
	if(empty($softpos_setting_get['clientid'])){
		//echo "<br/><hr/>id=>".@$id;
		//echo "<br/><hr/>clientid=>".@$softpos_setting_get['clientid'];
	}
		...
		if(!isset($post['softpos_pa'])||!$post['softpos_pa']){
			...
			if(empty($softpos_setting_get['clientid'])){
				...
				"`clientid`,`sub_merchantId`,`vpa`,`qr_fullname`,`qr_email`,`product_name`,`merchantAddressLine`,`merchantCity`,`merchantState`,`merchantPinCode`,`mobileNumber`,`panNumber`,`settlementAcSameAsParent`,`profile_pic`,`currency`,`softpos_terNO`,`softpos_status`,`json_value`".
					")VALUES(".	
					"'$id','{$json_value['eazypayMerchantID']}','".encode_f($postDataDMO['virtualAddress'])."','{$post['qr_fullname']}','".encrypts_decrypts_emails($post['qr_email'],1)."','{$post['product_name']}','{$post['merchantAddressLine']}','{$post['merchantCity']}','{$post['merchantState']}','{$post['merchantPinCode']}','{$post['mobileNumber']}','".strtoupper($post['panNumber'])."','{$post['settlementAcSameAsParent']}','{$post['profile_pic']}','{$post['currency']}','{$post['softpos_terNO']}','1','".json_encode($json_value)."')";
				*/
					...
					$sqlStmt="INSERT INTO `{$data['DbPrefix']}softpos_setting`(".
				"`clientid`,`softpos_pa`,`softpos_pn`,`softpos_terNO`,`softpos_public_key`,`vpa`,`sub_merchantId`,`qr_fullname`,`qr_email`,`product_name`,`merchantAddressLine`,`merchantCity`,`merchantState`,`merchantPinCode`,`mobileNumber`,`panNumber`,`settlementAcSameAsParent`,`profile_pic`,`currency`,`softpos_status`,`json_value`".
					")VALUES(".	
					"'$id','{$post['softpos_pa']}','{$post['softpos_pn']}','{$post['softpos_terNO']}','{$post['softpos_public_key']}','{$post['vpa']}','{$post['sub_merchantId']}','{$post['qr_fullname']}','".encrypts_decrypts_emails($post['qr_email'],1)."','{$post['product_name']}','{$post['merchantAddressLine']}','{$post['merchantCity']}','{$post['merchantState']}','{$post['merchantPinCode']}','{$post['mobileNumber']}','".strtoupper($post['panNumber'])."','{$post['settlementAcSameAsParent']}','{$post['profile_pic']}','{$post['currency']}','{$post['softpos_status']}','{$post['json_value']}' )";

					db_query($sqlStmt,@$data['cqp']);
				...
				db_query(
					...
					" SET `softpos_pa`='{$post['softpos_pa']}', `softpos_pn`='{$post['softpos_pn']}', `softpos_terNO`='{$post['softpos_terNO']}', `softpos_public_key`='{$post['softpos_public_key']}', `vpa`='{$post['vpa']}', `sub_merchantId`='{$post['sub_merchantId']}', `qr_fullname`='{$post['qr_fullname']}', `qr_email`='".encrypts_decrypts_emails($post['qr_email'],1)."', `product_name`='{$post['product_name']}', `merchantAddressLine`='{$post['merchantAddressLine']}', `merchantCity`='{$post['merchantCity']}', `merchantState`='{$post['merchantState']}', `merchantPinCode`='{$post['merchantPinCode']}', `mobileNumber`='{$post['mobileNumber']}', `panNumber`='".strtoupper($post['panNumber'])."', `settlementAcSameAsParent`='{$post['settlementAcSameAsParent']}', `profile_pic`='{$post['profile_pic']}', `currency`='{$post['currency']}', `softpos_status`='{$post['softpos_status']}', `json_value`='{$post['json_value']}'  ".
					" WHERE `clientid`='".$id."'  ",0
				);
...
if(isset($post['action'])&&$post['action']=='detail')
{
	...
	if(((isset($_SESSION['sub_admin_id']))&&($_SESSION['get_mid']!='M. All'))){
		...
		if(isset($_SESSION['get_mid'])&&@$_SESSION['get_mid']) $_SESSION['get_mid_array']=explode(",",@$_SESSION['get_mid']);

		if(isset($_SESSION['get_mid_array'])&&!in_array($post['gid'],$_SESSION['get_mid_array']))
		{
			if(isset($_SESSION)&&(isset($_SESSION['sub_admin_id'])&&$_SESSION['sub_admin_id'])&&isset($_SESSION['s11'])){
				...
				echo " | gid=>".$post['gid'];
				echo " | get_mid_array=>";print_r($_SESSION['get_mid_array']);
	...
	if(@$post['PayinInfo']['settlement_optimizer']) $_SESSION['settlement_optimizer']=@$post['PayinInfo']['settlement_optimizer'];
	/*
	if(isset($payin_setting_get)&&$payin_setting_get) $post=array_merge($post,$payin_setting_get);
	if(isset($_POST)&&$_POST) $post=array_merge($post,$_POST);
	*/
	...
	$member_json_value=jsondecode($post['MemberInfo']['json_value']);
	
	//Dev Tech: 24-12-30 view from Account manager ID 
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y')&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1))){
		$post['MemberInfo']['account_manager_view']=account_manager_fetch(@$member_json_value['account_manager_id'],1);
	}
...
if(isset($post['action'])&&$post['action']=='select'){
	...
	if (isset($post['keyword']) && !empty($post['keyword']) && isset($post['sfield']) && !empty($post['sfield'])) {
		...
		if(@$post['sfield']=='em'&&@$post['keyword']) {
			if(isset($_REQUEST['dtest']))echo "<br/>keyword=>". $post['keyword'];
			$post['keyword']=str_replace(" ","+",$post['keyword']);
			if(isset($_REQUEST['dtest']))echo "<br/>keyword after replace=>". $post['keyword'];
			
			$post['keyword']=encrypts_decrypts_emails($post['keyword'],1); 
			if(isset($_REQUEST['dtest'])) echo "<br/>keyword via encrypts for Email=>". $post['keyword']."<br/><br/>";
...
///=========================================
		...
		if(isset($post['order'])&&$post['type']=='online'){
			...
			if(isset($post['keyword'])&&trim($post['keyword']) && trim($fieldname)){
				...
				$fieldname;
				if($data['connection_type']=='PSQL')
					$where.=" AND `m`.{$fieldname} LIKE '%{$pkeyword}%' ";
				else $where.=" AND CONVERT(`m`.{$fieldname} USING utf8) LIKE '%{$pkeyword}%' ";
				$status=6; 
			}
