...
###############################################################################
$data['PageName']='Store';
$data['PageFile']='fino_vpa'; 
###############################################################################
include('../config.do');
$data['PageTitle'] = 'Generate Fino VPA - '.$data['domain_name'];
###############################################################################
...
if(isset($post['send']))
{
	...
	if(!isset($post['clientTxnId'])||!$post['clientTxnId']){
		$data['Error']='Can not empty for Client Txn Id';
		$data['ErroFocus']='clientTxnId';
	}elseif(!isset($post['serviceId'])||!$post['serviceId']){
		$data['Error']='Can not empty for Service Id';
		$data['ErroFocus']='serviceId';
	}
	else{
		...
		if(@$qp) echo "<hr/><br/><b style='font-size:18px'>Header_Encryption_Key via header:</b><br/> $Header_Encryption_Key\n";

			

		$encryptedDataHeader = CryptoJSAES::encrypt($headeRequest,  $Header_Encryption_Key);//header encrypted request
		if(@$qp) echo "<hr/><br/>Encrypted Header Dynamic:<br/> $encryptedDataHeader\n";
		...
		//vpa request data via body 132072023112178

		$dataVPA = '{
		"clientTxnId": "'.$post['clientTxnId'].'",
		"serviceId": "'.$post['serviceId'].'",
		"mobilenumber": "'.$post['mobilenumber'].'",
		"vpasubstring": "'.$post['vpasubstring'].'",
		"firstname": "'.$post['firstname'].'",
		"lastname": "'.$post['lastname'].'",
		"accountnumber": "'.$post['accountnumber'].'",
		"ifsc": "'.$post['ifsc'].'",
		"accounttype": "'.$post['accounttype'].'",
		"mcccode": "'.$post['mcccode'].'",
		"franchisename": "'.$post['franchisename'].'",
		"brandname": "'.$post['brandname'].'",
		"Settlement_Account": "'.$post['Settlement_Account'].'",
		"Settlement_IFSC": "'.$post['Settlement_IFSC'].'",
		"Bank_Name": "'.$post['Bank_Name'].'",
		"Bene_Name": "'.$post['Bene_Name'].'",
		"MDR_Per": "'.$post['MDR_Per'].'",
		"Minimum_Charge": "'.$post['Minimum_Charge'].'",
		"MDR_Type": "'.$post['MDR_Type'].'"
		}';
		...
		if(@$qp) echo "<hr/><br/><b style='font-size:18px'>Encryption_Decryption_Key for request vpa data via  body:</b><br/> $passphrase\n";



		// Encrypt the data
		$encryptedData = CryptoJSAES::encrypt($dataVPA, $passphrase);
		if(@$qp) echo "<hr/><br/>Encrypted request dataVPA:<br/> $encryptedData\n";

		// Decrypt the data
		$decryptedData = CryptoJSAES::decrypt($encryptedData, $passphrase);
		if(@$qp) echo "<hr/><br/>Decrypted request dataVPA:<br/> $decryptedData\n";


		if(@$qp)echo "<hr/><br/>dataVPA request:<br/> $dataVPA\n";
		...
		if(@$qp) echo "<hr/><br/>vpaUrl: $vpaUrl\n";
		...
		if(@$qp) echo "<hr/><br/>response: $response\n";

		if(@$qp) echo "<hr/><br/>response json_decode:\n";
		if(@$qp) print_r(@$req);
