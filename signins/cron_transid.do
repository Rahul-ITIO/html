...
#########################################################################
$data['PageName']='Status Update via TransID';
$data['PageFile']='cron_transid'; 
##########################################################################
include('../config.do');
        ...
        if(strpos($file_ext,'csv') !== false)
        {
            ...
            while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
            {
                ...
                else
                {
                    ...
                    $pramPost['transID']=@$transID=$filesop[0]; //first column our transID 
                    ...
                    if(trim($transID)) 
                    {
                        ...
                        else 
                        {
                            ...
                            ######################################################################
                                ...
                                if(isset($transID)&&trim($transID)){
                                    db_query("UPDATE `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` SET `trans_status`='0'  WHERE `transID`={$transID}  ",$qp);
                                }
                            ...
                            // subquery url for retrun url 
                                ...
                                @$subQuery='&action=webhook&admin=1&actionurl=admin_direct&tdateSkip=tdateSkip&destroy=2&acquirer_retrun_json_response=Y&cron_tab=cron_tab&cron_host_response='.@$admin_id.@$dbad_link_2;
                            ...
                            // condition define as per success of failed 
                                ...
                                */
                                ...
                                // $data_send['admin']='1';
                                    ...
                                    $data_send['no_merchant_redirect']='1';
                                ...
                                // $data_send = $_POST = strip_tags_d($data_send);
                                    ...
                                    $status_url_include = "status{$data['ex']}?transID=$transID&action=webhook".@$subQuery;
                                    
                                    $status_url = $data['Host']."/".@$status_url_include;

                                // $status_url=str_replace(" ","+",$status_url);
                                ...
                                //include($data['Path'].'/'.$status_url_include);

                                //$curlResponse=use_curl($status_url,$data_send);

                                if(@$qp)
                                {
                                    echo "<br/>status_url=>".@$status_url;
                                    echo "<br/>pramPost=>";
                                    print_r($get_string);
                                }
