...
function delete_unreg_clients_pay($id) {
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}unreg_clients_pays_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}unreg_clients_pays`)+1);",0
		);
	}
}
// user confirm registration
function update_unreg_clients_pays($receiver) {
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}temp_pays_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}temp_pays`)+1);",0
		);
	}
...
function block_clients($uid){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}login_attempts_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}login_attempts`)+1);",0
		);
	}
}
...
function save_remote_ip($uid, $address, $mid=0, $ref=""){
	global $data;
	if($ref=="")$ref=$_SERVER['HTTP_REFERER'];
	db_query(
		"INSERT INTO `{$data['DbPrefix']}login_ip_history`(`clientid`,`date`,`address`,`subadmin_id`,`source_url`".
...
function delete_card($gid){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}cards_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}cards`)+1);",0
		);
	}
}
...
function delete_bank($gid, $primary=0){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}zt_banks_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}zt_banks`)+1);",1
		);
		db_query(
			"SELECT setval('{$data['DbPrefix']}zt_coin_wallet_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}zt_coin_wallet`)+1);",1
		);
	}
...
function delete_access_roles($id){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}access_roles_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}access_roles`)+1);",0
		);
	}
}
function delete_subadmin_roles($id){
	...
	if($data['connection_type']=='PSQL'){
		db_query(
			"SELECT setval('{$data['DbPrefix']}subadmin_id_seq', (SELECT MAX(`id`) FROM `{$data['DbPrefix']}subadmin`)+1);",0
		);
	}
}
...
function insert_subadmin_info($post){
	...
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y')&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1))){
		$qry_1.=",`account_manager_id`";
		$qry_2.=",'{$post['account_manager_id']}'";
	}
	...
	$qry= "INSERT INTO `{$data['DbPrefix']}subadmin`(".
		...
		"'{$post['heading_bg_color']}','{$post['heading_text_color']}','{$post['front_ui_panel']}','{$post['dashboard_notice']}','{$post['notice_type']}'".$qry_2.
		")";
...
function update_subadmin_info($post, $sid){
	...
	if((isset($data['ACCOUNT_MANAGER_ENABLE'])&&@$data['ACCOUNT_MANAGER_ENABLE']=='Y')&&((isset($_SESSION['login_adm']))||(isset($_SESSION['account_manager'])&&$_SESSION['account_manager']==1))){
		$multiple_assign.=",`account_manager_id`='{$post['account_manager_id']}'";
	}
...
function insert_bank_info($post, $uid, $notify=true, $admin=false){
	...
	if($admin){	// if login with admin then can set value into primary field 
		...
		db_query(
			...
			"`status`,`default`,`full_address`,`bank_doc`,`settlement_webhook_url`".
			")VALUES(".
			...
			"0,0,'{$post['full_address']}','{$post['upload_logo']}','{$post['settlement_webhook_url']}')",$qp
		);
...
function update_bank_info($post, $gid, $uid, $notify=true, $admin=false){
	...
	"`baccount`='".encrypts_string($post['baccount'])."',`btype`='{$post['btype']}',`adiinfo`='{$post['adiinfo']}',`required_currency`='{$post['required_currency']}',".$img_updt.
		"`brtgnum`='{$post['brtgnum']}',`intermediary`='{$post['intermediary']}',`intermediary_bank_name`='{$post['intermediary_bank_name']}',`intermediary_bank_address`='{$post['intermediary_bank_address']}',`bswift`='{$post['bswift']}',`full_address`='{$post['full_address']}',`settlement_webhook_url`='{$post['settlement_webhook_url']}'".
	" WHERE `id`='{$gid}'",0
		);
...
function support_tickets_count_by_status($merchant_id,$status){
	...
	if($data['connection_type']=='PSQL')
		//$order_field=' ARRAY_POSITION(ARRAY[2,4,0,1,91], "status") ';
		$order_field=' ';
	else
		$order_field=" ORDER BY FIELD(`status`, 2,4,0,1,91) DESC ";
	

	$result=db_rows(
		"SELECT COUNT(*) AS count".
		" FROM {$data['DbPrefix']}support_tickets ".
		" WHERE `clientid`='{$merchant_id}' ".$qr." AND `status` NOT IN (90,92) ". $order_field .
		" LIMIT 1",0
	);
...
function user_block_time($uid,$tbl='clientid_table'){	
	...
	if($uid>0) db_query("UPDATE `{$data['DbPrefix']}{$tbl}`"." SET `ip_block_client`='{$ip_block_client}' WHERE `id`='{$uid}'",0);
}// end function
...
function user_un_block_time($uid,$tbl='clientid_table'){	
	global $data;
	if($uid>0) db_query("UPDATE `{$data['DbPrefix']}{$tbl}`"." SET `ip_block_client`='' WHERE `id`='{$uid}'");
}// end function
...
//Dev Tech : 24-09-25 Get parma for post via Checkout api 
...
if(isset($_GET['terNO']))$post['terNO']=$_GET['terNO'];
...
if(isset($_GET['bill_ip']))$post['bill_ip']=$_GET['bill_ip'];
if(isset($_GET['source']))$post['source']=$_GET['source'];
if(isset($_GET['source_url']))$post['source_url']=$_GET['source_url'];
if(isset($_GET['bill_amt']))$post['bill_amt']=$_GET['bill_amt'];
if(isset($_GET['bill_currency']))$post['bill_currency']=$_GET['bill_currency'];
if(isset($_GET['product_name']))$post['product_name']=$_GET['product_name'];
if(isset($_GET['fullname']))$post['fullname']=$_GET['fullname'];
if(isset($_GET['bill_email']))$post['bill_email']=$_GET['bill_email'];
if(isset($_GET['bill_address']))$post['bill_address']=$_GET['bill_address'];
if(isset($_GET['bill_city']))$post['bill_city']=$_GET['bill_city'];
if(isset($_GET['bill_state']))$post['bill_state']=$_GET['bill_state'];
if(isset($_GET['bill_country']))$post['bill_country']=$_GET['bill_country'];
if(isset($_GET['bill_zip']))$post['bill_zip']=$_GET['bill_zip'];
if(isset($_GET['bill_phone']))$post['bill_phone']=$_GET['bill_phone'];
if(isset($_GET['reference']))$post['reference']=$_GET['reference'];
if(isset($_GET['webhook_url']))$post['webhook_url']=$_GET['webhook_url'];
if(isset($_GET['return_url']))$post['return_url']=$_GET['return_url'];
if(isset($_GET['checkout_url']))$post['checkout_url']=$_GET['checkout_url'];
if(isset($_GET['mop']))$post['mop']=$_GET['mop'];
if(isset($_GET['os']))$post['os']=$_GET['os'];
if(isset($_GET['ccno']))$post['ccno']=$_GET['ccno'];
if(isset($_GET['ccvv']))$post['ccvv']=$_GET['ccvv'];
if(isset($_GET['month']))$post['month']=$_GET['month'];
if(isset($_GET['year']))$post['year']=$_GET['year'];
