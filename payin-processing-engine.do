...
if(!isset($post['step'])){
	...
	##################################################################
		...
		*/
		...
		}elseif((!empty($_REQUEST['bill_ip']))){
			$data['Addr']=$_REQUEST['bill_ip'];
			$_SESSION['bill_ip']=$_REQUEST['bill_ip'];
		}else{
		...
		*/
		...
		if($data['process_file']=='quay'){
			...
			/*
			$ipCountResult=db_rows("SELECT COUNT(*) AS `count` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE ( `bill_ip`='{$_SESSION['bill_ip']}' ) AND ( `tdate` IS NOT NULL ) AND ( `tdate` BETWEEN (DATE_FORMAT('{$date_1st}', '%Y%m%d%H%i%s')) AND (DATE_FORMAT('{$date_2nd}', '%Y%m%d%H%i%s')) )  ORDER BY id DESC LIMIT 1 ",0);
	...
	if(isset($_REQUEST['encrypted_payload'])&&$_REQUEST['encrypted_payload']){
		...
		if(isset($data['encrypted_payload_length'])&&trim($data['encrypted_payload_length']))
			$post['public_key']=substr( $encrypted_payload1, -(int)$data['encrypted_payload_length'] );
		else 
	...
	if(isset($post['public_key'])){	//check public_key is exists or not
		...
		{	//if data in $product not avaible then print following error code and message
			...
			if(isset($post['integration-type'])&&$post['integration-type']=='s2s')
			{
				...
				//echo "<br/>source_url_s2s=>".$source_url_s2s;
	...
	if(isset($post['clientid'])&&$post['clientid']>0){
		...
		if($get_clientid_details){ // if merchant exists then execute following section
			...
			if((isset($data['reference_unique'])||(isset($post['unique_reference'])&&$post['unique_reference']=='Y'))&&isset($post['reference'])){
				...
				$reference_unique=db_rows("SELECT COUNT(*) AS `count` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE ( `merID`={$_SESSION['clientid']} ) AND ( `reference`='{$post['reference']}' ) LIMIT 1 ",0);
				
				$reference_unique_count=(isset($reference_unique[0]['count'])?$reference_unique[0]['count']:0);
				if($reference_unique_count>0){
					$data['Error']=666;
					$data['Message']="Reference should be unique.";
					error_print($data['Error'],$data['Message']);	//print error
				}
		...
		foreach($bank_getway as $key=>$value){	//loop for fetch row by data from $bank_getway
			...
			if($value['acquirer_status']>0)
			{
				...
				if(isset($value['block_countries'])&&trim($value['block_countries'])){
					...
					$_SESSION["b_".$value['acquirer_id']]['block_countries_arr']=jsondecode(jsonencode($value['block_countries']));	//store country list which not allow transactions
				}
...
if(isset($_SESSION['public_key'])&&$_SESSION['public_key']&&isset($_SESSION['post']['bill_email'])&&$_SESSION['post']['bill_email']&&isset($_SESSION['post']['bill_amt'])&&$_SESSION['post']['bill_amt']&&$unique_reference==1){
	...
	//fetch the reference, transID from trans between above define date_s_1st and date_s_2nd with following conditions
		$mrid_get=db_rows("SELECT `reference`,`transID` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE ( `trans_status` IN (1) ) AND ( `bill_amt` IN ({$_SESSION['post']['bill_amt']}) ) AND ( `bill_email` IN ('{$_SESSION['post']['bill_email']}') ) AND ( `merID` IN ({$clients_uid})) AND (`terNO` IN ({$product_id})) AND ( `tdate` BETWEEN '{$date_s_1st}' AND '{$date_s_2nd}' )   ORDER BY id DESC LIMIT 1 ",0);
		
		//( `tdate` BETWEEN (DATE_FORMAT('{$date_s_1st}', '%Y%m%d%H%i%s')) AND (DATE_FORMAT('{$date_s_2nd}', '%Y%m%d%H%i%s')) ) 
...
$_SESSION['json_value']['checkout_theme']=$data['frontUiName'];
 ...
 // fetch the subadmin customer service email via customer_service_email 
        
 if(isset($domain_server['customer_service_email'])&&$domain_server['customer_service_email'])
 $customer_service_email=$_SESSION['customer_service_email']=$domain_server['customer_service_email'];

 //echo '<br/>customer_service_email =>  <b>'.$_SESSION['customer_service_email'].'</b>';
...
if(isset($post['fullname'])&&$post['fullname']&&empty($post['ccholder'])){
	...
	if(empty($post['ccholder_lname'])) $post['ccholder_lname']=$fullname;
}
...
{
	$post['bill_address']=$post['bill_street_1']=stf($post['bill_address']);
	...
	//if(empty($post['bill_street_1'])) $post['bill_street_1']='NA';
	if(empty($post['bill_street_2'])) $post['bill_street_2']='NA';
}

if(isset($data['cqp'])&&$data['cqp']==7){
	echo "<hr/><br/>con_name=>".$data['con_name'];
	echo "<br/>_SESSION bill_amt=>".$_SESSION['bill_amt'];
...
if($data['con_name']=='clk'||$data['con_name']=='IND'){
...
if(isset($data['cqp'])&&$data['cqp']==7){
	echo "<br/><br/><hr/><br/>con_name=>".$data['con_name'];
	echo "<br/>_SESSION bill_amt=>".$_SESSION['bill_amt'];
	echo "<br/>post bill_amt=>".$post['bill_amt'];
	echo "<br/><br/><hr/><br/>_POST=>";print_r($_POST);
	...
	//if array AccountInfoByMerchant exists then remove duplicate data from AccountInfoByMerchant array
		...
		foreach($post['AccountInfoByMerchant'] as $value){
			if($value){
				...
				if(isset($_SESSION['b_'.$value]['channel_type']) && (in_array($_SESSION['b_'.$value]['channel_type'],[4,11,99,7,90]))) {
				...
				// MOP - 90 Network Payment if find scanqr from popup_msg_web for scan qr code array
				
				if(isset($_SESSION["b_".$value]['aLj']['popup_msg_web'])&&trim($_SESSION["b_".$value]['aLj']['popup_msg_web'])&&(strpos($_SESSION["b_".$value]['aLj']['popup_msg_web'],'scanqrArray')!==false))
				{
					@$post['scan_qrcode_array'][$value]= 'upi_qr_border_'.@$value;
				}
...
//echo "<br/>upi_qr_border_ scan_qrcode_array=>"; print_r(@$post['scan_qrcode_array']);
...
if($post['step']==1){
	...
	//if(isset($post['integration-type'])&&$post['integration-type']){ //start
		...
		elseif(isset($acquirer)&&$acquirer&&((isset($_SESSION['inactive_bank_ac_list'])&&in_array($acquirer, $_SESSION['inactive_bank_ac_list']))||(isset($_SESSION['all_bank_ac_list'])&&!in_array($acquirer, $_SESSION['all_bank_ac_list'])))){
			$data['Error']='Requested Acquirer Id has been terminated!!!';	
			$data['ErrorNo']=1044;
		}
		elseif(isset($web_mc)&&isset($account_type_arr)&&$account_type_arr&&!in_array($acquirer,$account_type_arr)&&$curl_action==true){
			$data['Error']='MCC root not found ';	
			$data['ErrorNo']=1045;
		...
		elseif((((!isset($post['bill_email']))||(empty(trim($post['bill_email']))))||!verify_email2($post['bill_email']))&&($addressValidation)){
			...
			$data['ErrorNo']=82;
		}elseif(((!isset($post['bill_address']))||(empty(trim($post['bill_address']))))&&($addressValidation)){	//print this message if Billing Address  mandatory
			$data['Error']='Billing Address can not be empty.';
			$data['ErrorNo']=83;
		}elseif(((!isset($post['bill_city']))||(empty(trim($post['bill_city']))))&&($addressValidation)){
			$data['Error']='Billing bill_city can not be empty.';	//print this message if Billing bill_city mandatory
			$data['ErrorNo']=84;
		}elseif(((!isset($post['bill_state']))||(empty(trim($post['bill_state']))))&&($addressValidation)&&($stateValidation)){
			$data['Error']='Billing bill_state can not be empty.';	//print this message if Billing bill_state mandatory
			$data['ErrorNo']=85;
		}
		elseif(!isset($post['bill_country'])&&empty($post['bill_country'])&&$addressValidation){
			...
			$data['ErrorNo']=861;
		}elseif(((!isset($post['bill_zip']))||(empty(trim($post['bill_zip']))))&&($addressValidation)){
			$data['Error']='Billing bill_zip bill_address can not be empty.';	//print this message if Billing bill_zip mandatory
			$data['ErrorNo']=87;
		}elseif((!isset($post['bill_phone']))||(empty(trim($post['bill_phone']))) ){
...
if($post['step']==2){
	...
	if(isset($_SESSION['processing_currency_markup'])) unset($_SESSION['processing_currency_markup']);	//UNSET processing_currency_markup if set in $_SESSION
	...
	*/
	...
	if(isset($post['integration-type']))
	{
		...
		//echo "<hr/>acquirer=>".@$acquirer;
		//echo "<hr/>bank_cards_name=>".@$bank_cards_name;
		...
		}elseif(isset($acquirer)&&(empty($bank_cards_name))&&($validation==true)){
		...
		}elseif(isset($acquirer)&&(!isset($post['ccno'])||!$post['ccno'])&&($validation==true)){
		...
		}elseif(isset($acquirer)&&(!isset($post['validateCard'])||!$post['validateCard']) && ($validation==true) && (empty($post['validateCard'])) && ($testcardno==false) && (@$luhn_validation==true)){
		...
		}elseif(isset($acquirer)&&((!isset($post['ccvv'])||!$post['ccvv'])) && ($validation==true)){
		...
		}elseif(isset($acquirer)&&((!isset($post['month'])||!$post['month'])) && ($validation==true)){
		...
		}elseif(isset($acquirer)&&(!isset($post['year']) || !$post['year']) && ($validation==true)){
		...
		}elseif(isset($acquirer)&&(!isset($_SESSION["sp".$acquirer])) && ($settlementVali==true)){
			$data['Error']=157;
			$data['Message']="Settlement Period missing in Acquirer. Contact to Support";
			error_print($data['Error'],$data['Message']);	//print error
		}elseif(isset($acquirer)&&(isset($_SESSION["b_".$acquirer]['block_countries']))&&(trim($_SESSION["b_".$acquirer]['block_countries']))&&(isset($curl_action)&&$curl_action==true)&&in_array(@$post['country_two'],@$_SESSION["b_".$acquirer]['block_countries_arr'])){
			$jsonError['Error']=158;
			$jsonError['Message']="Billing address from a restricted country (".$post['country_two'].").";
			$jsonError['block_countries_list']=@$_SESSION["b_".$acquirer]['block_countries_arr'];
			json_print($jsonError);exit;
		}elseif((!isset($acquirer)) && ($validation==true)){
	...
	###### mid apJson :start #######################################
			...
			if($print_salt){ echo "<hr/>sm_=> ".$_SESSION["apJson$acquirer"];}
...
###### acquirer_processing_currency from apJson - acquirer :start #####################
	...
	###### acquirer_processing_currency from apJson - acquirer :end #####################
			...
			//addon for transaction rate processing currency markup - processing_currency_markup % on total amount 

			if(isset($_SESSION['b_'.$acquirer]['processing_currency_markup'])&&trim($_SESSION['b_'.$acquirer]['processing_currency_markup'])&&!is_string(@$_SESSION['b_'.$acquirer]['processing_currency_markup'])&&@$_SESSION['b_'.$acquirer]['processing_currency_markup']>0) 
			{
				$processing_currency_markup=@$_SESSION['b_'.$acquirer]['processing_currency_markup'];

				$_SESSION['json_value']['processing_currency_markup']=@$processing_currency_markup;

				$total_payment_or_bank_processing_amount=((isset($_SESSION['bank_processing_amount'])&&trim($_SESSION['bank_processing_amount']))?@$_SESSION['bank_processing_amount']:@$total_payment);

				$_SESSION['processing_currency_markup']=($total_payment_or_bank_processing_amount * $processing_currency_markup / 100);

				$_SESSION['json_value']['processing_currency_markup_calc_amount_via_percentage']=@$_SESSION['processing_currency_markup'];

				$_SESSION['json_value']['total_payment_with_processing_currency_markup']=number_formatf2(@$total_payment_or_bank_processing_amount+@$_SESSION['processing_currency_markup']);
			}
			...
			//echo "<br/><hr/>post=><br/>"; print_r(@$post);echo "<br/><hr/>json_value encode=><br/>".json_encode(@$_SESSION['json_value']);

			//Dev tech : 24-09-18 Removes double quotes from input of post and fixes suggestion values.
			$bill_param_arr=["product_name","fullname","bill_email","bill_address","bill_city","bill_state","bill_country","bill_zip","bill_phone","reference"];

			foreach($bill_param_arr as $bpa){
				if(isset($post[$bpa])){
					$post[$bpa]=$_SESSION['json_value']['post'][$bpa]=replacepost_api($post[$bpa]);
				}
			}
			
			
			//echo "<br/><hr/>post=><br/>"; print_r(@$post);echo "<br/><hr/>json_value=><br/>"; print_r(@$_SESSION['json_value']);echo "<br/><hr/>json_value encode=><br/>".json_encode(@$_SESSION['json_value']);exit;
			...
			// Dev Tech: 24-01-13 If countPost in json post than retry to max 3 post as same transID as per define condition via aquirer api 
			if(isset($_SESSION['json_value']['countPost'])&&!empty($_SESSION['json_value']['countPost']))
					$thisCountPost=(int)$_SESSION['json_value']['countPost'];
				
			if(isset($_SESSION['transID'])&&!empty($_SESSION['transID'])&&isset($_SESSION['json_value']['post']['countPost'])&&$_SESSION['json_value']['post']['countPost']<$thisCountPost)
				$thisTrInst=0;
		...
		//	if(isset($post['ccno'])&&$post['ccno']) 	$_SESSION['bin_no']		=$post['ccno'];
			...
			if($thisTrInst){
				...
				create_new_trans(
					...
					((isset($post['ccno'])&&trim($post['ccno']))?@$post['ccno']:''),
			...
			// if bank processing amount for converted currency amount  
			if(isset($_SESSION['bank_processing_amount'])&&$_SESSION['bank_processing_amount']>0&&$currConverter==true){
				$total_payment=number_formatf2($_SESSION['bank_processing_amount']);
				$_SESSION['curr']=$bpc;
			}
			

			
			//total payment + amount processing currency markup % on total amount 
			if(isset($_SESSION['processing_currency_markup'])&&@$_SESSION['processing_currency_markup']>0)
			{
				$total_payment=number_formatf2($total_payment+@$_SESSION['processing_currency_markup']);
			}
			...
			*/
			...
			if(@$scrubbed){
				if($scrubbed['scrubbed_status']==true){
					...
					if(isset($curl_action)&&$curl_action==true){
						$jsonError['Error']=159;
						$jsonError['Message']=$transID.@$scrubbed_msg;
						json_print($jsonError);exit;
					}
