...
$payable_amt_of_txn_row_2=1;
$payable_amt_of_txn_row_update_2=0;

if(isset($_GET['payable_amt_of_txn_row_update_2'])&&$_GET['payable_amt_of_txn_row_update_2']){
	
	$payable_amt_of_txn_row_update_2=$_GET['payable_amt_of_txn_row_update_2'];
}
...
if($payable_amt_of_txn_row_2){

	
		echo "<h3>Check not updated payable_amt_of_txn in trans_status 1 and 7 </h3><br/>";

		$transactions_select_2 = db_rows("SELECT `id`, 
		
		`transID`, `merID` , `acquirer`, `trans_status`, `bill_currency`,`bill_amt`, `trans_currency`,`trans_amt`, `buy_mdr_amt`,`buy_txnfee_amt`, `rolling_amt`, `mdr_cb_amt`, `mdr_cbk1_amt`, `mdr_refundfee_amt`, `payable_amt_of_txn` ".
		" FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` ".
		" WHERE  ". $merID_sender . 
		"  (`trans_type` IN (11))  
		AND  ( `trans_status` IN (1,7) )
		AND (`payable_amt_of_txn` is null OR `payable_amt_of_txn`='0.00' OR `payable_amt_of_txn`='0' ) ".
		///" LIMIT 10 ". //CMN
		" ",1); 


	$transactions_select_2_count =	count($transactions_select_2);
	
	echo "<hr/>size=>".$transactions_select_2_count."<br/><hr/>";
	...
	foreach($transactions_select_2 as $key=>$value){
		...
		
		{
			...
			/*
			if($payable_amt_of_txn_row_update_2 && ($diff>0 || $diff<(-0.99))){
	...
	if($transactions_select_2_count>0)
	{
	?><a href="javascript:void(0)" onclick="myConfirm('Are you Sure to Update: <?=($diff_item)?> records?','<?=$_SERVER['PHP_SELF'];?>?payable_amt_of_txn_row_update_2=1&bid=<?=$merID?>');" style="text-transform:uppercase;font-weight:bold; font-size:16px; text-decoration:none; color:blue">Missing Payout : Records (<?=$transactions_select_2_count?>)</a>
	<hr />
	<br />
	<?
...
if($payable_amt_of_txn_row){
	...
	echo "<hr/>size=>".count($transactions_select)."<br/><hr/>";
