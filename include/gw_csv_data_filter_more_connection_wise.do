...
$is_adm_login=1; $merchant_csv_data=0; $db_more_connection_loop=1;
...
$csv_q .= ' AND ( `t`.`trans_status` NOT IN (9,10) )';
...
if($qprint){
	...
	$sub_q = "";
	$wv3_arr=[]; // SubQuery array for wd - custom settlement optimizer 
	...
	if(isset($_POST['transID']) && !empty($_POST['transID'])){
		...
		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`transID`";
	}
	if(isset($_POST['reference']) && !empty($_POST['reference'])) {
		$HeadingArr[] = "Reference";

		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`reference`";
	}
	if(isset($_POST['created_date']) && !empty($_POST['created_date'])) {
		$HeadingArr[] = "Created Timestamp";

		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`created_date`";
	}

	if(isset($_POST['tdate']) && !empty($_POST['tdate'])) {
		$HeadingArr[] = "Updated Timestamp";

		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`tdate`";
	}

	if(@$qprint)
	{
		echo "<br/><hr/><br/>date_2nd=>".$_REQUEST['date_2nd']."<br/><br/>";
		echo "<br/><hr/><br/>RUNTIME_DB_DATE_2ND=>".$data['RUNTIME_DB_DATE_2ND']."<br/><br/>";

		echo "<br/><hr/><br/>date_2nd=>".date("Ymd",strtotime($_REQUEST['date_2nd']))."<br/><br/>";
		echo "<br/><hr/><br/>RUNTIME_DB_DATE_2ND=>".date("Ymd",strtotime($data['RUNTIME_DB_DATE_2ND']))."<br/><br/>";
	}

	if(isset($_POST['runtime']) && !empty($_POST['runtime']) && isset($_REQUEST['date_2nd']) && isset($data['RUNTIME_DB_DATE_2ND']) &&  date("Ymd",strtotime($_REQUEST['date_2nd'])) >= date("Ymd",strtotime($data['RUNTIME_DB_DATE_2ND'])) ) 
	{
		$HeadingArr[] = "Runtime";
		...
		$db_more_connection_loop=0;
	}

	if(@$qprint) echo "<br/><hr/><br/>db_more_connection_loop=>".$db_more_connection_loop."<br/><br/>";

	if(isset($_POST['fullname']) && !empty($_POST['fullname'])) {
		$HeadingArr[] = "Full Name";

		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`fullname`";
	}
	...
	if(isset($_POST['trans_status']) && !empty($_POST['trans_status'])) {
		...
		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`trans_status`";
	}
	if(isset($_POST['acquirer']) && !empty($_POST['acquirer'])) {
		$HeadingArr[] = "Acquirer";

		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`acquirer`";
	}
	...
	if(isset($_POST['rrn']) && !empty($_POST['rrn'])) {
		$HeadingArr[] = "RRN";
		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "{$mts}.`rrn`";
	}
	if(isset($_POST['gst_amt']) && !empty($_POST['gst_amt'])) {
		$HeadingArr[] = "GST Fee";
		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "`t`.`gst_amt`";
	}
	...
	if(isset($_POST['trans_response']) && !empty($_POST['trans_response'])) {
		...
		if($sub_q) $sub_q .= ", ";
		$sub_q .= $wv3_arr[]= "{$mts}.`trans_response`";
	}
...
//wd v3 fields filter 
if(isset($wv3_arr)&&count($wv3_arr)>0) 
{
	$sub_q_wd_v3=implode(",",$wv3_arr);
	$sub_q_wd_v3 = str_replace(["t.","`t`.","ad.","`ad`."], "`t`.", $sub_q_wd_v3);
	//if(isset($sub_q_wd_v3)&&$sub_q_wd_v3) $sub_q_wd_v3 = str_replace(["t.*","`t`.*","ad.*","`ad`.*"], $sub_q_wd_v3, $csv_q);
}
else $sub_q_wd_v3='*';
...
if($data['cqp']==6) 
{
	...
	echo "<br/><br/>sub_q_wd_v3=>".$sub_q_wd_v3."<br/><hr/><br/>"; 
}
...
if(!isset($data['maxCountLimit'])) $data['maxCountLimit']=5000000;
//if(!isset($data['maxCountLimit'])) $data['maxCountLimit']=16000;

$maxCountLimit=@$data['maxCountLimit'];

// make a query for count via default connection 

$sqlStmt_count_qr = "SELECT COUNT(*) AS `count` FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE ".explode(" WHERE ",$csv_q)[1]. " LIMIT 1";

//$sqlStmt_count_qr = str_replace(["t.","`t`.","ad.","`ad`."], "", $sqlStmt_count_qr);
$sqlStmt_count_qr = str_replace(["t.","`t`."], "", $sqlStmt_count_qr);


// fetch count via default connection 
$sqlStmt_count_qr_row = db_rows($sqlStmt_count_qr,$qprint); 
$data_count=$sqlStmt_count_qr_row[0]['count'];


if($data_count>$maxCountLimit) {
    if(isset($HeadingArr)) unset($HeadingArr);
    if(@$qprint) echo "Current count is {$data_count}. Cannot download because maximum count is {$maxCountLimit} limit.\r\n Please check the date range.";
    echo "Please modify the search filter or data range of your query to download the file. Your query exceeds the limit of 5,000,000 records that can be downloaded at once. We apologize for any inconvenience.";
    exit;
}

if($data['cqp']==6) {
    echo "<br/><br/>sqlStmt_count_qr=>".$sqlStmt_count_qr."<br/>"; 
    echo "<br/>data_count=>".$data_count."<br/><br/>"; 
   // exit;
}
...
// via default connection 
$rows_get = db_rows($sqlStmt,$qprint); 
if(@$rows_get) $rows=array_merge($rows,$rows_get);

######################################################################################################################

//Dev Tech: 24-05-11 Fetch more db connection



if( @$data_count<@$maxCountLimit && isset($data['DB_CON']) && !isset($_GET['DB_CON']) && function_exists('config_db_more_connection') && @$db_more_connection_loop==1 )
{
    ...
    { 
        ...
        if(isset($v4['MORE_ADDITIONAL'])&&(isset($data['MASTER_TRANS_ADDITIONAL'])))
        {
            foreach($v4['MORE_ADDITIONAL'] as $ak4 => $av4){
                ...
                config_db_more_connection($k4,$ak4,'',1);
                 ...
                 $join_additional_1=join_additional('t');
                 $sqlStmt_ma = "SELECT {$sub_q} FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` AS `t` {$join_additional_1} WHERE ".explode(" WHERE ",$csv_q)[1];

                    if(isset($_REQUEST['dtest'])) {
                        echo "<br/>MA=>".$k4."_".$ak4."_".$av4['NAME']." | connection_type:- ".$data['connection_type'];
                        echo "<br/>MA Trans Table=>".@$data['MASTER_TRANS_TABLE'];
                        echo "<br/>MA Addit Table=>".@$data['ASSIGN_MASTER_TRANS_ADDITIONAL'];
                    ...
                    //$rows_get = db_rows($sqlStmt,$qprint); 
                    $rows_get = db_rows($sqlStmt_ma,$qprint); 
                    
                    if(@$rows_get&&count($rows_get)>0) 
					{
						 $rows=array_merge($rows,$rows_get);

						//max count limit check 
						$data_count=count(@$rows);
						if($data_count>$maxCountLimit&&!isset($_REQUEST['all'])) break;
        ...
        if(isset($v4['MORE_MASTER']))
        {
            ...
            foreach($v4['MORE_MASTER'] as $mk4 => $mv4)
            {
                ...
                if(isset($_REQUEST['dtest'])) {
                    echo "<br/>MM=>".$k4."_".$mk4."_".$mv4['NAME']." | connection_type:- ".$data['connection_type'];
                    echo "<br/>MM Table=>".$data['MASTER_TRANS_TABLE'];
                    echo "<br/>tr_count=>".$data['tr_count'];
                }
                
                
                $sqlStmt_mm = "SELECT * FROM `{$data['DbPrefix']}{$data['MASTER_TRANS_TABLE']}` WHERE ".explode(" WHERE ",$csv_q)[1];

                $sqlStmt_mm = str_replace(["t.","`t`.","ad.","`ad`."], "", $sqlStmt_mm);
                //$sqlStmt_mm = str_replace(["t.","`t`."], "", $sqlStmt_mm);

                $rows_get = db_rows($sqlStmt_mm,$qprint); 
...
// Fetch for custom from settlement optimizer and Y is CUSTOM_SETTLEMENT_WD_V3
if(isset($data['CUSTOM_SETTLEMENT_OPTIMIZER_V3'])&&isset($data['CUSTOM_SETTLEMENT_WD_V3'])&&$data['CUSTOM_SETTLEMENT_WD_V3']=='Y')
{

	@$sqlStmt_ma = "SELECT {$sub_q_wd_v3} FROM `{$data['DbPrefix']}{$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3']}` AS `t` WHERE ".explode(" WHERE ",$csv_q)[1];

	if(isset($_REQUEST['dtest'])) 
	{
		echo "<hr/><h4 style='font-size:22px!important;font-weight:bold;color:#3a8411;'>WD V3 :: CUSTOM_SETTLEMENT_WD_V3=> ".@$data['CUSTOM_SETTLEMENT_WD_V3']."</h4>";

		echo "<br/>WD V3 connection_type:- ".$data['connection_type'];
		echo "<br/>WD V3 Table=>".@$data['CUSTOM_SETTLEMENT_OPTIMIZER_V3'];
		//echo "<br/>MA sqlStmt_ma=>".@$sqlStmt_ma;
		echo "<br/>tr_count=>".$data['tr_count'];
	}
...
//for($q_num = 0;$q_num<100;$q_num++) {
    ...
    */
	...
	if(isset($_POST['created_date']) && !empty($_POST['created_date'])) 
		array_multisort(array_column($rows, 'created_date'), SORT_ASC, $rows);
	elseif(isset($_POST['tdate']) && !empty($_POST['tdate'])) 
	...
	foreach($rows as $key=>$row){
		...
		//if(!(empty($_POST['rolling_amt']) && $row['acquirer']==3))
		{
			...
			if(isset($_POST['created_date']) && !empty($_POST['created_date']))
				...
				$dataset[] .= date("Y-m-d H:i:s",strtotime($row['created_date']));

			if(isset($_POST['tdate']) && !empty($_POST['tdate']))
				$dataset[] .= date("Y-m-d H:i:s",strtotime($row['tdate']));
			...
			if(isset($_POST['settelement_date']) && !empty($_POST['settelement_date']))
				if($row['settelement_date']!=NULL) $dataset[] .= date("Y-m-d",strtotime($row['settelement_date']));
				else $dataset[] .= '';
			if(isset($_POST['rolling_date']) && !empty($_POST['rolling_date']))
			{
				if($row['rolling_date']!=NULL) $dataset[] .= date("Y-m-d",strtotime($row['rolling_date']));
				else $dataset[] .= '';
			}
			if(isset($_POST['merID']) && !empty($_POST['merID']))
				$dataset[] .= ($row['merID']." ");
