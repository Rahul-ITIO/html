<??>
	...
	if(isset($data['TIME_TO_COMPLETION_TRANSACTION_SECONDS'])&&@$data['TIME_TO_COMPLETION_TRANSACTION_SECONDS']=='Y'  &&  date("Ymd",strtotime($_REQUEST['date_2nd'])) >= date("Ymd",strtotime($data['RUNTIME_DB_DATE_2ND'])) )
		$field_list7= array_merge($field_list7, ['runtime'=>'Runtime']);

	
	if($data['con_name']=='clk'){
		$clk_field_list7=['gst_amt'=>'GST Amt'];
		$field_list7 = array_merge($field_list7, $clk_field_list7);
	}
	
	$field_list7_uncheck = array('available_balance', 'rolling_date','merID','bill_address','bill_city','bill_state','bill_zip','bill_phone','trans_type','acquirer_ref','table_id', 'cardbin', 'risk_ratio','bill_ip','trans_response','acquirer');
