...
if(isset($_REQUEST['s1'])&&$_REQUEST['s1'])
	echo "<br/>security_f_path from =>security_function_256.do<br/>";
...
function card_decryption_required($ccno) {
	...
	if($not_encrypted){
		...
		if(is_string($ccno)&&strpos($ccno, 'decrypt') !== false)
		{
			if(isset($_REQUEST['s1'])&&$_REQUEST['s1']) echo "<br/>1 ccno=>".$ccno."<br/>";
	...
	if(isset($_REQUEST['s1'])&&$_REQUEST['s1']) echo "<br/>2 ccno=>".$ccno."<br/>";
...
function decrypts256($string,$dek_key,$secret_key) {
	...
	$output = openssl_decrypt( base64_decode( trim(@$string) ), $encrypt_method, $secret_key, 0, $iv );
    return $output;
}
