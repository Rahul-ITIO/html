...
function vlc($fileds){
	...
	return $result;
		...
		fputcsv($output, array("Sl.", 'transID', 'Bank Reference No.', 'Bank Status', 'MDR Amt.', 'Transaction Fee', 'Wire Fee', 'Monthly Fee', 'GST Fee', 'mid', 'username', 'CompanyName', 'settlement Amt', 'settlement Currency', 'BankName', 'IfscCode', 'AccountNumber', 'Nodal Bank Status'));
	
		$i=1;
		
		$csv_json_0=trim(@$pr_slc[0]['csv_json'],'[]');

		$csv_json=jsondecode("[".$csv_json_0."]",1,1);

		if($qprint){
			echo '<br/><hr/><br/>csv_json=><br/>';
			print_r($csv_json);
		}
