...
if($api_key)
{
	...
	//	print_r($locations);
		...
		if(isset($_REQUEST['action'])&&$_REQUEST['action']=='json'&&@$response)
		{
			//echo $response;

			//remove tab and new line from json encode value 
			$arrayIPRes2 = preg_replace('~[\r\n\t]+~', '', $response);
					
			$arrayIPRes2=stripslashes($arrayIPRes2); $arrayIPRes2=str_replace(array('"{','}"'),array('{','}'),$arrayIPRes2);
