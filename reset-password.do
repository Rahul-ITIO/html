...
if(isset($post['cancel'])&&$post['cancel']){
	...
	if($data['cqp']>0) print_r($_POST);
	
	if($data['cqp']>0) echo "<br/>1 step=>".$post['step'];
	
	if($post['step']==1){
		if($data['cqp']>0) echo "<br/>2 step=>".$post['step'];
	//	if(!$post['registered_email']||verify_email($post['registered_email'])){
		if(!isset($post['registered_email'])||empty($post['registered_email'])){
		...
		/*elseif((strpos($_SERVER['HTTP_REFERER'],'/forgot')===false||strpos($_SERVER['HTTP_REFERER'],'/reset-password')===false)&&($http_referer_validation == true)){ 
			...
			if($data['cqp']>5) 
			{ 
				echo "<br/>daily_password_count_info=>";
			...
			if($restorePassword==1){
				...
				$query_upd.	
				"`password_updated_date`='{$date_fetch}'".$daily_password_count.
				" WHERE `id`={$info['id']}",$data['cqp']
				);
